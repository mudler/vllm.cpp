...
|---|---|---|---|---|---|---|---|
| `BACKEND-GATE-CUDA-VLLM` | CUDA correctness and every-axis parity vs pinned vLLM | local vLLM `e24d1b24`; [benchmark protocol](benchmark-protocol.md) | [bench main:1](../examples/bench/main.cpp#L1); [online driver](../scripts/dgx-online-serving.sh#L1) | both token-exact greedy model gates pass (`tests/parity/test_qwen36_paged_engine.cpp:140`; `tests/parity/test_qwen27_paged_engine.cpp:80`). Exact clean pushed-`b5c6e4f` 27B online groups remain binding below floor: c1→c32 totals **0.9933/0.9520/0.9657/0.9760/1.0213/1.0218×**, **4/4/5/4/17/14 of 20** performance and **2/4** memory axes. W3-A's +0.816% c16 component strict-fails and changes no denominator. W3-B now mirrors pre-serve in-memory tuning in the shared loader; its disposable GB10 stage covers 80/80 profiles before HTTP readiness, reports zero lazy misses and preserves 16/16 correctness. Clean pushed-SHA sanitizer, selection/component, trace and exact-oracle gates are pending, so no speed/parity state changes. Pip-vLLM's persistent cache remains disabled. Repeat full 27B only after immutable W3-B classification, before 35B; device-residency wins and teardown debt remain separate; [benchmark checkpoint](../docs/BENCHMARKS.md) | [CUDA inventory](specs/cuda-architecture-inventory.md) | `PARTIAL` | - |
