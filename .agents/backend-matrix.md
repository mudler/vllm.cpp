...
|---|---|---|---|---|---|---|---|
| `BACKEND-GATE-CUDA-VLLM` | CUDA correctness and every-axis parity vs pinned vLLM | local vLLM `e24d1b24`; [benchmark protocol](benchmark-protocol.md) | [bench main:1](../examples/bench/main.cpp#L1); [online driver](../scripts/dgx-online-serving.sh#L1) | both token-exact greedy model gates pass (`tests/parity/test_qwen36_paged_engine.cpp:140`; `tests/parity/test_qwen27_paged_engine.cpp:80`). Exact clean pushed-`b5c6e4f` 27B online groups remain binding below floor: c1→c32 totals **0.9933/0.9520/0.9657/0.9760/1.0213/1.0218×**, **4/4/5/4/17/14 of 20** performance and **2/4** memory axes. W3-A `71f1e89` mirrors production's 1-ms delayed timing and passes correctness/safety. Its c16 component is **1.008156×** mean total but strict-fails **13/20 timing** and **2/4 memory** with only **5/35** delayed keys stable; no speed/parity denominator changes. Pip-vLLM's persistent cache is disabled, making active W3-B pre-serve in-memory all-bucket warmup the next mirror. Repeat full 27B before 35B; device-residency wins and teardown debt remain separate; [benchmark checkpoint](../docs/BENCHMARKS.md) | [CUDA inventory](specs/cuda-architecture-inventory.md) | `PARTIAL` | - |
