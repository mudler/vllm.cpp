...
|---|---|---|---|---|---|---|---|
| `BACKEND-GATE-CUDA-VLLM` | CUDA correctness and every-axis parity vs vLLM v0.25.0 | current porting pin `e24d1b24`; executable oracle/target `702f481`; [benchmark protocol](benchmark-protocol.md) | [bench main:1](../examples/bench/main.cpp#L1); [v0.25 oracle contract](../tools/bench/online_gate.py#L48); [model-scoped summary](../tools/bench/online_gate_summary.py#L584); [online driver](../scripts/dgx-online-serving.sh#L356) | both token-exact greedy model gates pass (`tests/parity/test_qwen36_paged_engine.cpp:140`; `tests/parity/test_qwen27_paged_engine.cpp:80`). Immutable `9cc7191` completed its v0.25.0/FlashInfer 0.6.13 27B cache-off grid; all 124 axes bind, **54 pass / 70 fail**. Clean `3f256ab` W3-D correctness passes; its component is **1.005049×** mean-positive but strict-fails at **14/20 timing + 2/4 memory**. The post-pack node trace passes and confirms **208.192 vs vLLM 208 FP4 GEMMs/forward**, closing the selected structural launch gap. This is still not a fresh oracle denominator: the binding 54/124 result remains, the exact rerun is next, and 35B remains prohibited. See the [benchmark checkpoint](../docs/BENCHMARKS.md) | [CUDA inventory](specs/cuda-architecture-inventory.md) | `PARTIAL` | - |
