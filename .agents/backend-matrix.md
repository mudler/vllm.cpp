...
|---|---|---|---|---|---|---|---|
| `BACKEND-GATE-CUDA-VLLM` | CUDA correctness and every-axis parity vs vLLM v0.25.0 | current porting pin `e24d1b24`; executable oracle/target `702f481`; [benchmark protocol](benchmark-protocol.md) | [bench main:1](../examples/bench/main.cpp#L1); [v0.25 oracle contract](../tools/bench/online_gate.py#L48); [online driver](../scripts/dgx-online-serving.sh#L1) | both token-exact greedy model gates pass (`tests/parity/test_qwen36_paged_engine.cpp:140`; `tests/parity/test_qwen27_paged_engine.cpp:80`). The complete `b5c6e4f` grid is historical because it used vLLM 0.24.0 + FlashInfer 0.6.12; `3cc490c` is VOID at 28/36 groups, 1,602/2,016 requests, four returns and no trace. No current ratio binds. Canonical v0.25.0 + FlashInfer 0.6.13 validation/activation passes exact package/import/CLI, lock-held real 27B production-graph generation and server health/completion smoke with v0.24 rollback preserved. Run the new exact 27B grid and paired traces; no 35B run is authorized before every 27B axis passes. [Benchmark checkpoint](../docs/BENCHMARKS.md) | [CUDA inventory](specs/cuda-architecture-inventory.md) | `PARTIAL` | - |
