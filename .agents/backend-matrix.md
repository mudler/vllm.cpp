...
|---|---|---|---|---|---|---|---|
| `BACKEND-GATE-CUDA-VLLM` | CUDA correctness and every-axis parity vs vLLM v0.25.0 | current porting pin `e24d1b24`; executable oracle/target `702f481`; [benchmark protocol](benchmark-protocol.md) | [bench main:1](../examples/bench/main.cpp#L1); [v0.25 oracle contract](../tools/bench/online_gate.py#L48); [model-scoped summary](../tools/bench/online_gate_summary.py#L584); [online driver](../scripts/dgx-online-serving.sh#L356) | both token-exact greedy model gates pass (`tests/parity/test_qwen36_paged_engine.cpp:140`; `tests/parity/test_qwen27_paged_engine.cpp:80`). Immutable `9cc7191` completed its v0.25.0/FlashInfer 0.6.13 27B cache-off grid; all 124 axes bind, **54 pass / 70 fail**. Replacement `def5f75` selected packed QKV from local ~240 versus vLLM 208 FP4 GEMMs/step. Clean `3f256ab` packed/split correctness passes **235/235 + 16/16** each; its c16/96 component means are **812.231/808.150 tok/s = 1.005049×**, but strict acceptance fails at **14/20 timing + 2/4 memory**. This is not a fresh oracle denominator: the binding 54/124 result remains, node re-trace/exact rerun are next, and 35B remains prohibited. See the [benchmark checkpoint](../docs/BENCHMARKS.md) | [CUDA inventory](specs/cuda-architecture-inventory.md) | `PARTIAL` | - |
