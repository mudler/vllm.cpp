...
|---|---|---|---|---|---|---|---|
| `BACKEND-GATE-CUDA-VLLM` | CUDA correctness and every-axis parity vs vLLM v0.25.0 | current porting pin `e24d1b24`; executable oracle/target `702f481`; [benchmark protocol](benchmark-protocol.md) | [bench main:1](../examples/bench/main.cpp#L1); [v0.25 oracle contract](../tools/bench/online_gate.py#L48); [model-scoped summary](../tools/bench/online_gate_summary.py#L584); [online driver](../scripts/dgx-online-serving.sh#L523) | both token-exact greedy model gates pass (`tests/parity/test_qwen36_paged_engine.cpp:140`; `tests/parity/test_qwen27_paged_engine.cpp:80`). Immutable `9cc7191` completed its v0.25.0/FlashInfer 0.6.13 27B cache-off grid: model gate, 36/36 groups, 2,016/2,016 requests, six returns and paired traces pass the evidence contract. All 124 axes bind; **54 pass / 70 fail**. Total-throughput ratios c1→c32 are **0.9901/0.9491/0.9633/0.9770/1.0288/1.0467×**, and peak PSS/RSS normalize to **0.5855/0.5934×**. The scoped summary exits 1 and hashes to `c46595b8…a894` / `231ec9fd…7591`; see the [benchmark checkpoint](../docs/BENCHMARKS.md). This is a stable failed/open 27B gate, not near parity; actual trace differences now own repair, and 35B remains prohibited | [CUDA inventory](specs/cuda-architecture-inventory.md) | `PARTIAL` | - |
