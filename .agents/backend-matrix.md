...
|---|---|---|---|---|---|---|---|
| `BACKEND-GATE-CUDA-VLLM` | CUDA correctness and every-axis parity vs vLLM v0.25.0 | current pin `e24d1b24`; oracle `702f481`; async default `vllm/config/vllm.py:952-1043`; depth-2 engine path `vllm/v1/engine/core.py:519-607`; async output `vllm/v1/worker/gpu/async_utils.py:12-70`; [benchmark protocol](benchmark-protocol.md) | Existing schema-v5 validator/summary remain. The trace-only [controller](../include/vt/cuda/cuda_profiler_control.h#L13), one-lock packed/rollback [driver](../scripts/dgx-online-serving.sh), mode-keyed [validator](../tools/bench/online_gate.py#L155), and marker-last [packed finalizer](../tools/bench/finalize_gdn_packed_trace.py#L218) preserve historical evidence and enforce explicit B=2 provenance; production builds have no observer | Binding `3f256ab` stays **55/124**. Clean `7ff713e` completes 12/12 packed + 12/12 rollback ranges and both exact vLLM oracle traces; retained correctness anchors are the [27B default/rollback gate](../tests/parity/test_qwen27_paged_engine.cpp#L125) and [CUDA GDN tests](../tests/vt/test_ops_gdn.cpp#L1039). New fingerprints are bounded solely to cached generated-RMSNorm 48/50-register allocation; exact [finalizer regressions](../tests/tools/test_low_batch_trace_summary.py#L112), imported-validator hash provenance and full dry finalization pass `complete-structural`. The pushed marker and c2/c16 remain pending. No speed credit exists; host PSS/RSS remains **22.920 GiB** and 35B performance is blocked | [CUDA inventory](specs/cuda-architecture-inventory.md); [online gate](specs/cuda-online-serving-gate.md); [packed decode](specs/gdn-packed-decode.md); [W3-I](specs/nvfp4-fused-silu-producer.md) | `PARTIAL` | - |
