...
|---|---|---|---|---|---|---|---|
| `BACKEND-GATE-CUDA-VLLM` | CUDA correctness and every-axis parity vs pinned vLLM | local vLLM `e24d1b24`; [benchmark protocol](benchmark-protocol.md) | [bench main:1](../examples/bench/main.cpp#L1); [online driver](../scripts/dgx-online-serving.sh#L1) | both token-exact greedy model gates pass (`tests/parity/test_qwen36_paged_engine.cpp:140`; `tests/parity/test_qwen27_paged_engine.cpp:80`). Exact pushed-`4e1d8ca` 27B online groups are all binding but below floor: c1→c32 median total ratios **0.9661/0.9274/0.9378/0.9466/0.9808/0.9910×**, **0/2/5/3/3/5 of 20** performance and **2/4** memory axes pass. The fixed HTTP ladder is healthy/neutral. FP4 W1 exact buckets/key/single-flight now clean-build for sm_121a; pushed `1a802ac` focused default/legacy CUDA runs each pass 10/10 and 1,945/1,945. The expanded real capture case is staging-green only; immutable capture/memcheck/model/performance gates remain, while direct M=16 retuning and vLLM's missing narrow Stream-K/static-persistent tactics remain the before-state. Repeat 27B before 35B, while W0/W1 device-residency wins and teardown debt remain separate; [benchmark checkpoint](../docs/BENCHMARKS.md) | [CUDA inventory](specs/cuda-architecture-inventory.md) | `PARTIAL` | - |
