...
|---|---|---|---|---|---|---|---|
| `BACKEND-GATE-CUDA-VLLM` | CUDA correctness and every-axis parity vs pinned vLLM | local vLLM `e24d1b24`; [benchmark protocol](benchmark-protocol.md) | [bench main:1](../examples/bench/main.cpp#L1); [online driver](../scripts/dgx-online-serving.sh#L1) | both token-exact greedy model gates pass (`tests/parity/test_qwen36_paged_engine.cpp:140`; `tests/parity/test_qwen27_paged_engine.cpp:80`). Exact pushed-`bce2627` 27B online groups are all binding but below floor: c1→c32 median total ratios **0.9680/0.9317/0.9403/0.9516/0.9944/1.0073×**, **4/4/5/4/4/12 of 20** performance and **2/4** memory axes pass. Model/trace status and 2,016 requests/six returns validate; summary/ratio/trace hashes are `06a4bd7a…e41d` / `1e9643e9…c4b9` / `ef9ce611…3a14`. FP4 W1 is safety/correctness/performance classified: positive at c8/c32 but strict component and full-grid gates fail. Fresh oracle execution attributes 25.1% of profiled time to the absent 128x32x256 Stream-K/static pair, making W2 the next lever. Repeat component/trace/full 27B before 35B, while device-residency wins and teardown debt remain separate; [benchmark checkpoint](../docs/BENCHMARKS.md) | [CUDA inventory](specs/cuda-architecture-inventory.md) | `PARTIAL` | - |
