...
|---|---|---|---|---|---|---|---|
| `BACKEND-GATE-CUDA-VLLM` | CUDA correctness and every-axis parity vs vLLM v0.25.0 | current pin `e24d1b24`; oracle `702f481`; async default `vllm/config/vllm.py:952-1043`; depth-2 engine path `vllm/v1/engine/core.py:519-607`; async output `vllm/v1/worker/gpu/async_utils.py:12-70`; [benchmark protocol](benchmark-protocol.md) | Existing schema-v5 validator/summary remain. The trace-only [controller](../include/vt/cuda/cuda_profiler_control.h#L13), [driver](../scripts/dgx-online-serving.sh#L14), batch-keyed [validator](../tools/bench/online_gate.py#L99), and fail-closed [c2 finalizer](../tools/bench/finalize_low_batch_trace.py#L1) preserve c16 and enforce explicit B=2 evidence; production builds have no observer. Oracle mode remains [profile_vllm_online_gate.py](../tools/bench/profile_vllm_online_gate.py#L21) | [mode contracts](../tests/tools/test_online_gate_trace.py#L30), [async summary cases](../tests/tools/test_async_credit_summary.py#L1), [c2 finalizer contracts](../tests/tools/test_low_batch_trace_summary.py#L1), and requested-batch rejection in [client contracts](../tests/tools/test_online_gate_client.py#L860) are green. Binding `3f256ab` stays **55/124**. Finalized `179a0fc` passes the model gate, all **12/12** local **1,011+7+1** ranges, and a fresh oracle SHA `2b3bf412…785c` with **1,522×1,160** steady B=2 windows plus two drains. Status `9e0143fa…7b57` is `complete-diagnostic` at median kernel time **111.076528 / 105.520831 ms = 1.052650×**; BF16 GEMM and RMSNorm lead while FP4 tactics match. W1C `f925294` passes exact BA projection and 35B/GGUF inertness but BF16 27B remains 233/235 downstream. No speed credit exists; host PSS/RSS remains **22.920 GiB**, W3-I is off and 35B blocked | [CUDA inventory](specs/cuda-architecture-inventory.md); [online gate](specs/cuda-online-serving-gate.md); [W3-I](specs/nvfp4-fused-silu-producer.md) | `PARTIAL` | - |
