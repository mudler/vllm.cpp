...
|---|---|---|---|---|---|---|---|
| `BACKEND-GATE-CUDA-VLLM` | CUDA correctness and every-axis parity vs vLLM v0.25.0 | current pin `e24d1b24`; oracle `702f481`; async default `vllm/config/vllm.py:952-1043`; depth-2 engine path `vllm/v1/engine/core.py:519-607`; async output `vllm/v1/worker/gpu/async_utils.py:12-70`; [benchmark protocol](benchmark-protocol.md) | Existing schema-v5 validator/summary remain. The trace-only [controller](../include/vt/cuda/cuda_profiler_control.h#L13), [driver](../scripts/dgx-online-serving.sh#L14), batch-keyed [validator](../tools/bench/online_gate.py#L99), and fail-closed [c2 finalizer](../tools/bench/finalize_low_batch_trace.py#L1) preserve c16 and enforce explicit B=2 evidence; production builds have no observer. Oracle mode remains [profile_vllm_online_gate.py](../tools/bench/profile_vllm_online_gate.py#L21) | [mode contracts](../tests/tools/test_online_gate_trace.py#L30) and the [packed boundary replay](../tests/parity/test_op_parity.cpp#L1817) ground the current evidence. Binding `3f256ab` stays **55/124**. `179a0fc` proves the 193-vs-97 BF16 topology and matching FP4 tactics; `0091cd1` closes BA structure; `f925294` passes exact BA projection and 35B/GGUF inertness, but BF16 27B remains 233/235. Clean `f18ca23` regenerates the official packed fixture byte-for-byte and passes CUDA **10/10**, proving full F32 in-kernel q/k normalization plus dtype-rounded beta reaches `0/1` while beta-only does not. Production port, 235/235 and c2/c16 are pending. No speed credit exists; host PSS/RSS remains **22.920 GiB**, W3-I is off and 35B blocked | [CUDA inventory](specs/cuda-architecture-inventory.md); [online gate](specs/cuda-online-serving-gate.md); [packed decode](specs/gdn-packed-decode.md); [W3-I](specs/nvfp4-fused-silu-producer.md) | `PARTIAL` | - |
