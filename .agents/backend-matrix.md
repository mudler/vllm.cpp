...
|---|---|---|---|---|---|---|---|
| `BACKEND-GATE-CUDA-VLLM` | CUDA correctness and every-axis parity vs vLLM v0.25.0 | current pin `e24d1b24`; oracle `702f481`; cache lifecycle `kernel_warmup.py:133-213`; [benchmark protocol](benchmark-protocol.md) | [oracle contract](../tools/bench/online_gate.py#L48); [summary](../tools/bench/online_gate_summary.py#L584); [driver](../scripts/dgx-online-serving.sh#L356) | token-exact [35B](../tests/parity/test_qwen36_paged_engine.cpp#L140) remains historical; W3-C2 runtime/cache tests and memcheck pass. Immutable `d211b8f` proves six fresh [27B](../tests/parity/test_qwen27_paged_engine.cpp#L116) processes with identical **64/64** native maps, zero tuning/misses and **235/235 + 16/16** each. The first same-plan c2 direct/fallback pair preserves that map and exact token counts but only **2/6** long texts match; C3 is stopped and partial performance is `VOID`. `3f256ab` therefore remains **55/124**; exact rerun and 35B performance are blocked on fixed-plan localization. See [BENCHMARKS](../docs/BENCHMARKS.md) | [CUDA inventory](specs/cuda-architecture-inventory.md); [W3-C spike](specs/nvfp4-persistent-plan-cache.md) | `PARTIAL` | - |
