...
|---|---|---|---|---|---|---|---|
| `BACKEND-GATE-CUDA-VLLM` | CUDA correctness and every-axis parity vs vLLM v0.25.0 | current pin `e24d1b24`; oracle `702f481`; cache lifecycle `kernel_warmup.py:133-213`; model-owned alpha `compressed_tensors_w4a4_nvfp4.py:95-141`, `kernels/linear/nvfp4/flashinfer.py:125-170`; FA2 backend `vllm/v1/attention/backends/flash_attn.py:674-721,935-977` and dependency `flash_api.cpp:262-327,587-714`; [benchmark protocol](benchmark-protocol.md) | [schema-v3 oracle contract and lossless validator](../tools/bench/online_gate.py); [summary](../tools/bench/online_gate_summary.py); [three-capture driver](../scripts/dgx-online-serving.sh); [repeated-range tests](../tests/tools/test_online_gate_client.py#L836); diagnostic [graceful target stop](../examples/server/main.cpp); W3-G exact [FA2 decode adapter](../src/vt/cuda/cuda_flash_attn_fa2.cu#L429), [dispatch](../src/vt/cuda/cuda_paged_attn.cu#L2501), [model gate](../src/vllm/model_executor/models/qwen3_5.cpp#L2692) and [ported tests](../tests/vt/test_ops_paged_attn.cpp#L1354) | W3-E/W3-F/W3-G strict-fail, so `3f256ab` remains **55/124** and none earns speed credit. W3-H1a/H1b/H1c and H1d attempts through `b2c940c` are `VOID`. `b2c940c` repeats the exact **154/154** build, passes the 27B gate **1/1 in 17.30 s**, completes the clients, frozen plans, FIFO lifecycle/removal and target/Nsight zero exit. Its four reports each contain one complete 1,107-kernel replay and zero eager CUDA rows, but all emit severity-2 possible loss and schema v3 expects one combined report. Schema v4 over three sessions x four reports with synchronous generation/device flush is accepted but not implemented; fresh evidence and 35B performance remain blocked. The [repeated-range regression](../tests/tools/test_online_gate_client.py#L836) and focused harness **31/31**, Python/shell syntax, ShellCheck and CUDA-off server build pass; the prior full CUDA-off suite is **105/106** twice on an unrelated C API timing case that passes alone. See [W3-H spike/evidence contract](specs/nvfp4-bf16-producer-vectorization.md) and [BENCHMARKS](../docs/BENCHMARKS.md) | [CUDA inventory](specs/cuda-architecture-inventory.md); [W3-C spike](specs/nvfp4-persistent-plan-cache.md); [W3-F spike](specs/nvfp4-device-alpha.md); [W3-G spike](specs/fa2-gqa-split-kv-decode.md); [W3-H spike](specs/nvfp4-bf16-producer-vectorization.md) | `PARTIAL` | - |
