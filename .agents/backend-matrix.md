...
|---|---|---|---|---|---|---|---|
| `BACKEND-GATE-CUDA-VLLM` | CUDA correctness and every-axis parity vs pinned vLLM | local vLLM `e24d1b24`; [benchmark protocol](benchmark-protocol.md) | [bench main:1](../examples/bench/main.cpp#L1); [online driver](../scripts/dgx-online-serving.sh#L1) | both token-exact greedy model gates pass (`tests/parity/test_qwen36_paged_engine.cpp:140`; `tests/parity/test_qwen27_paged_engine.cpp:80`). Exact clean pushed-`b5c6e4f` 27B online groups are binding below floor: c1→c32 median total ratios **0.9933/0.9520/0.9657/0.9760/1.0213/1.0218×**, **4/4/5/4/17/14 of 20** performance and **2/4** memory axes; 2,016 requests/six returns and model/trace status pass, with hashes `0056bf62…c5c59` / `632e087b…192c` / `0190a7e1…ad3e`. FP4 W2 materially improves every concurrency and wins c16/c32 throughput, but c1-c8 and mean TPOT/ITL remain red. Paired traces show vLLM's 128x32x256 Stream-K/static selection versus our wide selected tactics, promoting W3 pre-serve warmup/selection/cache parity. Repeat full 27B before 35B; device-residency wins and teardown debt remain separate; [benchmark checkpoint](../docs/BENCHMARKS.md) | [CUDA inventory](specs/cuda-architecture-inventory.md) | `PARTIAL` | - |
