...
|---|---|---|---|---|---|---|---|
| `BACKEND-GATE-CUDA-VLLM` | CUDA correctness and every-axis parity vs pinned vLLM | local vLLM `e24d1b24`; [benchmark protocol](benchmark-protocol.md) | [bench main:1](../examples/bench/main.cpp#L1); [online driver](../scripts/dgx-online-serving.sh#L1) | both token-exact greedy model gates pass (`tests/parity/test_qwen36_paged_engine.cpp:140`; `tests/parity/test_qwen27_paged_engine.cpp:80`). Exact pushed-`bce2627` 27B online groups remain the binding below-floor before-state: c1→c32 median total ratios **0.9680/0.9317/0.9403/0.9516/0.9944/1.0073×**, **4/4/5/4/4/12 of 20** performance and **2/4** memory axes; 2,016 requests/six returns and model/trace status pass, with hashes `06a4bd7a…e41d` / `1e9643e9…c4b9` / `ef9ce611…3a14`. FP4 W2 now ports the formerly absent 32-tactic family, merged CT gate/up scaling and fused SiLU→NVFP4 producer; focused CUDA/sanitizer/model gates pass. The fusion component is 1.015211× but misses strict timing/memory axes; its 1.020843×/20-of-20 paired trace is structural only. Rebuild the clean pushed SHA and repeat the full c1-c32 27B oracle before 35B, while device-residency wins and teardown debt remain separate; [benchmark checkpoint](../docs/BENCHMARKS.md) | [CUDA inventory](specs/cuda-architecture-inventory.md) | `PARTIAL` | - |
