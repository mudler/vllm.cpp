...
|---|---|---|---|---|---|---|---|
| `BACKEND-GATE-CUDA-VLLM` | CUDA correctness and every-axis parity vs vLLM v0.25.0 | current porting pin `e24d1b24`; executable oracle/target `702f481`; [benchmark protocol](benchmark-protocol.md) | [bench main:1](../examples/bench/main.cpp#L1); [v0.25 oracle contract](../tools/bench/online_gate.py#L48); [model-scoped summary](../tools/bench/online_gate_summary.py#L584); [online driver](../scripts/dgx-online-serving.sh#L356) | both token-exact greedy model gates pass (`tests/parity/test_qwen36_paged_engine.cpp:140`; `tests/parity/test_qwen27_paged_engine.cpp:80`). Immutable `9cc7191` completed its v0.25.0/FlashInfer 0.6.13 27B cache-off grid: model gate, 36/36 groups, 2,016/2,016 requests and six returns pass. All 124 axes bind; **54 pass / 70 fail**. Total-throughput ratios c1→c32 are **0.9901/0.9491/0.9633/0.9770/1.0288/1.0467×**, and peak PSS/RSS normalize to **0.5855/0.5934×**. Replacement `def5f75` passes the required node-level one-lock trace: 2,315,412 graph-child rows close attribution and exact counts promote packed QKV from the local ~240 versus vLLM 208 FP4 GEMMs/step. Timing/memory evidence remains failed/open, the trace itself claims no speedup, packed-QKV A/B is next, and 35B remains prohibited. See the [benchmark checkpoint](../docs/BENCHMARKS.md) | [CUDA inventory](specs/cuda-architecture-inventory.md) | `PARTIAL` | - |
