...
|---|---|---|---|---|---|---|---|
| `BACKEND-GATE-CUDA-VLLM` | CUDA correctness and every-axis parity vs pinned vLLM | local vLLM `e24d1b24`; [benchmark protocol](benchmark-protocol.md) | [bench main:1](../examples/bench/main.cpp#L1); [online driver](../scripts/dgx-online-serving.sh#L1) | both token-exact greedy model gates pass (`tests/parity/test_qwen36_paged_engine.cpp:140`; `tests/parity/test_qwen27_paged_engine.cpp:80`). Exact pushed-`a531e05` 27B online groups are all binding but below floor: c1→c32 median total ratios **0.9679/0.9338/0.9482/0.9547/1.0028/0.9268×**, **4/2/5/3/10/8 of 20** performance and **2/4** memory axes pass. Runtime evidence confirms c32 HTTP worker under-capacity and FP4 M=1/2/4/8/16 key aliasing; direct M=16 retuning matches vLLM mean TPOT, while vLLM also runs missing narrow Stream-K/static-persistent tactics. Repair those and repeat 27B before 35B; W0/W1 component wins and teardown debt remain separate; [benchmark checkpoint](../docs/BENCHMARKS.md) | [CUDA inventory](specs/cuda-architecture-inventory.md) | `PARTIAL` | - |
