...
|---|---|---|---|---|---|---|---|
| `BACKEND-GATE-CUDA-VLLM` | CUDA correctness and every-axis parity vs vLLM v0.25.0 | current porting pin `e24d1b24`; executable oracle/target `702f481`; [benchmark protocol](benchmark-protocol.md) | [bench main:1](../examples/bench/main.cpp#L1); [v0.25 oracle contract](../tools/bench/online_gate.py#L48); [online driver](../scripts/dgx-online-serving.sh#L1) | both token-exact greedy model gates pass (`tests/parity/test_qwen36_paged_engine.cpp:140`; `tests/parity/test_qwen27_paged_engine.cpp:80`). The complete `b5c6e4f` grid is historical and `3cc490c` is VOID at 28/36 groups; no ratio binds. Canonical v0.25.0 + FlashInfer 0.6.13 validation/activation passes. Immutable `9cc7191` source/build/oracle/corpus preflight is now complete and hashed; its whole-lock 27B model gate, 36 timed groups, six returns and paired traces are `ACTIVE` but not yet executed. One corrected no-GPU recorder preflight supersedes a missing-module-path command failure. No 35B run is authorized before every 27B axis passes. [Benchmark checkpoint](../docs/BENCHMARKS.md) | [CUDA inventory](specs/cuda-architecture-inventory.md) | `PARTIAL` | - |
