...
| `BACKEND-CUDA-COMP-FA` | FlashAttention | FA2 all filtered targets; FA3 >=12.3; FA4 CuTe package; `vllm_flash_attn.cmake:1-46`, `fa_utils.py:132-250` | FA2 BF16 causal head-256 prefill plus bounded ratio-6 non-causal split-KV decode [CMakeLists.txt:428](../CMakeLists.txt#L428), [adapter](../src/vt/cuda/cuda_flash_attn_fa2.cu#L429) | [paged-attention tests](../tests/vt/test_ops_paged_attn.cpp#L585), [27B gate](../tests/parity/test_qwen27_paged_engine.cpp#L110); immutable `ae9e8ff` clean sm_121a compile/operator/memcheck/model/paired-trace pass; c2/c16 component active under one lock with result pending | [CUDA inventory](specs/cuda-architecture-inventory.md); [W3-G spike](specs/fa2-gqa-split-kv-decode.md) | `PARTIAL` | - |
...
|---|---|---|---|---|---|---|---|
| `BACKEND-GATE-CUDA-VLLM` | CUDA correctness and every-axis parity vs vLLM v0.25.0 | current pin `e24d1b24`; oracle `702f481`; cache lifecycle `kernel_warmup.py:133-213`; model-owned alpha `compressed_tensors_w4a4_nvfp4.py:95-141`, `kernels/linear/nvfp4/flashinfer.py:125-170`; FA2 backend `vllm/v1/attention/backends/flash_attn.py:674-721,935-977` and dependency `flash_api.cpp:262-327,587-714`; [benchmark protocol](benchmark-protocol.md) | [oracle contract](../tools/bench/online_gate.py#L48); [summary](../tools/bench/online_gate_summary.py#L584); [driver](../scripts/dgx-online-serving.sh#L356); W3-G exact [FA2 decode adapter](../src/vt/cuda/cuda_flash_attn_fa2.cu#L429), [dispatch](../src/vt/cuda/cuda_paged_attn.cu#L2501), [model gate](../src/vllm/model_executor/models/qwen3_5.cpp#L2692) and [ported tests](../tests/vt/test_ops_paged_attn.cpp#L1354) | W3-E strict-fails **39/40 timing + 1/8 memory** and W3-F strict-fails **27/40 + 3/8**, so `3f256ab` remains **55/124**. W3-G immutable `ae9e8ff` passes clean sm_121a/operator/zero-error-zero-leak sanitizer, both 27B default/fallback **235/235 + 16/16**, correctness-only 35B **315/315** and paired node trace. Trace dispatch is exact and all FP4 plans/topology match; the short trace is performance-negative and non-binding. The **40+8** component is `ACTIVE` under one uninterrupted lock after both model arms passed; every partial leg remains non-binding. Exact rerun and 35B performance stay blocked. See [W3-G ledger evidence](parity-ledger.md#L381) and [BENCHMARKS](../docs/BENCHMARKS.md) | [CUDA inventory](specs/cuda-architecture-inventory.md); [W3-C spike](specs/nvfp4-persistent-plan-cache.md); [W3-F spike](specs/nvfp4-device-alpha.md); [W3-G spike](specs/fa2-gqa-split-kv-decode.md) | `PARTIAL` | - |
