...
|---|---|---|---|---|---|---|---|
| `BACKEND-GATE-CUDA-VLLM` | CUDA correctness and every-axis parity vs vLLM v0.25.0 | current pin `e24d1b24`; oracle `702f481`; async default `vllm/config/vllm.py:952-1043`; depth-2 engine path `vllm/v1/engine/core.py:519-607`; async output `vllm/v1/worker/gpu/async_utils.py:12-70`; [benchmark protocol](benchmark-protocol.md) | Existing schema-v5 validator/summary/driver remain; diagnostic oracle mode and absolute-path bootstrap are [profile_vllm_online_gate.py](../tools/bench/profile_vllm_online_gate.py#L21), with no local engine-default change | [mode-control/direct-script contract](../tests/tools/test_online_gate_trace.py#L30) is green. Binding `3f256ab` stays **55/124**. Async `9b1774c` is VOID after six timing legs and ON capture because H1d exact-count validation stopped before shape-neutral summary/OFF; provisional ON/OFF total is **1.000464×** and earns no credit. Fresh shape-neutral timing + traces are `PENDING`, so W3 stays `READY`/uncredited. Host PSS/RSS remains **22.920 GiB** CPU weights plus mmap pages; W3-I off and 35B blocked. See [async W3](specs/async-serving.md) and [scoreboard](../docs/BENCHMARKS.md) | [CUDA inventory](specs/cuda-architecture-inventory.md); [async W3](specs/async-serving.md); [W3-I](specs/nvfp4-fused-silu-producer.md) | `PARTIAL` | - |
