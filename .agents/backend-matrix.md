...
| `BACKEND-CUDA-COMP-FA` | FlashAttention | FA2 all filtered targets; FA3 >=12.3; FA4 CuTe package; `vllm_flash_attn.cmake:1-46`, `fa_utils.py:132-250` | FA2 BF16 causal head-256 subset [CMakeLists.txt:428](../CMakeLists.txt#L428) | [paged-attention tests](../tests/vt/test_ops_paged_attn.cpp#L585), [27B gate](../tests/parity/test_qwen27_paged_engine.cpp#L110) | [CUDA inventory](specs/cuda-architecture-inventory.md) | `PARTIAL` | - |
...
|---|---|---|---|---|---|---|---|
| `BACKEND-GATE-CUDA-VLLM` | CUDA correctness and every-axis parity vs vLLM v0.25.0 | current porting pin `e24d1b24`; executable oracle/target `702f481`; v0.25 persistent cache `vllm/model_executor/warmup/kernel_warmup.py:133-213`; [benchmark protocol](benchmark-protocol.md) | [bench main:1](../examples/bench/main.cpp#L1); [v0.25 oracle contract](../tools/bench/online_gate.py#L48); [model-scoped summary](../tools/bench/online_gate_summary.py#L584); [online driver](../scripts/dgx-online-serving.sh#L356) | both token-exact greedy gates pass ([35B](../tests/parity/test_qwen36_paged_engine.cpp#L140), [27B](../tests/parity/test_qwen27_paged_engine.cpp#L116)). Immutable `3f256ab` completes the 27B grid/lifecycle/trace; **55/124 axes pass**. Binding totals are **0.993504/0.954464/0.966438/0.980678/1.027889/1.039417×**; PSS/RSS fail at 0.584689/0.592269. W3-E removes **624** activation swizzles but its component strict-fails **32/40 timing + 6/8 memory**. Binding v0.25 log/cache SHA `367af18d…2da` / `b41a8ecc…677` proves a 64-plan cache hit, while ours retuned and matched only 18--33/64 paired IDs. [W3-C](specs/nvfp4-persistent-plan-cache.md) is `READY` for persistent/imported/frozen plan parity; code, 64/64 fresh-process stability, same-plan component and any exact rerun are `PENDING`. No speed credit or 35B performance result; see [BENCHMARKS](../docs/BENCHMARKS.md) | [CUDA inventory](specs/cuda-architecture-inventory.md); [W3-E spike](specs/nvfp4-direct-swizzled-scales.md); [W3-C spike](specs/nvfp4-persistent-plan-cache.md) | `PARTIAL` | - |
