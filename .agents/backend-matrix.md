...
|---|---|---|---|---|---|---|---|
| `BACKEND-GATE-CUDA-VLLM` | CUDA correctness and every-axis parity vs vLLM v0.25.0 | current pin `e24d1b24`; oracle `702f481`; cache lifecycle `kernel_warmup.py:133-213`; model-owned alpha `compressed_tensors_w4a4_nvfp4.py:95-141`, `kernels/linear/nvfp4/flashinfer.py:125-170`; FA2 backend `vllm/v1/attention/backends/flash_attn.py:674-721,935-977` and dependency `flash_api.cpp:262-327,587-714`; [benchmark protocol](benchmark-protocol.md) | [oracle contract and lossless validator](../tools/bench/online_gate.py); [summary](../tools/bench/online_gate_summary.py); [three-capture driver](../scripts/dgx-online-serving.sh); W3-G exact [FA2 decode adapter](../src/vt/cuda/cuda_flash_attn_fa2.cu#L429), [dispatch](../src/vt/cuda/cuda_paged_attn.cu#L2501), [model gate](../src/vllm/model_executor/models/qwen3_5.cpp#L2692) and [ported tests](../tests/vt/test_ops_paged_attn.cpp#L1354) | W3-E strict-fails **39/40 timing + 1/8 memory** and W3-F strict-fails **27/40 + 3/8**, so `3f256ab` remains **55/124**. W3-G immutable `ae9e8ff` passes clean sm_121a/operator/zero-error-zero-leak sanitizer, both 27B default/fallback **235/235 + 16/16**, correctness-only 35B **315/315** and paired node trace. Its completed 12-leg/612-request c2/c16 component reaches **1.017668×/1.006548×** mean total throughput but strict-fails **35/40 timing + 5/8 memory**. W3-G earns no speed credit. W3-H1a/H1b are `VOID`. Immutable `d1f8e33` H1c is also `VOID`: its 19.20-s model gate and 48/48 capture-1 client completed, but Nsight warned that not all 818,537 CUDA events might have been collected; fail-closed execution stopped before r2/r3/vLLM and the process log retained no frozen-plan records. H1d must add lossless collection plus exact plan/workload/graph/capture/vLLM semantic validation; exact rerun and 35B performance remain blocked. [Trace validator tests](../tests/tools/test_online_gate_client.py#L320) and [indexed summary-artifact test](../tests/tools/test_online_gate_summary.py#L529) cover the earlier contract but not the newly audited gaps; see [W3-H spike/evidence contract](specs/nvfp4-bf16-producer-vectorization.md) and [BENCHMARKS](../docs/BENCHMARKS.md) | [CUDA inventory](specs/cuda-architecture-inventory.md); [W3-C spike](specs/nvfp4-persistent-plan-cache.md); [W3-F spike](specs/nvfp4-device-alpha.md); [W3-G spike](specs/fa2-gqa-split-kv-decode.md); [W3-H spike](specs/nvfp4-bf16-producer-vectorization.md) | `PARTIAL` | - |
