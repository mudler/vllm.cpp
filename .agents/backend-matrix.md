...
|---|---|---|---|---|---|---|---|
| `BACKEND-GATE-CUDA-VLLM` | CUDA correctness and every-axis parity vs pinned vLLM | local vLLM `e24d1b24`; [benchmark protocol](benchmark-protocol.md) | [bench main:1](../examples/bench/main.cpp#L1); [online driver](../scripts/dgx-online-serving.sh#L1) | both token-exact greedy model gates pass (`tests/parity/test_qwen36_paged_engine.cpp:140`; `tests/parity/test_qwen27_paged_engine.cpp:80`). Exact clean pushed-`b5c6e4f` remains binding below floor: c1→c32 totals **0.9933/0.9520/0.9657/0.9760/1.0213/1.0218×**, **4/4/5/4/17/14 of 20** performance and **2/4** memory axes. Corrected W3-B tracing closes the original FP4 tactic-family mismatch. Clean pushed `3cc490c` is now running the replacement exact 27B gate under one uncontended lock; model/corpus/cache preconditions, ours repetition 1 c1-c32 and its memory return pass, while vLLM repetition 1 is active. No partial ratio binds and no 35B run is authorized; [benchmark checkpoint](../docs/BENCHMARKS.md) | [CUDA inventory](specs/cuda-architecture-inventory.md) | `PARTIAL` | - |
