...
|---|---|---|---|---|---|---|---|
| `BACKEND-GATE-CUDA-VLLM` | CUDA correctness and every-axis parity vs vLLM v0.25.0 | current pin `e24d1b24`; oracle `702f481`; cache lifecycle `kernel_warmup.py:133-213`; model-owned alpha `compressed_tensors_w4a4_nvfp4.py:95-141`, `kernels/linear/nvfp4/flashinfer.py:125-170`; [benchmark protocol](benchmark-protocol.md) | [oracle contract](../tools/bench/online_gate.py#L48); [summary](../tools/bench/online_gate_summary.py#L584); [driver](../scripts/dgx-online-serving.sh#L356) | W3-C frozen-plan reproduction control completes; corrected W3-E fails **39/40 timing + 1/8 memory**. W3-F is now spiked/`READY` from exact trace evidence: local steady graph executes 208.123 scalar stages/forward, vLLM zero, and its FlashInfer chain passes the persistent device pointer. No W3-F runtime/result. `3f256ab` remains **55/124**; exact rerun and 35B performance remain blocked. See [W3-F ledger evidence](parity-ledger.md#L375) and [BENCHMARKS](../docs/BENCHMARKS.md) | [CUDA inventory](specs/cuda-architecture-inventory.md); [W3-C spike](specs/nvfp4-persistent-plan-cache.md); [W3-F spike](specs/nvfp4-device-alpha.md) | `PARTIAL` | - |
