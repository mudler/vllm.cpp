...
|---|---|---|---|---|---|---|---|
| `BACKEND-GATE-CUDA-VLLM` | CUDA correctness and every-axis parity vs pinned vLLM | local vLLM `e24d1b24`; [benchmark protocol](benchmark-protocol.md) | [bench main:1](../examples/bench/main.cpp#L1); [online driver](../scripts/dgx-online-serving.sh#L1) | both token-exact greedy model gates pass (`tests/parity/test_qwen36_paged_engine.cpp:140`; `tests/parity/test_qwen27_paged_engine.cpp:80`). Exact clean pushed-`b5c6e4f` 27B online groups remain binding below floor: c1→c32 totals **0.9933/0.9520/0.9657/0.9760/1.0213/1.0218×**, **4/4/5/4/17/14 of 20** performance and **2/4** memory axes. Clean `d7cdf66` W3-B remains correctness/safety-green and first-use-positive but its component strict-fails. Corrected trace attempt 2 completes all three **144/144** arms under one lock with unchanged 49-file inventories and memory return. Prewarm/lazy/vLLM FP4 sums are **110.623/114.229/109.932 s**; prewarm matches the narrow pair at **70.333/70.986 s** and removes 480 lazy delay launches. Profiled normalized mean TPOT remains **0.9673x**, so the rates are diagnostic and `b5c6e4f` is not replaced. Pip-vLLM's persistent cache remains disabled. Run the fresh exact W3-B c1-c32 grid, then use any remaining executed-kernel gap before 35B; device-residency wins and teardown debt remain separate; [benchmark checkpoint](../docs/BENCHMARKS.md) | [CUDA inventory](specs/cuda-architecture-inventory.md) | `PARTIAL` | - |
