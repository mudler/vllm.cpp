...
|---|---|---|---|---|---|---|---|
| `BACKEND-GATE-CUDA-VLLM` | CUDA correctness and every-axis parity vs pinned vLLM | local vLLM `e24d1b24`; [benchmark protocol](benchmark-protocol.md) | [bench main:1](../examples/bench/main.cpp#L1); [online driver](../scripts/dgx-online-serving.sh#L1) | both token-exact greedy model gates pass (`tests/parity/test_qwen36_paged_engine.cpp:140`; `tests/parity/test_qwen27_paged_engine.cpp:80`). Exact clean pushed-`b5c6e4f` 27B online groups remain binding below floor: c1→c32 totals **0.9933/0.9520/0.9657/0.9760/1.0213/1.0218×**, **4/4/5/4/17/14 of 20** performance and **2/4** memory axes. W3-A's +0.816% c16 component strict-fails and changes no denominator. W3-B now mirrors pre-serve in-memory tuning in the shared loader. Clean pushed `d7cdf66` covers 80/80 profiles before HTTP readiness, reports zero lazy misses, preserves 16/16 correctness and passes 24,586/24,586 memcheck assertions with zero errors. Its repeated prewarm/lazy component is **1.000293×**, strict-fails **15/20 timing + 2/4 memory**, and retains only **20/80** stable prewarm tactic IDs; first-use improves, but no speed/parity state changes. Trace attempt 1 is VOID after prewarm-only 144/144 because cache inventory changed 50→58; lazy/vLLM did not run and no kernel comparison is accepted. Pip-vLLM's persistent cache remains disabled. Rerun the corrected paired trace, then use it to choose the next lever and repeat full 27B before 35B; device-residency wins and teardown debt remain separate; [benchmark checkpoint](../docs/BENCHMARKS.md) | [CUDA inventory](specs/cuda-architecture-inventory.md) | `PARTIAL` | - |
