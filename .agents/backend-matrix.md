...
|---|---|---|---|---|---|---|---|
| `BACKEND-GATE-CUDA-VLLM` | CUDA correctness and every-axis parity vs vLLM v0.25.0 | current porting pin `e24d1b24`; executable oracle/target `702f481`; [benchmark protocol](benchmark-protocol.md) | [bench main:1](../examples/bench/main.cpp#L1); [v0.25 oracle contract](../tools/bench/online_gate.py#L48); [model-scoped summary](../tools/bench/online_gate_summary.py#L584); [online driver](../scripts/dgx-online-serving.sh#L356) | both token-exact greedy model gates pass ([35B](../tests/parity/test_qwen36_paged_engine.cpp#L140), [27B](../tests/parity/test_qwen27_paged_engine.cpp#L116)). Immutable `3f256ab` completes the exact 27B grid/lifecycle/trace; **55/124 axes pass and 69 fail**. Binding total ratios are **0.993504/0.954464/0.966438/0.980678/1.027889/1.039417×**; PSS/RSS fail at 0.584689/0.592269, GPU/drop pass at 1.829076/1.227760. Generated-body inspection refutes norm+FP4 fusion. W3-E direct scales pass implementation/correctness/sanitizer/trace and remove **624** activation swizzles. Immutable `53ab149` component direct/fallback mean total throughput is **1.002108× c2 / 1.006222× c16**, but only **32/40 timing + 6/8 memory** axes pass; five of six long-output hash pairs differ amid unstable tactic IDs. The component strict-fails, so no exact rerun or 35B performance run occurred and `3f256ab` remains binding. See the [benchmark checkpoint](../docs/BENCHMARKS.md) | [CUDA inventory](specs/cuda-architecture-inventory.md); [W3-E spike](specs/nvfp4-direct-swizzled-scales.md) | `PARTIAL` | - |
