...
|---|---|---|---|---|---|---|---|
| `BACKEND-GATE-CUDA-VLLM` | CUDA correctness and every-axis parity vs vLLM v0.25.0 | current pin `e24d1b24`; oracle `702f481`; async default `vllm/config/vllm.py:952-1043`; depth-2 engine path `vllm/v1/engine/core.py:519-607`; async output `vllm/v1/worker/gpu/async_utils.py:12-70`; [benchmark protocol](benchmark-protocol.md) | Existing schema-v5 validator/summary remain. The trace-only [controller](../include/vt/cuda/cuda_profiler_control.h#L13), [driver](../scripts/dgx-online-serving.sh#L14), and batch-keyed [validator](../tools/bench/online_gate.py#L99) preserve c16 and enforce an explicit B=2 contract; production builds have no observer. Oracle mode remains [profile_vllm_online_gate.py](../tools/bench/profile_vllm_online_gate.py#L21) | [mode/finalizer contracts](../tests/tools/test_online_gate_trace.py#L30), [async summary cases](../tests/tools/test_async_credit_summary.py#L1), and requested-batch/graph-contract rejection in [client contracts](../tests/tools/test_online_gate_client.py#L860) are green. Binding `3f256ab` stays **55/124**. Oracle c2 has **1,524×1,160** kernels. First local `ad8b58f` passes the model gate and one four-range session (**1,011+7+1**, lossless, identical), then **FAILS / VOIDS** on the former c16-only 1,107 assertion; sessions 2/3 and fresh oracle are absent. Batch-aware retry/final status is **PENDING**, so no speed credit; host PSS/RSS remains **22.920 GiB** CPU weights plus mmap pages, W3-I is off and 35B blocked | [CUDA inventory](specs/cuda-architecture-inventory.md); [online gate](specs/cuda-online-serving-gate.md); [W3-I](specs/nvfp4-fused-silu-producer.md) | `PARTIAL` | - |
