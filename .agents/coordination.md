...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local `/home/mudler/_git/vllm.cpp-serve-gate`; closed diagnostic DGX dir `~/work/vllm.cpp-latency`; binding evidence root `~/work/vllm.cpp-online-gate` | `codex/serve-gate-harness` (merged `b0acec2`; corrected trace/gate contract active) | `tools/bench/online_gate*`, closed-loop `profile_vllm_online_gate.py`, `summarize_torch_kernels.py`, `scripts/dgx-online-serving.sh`, `tests/tools/test_online_gate_*`; cache-eviction helper; explicit server scheduling/cache CLI; DGX exact build/harness/results; matrix/roadmap/ledger/state. ONE `flock` around each complete model A/B+trace series | `ACTIVE` | 2026-07-11 (exact pushed-`f065fce` 27B series complete: 2,016/2,016 timed requests, six memory returns, model and trace contracts pass. Mean c1→c32 throughput ratios 0.9616/0.9258/0.9364/0.9450/0.9775/0.9722×; 3–5/20 timing axes and 2/4 memory axes pass. Corrected trace-status SHA-256 `f8de…7e2a`; no parity claim. Historical denominators remain void. `KV-DEVICE-RESIDENCY` owns the trace-grounded repair) |
| `CLAIM-KV-DEVICE-1` | `KV-DEVICE-RESIDENCY` | root | main worktree `/home/mudler/_git/vllm.cpp`; baseline evidence `~/work/vllm.cpp-online-gate/evidence/f065fce18027b52a178b246f490b091b9d9e07a3`; W0 evidence `~/work/vllm.cpp-kv-device/7d29e0cd69709f6f96ebbf86a320f8885d7362d1/{w0-ab-27,w0-sanitizer,w0-sanitizer-access,w0-sanitizer-leaks-35}`; W1 evidence `~/work/vllm.cpp-kv-device/w1-precommit-bec1ff5fd5f3/{evidence,w1-ab-27}` | `main` | W0 move-only backend cache ownership/device residency plus W1 persistent metadata/indexed GDN gather-scatter; next fresh direct/online oracle closure, W2 only if a gap remains. Primary files `include/vllm/v1/worker/gpu/runner.h`, `src/vllm/v1/worker/gpu/runner.cpp`, `src/vllm/model_executor/models/qwen3_5.cpp`, vt CUDA op/tests, owning spec/matrices/README/BENCHMARKS/ledger/state. Every GPU A/B/trace series holds one uncontended lock | `ACTIVE` | 2026-07-11 (W0 A/B 1.021239×/20-of-20; both-model access clean; inherited 47.29 MB/36.82 GB pools fail zero-leak with W0 caches absent. W1 focused CPU/sanitizer/CUDA/op, indexed/fallback 27B+35B and turnover gates pass; c16/48 AB/BA/AB is 781.799/776.946 tok/s = 1.006246×, 20/20 axes, six memory returns. Trace collapses async copies 163,540→7,508 and D2D 142,717→1,231; profiler throughput is perturbed/inverted and non-binding. Manifest `34285a91…a5b`. Fresh vLLM parity and teardown remain required; no oracle parity claim) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + `KV-DEVICE-RESIDENCY` | Exact f065 27B before-state is below floor. W0 is green at +2.1239%/20-of-20. W1 indexed state I/O is focused-gated at +0.6246%/20-of-20 and collapses traced async copies 163,540→7,508; inherited pools still fail zero-leak | run fresh exact direct-library/online 27B parity now; implement W2 direct indexed convolution update only if a gap survives, then close 35B. Repair inherited pool teardown before row closure. Later roadmap blocks remain behind this priority-zero gate | `ACTIVE` |
| 2 | `SERVE-ASYNC-LLM` block (`ENG-CORE-BUSY-LOOP`, `SERVE-ASYNC-LLM`, `ENG-ASYNC-SCHED`, `ENG-PRIORITY-SCHED`) | joint spike accepted ([async-serving.md](specs/async-serving.md)); W2 removes the fake-SSE structural blocker, but `SERVE-GATE-ONLINE` cannot close until its real GB10 gates pass | W1 + W2 + W4 are implemented and `GATING` with claims released. W2 CPU full suite 94/94, focused TSan 5/5, live completion/chat SSE, disconnect abort, concurrent HTTP requests and 17-symbol C ABI are green. The void `a40a9e3` campaign exercised the post-W2 server and proved live/native-count behavior, but supplies no gate result. The fresh whole-series lock must close W1 G1/G4, W2 G1/G3/G4/G6 and W4 G1; W3 overlap remains READY | `GATING` |
