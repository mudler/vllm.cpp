...
| `CLAIM-NVFP4-SMALL-M-3` | `KERNEL-GEMM-NVFP4-W4A4` (`W3`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; immutable W3-A evidence `~/work/vllm.cpp-nvfp4-small-m/71f1e894d0c5e496607d08cfe9089a9944128271/w3`; immutable W3-B evidence `~/work/vllm.cpp-nvfp4-small-m/d7cdf66db0cfcc53d68d49613623ec6cd3807641/w3b`; exact online evidence `~/work/vllm.cpp-online-gate/evidence/3cc490cfa6314e81a69451c5f175b071e7970506` | `codex/nvfp4-small-m` | W3 only: A mirrors production pip-vLLM/FlashInfer timing; B owns shared-loader max-token/all-bucket warmup and its exact 27B classification; C remains separately optional. No new quant layout, alpha residency, topology, HTTP, scheduler, GDN or device-residency change before W3 closes | `ACTIVE` | 2026-07-12 (`3cc490c` exact campaign active under one uncontended model-wide lock. Commit-bound 27B gate and deterministic corpus/cache preconditions pass; ours repetition 1 retained c1-c32 and returned memory; vLLM repetition 1 is running. No partial ratio binds, `b5c6e4f` remains the floor, and 35B is held) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + FP4 repair | Exact clean `b5c6e4f` remains binding below floor. Corrected W3-B tracing is lifecycle-clean and structurally closes the original tactic mismatch. Clean pushed `3cc490c` is now active in the replacement exact c1-c32 campaign; its model/corpus/cache preconditions, ours repetition 1 and return pass, while vLLM repetition 1 runs. No partial rate binds | finish all 36 timed groups, six returns and paired trace; classify the 27-only summary, then amend the spike for the highest-ranked remaining executed-kernel gap if any axis remains red. Gate W3-C separately; run 35B only after full 27B closure | `ACTIVE` |
