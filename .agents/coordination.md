...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; active worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; accepted H1d root `~/work/vllm.cpp-executed-path-refresh-h1d/c498a4131af7e6cf0ac678841212af80f4f12d53`; validator root `~/work/vllm.cpp-trace-validator/71128642ce04c191f559ea4ccabe4b7e33a66b0f`; superseded roots remain in state/ledger only | current checkpoint `codex/nvfp4-small-m` | online harness/evidence/matrices, completed W3-G gates, [W3-H](specs/nvfp4-bf16-producer-vectorization.md), and accepted [W3-I fused-producer spike](specs/nvfp4-fused-silu-producer.md). Claim now extends only through opt-in W3-I1 implementation, tests, SASS/trace/model checks and its strict c2/c16 component. Normal H2, W3-I2+, host-weight, GDN, scheduler, exact grid and 35B performance remain excluded until the preceding gate passes. ONE `flock` around any complete GPU series | `ACTIVE` | 2026-07-13 (W3-I0 complete: generated zero-buffer lifecycle plus local/vLLM 1,480/384-instruction body gap grounded; I1 `READY`, not implemented. `3f256ab` remains 55/124) |
...
spike and authorizes only opt-in I1 implementation plus its precommitted gates;
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + W3-I fused producer | `3f256ab` remains 55/124; W3-E/W3-F/W3-G earn no speed credit. Schema-v5 `c498a413` selects fused SiLU→FP4 in 12/12 reports; W3-I0 source/generated-code/SASS/test/gate spike is accepted. GPU/lock/ports are idle | implement opt-in W3-I1, pass operator/model/sanitizer/SASS/trace gates, then run the complete c2/c16 48-axis component. No normal H2, I2+, exact grid, or 35B performance before it passes | `ACTIVE` |
