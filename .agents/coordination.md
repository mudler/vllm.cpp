...
|---|---|---|---|---|---|---|---|
| `CLAIM-PR3` | `KERNEL-GDN-AOT-BF16`, `KERNEL-GDN-SCRATCH` | root takeover of stopped `validate_pr3` / `complete_pr3` stream | primary recovery tree `/home/mudler/_git/vllm.cpp-pr3-validate`; 27B default/component integration in `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; DGX evidence `~/work/vllm.cpp-noPy` plus `~/work/vllm.cpp-nvfp4-small-m/debug/gdn-out-bf16-c16-ab-20260712` | recovery branch integrated into `main` by `a767188`; current checkpoint on `codex/nvfp4-small-m` | PR #3 files and rows `KERNEL-GDN-AOT-BF16`+`KERNEL-GDN-SCRATCH`; 27B-only `GdnOutDType` default/f32 override in `qwen3_5.cpp`; ledger/inventory/roadmap evidence. No 35B default change. GPU lock: residual trace/pool classification remains separate from FP4 W3 | `ACTIVE` | 2026-07-13 (vendored BF16 H32/H48 AOT/safety evidence and native 16/16 correctness remain green. Binding `3f256ab` has c16 total at 1.027889× but mean TPOT/ITL at 0.987450×. Cross-profiler GDN totals remain diagnostic only; W3-E direct scales is selected ahead of another GDN change. All 35B paths keep f32) |
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local `/home/mudler/_git/vllm.cpp-serve-gate`; binding grid `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; W3-D component/post-pack trace as recorded | `codex/serve-gate-harness` (merged `b0acec2`; current checkpoint on `codex/nvfp4-small-m`) | `tools/bench/online_gate*`, profiler/summarizer/driver contracts, exact DGX evidence, matrices/roadmap/ledger/state. ONE `flock` around each complete model A/B+trace series | `ACTIVE` | 2026-07-13 (`3f256ab` completes 36/36 groups, 2,016 requests, six returns and paired trace; 55/124 pass, 69 fail. Norm+FP4 is refuted; W3-E direct scales is trace/source-selected `READY`. Its implementation/component gate precedes any exact rerun; 35B held) |
...
| `CLAIM-NVFP4-SMALL-M-3` | `KERNEL-GEMM-NVFP4-W4A4` (`W3`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; immutable W3-A/B and binding evidence as above; W3-D component/post-pack trace and exact grid roots listed in `CLAIM-SERVE-GATE-1` | `codex/nvfp4-small-m` | W3 owns FP4 tactic/topology repair. C persistence stays optional; D owns packed QKV and `VT_FP4_MERGED_QKV=0`. Accepted W3-E scope is only explicit linear/direct-swizzled producer layouts, padded zero contract, true-W4A4 CUTLASS dispatch, ported tests and `VT_FP4_DIRECT_SF=0` per [spike](specs/nvfp4-direct-swizzled-scales.md). No other lever stacks into its A/B | `ACTIVE` | 2026-07-13 (W3-D complete/strict-failed as recorded. Exact body/source scan attributes **320,099 / 1.238881 s** local standalone swizzles and zero vLLM standalone equivalents. W3-E is `READY`; no code/test/A-B/speed result exists yet. Do not run 35B performance) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + FP4 repair | Binding immutable `3f256ab` v0.25 27B grid/trace is complete: 55/124 pass, 69 fail. W3-D closes 240→208; norm+FP4 is refuted. Clean local slice selects W3-E from **320,099 / 1.238881 s** standalone swizzles versus vLLM direct in-producer addresses. GPU/ports/lock idle | implement W3-E explicit layouts/padding/fallback and port tests; run sanitizer, both model gates, node trace and c2/c16 same-binary A/B. Exact rerun only if the component accepts. Modernize/delete only trace-proven paths; 35B performance only after full 27B closure | `ACTIVE` |
