...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; active worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; finalized c2 root `~/work/vllm.cpp-executed-path-c2/179a0fc2afc1c33b63d14de8e50d3fde976c7356`; W1 safety root `~/work/vllm.cpp-gdn-ba/immutable-581d335fec2e5a96d9ccbb38c1ec001c39ac1789`; completed structural root `~/work/vllm.cpp-gdn-ba-trace/0091cd192d9a6baa2197a4f3bdb0561bd859baf5` | current checkpoint `codex/nvfp4-small-m` | Online-gate execution and harness work, including immutable evidence for the now-unclaimed `GATING` W1 BA implementation. qkvz code, loader-memory repair, exact grid, and 35B performance are excluded. Any A/B or trace series uses one `flock`; further `KERNEL-GEMM-BF16` production code requires a fresh `ACTIVE` claim | `ACTIVE` | 2026-07-14 (W1 core correctness/safety remains closed at `581d335`; BF16 output fails **233/235**. Immutable `0091cd1`, finalized by pushed `8a1f923`, is `complete-structural`: 12/12 merged 963/145 versus 12/12 split 1,011/193, exact 48-BF16-only removal, unchanged selected non-BF16 families, `benchmark_binding=false`. Close GGUF/legacy inertness, BF16 rounding and c2/c16 component next; binding 55/124, no speed credit) |
| `CLAIM-GDN-BA-ROUNDING-1` | `KERNEL-GEMM-BF16` (`W1C`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-gdn-ba-rounding`; planned DGX root `~/work/vllm.cpp-gdn-ba-rounding/<pushed-sha>` | `codex/gdn-ba-rounding-w1c` | Leads only `ProjectGdnBA` output-dtype/algorithm grounding in `src/vllm/model_executor/models/qwen3_5.cpp`, its projection-oracle and 35B legacy/GGUF inertness tests, and the c2/c16 same-binary BA component harness/evidence. This is the explicit overlap lead over `CLAIM-PR3` for that function and over `CLAIM-SERVE-GATE-1` for the W1C component. qkvz, host-weight repair, binding-grid execution and 35B performance are excluded. One `flock /tmp/gpu` covers every GPU correctness or A/B series | `ACTIVE` | 2026-07-14 (default-off BF16 output, exact full-digest projection test and 35B/GGUF selection assertions implemented. vLLM 0.25.0 GB10 M=1/2/4/16/32 is stable 3×; synthetic GGUF 97/97 and CPU parity 123/123 pass. Local CUDA/model/native-legacy-35B/real-GGUF gates and 40+8 component remain pending; no timing, memory or speed result) |
...
GGUF/legacy inertness, exact BF16 rounding and the component result remain open.
...
|---|---|---|---|---|
| 1 | `KERNEL-GEMM-BF16` merged GDN projections for `SERVE-GATE-ONLINE` | `3f256ab` remains 55/124; pushed `581d335` closes F32-output core correctness/safety; finalized `0091cd1` closes exact structure with `benchmark_binding=false` | close GGUF/legacy inertness and BF16 rounding, then complete c2/c16 before qkvz | `GATING` |
