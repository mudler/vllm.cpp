...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local `/home/mudler/_git/vllm.cpp-serve-gate`; closed diagnostic DGX dir `~/work/vllm.cpp-latency`; binding evidence root `~/work/vllm.cpp-online-gate` | `codex/serve-gate-harness` (merged `b0acec2`; merged-DELTA validator repair active) | committed `tools/bench/online_gate*`, `tools/bench/profile_vllm_online_gate.py`, `tools/bench/summarize_torch_kernels.py`, `scripts/dgx-online-serving.sh`, `tests/tools/test_online_gate_*`; rootless verified cache-eviction helper `tools/bench/drop_file_cache.py`; explicit server scheduling-capacity CLI in `examples/server/main.cpp` + help test/README; DGX current-main build/harness/results; engine matrix, roadmap, ledger/state. ONE `flock` around each complete model A/B+trace series | `ACTIVE` | 2026-07-11 (`a40a9e3` proved exact native usage on 1,488 retained 27B requests, then stopped after c16-r3 because the validator rejected 12 upstream-legal producer-ahead merged DELTAs with 126 ITLs. Evidence is void; GPU is idle/unlocked. Merge the corrected inter-chunk contract, regenerate every commit-bound artifact, then restart 27B followed by 35B. No prior ratio is reusable) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` | All partial campaigns are invalid. `a40a9e3` closed the missing-usage symptom across 1,488 retained requests, but its validator stopped on pinned-vLLM's legal producer-ahead merged DELTAs before the third pair/traces/finalization | merge the inter-chunk validator repair, regenerate commit-bound corpus/evidence, and restart 27B followed by 35B under new whole-model locks | `ACTIVE` |
