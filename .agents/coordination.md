...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + W3-H executed-path refresh | `3f256ab` remains 55/124; W3-C reproduction control is green, while W3-E/W3-F/W3-G strict-fail and earn no speed credit. W3-H H1a/H1b/H1c are `VOID`; H1c failed closed on capture-1 event loss and also lacks plan-log proof. Implementation remains prohibited; GPU/lock/ports are idle | harden lossless collection and exact plan/workload/graph/capture/vLLM semantics, then run immutable H1d at cache-off input-1,024/output-128 with the exact read-only 64-plan fixture. Implement only the highest losslessly verified lever; no exact grid/35B performance until a component passes every axis | `ACTIVE` |
