...
|---|---|---|---|---|---|---|---|
| `CLAIM-BACKEND-ABI-VT-W0-1` | `BACKEND-ABI-VT` | `backend-abi-vt-w0` (Codex) | local `/home/mudler/_git/vllm.cpp-backend-abi-w0` (own build dirs); planned DGX `~/work/vllm.cpp-backend-abi-w0` after the active serve claim releases the GPU | `codex/backend-abi-vt-w0` -> `main` | W0 ABI spine only: common semantic scalar/layout descriptor; `include/vt/{device,backend,ops}.h`; `src/vt/{backend,ops}.cpp`; new `src/vt/cuda/cuda_dropin.{h,cu}`; new `tests/vt/test_dropin_abi.cpp`; narrow source/test-list lines in `CMakeLists.txt` and `tests/CMakeLists.txt`; `BACKEND-ABI-VT` matrix/ROAD-V1-C1/coordination/README-if-visible/porting-inventory/ledger/state closure lines. Shared record and CMake files are row/line-scoped; this claim leads only its W0 lines and rebases around other claims. No production adapter or kernel-family TU. GPU lock plan: one flock-held W0 CUDA probe/test window after `CLAIM-SERVE-GATE-1`. | `ACTIVE` | 2026-07-10 (W0 claimed before implementation edits) |
| `CLAIM-PR3` | `KERNEL-GDN-AOT-BF16`, `KERNEL-GDN-SCRATCH` | root takeover of stopped `validate_pr3` / `complete_pr3` stream | local `/home/mudler/_git/vllm.cpp-pr3-validate`; DGX `~/work/vllm.cpp-noPy` (own build dir) | `merge-pr3-validation` (PR head `85dfb48`; must integrate current main before closure) | PR #3 files; claim/matrix rows `KERNEL-GDN-AOT-BF16`+`KERNEL-GDN-SCRATCH`; ledger/inventory/roadmap rows for PR #3 closure. GPU lock: queued preliminary test series + one pool A/B series, then final post-integration tests/traces/benchmarks under fresh locks | `ACTIVE` | 2026-07-10 (recovered current worktree: preliminary GPU jobs queued behind serving; review found missing vendored BF16 `chunk_o` artifacts/MANIFEST entries and no assertable Triton-dispatch/dirty-buffer-reuse test, so queued results cannot close the claim) |
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local main (docs only); DGX `~/work/vllm.cpp-latency` (harness/build @ 83010c7) + `~/work/vllm.cpp-latency/latres2` | `main` (docs-only edits per push policy) | DGX latency dir + `lat_*.sh` harness; engine-matrix `SERVE-GATE-ONLINE` row; roadmap `ROAD-V1-A` row; ledger appends. ONE `flock` per model A/B series (27B ours+vLLM single hold; 35B ours+vLLM single hold) | `ACTIVE` | 2026-07-10 (35B exact-shape sanitizer diagnostic survived 3/3 with zero sanitizer reports; second 27B same-lock campaign is active, ours c1/c2 complete and c4 running; vLLM denominator and 35B series remain) |
| `CLAIM-SERVE-ASYNC-W2-1` | `SERVE-ASYNC-LLM` | `resume-roadmap` (Codex root) | local `/home/mudler/_git/vllm.cpp-async-llm-w2` (own CPU build dir); DGX handoff only after `CLAIM-SERVE-GATE-1` releases the GPU | `codex/serve-async-llm-w2` -> `main` | W2 only: `async_llm.{h,cpp}`; collector additions in `output_processor.{h,cpp}`; live chunk sources in completion/chat serving; `api_server.cpp`; additive non-blocking C ABI in `include/vllm.h` + `src/capi/vllm_c.cpp`; server construction/wiring; matching W2 unit/conformance/C-ABI tests and narrow CMake lines; owned `SERVE-ASYNC-LLM` matrix/ROAD-V1-C6/porting-inventory/ledger/state/coordination closure lines. No W3 async scheduler, runner copy-stream/kernel, or GPU execution while the online campaign owns dgx | `ACTIVE` | 2026-07-10 (crash recovery complete; W2 claimed on merged W1 queue split before implementation edits) |
| `CLAIM-MODEL-FACTORY-1` | `MODEL-FACTORY-registry` | `resume_mtp0` (Codex) | local `/home/mudler/_git/vllm.cpp-model-factory-registry` (own CPU build dir); no GPU execution while `CLAIM-SERVE-GATE-1` owns dgx | `codex/model-factory-registry` -> `main` | Registry leaf only: new `include/vllm/model_executor/models/model_registry.h` + `src/vllm/model_executor/models/model_registry.cpp`; superseded `models/registry.{h,cpp}`; `entrypoints/model_loader.{h,cpp}`; only the type-erasure indirection required in `v1/worker/gpu/runner.{h,cpp}`; both existing Qwen registrations; `tests/vllm/models/test_model_registry.cpp`; narrow root/tests CMake lines; owned `MODEL-FACTORY-registry` matrix/ROAD-V1-C2/porting-inventory/ledger/state/coordination closure lines. No new family, suffix normalization, dynamic Transformers/OOT runtime loading, multimodal/pooling/reward routing, or GPU execution | `ACTIVE` | 2026-07-10 (accepted spike claimed before implementation edits) |
...
| 7 | C2/C4 models + quantizations | accepted comprehensive inventories | `MODEL-FACTORY-registry` claimed by `CLAIM-MODEL-FACTORY-1` ([model-factory-registry.md](specs/model-factory-registry.md)); CPU implementation/gates first, then exact two-model GPU no-regression handoff and Llama dense. `QUANT-GGUF-COMPUTE` leaves split — `QUANT-GGUF-CPU-THREADPOOL` claimed by `CLAIM-QGCT-1`; keep-quant loader + CIQ GEMM next | `ACTIVE` |
