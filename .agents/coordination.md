...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local main (docs only); DGX `~/work/vllm.cpp-latency` (harness/build @ 83010c7) + `~/work/vllm.cpp-latency/latres2` | `main` (docs-only edits per push policy) | DGX latency dir + `lat_*.sh` harness; engine-matrix `SERVE-GATE-ONLINE` row; roadmap `ROAD-V1-A` row; ledger appends. ONE `flock` per model A/B series (27B ours+vLLM single hold; 35B ours+vLLM single hold) | `ACTIVE` | 2026-07-11 (35B exact-shape sanitizer diagnostic survived 3/3 with zero reports; second same-lock campaign completed our full 27B arm; matched vLLM 27B greedy c1-c32 completed all three repetitions, sampled c1 completed both repetitions, and sampled c16 repetition 1 is active; its later sampled points and both 35B arms remain) |
| `CLAIM-BACKEND-SGLANG-PREFLIGHT-1` | `BACKEND-BENCH-CUDA-SGLANG-PREFLIGHT` | root | local `/home/mudler/_git/vllm.cpp-sglang-preflight` | `codex/sglang-preflight-p1` | P1 CPU slice only: `tools/bench/*serve_low*`, `scripts/dgx-sglang-low-concurrency.sh`, `tests/tools/test_*serve_low*`, `tests/tools/test_process_memory_sampler.py`, and Python-test CMake integration plus the owning backend/roadmap/record rows. No image pull, SGLang install, checkpoint mutation, DGX command beyond read-only monitoring, or P2/P3 performance claim | `ACTIVE` | 2026-07-11 (accepted spike read; deterministic corpus/client/summary/memory/dry-run contracts bounded; serving campaign retains dgx and provisioning is explicitly deferred) |
...
| 5 | `BACKEND-BENCH-CUDA-SGLANG-PREFLIGHT` | accepted spike; no dependency on async serving | P1 CPU harness/dry-run is active under `CLAIM-BACKEND-SGLANG-PREFLIGHT-1`; provisioning waits for a non-benchmark window, then P2 classifies each exact checkpoint | `ACTIVE` |
