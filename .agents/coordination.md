...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid/trace `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; W3-F immutable evidence `~/work/vllm.cpp-nvfp4-device-alpha/7517af4f983fe322ac88ce2d9869e1441b7be3fd/evidence`; W3-G immutable DGX root `~/work/vllm.cpp-fa2-decode/ae9e8ff0576badabdda7289beeacaa1041c55d21`; W3-H local `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; H1a `~/work/vllm.cpp-executed-path-refresh/5d8af792…`; H1b `~/work/vllm.cpp-executed-path-refresh-frozen-h1b/5d8af792…`; pending immutable H1c root `~/work/vllm.cpp-executed-path-refresh/<SHA>` | current checkpoint `codex/nvfp4-small-m` | online harness/evidence/matrices plus completed W3-G ratio-6 FA2 gates and [W3-H normal-producer spike](specs/nvfp4-bf16-producer-vectorization.md). H1 owns only exact-workload paired ours/vLLM traces, lossless-trace validation and residual re-ranking; implementation is prohibited until lossless H1c. Host-weight, fused producer, GDN changes, scheduler and 35B performance excluded. ONE `flock` around the complete three-capture plus vLLM series | `ACTIVE` | 2026-07-13 (W3-G remains without speed credit. H1a is `VOID`: writable cache changed 37/64 tactics. Frozen-map H1b completed three 48/48 legs and vLLM but is `VOID`: Nsight reported lost CUDA events and the 1,107-node graph has 930 nodes at 1,372 versus 177 at 1,373 replays. The harness now runs three independent captures, flushes every 10 seconds, hashes/validates each SQLite and fails before vLLM on loss. H1c is `PENDING`; no implementation, ratio, exact grid or 35B performance exists. GPU, `/tmp/gpu` and serving ports are idle; `3f256ab` remains 55/124) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + W3-H executed-path refresh | `3f256ab` remains 55/124; W3-C reproduction control is green, while W3-E/W3-F/W3-G strict-fail and earn no speed credit. W3-H H1a/H1b are both `VOID` from plan-map drift and Nsight event loss; implementation remains prohibited. The three-capture lossless validator is CPU-tested; GPU/lock/ports are idle | push the hardening checkpoint, then run immutable H1c at cache-off input-1,024/output-128 with the exact read-only 64-plan fixture. Each of three independent Nsight reports must have zero rejected diagnostics and uniform dominant-graph replay before vLLM. Implement only the highest losslessly verified lever; no exact grid/35B performance until a component passes every axis | `ACTIVE` |
