...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; active worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; accepted H1d root `~/work/vllm.cpp-executed-path-refresh-h1d/c498a4131af7e6cf0ac678841212af80f4f12d53`; W3-I root `~/work/vllm.cpp-nvfp4-fused/15c6b8933d982019aa8965d218deb0eb1d9dc3f4-r2`; accepted async root `~/work/vllm-async-credit/3812d8d2b4a68d2e501007d01fe10cdf17751d02`; pending c2 root `~/work/vllm.cpp-executed-path-c2/$SHA` | current checkpoint `codex/nvfp4-small-m` | online harness/evidence/matrices and exact low-batch ours/vLLM RMSNorm/generated-partition + FP4-tactic mapping. `ENG-ASYNC-SCHED`, loader/memory implementation, W3-H2/I2+, exact-grid, and 35B implementation remain excluded until a complete kernel lever is selected. Any A/B or trace series uses ONE `flock` | `ACTIVE` | 2026-07-14 (oracle c2 contract fixed at **1,524 B=2 windows / 1,160 kernels each**, seven generated families, 128+80 FP4 tactics. Trace-only local B=2 observer/harness ready; fresh paired capture/finalization pending. Binding 55/124; no speed credit) |
...
The accepted oracle c2 trace now fixes its exact topology and FP4 tactic split;
the trace-only local observer can target B=2 without changing production builds.
Fresh paired capture/finalization is still pending. W3-H2, W3-I2+, the exact grid, and 35B
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` low-batch kernel mapping | `3f256ab` remains 55/124; accepted oracle c2 trace is exact at 1,524 B=2 windows and 1,160 kernels/window, while local B=2 evidence is pending | run/finalize the three-session paired c2 capture, map local RMSNorm nodes and FP4 tactics against the fixed oracle topology, then spike/gate the highest complete 4–6% lever before the exact grid | `ACTIVE` |
