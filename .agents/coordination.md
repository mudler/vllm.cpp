...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; active worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; finalized c2 root `~/work/vllm.cpp-executed-path-c2/179a0fc2afc1c33b63d14de8e50d3fde976c7356`; packed trace root `~/work/vllm.cpp-gdn-packed-trace/7ff713e377457130db4ed15929133d1b463aff96` | current checkpoint `codex/nvfp4-small-m` | Online-gate execution and harness work. `CLAIM-GDN-BA-ROUNDING-1` leads packed-decode code and its c2/c16 component; this claim retains the binding grid. qkvz code, loader-memory repair, exact grid, and 35B performance are excluded. Any A/B or trace series uses one `flock` | `ACTIVE` | 2026-07-14 (packed structure is accepted. The pre-GPU invocation defect is repaired at focused **45/45**, all-tools **127/127**; fresh-SHA component execution is next. Binding stays 55/124 with no speed credit) |
| `CLAIM-GDN-BA-ROUNDING-1` | `KERNEL-GEMM-BF16` (`W1C`), `KERNEL-GDN-PACKED-DECODE` (`W1D`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-gdn-ba-rounding`; immutable G2 root `~/work/vllm.cpp-gdn-packed-decode/f344decf457a4d50c3bcae78a2903d7fe176a511/evidence-g2`; accepted trace root `~/work/vllm.cpp-gdn-packed-trace/7ff713e377457130db4ed15929133d1b463aff96`; failed pre-GPU component root `~/work/vllm.cpp-gdn-packed-component/593996d7a40ad323834a96eaa542c943142f788e`; next root `~/work/vllm.cpp-gdn-packed-component/<pushed-repair-sha>` | `codex/gdn-ba-rounding-w1c` | Leads exact packed decode, model dispatch/rollback, trace and c2/c16 component. Prefill/mixed/spec GDN, qkvz, host-weight repair, binding-grid execution and 35B performance are excluded. One `flock /tmp/gpu` covers every GPU correctness or A/B series | `ACTIVE` | 2026-07-14 (test-first exact `--profile-control off` repair passes focused **45/45**, all tools **127/127**, shell and dry-run gates. Push, create a new SHA-owned root, then execute the one-lock **40+8** series. Speed credit remains absent) |
...
runner/finalizer is CPU-green **45/45**; its one-lock DGX series is the only
remaining packed component checkpoint. The missing `--profile-control off` is
test-first repaired; focused **45/45** and all-tools **127/127** pass. Push and
use a fresh root next. qkvz, exact-grid and 35B performance stay excluded until
that gate closes.
...
|---|---|---|---|---|
| 1 | `KERNEL-GEMM-BF16` + `KERNEL-GDN-PACKED-DECODE` for `SERVE-GATE-ONLINE` | `3f256ab` remains 55/124; correctness/structure close at `f344dec` and `7ff713e` + `24cea4f`; repaired driver is focused **45/45**, all-tools **127/127** | push, then execute/finalize the fresh-SHA one-lock c2/c16 **40+8** series before qkvz | `GATING` |
