...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; active worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; accepted H1d root `~/work/vllm.cpp-executed-path-refresh-h1d/c498a4131af7e6cf0ac678841212af80f4f12d53`; W3-I root `~/work/vllm.cpp-nvfp4-fused/15c6b8933d982019aa8965d218deb0eb1d9dc3f4-r2`; accepted async root `~/work/vllm-async-credit/3812d8d2b4a68d2e501007d01fe10cdf17751d02`; failed c2 root `~/work/vllm.cpp-executed-path-c2/ad8b58f8708ce9bdf32aa9043611b3f6049be7fd`; pending retry `~/work/vllm.cpp-executed-path-c2/$SHA` | current checkpoint `codex/nvfp4-small-m` | online harness/evidence/matrices and exact low-batch ours/vLLM RMSNorm/generated-partition + FP4-tactic mapping. `ENG-ASYNC-SCHED`, loader/memory implementation, W3-H2/I2+, exact-grid, and 35B implementation remain excluded until a complete kernel lever is selected. Any A/B or trace series uses ONE `flock` | `ACTIVE` | 2026-07-14 (`ad8b58f` model gate + one four-range B=2 session pass; all ranges are lossless/identical at **1,011+7+1**. Run **VOID** on stale c16 1,107-kernel assertion before sessions 2/3/oracle. Batch-aware repair green; fresh full retry/finalization pending. Binding 55/124; no speed credit) |
...
The accepted oracle c2 trace fixes its exact topology. The first local B=2
attempt is void after one valid four-range session exposed a c16-only validator
assertion; the batch-aware repair is green and a fresh complete retry is pending.
W3-H2, W3-I2+, the exact grid, and 35B
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` low-batch kernel mapping | `3f256ab` remains 55/124; oracle is exact. Void `ad8b58f` contributes one lossless local session only; batch-aware validator repair is ready | repeat/finalize all three local c2 sessions plus fresh oracle, map RMSNorm nodes/tactics, then spike/gate the highest complete 4–6% lever before the exact grid | `ACTIVE` |
