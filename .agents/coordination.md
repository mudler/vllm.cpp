...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid/trace `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; W3-F immutable evidence `~/work/vllm.cpp-nvfp4-device-alpha/7517af4f983fe322ac88ce2d9869e1441b7be3fd/evidence`; W3-G immutable DGX root `~/work/vllm.cpp-fa2-decode/ae9e8ff0576badabdda7289beeacaa1041c55d21`; W3-H local `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; H1a `~/work/vllm.cpp-executed-path-refresh/5d8af792…`; H1b `~/work/vllm.cpp-executed-path-refresh-frozen-h1b/5d8af792…`; H1c `~/work/vllm.cpp-executed-path-refresh-h1c/d1f8e332…`; H1d failed roots `~/work/vllm.cpp-executed-path-refresh-h1d/{7bae38afe8f97603fec525e8a2b8833bcba983cf,2d16c681f3fe64130a7ab9b6fc55ad7e21c0808e,5f8fab17042ce2939115a3f6ac64b1b9418136c8,d063f20f340cdfeab79c95a61c6fc08c82c1f4a9,b1c7eb6a605df754a958799c42010b2342636354,e1acb7563cb444b1378245f096031a1ea7ce3bcf,a96e89929e76b8965d2c9328e4de91690e3ff984,3c1d7b79243ac173e2828e1fb3a74de4e72fbd35,219f4f2449aaf6afe7a9a8a41a0dabb58c85e0f3,b2c940cef40ac3d0852352d81ac5ca4448a213e5}`; schema-v4 3-session x 4-report synchronous-flush repair implemented/CPU-gated, fresh immutable DGX root pending | current checkpoint `codex/nvfp4-small-m` | online harness/evidence/matrices plus completed W3-G ratio-6 FA2 gates and [W3-H normal-producer spike](specs/nvfp4-bf16-producer-vectorization.md). H1 owns the diagnostic-only four-replay controller, graceful target stop, target-build/dispatch manifest, exact-workload paired ours/vLLM trace validation and residual re-ranking; W3-H2 inference-kernel implementation remains prohibited until valid H1d evidence. Host-weight, fused producer, GDN changes, scheduler and 35B performance excluded. ONE `flock` around the complete series | `ACTIVE` | 2026-07-13 (H1a/H1b/H1c and H1d attempts through `b2c940c` are `VOID`. `b2c940c` completed exact **154/154** build, 27B gate **1/1 in 17.30 s**, both clients, frozen plans, FIFO removal and zero-exit profiler. Four reports each have one complete 1,107-kernel replay and zero eager CUDA rows, but all emit severity-2 possible loss and schema v3 expects one combined report. Schema v4 is CPU-gated at 31/31 focused and final 106/106 CUDA-off. No ratio, exact grid or 35B performance exists. GPU, `/tmp/gpu` and ports are idle; fresh immutable DGX execution is next and `3f256ab` remains 55/124) |
...
`CLAIM-SERVE-GATE-1` H1d's frozen trace-only controller, build manifest and
schema-v4 validators are implemented and CPU-gated. `219f4f2` proves the FIFO
shutdown path but exposes the continuous-range sampler gap. `b2c940c` proves
four isolated reports are exact and zero-eager, but every report retains
severity-2 possible loss and schema v3 expects one combined report. Schema v4,
the 3-session x 4-report artifact contract and synchronous generation/device
flush are now implemented; a fresh immutable DGX execution is next. The
claim continues to own only this diagnostic harness and its resulting paired
traces. The retained
`7bae38a`/`2d16c68` pre-build failures, invalid-config `5f8fab1` correctness
failure, pre-plan `d063f20` artifact-order failure, pre-build `b1c7eb6`
bootstrap failure, exact-build/gate `e1acb75` profiler-topology failure and
exact-four-replay `a96e899` exit-143 plus `3c1d7b7` exit-138 failures,
exact-FIFO/zero-exit `219f4f2` continuous-range failure and `b2c940c`
multi-report/loss failure cannot be reused;
no W3-H2 inference-kernel code is authorized before three valid target-path
captures re-rank the residual.

## Handoff queue

| Priority | Row/block | Dependency | Next handoff | State |
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + W3-H executed-path refresh | `3f256ab` remains 55/124; W3-E/W3-F/W3-G strict-fail. H1a/H1b/H1c and H1d attempts through `b2c940c` are `VOID`; the latest proves exact build/gate, frozen plans, FIFO/zero-exit and four complete single-replay reports with zero eager CUDA rows, but all emit severity-2 possible loss and schema v3 expects one combined report. Schema v4 is CPU-gated; GPU/lock/ports are idle | execute a fresh immutable root and require correctness plus 12 zero-eager/lossless reports, direct graph-child correlation, identical resources and exact plans before implementing any lever. No exact grid/35B performance until a later component passes every axis | `ACTIVE` |
