...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; active worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; finalized c2 root `~/work/vllm.cpp-executed-path-c2/179a0fc2afc1c33b63d14de8e50d3fde976c7356`; W1 safety root `~/work/vllm.cpp-gdn-ba/immutable-581d335fec2e5a96d9ccbb38c1ec001c39ac1789`; completed structural root `~/work/vllm.cpp-gdn-ba-trace/0091cd192d9a6baa2197a4f3bdb0561bd859baf5` | current checkpoint `codex/nvfp4-small-m` | Online-gate execution and harness work, including immutable evidence for the now-unclaimed `GATING` W1 BA implementation. qkvz code, loader-memory repair, exact grid, and 35B performance are excluded. Any A/B or trace series uses one `flock`; further `KERNEL-GEMM-BF16` production code requires a fresh `ACTIVE` claim | `ACTIVE` | 2026-07-14 (W1 core safety and exact structure remain closed. Active W1C root `f925294` passes projection 14/14 and native/legacy/GGUF inertness but BF16 27B remains 233/235, so downstream first-divergence and c2/c16 remain open. Binding 55/124, no speed credit) |
| `CLAIM-GDN-BA-ROUNDING-1` | `KERNEL-GEMM-BF16` (`W1C`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-gdn-ba-rounding`; DGX root `~/work/vllm.cpp-gdn-ba-rounding/f9252943d1e96dbfa43e3b8f2d06dec1aa5f20d3` | `codex/gdn-ba-rounding-w1c` | Leads only `ProjectGdnBA` output-dtype/algorithm grounding in `src/vllm/model_executor/models/qwen3_5.cpp`, its projection-oracle and 35B legacy/GGUF inertness tests, downstream b/a first-divergence instrumentation, and the c2/c16 same-binary BA component harness/evidence. This is the explicit overlap lead over `CLAIM-PR3` for that function and over `CLAIM-SERVE-GATE-1` for the W1C component. qkvz, host-weight repair, binding-grid execution and 35B performance are excluded. One `flock /tmp/gpu` covers every GPU correctness or A/B series | `ACTIVE` | 2026-07-14 (immutable local projection 14/14, loader 73/73, native 35B 315/315, real GGUF 28/28 and default 27B 235/235 pass. BF16 27B is 233/235 and exactly emulation, localizing the gap downstream of GEMM. Compare packed BA, contiguous b/a and consumer boundaries before any component timing; no speed credit) |
...
Projection and 35B/GGUF inertness close at `f925294`; downstream BF16 semantics
and the component result remain open.
...
|---|---|---|---|---|
| 1 | `KERNEL-GEMM-BF16` merged GDN projections for `SERVE-GATE-ONLINE` | `3f256ab` remains 55/124; pushed `581d335` closes F32-output core correctness/safety; finalized `0091cd1` closes exact structure; `f925294` closes projection/inertness but BF16 27B is 233/235 | identify the first downstream b/a divergence and restore token-exact BF16 semantics, then complete c2/c16 before qkvz | `GATING` |
