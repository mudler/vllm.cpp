...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; active worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; accepted H1d root `~/work/vllm.cpp-executed-path-refresh-h1d/c498a4131af7e6cf0ac678841212af80f4f12d53`; accepted async root `~/work/vllm-async-credit/3812d8d2b4a68d2e501007d01fe10cdf17751d02`; finalized c2 root `~/work/vllm.cpp-executed-path-c2/179a0fc2afc1c33b63d14de8e50d3fde976c7356` | current checkpoint `codex/nvfp4-small-m` | online harness/evidence/matrices and exact low-batch ours/vLLM BF16-GEMM/RMSNorm/FP4-tactic mapping. `ENG-ASYNC-SCHED`, loader/memory implementation, W3-H2/I2+, exact-grid, and 35B implementation remain excluded until a complete kernel lever is spiked. Any A/B or trace series uses ONE `flock` | `ACTIVE` | 2026-07-14 (`179a0fc` c2 is `complete-diagnostic`: model gate pass, **12/12** invariant local **1,011+7+1** ranges, fresh oracle **1,522×1,160** steady B=2 windows + two drains; status `9e0143fa…7b57`. BF16-GEMM launch parity is the next whole-chain spike. Binding 55/124; no speed credit) |
...
Complete `179a0fc` raw c2 evidence fixes both sides: 12/12 invariant local B=2
ranges and 1,522 exact oracle B=2 windows plus two bounded drains. Committed
finalizer `fe28003` records `complete-diagnostic`; summary / manifest / status /
artifact-set SHA values are `0ef6a124…0273` / `2556cfd0…2f21` /
`9e0143fa…7b57` / `cc248ad2…823a`.
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` low-batch kernel mapping | `3f256ab` remains 55/124; finalized `179a0fc` c2 status `9e0143fa…7b57` is `complete-diagnostic` and ranks the BF16-GEMM +96-launch residual first | whole-chain spike and gate BF16-GEMM launch parity before the exact grid | `ACTIVE` |
