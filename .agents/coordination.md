...
|---|---|---|---|---|---|---|---|
| `CLAIM-PR3` | `KERNEL-GDN-AOT-BF16`, `KERNEL-GDN-SCRATCH` | root takeover of stopped `validate_pr3` / `complete_pr3` stream | primary recovery tree `/home/mudler/_git/vllm.cpp-pr3-validate`; 27B default/component integration in `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; DGX evidence `~/work/vllm.cpp-noPy` plus `~/work/vllm.cpp-nvfp4-small-m/debug/gdn-out-bf16-c16-ab-20260712` | recovery branch integrated into `main` by `a767188`; current checkpoint on `codex/nvfp4-small-m` | PR #3 files and rows `KERNEL-GDN-AOT-BF16`+`KERNEL-GDN-SCRATCH`; 27B-only `GdnOutDType` default/f32 override in `qwen3_5.cpp`; ledger/inventory/roadmap evidence. No 35B default change. GPU lock: final paired trace/pool classification and fresh-vLLM comparison remain separate from FP4 exact campaign | `ACTIVE` | 2026-07-12 (vendored BF16 H32/H48 AOT/safety evidence remains green. 27B BF16 now restores native 16/16 tokens; c16 BF16/f32 means 789.279/783.023 = 1.007989×, 16/20 timing and 2/4 memory axes, six returns, summary `ee6d25c…c930b`. Strict component/trace/fresh-vLLM closure remains; all 35B paths keep f32 default) |
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local `/home/mudler/_git/vllm.cpp-serve-gate`; binding evidence root `~/work/vllm.cpp-online-gate` | `codex/serve-gate-harness` (merged `b0acec2`; corrected trace/gate contract active) | `tools/bench/online_gate*`, profiler/summarizer/driver contracts, exact DGX evidence, matrices/roadmap/ledger/state. ONE `flock` around each complete model A/B+trace series | `ACTIVE` | 2026-07-12 (exact pushed-`bce2627` 27B remains the binding before-state: 12/12 groups, 2,016/2,016 requests, six returns, model/trace pass; c1→c32 total ratios 0.9680/0.9317/0.9403/0.9516/0.9944/1.0073×, 4/4/5/4/4/12 timing and 2/4 memory axes. W2 implementation/component gates are complete enough for the next immutable-SHA oracle rerun; 35B held) |
| `CLAIM-NVFP4-SMALL-M-2` | `KERNEL-GEMM-NVFP4-W4A4` (`W2`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; DGX staging evidence under `~/work/vllm.cpp-nvfp4-small-m/debug`, immutable evidence will live under `~/work/vllm.cpp-nvfp4-small-m/<pushed-sha>/w2` plus the exact online-gate root | `codex/nvfp4-small-m` | W2 only: split CUDA template/dispatch TUs, exact 32-tactic descriptors/order, swap-AB/static/Stream-K raw launch, high-water workspace, forced-ID/upstream-ported tests, trace-required dense merged gate/up resident + maximum logical-shard CT divisor semantics, and one-input `SiluAndMulFp4Quant`. `VT_FP4_MERGED_SILU_QUANT=0` restores materialized activation+quant, `VT_FP4_MERGED_GATE_UP=0` restores split W2 and `VT_FP4_FULL_TACTICS=0` restores W1. No HTTP, scheduler, unrelated model topology or W3 persistence edits. One uncontended lock for every-tactic sanitizer + component AB/BA/AB + paired trace + exact 27B campaign | `ACTIVE` | 2026-07-12 (32/32 tactics, merged semantics, byte-exact fusion, sanitizer and native 27B gates pass. Fusion c16 A/B is 1.015211× with 17/20 timing and 0/4 sampled-memory axes; paired trace is 1.020843×/20-of-20 and removes ~8k launches. Component strict gate and clean pushed-SHA exact 27B oracle remain; no 35B) |
| `CLAIM-KV-DEVICE-1` | `KV-DEVICE-RESIDENCY` | root | main worktree `/home/mudler/_git/vllm.cpp`; W0/W1 evidence under `~/work/vllm.cpp-kv-device`; current oracle evidence `~/work/vllm.cpp-online-gate/evidence/bce262729726ce56991e5af9f98143227d94f9f4` | `main` | W0/W1 ownership/indexed-state implementation and open teardown; W2 direct convolution update remains scoped but unclaimed until post-FP4 evidence re-ranks it | `ACTIVE` | 2026-07-12 (W0/W1 component A/Bs remain 1.021239×/1.006246× and access/correctness green; inherited pools fail zero-leak. Fresh `bce2627` oracle remains below floor and confirms FP4 W2 first; no device-residency W2 implementation is inferred yet) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + FP4 repair | Exact `bce2627` 27B checkpoint is binding below floor. W2's 32 tactics, merged CT scaling and one-input SiLU→NVFP4 producer are implemented and correctness/sanitizer-gated; the fusion component is positive but misses strict timing/memory axes | checkpoint/push W2, rebuild its immutable SHA, then repeat the exact c1-c32 27B oracle campaign until every performance and memory axis passes; only then run 35B | `ACTIVE` |
