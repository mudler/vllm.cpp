...
| `CLAIM-NVFP4-SMALL-M-3` | `KERNEL-GEMM-NVFP4-W4A4` (`W3`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; immutable evidence `~/work/vllm.cpp-nvfp4-small-m/71f1e894d0c5e496607d08cfe9089a9944128271/w3` plus the exact online-gate root | `codex/nvfp4-small-m` | W3 only: A mirrors production pip-vLLM/FlashInfer's three-warmup, stream-sync, 1-ms GPU-delay, ten-repeat eager event selection; B adds pre-serve all-bucket in-memory warmup; C adds separately gated optional versioned device/CUDA/CUTLASS/model-shape persistence with atomic load/save/stale/collision rejection. Stable selected-plan evidence and lazy diagnostic fallback. No new tactic, model topology, HTTP, scheduler, GDN or device-residency changes. Same-binary selection A/B, paired trace, then exact 27B under one uncontended lock | `ACTIVE` | 2026-07-12 (W3-A `71f1e89` correctness/safety green. Its c16 delayed/off means are 809.932/803.379 tok/s = 1.008156x, but strict acceptance fails at 13/20 timing and 2/4 memory; only 5/35 delayed keys are ID-stable across three processes. W3-B pre-serve all-bucket tuning is active. Pip-vLLM disables its colliding file cache, so persistence cannot own the speed denominator) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + FP4 repair | Exact clean `b5c6e4f` 27B checkpoint is binding below floor. W2 improves every concurrency and wins c16/c32 throughput, but c1-c8, TPOT/ITL and host memory remain red. W3-A `71f1e89` is correctness/safety green and +0.816% in mean c16 total throughput, but strict-fails 13/20 timing and 2/4 memory with only 5/35 delayed keys stable | implement and same-binary gate W3-B pre-serve all-bucket in-memory warmup; gate optional collision-complete W3-C persistence separately, repeat exact 27B until every performance and memory axis passes, then run 35B | `ACTIVE` |
