...
| `CLAIM-HOST-WEIGHT-RESIDENCY-1` | `ENG-HOST-WEIGHT-RESIDENCY` | Codex:gpt-5 | local `/home/rich/c/vllm.cpp`; local RTX evidence under `/tmp/qwen35-memory-*` and `/tmp/qwen35-host-residency-*` | `local-blackwell-environment` | `.agents/specs/discrete-cuda-host-weight-residency.md`; `.agents/specs/safetensors-bounded-page-residency.md`; safetensors reader, dense loader/forward/model-registry files and focused tests; matrix-count invariant in `scripts/check-agent-record.py`; matching matrix/roadmap/README/BENCHMARKS/ledger/state rows. One `/tmp/gpu` lock around each complete local A/B series; no Spark/GB10 access or system configuration changes | `ACTIVE` | 2026-07-12 (W4 spike accepted after tracing vLLM's one-shard-scoped iterator against the project's all-shard mappings. Implement default-on fail-open page discard after globals/each layer, then measure same-binary OFF and fresh vLLM; direct-device loading remains required if the owned-model floor exceeds vLLM. No improved number yet) |
