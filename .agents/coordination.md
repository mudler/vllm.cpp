...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; active worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; finalized c2 root `~/work/vllm.cpp-executed-path-c2/179a0fc2afc1c33b63d14de8e50d3fde976c7356`; W1 safety root `~/work/vllm.cpp-gdn-ba/immutable-581d335fec2e5a96d9ccbb38c1ec001c39ac1789`; completed structural root `~/work/vllm.cpp-gdn-ba-trace/0091cd192d9a6baa2197a4f3bdb0561bd859baf5` | current checkpoint `codex/nvfp4-small-m` | Online-gate execution and harness work. `CLAIM-GDN-BA-ROUNDING-1` leads packed-decode code and its c2/c16 component; this claim retains the binding grid. qkvz code, loader-memory repair, exact grid, and 35B performance are excluded. Any A/B or trace series uses one `flock` | `ACTIVE` | 2026-07-14 (BA safety/structure/projection, packed semantic G0 and W1D1/G1 are closed. BF16 27B is 233/235; W1D2 model dispatch and c2/c16 remain open. Binding 55/124, no speed credit) |
| `CLAIM-GDN-BA-ROUNDING-1` | `KERNEL-GEMM-BF16` (`W1C`), `KERNEL-GDN-PACKED-DECODE` (`W1D`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-gdn-ba-rounding`; DGX G0 root `~/work/vllm.cpp-gdn-packed-decode/f18ca23691bc7e38adbf04912da92f819154379e`; immutable G1 root `~/work/vllm.cpp-gdn-packed-decode/9ad8fb76940e68737d2a13ad8ddd97d649bb577c` | `codex/gdn-ba-rounding-w1c` | Leads `ProjectGdnBA` output dtype plus the exact pure-non-spec packed-decode op in `include/vt/ops.h`, `src/vt/{ops.cpp,cpu/cpu_ops.cpp,cuda/cuda_gdn.cu}`, registrations, `qwen3_5.cpp`, focused oracle/tests, 35B inertness, node trace and c2/c16 component. This is the explicit overlap lead over `CLAIM-PR3` for pure decode/model dispatch and over `CLAIM-SERVE-GATE-1` for the component. Prefill/mixed/spec GDN, qkvz, host-weight repair, binding-grid execution and 35B performance are excluded. One `flock /tmp/gpu` covers every GPU correctness or A/B series | `ACTIVE` | 2026-07-14 (clean `9ad8fb7` closes W1D1/G1: focused packed **5/5**, full CUDA GDN **41/41**, direct fixture `0/1`, capture/canaries and memcheck **2/2, 0 errors/leaks** pass. W1D2 model dispatch/235/235 is next; speed credit remains absent) |
...
model semantics, trace and the component result remain open.
...
|---|---|---|---|---|
| 1 | `KERNEL-GEMM-BF16` + `KERNEL-GDN-PACKED-DECODE` for `SERVE-GATE-ONLINE` | `3f256ab` remains 55/124; BA evidence, packed G0 and clean `9ad8fb7` W1D1/G1 are closed | wire W1D2 and restore 235/235 in default/rollback arms, then complete trace plus c2/c16 before qkvz | `GATING` |
