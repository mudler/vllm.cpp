...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local `/home/mudler/_git/vllm.cpp-serve-gate`; closed diagnostic DGX dir `~/work/vllm.cpp-latency`; fresh current-main source/build/evidence dirs next | `codex/serve-gate-harness` (merged `b0acec2`) | committed `tools/bench/online_gate*`, `tools/bench/profile_vllm_online_gate.py`, `tools/bench/summarize_torch_kernels.py`, `scripts/dgx-online-serving.sh`, `tests/tools/test_online_gate_*`; explicit server scheduling-capacity CLI in `examples/server/main.cpp` + help test/README; DGX current-main build/harness/results; engine matrix, roadmap, ledger/state. ONE `flock` around each complete model A/B+trace series | `ACTIVE` | 2026-07-11 (harness merged/CPU-gated. Pre-W2 35B closed as dual failure: ours aborted after 1/6 c1 with surfaced illegal access; vLLM completed greedy c1-c32 + default c1, then hung >60m in default c16/np96 rep1 with 16 open sockets/no log progress. Snapshot+hash manifest: `latres2/diagnostic-vllm35-hang-20260711T0734CEST`; owned PGID 1140669 terminated, `/tmp/gpu` released, no compute owner. Next: fresh current-main DGX build, sanitizer reproduction if needed, then binding W2 G1/G3-G6) |
| `CLAIM-BACKEND-ABI-W0-GPU-1` | `BACKEND-ABI-VT` (`W0-GPU`) | root, surfaced by fresh serving-gate CUDA build | local `/home/mudler/_git/vllm.cpp-backend-abi-gpu-fix`; DGX `~/work/vllm.cpp-online-{src,build}` | `codex/backend-abi-w0-gpu` | exact GCC13/CUDA build blocker in `tests/vt/test_dropin_abi.cpp`; CUDA cross-build/runtime/capture evidence for additive W0; owning matrix/roadmap/ledger/state. No ABI or production-kernel redesign | `ACTIVE` | 2026-07-11 (syntax-only brace-initialization repair implemented with identical throw semantics; clean CPU focused build/CTest 1/1 and record gates pass; no ABI/runtime source or GPU claim. Push branch, repeat exact sm_121a build, then run focused W0 runtime under lock) |
