...
|---|---|---|---|---|---|---|---|
| `CLAIM-PR3` | `KERNEL-GDN-AOT-BF16`, `KERNEL-GDN-SCRATCH` | root takeover of stopped `validate_pr3` / `complete_pr3` stream | primary recovery tree `/home/mudler/_git/vllm.cpp-pr3-validate`; 27B default/component integration in `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; DGX evidence `~/work/vllm.cpp-noPy` plus `~/work/vllm.cpp-nvfp4-small-m/debug/gdn-out-bf16-c16-ab-20260712` | recovery branch integrated into `main` by `a767188`; current checkpoint on `codex/nvfp4-small-m` | PR #3 files and rows `KERNEL-GDN-AOT-BF16`+`KERNEL-GDN-SCRATCH`; 27B-only `GdnOutDType` default/f32 override in `qwen3_5.cpp`; ledger/inventory/roadmap evidence. No 35B default change. GPU lock: residual trace/pool classification remains separate from FP4 W3 | `ACTIVE` | 2026-07-12 (vendored BF16 H32/H48 AOT/safety evidence and native 16/16 correctness remain green. Binding `9cc7191` has c16 total at 1.028782× but mean TPOT/ITL at 0.988851×; the first ours trace omitted graph child nodes, so re-ranking waits for the node-level paired recapture. All 35B paths keep f32) |
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local `/home/mudler/_git/vllm.cpp-serve-gate`; immutable source/build `~/work/vllm.cpp-online-gate/checkpoints/9cc71918dbdc10f014c02feb9bab1d00963a16fe`; evidence `~/work/vllm.cpp-online-gate/evidence/9cc71918dbdc10f014c02feb9bab1d00963a16fe`; next node trace gets a fresh SHA-bound root | `codex/serve-gate-harness` (merged `b0acec2`; current checkpoint on `codex/nvfp4-small-m`) | `tools/bench/online_gate*`, profiler/summarizer/driver contracts, exact DGX evidence, matrices/roadmap/ledger/state. ONE `flock` around each complete model A/B+trace series | `ACTIVE` | 2026-07-12 (`9cc7191` grid remains binding at 54/124. Trace audit finds 1,226 whole-graph activities but zero child-node rows because ours omitted `--cuda-graph-trace=node`. New record contract requires node mode and `--trace-only` runs model gate + both profilers without the grid. Commit, prepare fresh immutable evidence, execute the one-lock node recapture; keep 35B held) |
...
| `CLAIM-NVFP4-SMALL-M-3` | `KERNEL-GEMM-NVFP4-W4A4` (`W3`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; immutable W3-A evidence `~/work/vllm.cpp-nvfp4-small-m/71f1e894d0c5e496607d08cfe9089a9944128271/w3`; immutable W3-B evidence `~/work/vllm.cpp-nvfp4-small-m/d7cdf66db0cfcc53d68d49613623ec6cd3807641/w3b`; binding grid `~/work/vllm.cpp-online-gate/evidence/9cc71918dbdc10f014c02feb9bab1d00963a16fe`; next node trace gets fresh evidence | `codex/nvfp4-small-m` | W3 owns the highest-ranked executed FP4 topology/selection repair. A/B correctness and structural evidence remain; C persistence stays separately optional. Do not implement direct-swizzled scales, resident alpha or another topology change until a node-level paired trace proves it is on the remaining path | `ACTIVE` | 2026-07-12 (`9cc7191` exact v0.25 grid completes with 54/124 axes passing, but its ours trace is whole-graph only and cannot rank child kernels. Run the fresh node-level trace-only checkpoint before topology work; 35B held) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + FP4 repair | Binding immutable `9cc7191` v0.25 27B grid is complete: all 124 axes bind, 54 pass and 70 fail. The first ours trace recorded whole graph activities but zero child nodes, so kernel attribution is pending. GPU/port/lock exit idle | commit the node-granularity/trace-only contract, prepare fresh immutable evidence, run model gate + paired node trace under one lock, then rank gain÷effort and run the highest-ranked same-binary 27B repair A/B. Modernize/delete only trace-proven paths and run 35B only after full 27B closure | `ACTIVE` |
