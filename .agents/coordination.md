...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local `/home/mudler/_git/vllm.cpp-serve-gate`; binding grid `~/work/vllm.cpp-online-gate/evidence/9cc71918dbdc10f014c02feb9bab1d00963a16fe`; accepted node trace `~/work/vllm.cpp-online-trace-node/evidence/def5f752896036d9b35841a278578fd812f75a0d`; immutable W3-D component `~/work/vllm.cpp-packed-qkv/3f256abdbb558e162bf8a2196284deb119648560/w3d/component-ab` | `codex/serve-gate-harness` (merged `b0acec2`; current checkpoint on `codex/nvfp4-small-m`) | `tools/bench/online_gate*`, profiler/summarizer/driver contracts, exact DGX evidence, matrices/roadmap/ledger/state. ONE `flock` around each complete model A/B+trace series | `ACTIVE` | 2026-07-12 (`9cc7191` remains binding at 54/124. Clean W3-D packed/split correctness passes 235/235 +16/16; c16 means 812.231/808.150 = 1.005049× but strict component acceptance fails at 14/20 timing +2/4 memory. Re-trace 240→208, repeat the exact grid and keep 35B held) |
...
| `CLAIM-NVFP4-SMALL-M-3` | `KERNEL-GEMM-NVFP4-W4A4` (`W3`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; immutable W3-A/B and binding/trace evidence as above; W3-D component `~/work/vllm.cpp-packed-qkv/3f256abdbb558e162bf8a2196284deb119648560/w3d/component-ab` | `codex/nvfp4-small-m` | W3 owns the highest-ranked executed FP4 topology/selection repair. C persistence stays optional; D owns only packed full-attention QKV, its max-shard scalar/resident/row-strided consumers, ported tests and `VT_FP4_MERGED_QKV=0` fallback | `ACTIVE` | 2026-07-12 (W3-D immutable correctness and c16 A/B complete: both arms 235/235 +16/16, profiles 64/80, packed/split 812.231/808.150 tok/s = 1.005049×, 14/20 timing +2/4 memory. Retain packed but record strict component fail; re-trace then exact grid. Do not stack other levers or run 35B) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + FP4 repair | Binding immutable `9cc7191` v0.25 27B grid is complete: all 124 axes bind, 54 pass and 70 fail. W3-D immutable packed/split correctness passes; its c16 component is 1.005049× mean-positive but strict-fails at 14/20 timing +2/4 memory. GPU/port/lock exit idle | commit/push this A/B checkpoint, node re-trace `3f256ab`, then repeat/re-rank the exact grid. Modernize/delete only trace-proven paths and run 35B only after full 27B closure | `ACTIVE` |
