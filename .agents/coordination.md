...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + FP4 repair | Exact clean `b5c6e4f` 27B checkpoint is binding below floor. W2 improves every concurrency and wins c16/c32 throughput, but c1-c8, TPOT/ITL and host memory remain red. W3-A `71f1e89` delayed timing is focused/model/memcheck green and resolves the traced ID 6/4 family; performance remains pending | run the delayed/off c16/96 AB/BA/AB, then W3-B pre-serve all-bucket in-memory warmup; gate optional collision-complete W3-C persistence separately, repeat exact 27B until every performance and memory axis passes, then run 35B | `ACTIVE` |
