...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; active worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; accepted H1d root `~/work/vllm.cpp-executed-path-refresh-h1d/c498a4131af7e6cf0ac678841212af80f4f12d53`; W3-I root `~/work/vllm.cpp-nvfp4-fused/15c6b8933d982019aa8965d218deb0eb1d9dc3f4-r2`; accepted async root `~/work/vllm-async-credit/3812d8d2b4a68d2e501007d01fe10cdf17751d02` | current checkpoint `codex/nvfp4-small-m` | online harness/evidence/matrices and exact low-batch ours/vLLM RMSNorm/generated-partition + FP4-tactic mapping. `ENG-ASYNC-SCHED`, loader/memory implementation, W3-H2/I2+, exact-grid, and 35B implementation remain excluded until a complete kernel lever is selected. Any A/B or trace series uses ONE `flock` | `ACTIVE` | 2026-07-14 (`3812d8` complete async control: ON/OFF total **1.002153×**, TPOT **1.010291×**, TTFT **0.862159×**, traced GPU time **1.002004×**; neutral/no W3 speed credit. Binding 55/124; low-batch kernel mapping next) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` low-batch kernel mapping | `3f256ab` remains 55/124; accepted async `3812d8` closes W3's speed hypothesis at only **1.002153×** total and **1.002004×** traced GPU time | capture/align exact c2 ours/vLLM kernels, map the 129 RMSNorm and generated partitions plus resolved FP4 tactics, then gate the highest complete 4–6% lever before the exact grid | `ACTIVE` |
