...
| `CLAIM-NVFP4-SMALL-M-3` | `KERNEL-GEMM-NVFP4-W4A4` (`W3`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; immutable W3-A/B and binding evidence as above; W3-D component/post-pack trace and exact grid roots listed in `CLAIM-SERVE-GATE-1` | `codex/nvfp4-small-m` | W3 owns only the implemented FP4 tactic/topology repair. C persistence stays optional; D owns packed full-attention QKV, its max-shard scalar/resident/row-strided consumers, ported tests and `VT_FP4_MERGED_QKV=0` fallback. No additional W3 lever stacks before body-level residual selection | `ACTIVE` | 2026-07-13 (W3-D correctness/A-B/trace complete: c16 1.005049×, strict 14/20 timing +2/4 memory; post-pack **296,674/1,425 = 208.192** vs vLLM 208 FP4 GEMMs/forward. Exact `3f256ab` improves binding disposition to 55/124 but strict-fails. Generated code proves vLLM also uses separate norm and FP4 quant launches; residual selection is open. Do not run 35B) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + FP4 repair | Binding immutable `3f256ab` v0.25 27B grid/trace is complete: 55/124 pass, 69 fail. W3-D closes 240→208. Generated code refutes the apparent 80/forward fused norm+FP4 gap: vLLM also launches quant separately. GPU/ports/lock idle | continue body-level trace/dispatch diffing and obtain a clean local slice before writing or claiming the next spike; then port tests, same-binary A/B and exact rerun. Modernize/delete only trace-proven paths and run 35B only after full 27B closure | `ACTIVE` |
