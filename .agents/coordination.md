...
| `CLAIM-HOST-WEIGHT-RESIDENCY-1` | `ENG-HOST-WEIGHT-RESIDENCY` | Codex:gpt-5 | local `/home/rich/c/vllm.cpp`; local RTX evidence under `/tmp/qwen35-memory-*`, `/tmp/qwen35-host-residency-*`, `/tmp/qwen35-peak-w4-*`, `/tmp/qwen35-direct-*` | `local-blackwell-environment` | `.agents/specs/discrete-cuda-host-weight-residency.md`; `.agents/specs/safetensors-bounded-page-residency.md`; `.agents/specs/dense-direct-device-loading.md`; safetensors reader, dense loader/forward/model-registry/entrypoint files and focused tests; matrix-count invariant in `scripts/check-agent-record.py`; matching matrix/roadmap/README/BENCHMARKS/ledger/state rows. One `/tmp/gpu` lock around each complete local A/B series; no Spark/GB10 access or system configuration changes | `ACTIVE` | 2026-07-12 (`54401bf`: all 18 local arms complete; ON/OFF/vLLM peak PSS 1.849/8.168/7.616 GiB, throughput 6605.55/6601.40/6717.87, mean TTFT 659/661/908 ms. Project output varies 121-124/128 while vLLM is stable, so metrics stay diagnostic. CPU/CUDA focused green, full CPU 104/105 + async isolated 3/3; GPU idle/kernel log empty) |
