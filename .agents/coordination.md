...
| `CLAIM-NVFP4-SMALL-M-3` | `KERNEL-GEMM-NVFP4-W4A4` (`W3`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; next immutable evidence under `~/work/vllm.cpp-nvfp4-small-m/<pushed-sha>/w3` plus the exact online-gate root | `codex/nvfp4-small-m` | W3 only: A mirrors production pip-vLLM/FlashInfer's three-warmup, stream-sync, 1-ms GPU-delay, ten-repeat eager event selection; B adds pre-serve all-bucket in-memory warmup; C adds separately gated optional versioned device/CUDA/CUTLASS/model-shape persistence with atomic load/save/stale/collision rejection. Stable selected-plan evidence and lazy diagnostic fallback. No new tactic, model topology, HTTP, scheduler, GDN or device-residency changes. Same-binary selection A/B, paired trace, then exact 27B under one uncontended lock | `ACTIVE` | 2026-07-12 (W3-A implementation staged; `VT_FP4_AUTOTUNE_DELAY=0` restores W2 and verbose output reports timing/stable ID. Disposable sm_121a delayed/off processes each pass 14/14 / 18,619/18,619 and select different synthetic IDs 1/0 vs 20/7; GPU/lock return idle. Clean commit-bound safety, real-model selected-plan/performance A/B remain pending. Installed pip-vLLM disables its colliding file cache, so persistence cannot own the speed denominator) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + FP4 repair | Exact clean `b5c6e4f` 27B checkpoint is binding below floor. W2 improves every concurrency and wins c16/c32 throughput, but c1-c8, TPOT/ITL and host memory remain red; paired traces localize the next mismatch to runtime tactic selection. W3-A delayed timing is staged/GPU-pending | clean-build and gate W3-A delayed/off selection, then W3-B pre-serve all-bucket in-memory warmup; gate optional collision-complete W3-C persistence separately, then repeat exact 27B until every performance and memory axis passes; only then run 35B | `ACTIVE` |
