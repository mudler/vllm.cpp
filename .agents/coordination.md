...
| `CLAIM-BUSY-LOOP-1` | `ENG-CORE-BUSY-LOOP` | `eng-core-busy-loop` (W1 impl agent) | local worktree `.claude/worktrees/agent-adb1b34e5b6ab3dc3` (own build dir); no DGX dir (CPU-only leaf; GPU gates deferred — see lock note) | `eng-core-busy-loop` | new `include/vllm/v1/engine/core_proc.h`, `src/vllm/v1/engine/core_proc.cpp`, `include/vllm/v1/engine/core_client.h`, `src/vllm/v1/engine/core_client.cpp`, new `tests/vllm/v1/test_engine_core_proc.cpp`; access-only touch of `include/vllm/v1/engine/core.h` (private→protected); `CMakeLists.txt` + `tests/CMakeLists.txt` source-list lines; engine-matrix `ENG-CORE-BUSY-LOOP` row; roadmap `ROAD-V1-C6` row; ledger/state appends. Lock plan: NONE — GPU busy with `CLAIM-SERVE-GATE-1` sanitizer+A/B campaign (flock held + queued waiter at claim time); G1/G4 GPU gates left as recorded gating handoff, not queued behind the campaign | `ACTIVE` | 2026-07-10 (claimed; W1 = EngineCoreProc busy loop + in-proc queue split per [async-serving.md](specs/async-serving.md)) |
| `CLAIM-EXPSTREAM-SPIKE-1` | `ENG-EXPERT-STREAM` | `expert-streaming-spike` spike agent (user-directed) | local main (docs only; spike is read-only outside spec/matrix/coordination rows); ds4 clone + measurements in session scratchpad; short dgx NVMe read probes (disk-only, no GPU work, no lock needed) | `main` (docs-only edits per push policy) | New spec `.agents/specs/expert-streaming.md`; engine-matrix `ENG-EXPERT-STREAM` row (+ mirror-floor inventory row `ENG-WEIGHT-OFFLOAD`) + summary tallies + checker `ENGINE_ROWS` pin; feature-matrix §2 rows; roadmap `ROAD-V1-D4` row; state.md append | `ACTIVE` | 2026-07-10 (ds4 anatomy done; NVMe bandwidth measured on dgx; writing spec) |
## Handoff queue
...
| 7 | C2/C4 models + quantizations | accepted comprehensive inventories | spike `MODEL-FACTORY-registry`; `QUANT-GGUF-COMPUTE` leaves split (`CLAIM-QGC-LEAVES` released) — `QUANT-GGUF-CPU-THREADPOOL` claimed by `CLAIM-QGCT-1`; keep-quant loader + CIQ GEMM next | `SPIKE` |
