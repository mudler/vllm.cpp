...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; active worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; latest failed/void H1d root `~/work/vllm.cpp-executed-path-refresh-h1d/b8c8086eea9a4f392774cb97a130a06a75ec920c`; superseded H1 roots remain in state/ledger only | current checkpoint `codex/nvfp4-small-m` | online harness/evidence/matrices plus completed W3-G ratio-6 FA2 gates and [W3-H normal-producer spike](specs/nvfp4-bf16-producer-vectorization.md). H1 owns the diagnostic-only four-replay controller, graceful target stop, target-build/dispatch manifest, exact-workload paired ours/vLLM trace validation and residual re-ranking; W3-H2 inference-kernel implementation remains prohibited until valid H1d evidence. Host-weight, fused producer, GDN changes, scheduler and 35B performance excluded. ONE `flock` around the complete series | `ACTIVE` | 2026-07-13 (`b8c8086` passes report-1 schema-v5 reconciliation but is VOID because the secondary summary omitted the model key. The argument-flow repair is CPU-green; execute a fresh immutable 12-report DGX run. `3f256ab` remains 55/124) |
...
traces only. Schema-v5 `b8c8086` is the current failed/void checkpoint: report
1 reconciles exactly, then the secondary summary omits the model key. The
argument-flow repair is CPU-green; the next work is a fresh immutable
12-report DGX root. W3-H2 inference code remains unauthorized until that gate
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + W3-H executed-path refresh | `3f256ab` remains 55/124; W3-E/W3-F/W3-G earn no speed credit. Schema-v5 `b8c8086` is VOID on a repaired model-key omission in the secondary summary path. GPU/lock/ports are idle | run a fresh immutable 12-report schema-v5 root before implementing any lever. No exact grid/35B performance until a later component passes every axis | `ACTIVE` |
