...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local `/home/mudler/_git/vllm.cpp-serve-gate`; binding grid `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; W3-E immutable component `~/work/vllm.cpp-direct-sf/53ab1492983282a9858cc301d4f7e9aad4784c48/component-ab-c2-c16` | `codex/serve-gate-harness` (merged `b0acec2`; current checkpoint on `codex/nvfp4-small-m`) | `tools/bench/online_gate*`, profiler/summarizer/driver contracts, exact DGX evidence, matrices/roadmap/ledger/state. ONE `flock` around each complete model A/B+trace series | `ACTIVE` | 2026-07-13 (`3f256ab` remains 55/124. W3-E completes 12 legs/612 requests/12 returns and gains 1.002108×/1.006222× c2/c16 mean total throughput, but strict-fails 32/40 timing +6/8 memory. No exact rerun or 35B performance run; re-scan next) |
...
| `CLAIM-NVFP4-SMALL-M-3` | `KERNEL-GEMM-NVFP4-W4A4` (`W3`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; binding grid as above; W3-E immutable component `~/work/vllm.cpp-direct-sf/53ab1492983282a9858cc301d4f7e9aad4784c48/component-ab-c2-c16` plus recorded mutable build/safety/trace | `codex/nvfp4-small-m` | W3 owns FP4 tactic/topology repair. W3-E scope is explicit linear/direct-swizzled producer layouts, padded zero contract, true-W4A4 CUTLASS dispatch, ported tests and `VT_FP4_DIRECT_SF=0` per [spike](specs/nvfp4-direct-swizzled-scales.md). No other lever stacked into its A/B | `ACTIVE` | 2026-07-13 (W3-E code/correctness/sanitizer/trace pass and remove 624 activation swizzles. Its 12-leg component is mean-positive at 1.002108× c2 / 1.006222× c16 but strict-fails 32/40 timing +6/8 memory; five of six long-output hash pairs differ and tactic IDs are unstable. No exact rerun or 35B performance run; re-scan before a new spike) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + FP4 repair | Binding immutable `3f256ab` remains 55/124. W3-E removes 624 activation swizzles and its immutable component gains 1.002108×/1.006222× c2/c16 mean total throughput, but strict-fails 32/40 timing +6/8 memory. GPU/ports/lock idle | commit/push the failed component checkpoint; do not run its conditional exact grid. Execute the dynamic hot-path re-scan against vLLM and spike only the next verified unstacked lever. 35B performance remains held until full 27B closure | `ACTIVE` |
