...
| `CLAIM-HOST-WEIGHT-RESIDENCY-1` | `ENG-HOST-WEIGHT-RESIDENCY` | Codex:gpt-5 | local `/home/rich/c/vllm.cpp`; local RTX evidence under `/tmp/qwen35-memory-*` | `local-blackwell-environment` | `.agents/specs/discrete-cuda-host-weight-residency.md`; `qwen3_5_weights.h`; dense loader/forward/model-registry files and focused tests; matrix-count invariant in `scripts/check-agent-record.py`; matching matrix/roadmap/README/BENCHMARKS/ledger/state rows. One `/tmp/gpu` lock around each complete local A/B series; no Spark/GB10 access or system configuration changes | `ACTIVE` | 2026-07-11 (W1+W2 memory series complete; first-forward W1 throughput regressed 1.77% and is rejected. W3 moves eager upload/synchronized release to model prepare; CPU/native-sm_120 builds and focused 4/4 pass, replacement memory/performance gates pending) |
