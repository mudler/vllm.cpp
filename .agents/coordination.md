...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; active worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; accepted H1d root `~/work/vllm.cpp-executed-path-refresh-h1d/c498a4131af7e6cf0ac678841212af80f4f12d53`; W3-I root `~/work/vllm.cpp-nvfp4-fused/15c6b8933d982019aa8965d218deb0eb1d9dc3f4-r2`; current VOID async root `~/work/vllm-async-credit/b8681ac80b3f84af71955cf3a20cece2a118ea1f`; fresh root follows direct-script bootstrap repair | current checkpoint `codex/nvfp4-small-m` | online harness/evidence/matrices and diagnostic-only vLLM async ON/OFF c2 timing + Torch traces. `ENG-ASYNC-SCHED`, loader/memory, RMSNorm, W3-H2/I2+, exact-grid, and 35B implementation remain excluded until the control selects a lever. ONE `flock` around the complete async series | `ACTIVE` | 2026-07-14 (`b8681ac` VOID: six timing legs complete, provisional ON/OFF total **1.001345×**, but first trace import failed and both traces/completion marker are absent; cleanup pass. Bootstrap repair pending fresh run; binding 55/124) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` async-credit control | `3f256ab` remains 55/124; `b8681ac` completed six timing legs at provisional **1.001345×** ON/OFF total but is VOID because both traces are absent after direct-script import failure | commit the bootstrap repair, create a fresh root, then repeat all six timing legs + two traces under one lock. If the provisional neutral result repeats, map low-batch RMSNorm/generated partitions; only complete 4–6% credit may activate W3 for speed | `ACTIVE` |
