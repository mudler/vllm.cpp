...
| 7 | C2/C4 models + quantizations | accepted comprehensive inventories | `MODEL-FACTORY-registry` implementation landed at `c707602`, CPU suite 94/94, claim released; row is `GATING` on the exact two-model greedy + adjacent-commit performance/memory handoff queued after `CLAIM-SERVE-GATE-1` ([model-factory-registry.md](specs/model-factory-registry.md)), then Llama dense. `QUANT-GGUF-COMPUTE` leaves split — `QUANT-GGUF-CPU-THREADPOOL` claimed by `CLAIM-QGCT-1`; keep-quant loader + CIQ GEMM next | `ACTIVE` |
