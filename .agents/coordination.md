...
| `CLAIM-C5-YARN-1` | `ATTN-YARN` | root | local `/home/mudler/_git/vllm.cpp-c5-yarn` | `codex/c5-yarn-w5` | W5 only: typed YaRN/MRoPE slices in `include/src/vllm/transformers_utils/hf_config.*`, new `include/src/vllm/model_executor/layers/rotary_embedding/{base,common,yarn_scaling_rope,mrope}.*`, the shared supplied-cache RoPE apply seam in `include/src/vt/ops.*` plus CPU/CUDA implementations, CMake/tests, pinned oracle dump/goldens, and C5 record rows. No W6-W8 Llama3/LongRoPE/dynamic formulas, model-family or multimodal preprocessing, local-attention edits, or GPU execution | `ACTIVE` | 2026-07-11 (W1-W4 merged/CPU-gated; port the common typed cache/factory foundation and complete plain + `mrope_section` YaRN semantics while the serving campaign retains dgx) |
...
| 8 | C3/C5/C7/C8 engine/API work | C3/C5/C6 specs accepted; C7/C8 leaf specs missing | C3 M-mtp-0 remains `GATING` on its GPU recipe. C5 W1-W4 are implemented and `GATING`; W5 `ATTN-YARN` is `ACTIVE` under `CLAIM-C5-YARN-1` for typed config/cache/apply + oracle work. Restored positive model rows and an idle GPU still gate local-attention runtime closure; W6-W8 follow W5. Then spike C7/C8 | `ACTIVE` |
