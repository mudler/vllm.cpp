...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local `/home/mudler/_git/vllm.cpp-serve-gate`; binding evidence root `~/work/vllm.cpp-online-gate` | `codex/serve-gate-harness` (merged `b0acec2`; corrected trace/gate contract active) | `tools/bench/online_gate*`, profiler/summarizer/driver contracts, exact DGX evidence, matrices/roadmap/ledger/state. ONE `flock` around each complete model A/B+trace series | `ACTIVE` | 2026-07-11 (exact pushed-`a531e05` 27B: 12/12 groups binding, 2,016/2,016 requests, six memory returns, model/trace pass. Median c1→c32 total ratios 0.9679/0.9338/0.9482/0.9547/1.0028/0.9268×; 4/2/5/3/10/8 of 20 performance and 2/4 memory axes pass. Campaign/trace `24d78fbc…e9d2a` / `1c702ef9…142a`. HTTP c32 and FP4 bucket/tactic defects reproduced; 35B held) |
| `CLAIM-SERVE-HTTP-POOL-1` | `SERVE-ASYNC-LLM` (W2 HTTP-capacity repair) | root | isolated worktree `/home/mudler/_git/vllm.cpp-serve-http-pool`; DGX evidence to be commit-bound under `~/work/vllm.cpp-online-gate` or a component-A/B sibling | `codex/serve-http-pool` | `api_server.{h,cpp}`, production server max-seqs/env A/B wiring, API socket tests, async-serving spec + owning README/BENCHMARKS/matrix/roadmap/ledger/state. No scheduler/kernel edits. Exact c32/full-ladder series holds one uncontended lock | `ACTIVE` | 2026-07-11 (fixed `max_num_seqs + 4` pool implemented; `VLLM_CPP_HTTP_FIXED_POOL=0` legacy arm. Release/help pass; capacity API test 100/100; ASan+UBSan and TSan 1/1. Full serial 104/105 only on known unrelated C-API flake; isolated pass. Commit/push CPU checkpoint, then fixed/legacy AB/BA/AB c32 + fresh oracle ladder) |
| `CLAIM-KV-DEVICE-1` | `KV-DEVICE-RESIDENCY` | root | main worktree `/home/mudler/_git/vllm.cpp`; W0/W1 evidence under `~/work/vllm.cpp-kv-device`; current oracle evidence `~/work/vllm.cpp-online-gate/evidence/a531e055f0ef81b1d7296a7cba99d8f09373a265` | `main` | W0/W1 ownership/indexed-state implementation and open teardown; W2 direct convolution update remains scoped but unclaimed until post-HTTP/FP4 evidence re-ranks it | `ACTIVE` | 2026-07-11 (W0/W1 component A/Bs remain 1.021239×/1.006246× and access/correctness green; inherited pools fail zero-leak. Fresh `a531e05` oracle is below floor, but confirmed HTTP capacity and FP4 key/tactic defects take priority; no W2 implementation is inferred yet) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + HTTP/FP4 repair | Exact `a531e05` 27B checkpoint is binding below floor; c32 HTTP capacity and small-M FP4 dispatch are reproduced. The fixed HTTP implementation is CPU/sanitizer-green; the full FlashInfer dependency-chain spike for `KERNEL-GEMM-NVFP4-W4A4` is accepted and `READY` | finish the immutable HTTP fixed/legacy + fresh-oracle campaign, then claim W1 from [the accepted FP4 spike](specs/nvfp4-small-m-dispatch.md): exact hybrid buckets/single-flight first, full 32-tactic family second, with separate AB/BA/AB and exact 27B campaigns. Repeat until every axis passes; only then run 35B | `ACTIVE` |
| 2 | `SERVE-ASYNC-LLM` block (`ENG-CORE-BUSY-LOOP`, `SERVE-ASYNC-LLM`, `ENG-ASYNC-SCHED`, `ENG-PRIORITY-SCHED`) | joint spike accepted ([async-serving.md](specs/async-serving.md)); exact c32 evidence confirms W2 transport capacity is not closed | W1/W2/W4 host behavior remains CPU/TSan-green. The active W2 repair has fixed persistent-stream capacity and CPU tests; G3 fixed-vs-legacy c32/full-ladder evidence remains. W3 overlap stays READY and must not be mixed into this A/B | `GATING` |
