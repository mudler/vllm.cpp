...
|---|---|---|---|---|
| 1 | `KERNEL-GEMM-BF16` + `KERNEL-GDN-PACKED-DECODE` for `SERVE-GATE-ONLINE` | `3f256ab` remains 55/124; correctness/structure close at `f344dec` and `7ff713e` + `24cea4f`; `d82d282` failed incomplete at first c16 timed batch | capture the discarded server exception, root-cause/repair test-first, then execute a fresh-SHA one-lock **40+8 / 144-paired** series before qkvz | `GATING` |
