...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + FP4 repair | Exact clean `b5c6e4f` 27B checkpoint is binding below floor. W2 improves every concurrency and wins c16/c32 throughput, but c1-c8, TPOT/ITL and host memory remain red. W3-A strict-fails. W3-B at clean `d7cdf66` is immutable build/correctness/safety-green at 80/80 profiles, zero lazy misses, 16/16 correctness and zero memcheck errors; no speed evidence exists yet | run repeated W3-B plan-stability/component and paired trace, gate optional W3-C persistence separately, repeat exact 27B until every performance and memory axis passes, then run 35B | `ACTIVE` |
