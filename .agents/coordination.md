...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local `/home/mudler/_git/vllm.cpp-serve-gate`; closed diagnostic DGX dir `~/work/vllm.cpp-latency`; binding evidence root `~/work/vllm.cpp-online-gate` | `codex/serve-gate-harness` (merged `b0acec2`; profiler preflight + parity diagnosis active) | committed `tools/bench/online_gate*`, `tools/bench/profile_vllm_online_gate.py`, `tools/bench/summarize_torch_kernels.py`, `scripts/dgx-online-serving.sh`, `tests/tools/test_online_gate_*`; rootless verified cache-eviction helper `tools/bench/drop_file_cache.py`; explicit server scheduling-capacity CLI in `examples/server/main.cpp` + help test/README; DGX current-main build/harness/results; engine matrix, roadmap, ledger/state. ONE `flock` around each complete model A/B+trace series | `ACTIVE` | 2026-07-11 (`31d053f` completed all repeated 27B timing/memory legs with exact counts but measured 0.926–0.961× throughput and broad latency gaps. Ours nsys completed; vLLM profile failed because its FlashInfer JIT subprocess could not find venv `ninja`. Series void, GPU idle/unlocked, 35B not started. Merge the hashed ninja/PATH repair, recapture a diagnostic paired trace, then optimize before the next full gate) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` | All partial campaigns are invalid. `31d053f` completed the repeated 27B timing/memory grid, proved exact usage, and exposed reproducible performance/latency gaps; required vLLM trace closure failed only because the profiler omitted the pinned venv `PATH` for `ninja` | merge the hashed ninja/PATH repair; recapture the diagnostic paired trace; rank and implement the top parity lever; then restart the full 27B gate before 35B | `ACTIVE` |
