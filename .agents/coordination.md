...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; active worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; finalized c2 root `~/work/vllm.cpp-executed-path-c2/179a0fc2afc1c33b63d14de8e50d3fde976c7356`; W1 safety root `~/work/vllm.cpp-gdn-ba/immutable-581d335fec2e5a96d9ccbb38c1ec001c39ac1789`; raw trace root `~/work/vllm.cpp-gdn-ba-trace/0091cd192d9a6baa2197a4f3bdb0561bd859baf5` | current checkpoint `codex/nvfp4-small-m` | Online-gate execution and harness work, including immutable evidence for the now-unclaimed `GATING` W1 BA implementation. qkvz code, loader-memory repair, exact grid, and 35B performance are excluded. Any A/B or trace series uses one `flock`; further `KERNEL-GEMM-BF16` production code requires a fresh `ACTIVE` claim | `ACTIVE` | 2026-07-14 (W1 core correctness/safety remains closed at `581d335`; BF16 output fails **233/235**. Raw `0091cd1` completes 12/12 merged 963/145 and 12/12 split 1,011/193 local ranges. First finalization exited 2 on new exact vLLM Inductor register signatures and wrote no marker; whole-chain diff preserves every structural field, and the two-hash repair passes **69/69**. Push/re-finalize before component work; binding 55/124, no speed credit) |
...
evidence execution only. Exact BF16 rounding, structural completion marker and
component result remain open. qkvz, exact-grid and 35B work stay excluded until
those gates close.
...
|---|---|---|---|---|
| 1 | `KERNEL-GEMM-BF16` merged GDN projections for `SERVE-GATE-ONLINE` | `3f256ab` remains 55/124; pushed `581d335` closes F32-output core correctness/safety; raw `0091cd1` passes exact local structure but first finalization rejected new vLLM register signatures and wrote no marker | push the exact two-signature repair and re-finalize immutable raw evidence; resolve BF16 rounding, then complete c2/c16 before qkvz | `GATING` |
