...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local main (docs only); DGX `~/work/vllm.cpp-latency` (harness/build @ 83010c7) + `~/work/vllm.cpp-latency/latres2` | `main` (docs-only edits per push policy) | DGX latency dir + `lat_*.sh` harness; engine-matrix `SERVE-GATE-ONLINE` row; roadmap `ROAD-V1-A` row; ledger appends. ONE `flock` per model A/B series (27B ours+vLLM single hold; 35B ours+vLLM single hold) | `ACTIVE` | 2026-07-11 (35B exact-shape sanitizer diagnostic survived 3/3 with zero reports; second same-lock campaign completed our full 27B arm; matched vLLM 27B c1 is complete and c2 greedy repetition 2 is active; both 35B arms remain) |
| `CLAIM-C5-DYNAMIC-1` | `ATTN-ROPE-DYNAMIC-NTK` | root | local `/home/mudler/_git/vllm.cpp-c5-dynamic` | `codex/c5-dynamic-w8` | W8 only: typed `alpha`/`max_trained_positions`, new `include/src/vllm/model_executor/layers/rotary_embedding/dynamic_ntk_{scaling,alpha}_rope.*`, exact factory branches, CMake/leaf config+formula tests, generic long-context oracle dumper/runner extensions, pinned factor/alpha f32+bf16 goldens, and C5 record rows. Preserve alpha-first dispatch and existing W5-W7 semantics. No other RoPE families, Hunyuan model port, attention edit, or GPU execution | `ACTIVE` | 2026-07-11 (pinned R2/R8 and test_config/test_pos_encoding read: alpha wins when present; factor uses optional trained length defaulting to max position; guard rotary_dim=2; serving campaign retains dgx) |
...
| 8 | C3/C5/C7/C8 engine/API work | C3/C5/C6 specs accepted; C7/C8 leaf specs missing | C3 M-mtp-0 remains `GATING` on its GPU recipe. C5 W1-W7 are implemented and `GATING`; W5-W7 retain CUDA/model/trace/every-axis debts. W8 `ATTN-ROPE-DYNAMIC-NTK` is `ACTIVE` under `CLAIM-C5-DYNAMIC-1`. Restored positive model rows and an idle GPU still gate runtime closure; then spike C7/C8 | `ACTIVE` |
