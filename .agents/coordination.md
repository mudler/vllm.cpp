...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; active worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; accepted H1d root `~/work/vllm.cpp-executed-path-refresh-h1d/c498a4131af7e6cf0ac678841212af80f4f12d53`; validator root `~/work/vllm.cpp-trace-validator/71128642ce04c191f559ea4ccabe4b7e33a66b0f`; superseded H1 roots remain in state/ledger only | current checkpoint `codex/nvfp4-small-m` | online harness/evidence/matrices plus completed W3-G ratio-6 FA2 gates and [W3-H normal-producer spike](specs/nvfp4-bf16-producer-vectorization.md). H1d diagnostic trace/ranking is complete. The next claim extension owns only the whole-chain fused-producer spike; normal W3-H2 and any fused implementation remain prohibited until that spike is accepted. Host-weight, GDN changes, scheduler and 35B performance remain excluded. ONE `flock` around any later complete series | `ACTIVE` | 2026-07-13 (`c498a413` passes status `84d15970…6e66`; fused SiLU→FP4 is the largest positive mapped residual in 12/12 reports and displaces normal H2. `3f256ab` remains 55/124) |
...
so W3-H2 stays unimplemented and the next work is a dedicated fused-producer
spike. No inference change begins before that spike is accepted. Superseded
attempt roots remain in the append-only record, not this live control plane.

## Handoff queue

| Priority | Row/block | Dependency | Next handoff | State |
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + W3-H executed-path refresh | `3f256ab` remains 55/124; W3-E/W3-F/W3-G earn no speed credit. Schema-v5 `c498a413` passes status `84d15970…6e66`; fused SiLU→FP4 ranks first in 12/12 reports. GPU/lock/ports are idle | write and accept the fused-producer whole-chain spike, then gate one bounded candidate. No normal H2, exact grid, or 35B performance before its required component gates | `ACTIVE` |
