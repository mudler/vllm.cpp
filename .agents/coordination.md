...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local `/home/mudler/_git/vllm.cpp-serve-gate`; binding evidence root `~/work/vllm.cpp-online-gate` | `codex/serve-gate-harness` (merged `b0acec2`; corrected trace/gate contract active) | `tools/bench/online_gate*`, profiler/summarizer/driver contracts, exact DGX evidence, matrices/roadmap/ledger/state. ONE `flock` around each complete model A/B+trace series | `ACTIVE` | 2026-07-12 (exact pushed-`bce2627` 27B: 12/12 groups binding, 2,016/2,016 requests, six memory returns, model/trace pass. Median c1→c32 total ratios 0.9680/0.9317/0.9403/0.9516/0.9944/1.0073×; 4/4/5/4/4/12 of 20 performance and 2/4 memory axes pass. W1 is measured; W2 owns the confirmed tactic gap. 35B held) |
| `CLAIM-NVFP4-SMALL-M-2` | `KERNEL-GEMM-NVFP4-W4A4` (`W2`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; DGX evidence will live under `~/work/vllm.cpp-nvfp4-small-m/<pushed-sha>/w2` plus the exact online-gate root | `codex/nvfp4-small-m` | W2 only: split CUDA template/dispatch TUs, exact 32-tactic descriptors/order, swap-AB/static/Stream-K raw launch, high-water workspace, forced-ID/upstream-ported tests, and the trace-required dense merged gate/up resident + maximum logical-shard CT divisor semantics in `qwen3_5.cpp`/dense model tests. `VT_FP4_MERGED_GATE_UP=0` restores split W2; `VT_FP4_FULL_TACTICS=0` restores W1. No HTTP, scheduler, unrelated model topology or W3 persistence edits. One uncontended lock for every-tactic sanitizer + component AB/BA/AB + paired trace + exact 27B campaign | `ACTIVE` | 2026-07-12 (32/32 raw tactics and sanitizer shapes pass. Default swap tactics first diverge in layer-1 MLP; forced narrow ordinary IDs 1/3/7 recover 16/16 tokens, but nsys proves oracle runs swap IDs 4/6. Upstream merged gate/up + max-divisor/one-alpha processing is the missing execution-chain link now added to the accepted spike; no 35B) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + FP4 repair | Exact `bce2627` 27B checkpoint is binding below floor; W1 is fully classified and improves the grid but fails strict component/full gates. Fresh runtime trace confirms the absent narrow Stream-K/static family | implement claimed W2's exact 32-tactic/workspace/test surface, then repeat component + trace + exact 27B until every axis passes; only then run 35B | `ACTIVE` |
