...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid/trace `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; W3-F immutable evidence `~/work/vllm.cpp-nvfp4-device-alpha/7517af4f983fe322ac88ce2d9869e1441b7be3fd/evidence`; W3-G immutable DGX root `~/work/vllm.cpp-fa2-decode/ae9e8ff0576badabdda7289beeacaa1041c55d21`; W3-H local `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; H1a `~/work/vllm.cpp-executed-path-refresh/5d8af792…`; H1b `~/work/vllm.cpp-executed-path-refresh-frozen-h1b/5d8af792…`; H1c `~/work/vllm.cpp-executed-path-refresh-h1c/d1f8e332…`; H1d failed setup `~/work/vllm.cpp-executed-path-refresh-h1d/7bae38afe8f97603fec525e8a2b8833bcba983cf`; replacement immutable root pending | current checkpoint `codex/nvfp4-small-m` | online harness/evidence/matrices plus completed W3-G ratio-6 FA2 gates and [W3-H normal-producer spike](specs/nvfp4-bf16-producer-vectorization.md). H1 owns the diagnostic-only four-replay controller, target-build/dispatch manifest, exact-workload paired ours/vLLM trace validation and residual re-ranking; W3-H2 inference-kernel implementation remains prohibited until valid H1d evidence. Host-weight, fused producer, GDN changes, scheduler and 35B performance excluded. ONE `flock` around the complete series | `ACTIVE` | 2026-07-13 (W3-G remains without speed credit. H1a/H1b/H1c are `VOID`; H1c both lost events and ran a CUTLASS-disabled naive/WMMA fallback. H1d's controller/validators are implemented and CPU-gated. Clean `7bae38a` failed before compilation, model load or GPU because CMake could not find Ninja on login `PATH`; configure SHA `ac9e4854…f616`. The replacement recipe pins oracle Ninja and needs a new immutable root. No ratio, exact grid or 35B performance exists. GPU, `/tmp/gpu` and serving ports are idle; `3f256ab` remains 55/124) |
...
execution is next from the pinned-Ninja replacement SHA. The retained `7bae38a`
root is a pre-build/pre-GPU failure and cannot be reused; no W3-H2
inference-kernel code is authorized before three valid target-path captures
re-rank the residual.

## Handoff queue

| Priority | Row/block | Dependency | Next handoff | State |
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + W3-H executed-path refresh | `3f256ab` remains 55/124; W3-E/W3-F/W3-G strict-fail. H1a/H1b/H1c are `VOID`; clean H1d `7bae38a` is also a pre-build/pre-GPU environment failure because login `PATH` omitted Ninja. The controller/schema remains CPU-gated; GPU/lock/ports are idle | execute the new clean pushed plan/reproduction repair in a fresh immutable root with oracle Ninja pinned by absolute path. Require three zero-exit unique Nsight sessions, lossless direct graph-child correlation, identical resources and exact plans before implementing any lever. No exact grid/35B performance until a later component passes every axis | `ACTIVE` |
