...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid/trace `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; W3-F immutable evidence `~/work/vllm.cpp-nvfp4-device-alpha/7517af4f983fe322ac88ce2d9869e1441b7be3fd/evidence`; W3-G local `/home/mudler/_git/vllm.cpp-nvfp4-small-m`, immutable DGX root `~/work/vllm.cpp-fa2-decode/ae9e8ff0576badabdda7289beeacaa1041c55d21` | current checkpoint `codex/nvfp4-small-m` | online harness/evidence/matrices plus completed W3-G ratio-6 FA2 split-KV decode adapter/gates and the fresh executed-path scan. The FP4 producer remains read-only/unspiked; host-weight, GDN, scheduler and 35B performance excluded. ONE `flock` around a complete model A/B+trace or component series | `ACTIVE` | 2026-07-13 (W3-G source/CUDA/safety/model/trace gates pass at `ae9e8ff`. Its completed 12-leg/612-request frozen c2/c16 component reaches **1.017668×/1.006548×** mean total throughput but strict-fails **35/40 timing + 5/8 memory**; lifecycle/cache/plan integrity passes. W3-G earns no speed credit and no exact grid/35B performance follows. GPU, `/tmp/gpu` and serving ports exit idle. `3f256ab` remains 55/124; fresh executed-path scanning must reverify and spike the next lever) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + executed-path parity scan | `3f256ab` remains 55/124; W3-C reproduction control is green, while W3-E/W3-F/W3-G strict-fail and earn no speed credit. W3-G's completed c2/c16 result is **1.017668×/1.006548×**, **35/40 timing + 5/8 memory**, with all integrity checks green; GPU/lock/ports are idle | reprofile ours and vLLM on the same workload, reverify the leading vectorized normal BF16→FP4 producer candidate, inventory the whole chain/tests/gates and commit its spike before implementation. No exact grid/35B performance until a component passes every axis | `ACTIVE` |
