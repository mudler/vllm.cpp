...
traces only. Schema-v4 `b9beccd` remains the failed/void checkpoint. The
minimal probe now classifies its exact warning as pinned-tool capture-boundary
behavior, and schema-v5 exact reconciliation is CPU-green. The next work is a
fresh immutable 12-report DGX root. W3-H2 inference code remains unauthorized
until that gate passes and re-ranks the residual. Superseded attempt roots
remain in the append-only record, not this live control plane.

## Handoff queue

| Priority | Row/block | Dependency | Next handoff | State |
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + W3-H executed-path refresh | `3f256ab` remains 55/124; W3-E/W3-F/W3-G earn no speed credit. Schema-v4 `b9beccd` remains VOID; its warning is calibrated and schema-v5 exact reconciliation is CPU-green. GPU/lock/ports are idle | run a fresh immutable 12-report schema-v5 root before implementing any lever. No exact grid/35B performance until a later component passes every axis | `ACTIVE` |
