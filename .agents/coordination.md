...
| `CLAIM-SERVE-TRANSPORT-1` | `SERVE-HTTP-TRANSPORT` | root | isolated worktree `/home/mudler/_git/vllm.cpp` tree `agent-a772a917b56dcdb09` | `main` (direct push) | api_server transport socket options (`src/vllm/entrypoints/openai/api_server.cpp` `set_tcp_nodelay`), its behavioral socket test (`tests/vllm/entrypoints/openai/test_api_server.cpp`), and this row's record surfaces (engine matrix, README, BENCHMARKS, roadmap order-0, state, ledger). Excludes everything GPU and the entire c16 diagnostic track (`scripts/dgx-gdn-packed-component.sh`, `tools/bench/gdn_packed_component.py`). No GPU work runs under this claim | `ACTIVE` | 2026-07-14 (mirror implemented + CPU-tested: RED accepted-socket `TCP_NODELAY` 0 → GREEN 1; full `test_openai_api_server` 22/22 cases / 242 assertions; `-Werror` clean. The single non-binding localhost GPU sizing is PENDING because the GPU is held by the c16 diagnostic; axis credit only via the authorized exact-grid rerun. Row stays `ACTIVE` while this claim is open — the record checker forbids a claim referencing a non-SPIKE/ACTIVE row — and converges to `GATING` on release) |
