...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; active worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; accepted H1d root `~/work/vllm.cpp-executed-path-refresh-h1d/c498a4131af7e6cf0ac678841212af80f4f12d53`; W3-I root `~/work/vllm.cpp-nvfp4-fused/15c6b8933d982019aa8965d218deb0eb1d9dc3f4-r2`; VOID async root `~/work/vllm-async-credit/4d85ead5e4d9c8eaa72698813869f5ac226474f3`; fresh root follows the repair commit | current checkpoint `codex/nvfp4-small-m` | online harness/evidence/matrices and diagnostic-only vLLM async ON/OFF c2 timing + Torch traces. `ENG-ASYNC-SCHED`, loader/memory, RMSNorm, W3-H2/I2+, exact-grid, and 35B implementation remain excluded until the control selects a lever. ONE `flock` around the complete async series | `ACTIVE` | 2026-07-14 (`4d85ead` VOID before requests: spawned EngineCore lacked `ninja`; cleanup pass. Fresh venv/CUDA-PATH repair pending; binding stays 55/124) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` async-credit control | `3f256ab` remains 55/124; scan complete. First `4d85ead` setup is VOID before requests on missing EngineCore `ninja`; zero measurements and cleanup pass | create a fresh commit-owned root, prepend oracle venv/CUDA tools, then repeat all six timing legs + two traces under one lock. If credit explains 4–6%, update claim and activate W3; if neutral, map low-batch RMSNorm partitions | `ACTIVE` |
