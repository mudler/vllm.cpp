...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; active worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; latest failed/void H1d root `~/work/vllm.cpp-executed-path-refresh-h1d/b9beccdab23d103bcdcb950bae89e78bfeceff15`; superseded H1 roots remain in state/ledger only | current checkpoint `codex/nvfp4-small-m` | online harness/evidence/matrices plus completed W3-G ratio-6 FA2 gates and [W3-H normal-producer spike](specs/nvfp4-bf16-producer-vectorization.md). H1 owns the diagnostic-only four-replay controller, graceful target stop, target-build/dispatch manifest, exact-workload paired ours/vLLM trace validation and residual re-ranking; W3-H2 inference-kernel implementation remains prohibited until valid H1d evidence. Host-weight, fused producer, GDN changes, scheduler and 35B performance excluded. ONE `flock` around the complete series | `ACTIVE` | 2026-07-13 (clean schema-v4 `b9beccd` passes exact 154/154 build, 27B 1/1 gate, plans, clients, FIFO/zero-exit and four complete zero-eager reports. Strict validation rejects pinned Nsight's possible-loss capture-range diagnostic before sessions 2/3 or vLLM, so it is VOID. Calibrate and reconcile the diagnostic before a fresh 12-report run; `3f256ab` remains 55/124) |
...
`CLAIM-SERVE-GATE-1` owns the diagnostic H1d harness and resulting paired
traces only. Clean schema-v4 `b9beccd` is the current failed/void checkpoint:
its exact target path and four graph reports are structurally complete, but the
Nsight capture-range diagnostic is unresolved. The next work is the pinned-tool
minimal calibration and an exact-reconciliation rule, followed by a fresh
12-report root. W3-H2 inference code remains unauthorized until that gate
passes and re-ranks the residual. Superseded attempt roots remain in the
append-only record, not this live control plane.

## Handoff queue

| Priority | Row/block | Dependency | Next handoff | State |
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + W3-H executed-path refresh | `3f256ab` remains 55/124; W3-E/W3-F/W3-G earn no speed credit. Clean schema-v4 `b9beccd` is VOID on pinned Nsight's capture-range diagnostic despite four complete zero-eager reports; GPU/lock/ports are idle | calibrate and exactly reconcile the diagnostic, then run a fresh 12-report root before implementing any lever. No exact grid/35B performance until a later component passes every axis | `ACTIVE` |
