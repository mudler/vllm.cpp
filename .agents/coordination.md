...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; active worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; accepted H1d root `~/work/vllm.cpp-executed-path-refresh-h1d/c498a4131af7e6cf0ac678841212af80f4f12d53`; accepted async root `~/work/vllm-async-credit/3812d8d2b4a68d2e501007d01fe10cdf17751d02`; current c2 raw root `~/work/vllm.cpp-executed-path-c2/179a0fc2afc1c33b63d14de8e50d3fde976c7356` | current checkpoint `codex/nvfp4-small-m` | online harness/evidence/matrices and exact low-batch ours/vLLM BF16-GEMM/RMSNorm/FP4-tactic mapping. `ENG-ASYNC-SCHED`, loader/memory implementation, W3-H2/I2+, exact-grid, and 35B implementation remain excluded until a complete kernel lever is selected. Any A/B or trace series uses ONE `flock` | `ACTIVE` | 2026-07-14 (complete raw c2 series: model gate pass, **12/12** invariant local **1,011+7+1** ranges, fresh oracle **1,522×1,160** steady B=2 windows + two drains. Read-only finalizer preflight passes; durable marker pending. Binding 55/124; no speed credit) |
...
ranges and 1,522 exact oracle B=2 windows plus two bounded drains. Read-only
finalizer preflight passes; its durable summary/manifest/marker is pending.
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` low-batch kernel mapping | `3f256ab` remains 55/124; complete `179a0fc` raw c2 evidence passes read-only finalizer preflight, with durable status pending | write the durable marker, then whole-chain spike/gate the highest complete BF16-GEMM/RMSNorm lever before the exact grid | `ACTIVE` |
