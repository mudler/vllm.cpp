...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local main (docs only); DGX `~/work/vllm.cpp-latency` (harness/build @ 83010c7) + `~/work/vllm.cpp-latency/latres2` | `main` (docs-only edits per push policy) | DGX latency dir + `lat_*.sh` harness; engine-matrix `SERVE-GATE-ONLINE` row; roadmap `ROAD-V1-A` row; ledger appends. ONE `flock` per model A/B series (27B ours+vLLM single hold; 35B ours+vLLM single hold) | `ACTIVE` | 2026-07-11 (second same-lock 27B ours/vLLM series completed every greedy/sampled point with both arms rc=0; the queued PR3 focused test passed and its pool A/B now owns `/tmp/gpu`; the scripted 35B series is waiting to reacquire the lock) |

## Handoff queue

| Priority | Row/block | Dependency | Next handoff | State |
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` | First campaign invalid: vLLM startup failure + ours-35B aborts | root recovered `CLAIM-SERVE-GATE-1`; 35B sanitizer diagnostic is clean, the complete second 27B same-lock ours→vLLM series is retained as pre-W2 baseline evidence, and its 35B continuation waits behind queued PR3 work | `ACTIVE` |
