...
| `CLAIM-NVFP4-SMALL-M-4` | `KERNEL-GEMM-NVFP4-W4A4` (`W3-F`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; read-only traces `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560/trace/27` and `~/work/vllm.cpp-nvfp4-direct-sf-evidence/trace`; future immutable evidence under `~/work/vllm.cpp-nvfp4-device-alpha/<SHA>/` | `codex/nvfp4-small-m` | W3-F only: model-owned device F32 alpha, tensor op ABI/validation, `VT_FP4_DEVICE_ALPHA=0` fallback, ported tests, safety/model/node-trace and frozen-plan c2/c16 gates per [spike](specs/nvfp4-device-alpha.md). Quant/GDN/attention/host-weight changes excluded | `ACTIVE` | 2026-07-13 (spike `READY`; local steady trace has 296,575 graph `SetScalar` / 1,425 forwards, vLLM zero. No implementation, GPU run, ratio or exact-grid/35B authorization) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + FP4 repair | `3f256ab` remains 55/124; W3-C reproduction control is green and W3-E fails at 39/40 timing + 1/8 memory. The resumed whole-chain scan selects W3-F device alpha from 208 local scalar stages/forward versus zero in vLLM; its spike/claim are ready and GPU/ports/lock are idle | implement W3-F with the same-binary host-alpha fallback, port upstream tensor-alpha tests, then run pushed safety/model/node-trace and frozen-plan c2/c16 gates. No exact grid/35B performance before an every-axis component passes | `ACTIVE` |
