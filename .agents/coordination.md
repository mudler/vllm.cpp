...
model semantics and immutable G2 close at clean `f344dec`. Paired trace and the
component result remain open. qkvz, exact-grid and 35B performance stay excluded until
those gates close.
...
|---|---|---|---|---|
| 1 | `KERNEL-GEMM-BF16` + `KERNEL-GDN-PACKED-DECODE` for `SERVE-GATE-ONLINE` | `3f256ab` remains 55/124; clean `f344dec` closes W1D2/G2 | complete paired node traces plus c2/c16 before qkvz | `GATING` |
