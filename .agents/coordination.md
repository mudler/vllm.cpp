...
exact-four-replay `a96e899` exit-143 failure cannot be reused;
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + W3-H executed-path refresh | `3f256ab` remains 55/124; W3-E/W3-F/W3-G strict-fail. H1a/H1b/H1c and H1d attempts through `a96e899` are `VOID`; the latest closes four replays but target SIGTERM yields profiler exit 143 before validation. GPU/lock/ports are idle | execute the clean pushed graceful-stop repair in a fresh immutable root with plan-first exact build, recorded Nsight ancestry, synchronous diagnostic SIGUSR1 target stop and zero exit. Require correctness plus three unique lossless sessions, direct graph-child correlation, identical resources and exact plans before implementing any lever. No exact grid/35B performance until a later component passes every axis | `ACTIVE` |
