...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local main (docs only); DGX `~/work/vllm.cpp-latency` (harness/build @ 83010c7) + `~/work/vllm.cpp-latency/latres2` | `main` (docs-only edits per push policy) | DGX latency dir + `lat_*.sh` harness; engine-matrix `SERVE-GATE-ONLINE` row; roadmap `ROAD-V1-A` row; ledger appends. ONE `flock` per model A/B series (27B ours+vLLM single hold; 35B ours+vLLM single hold) | `ACTIVE` | 2026-07-11 (35B exact-shape sanitizer diagnostic survived 3/3 with zero reports; second same-lock campaign completed our full 27B arm; matched vLLM 27B greedy c1-c16 completed all three repetitions, c32 repetitions 1-2 completed, and c32 repetition 3 is active; its later replicated/sampled points and both 35B arms remain) |
