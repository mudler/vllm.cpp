...
|---|---|---|---|---|---|---|---|
| `CLAIM-C5-SPIKE-1` | `KV-SLIDING-LOCAL-SPECS`, `KV-SLIDING-WINDOW-SPEC`, `KV-CHUNKED-LOCAL-SPEC`, `ATTN-YARN`, `ATTN-ROPE-FAMILY`, `ATTN-ROPE-LLAMA3`, `ATTN-ROPE-LONGROPE`, `ATTN-ROPE-DYNAMIC-NTK`, `ATTN-SLIDING-WINDOW`, `ATTN-CHUNKED-LOCAL` | `road-v1-c5-spike` (Codex) | local `/home/mudler/_git/vllm.cpp-road-v1-c5-spike` (read-only upstream analysis; no build/GPU directory) | `codex/road-v1-c5-spike` -> `main` | spike-only: new `specs/sliding-local-yarn-long-context.md`; the named C5 rows and lifecycle summaries in engine/feature matrices; `ROAD-V1-C5`; this claim; state append; pinned checker engine-row count. No implementation/source/test edits and no GPU execution. | `SPIKE` | 2026-07-10 (claim committed before deep analysis; six claimable leaves added while preserving the two prior IDs as joint-spike umbrellas) |
