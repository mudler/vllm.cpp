...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local `/home/mudler/_git/vllm.cpp-serve-gate`; DGX `~/work/vllm.cpp-latency` (pre-W2 build @ 83010c7) + a fresh current-main gate directory after the diagnostic lock releases | `codex/serve-gate-harness` | committed `tools/bench/online_gate_*`, `scripts/dgx-online-serving.sh`, `tests/tools/test_online_gate_*`; DGX harness/results; engine-matrix `SERVE-GATE-ONLINE`; roadmap `ROAD-V1-A`; ledger/state. ONE `flock` around every complete current-main ours/vLLM A/B series | `ACTIVE` | 2026-07-11 (pre-W2 27B diagnostic completed both arms rc=0; after PR3 released the lock, the pre-W2 35B local server completed only 1/6 c1 requests then aborted with a surfaced `cudaFree` illegal-memory-access error, so that arm and series are void; its vLLM arm continues only as diagnostic evidence while the committed fail-closed post-W2 harness is hardened) |

## Handoff queue

| Priority | Row/block | Dependency | Next handoff | State |
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` | First campaign invalid: vLLM startup failure + ours-35B aborts | root recovered `CLAIM-SERVE-GATE-1`; the second pre-W2 27B same-lock series is diagnostic-complete, while its 35B local arm is void after a 1/6-request illegal-memory-access abort and the still-running vLLM arm cannot repair it. Commit/fuzz the fail-closed post-W2 harness, then build current main and run W2 G1/G3-G6 under new whole-series locks | `ACTIVE` |
