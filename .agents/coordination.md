...
| `CLAIM-C5-LLAMA3-1` | `ATTN-ROPE-LLAMA3` | root | local `/home/mudler/_git/vllm.cpp-c5-llama3` | `codex/c5-llama3-w6` | W6 only: Llama 3 typed `low_freq_factor`/`high_freq_factor` config fields, new `include/src/vllm/model_executor/layers/rotary_embedding/llama3_rope.*`, the exact factory branch in `base.cpp`, CMake/leaf tests, extensions to the generic long-context oracle dumper/runner, pinned goldens, and C5 record rows. No W7/W8 LongRoPE/dynamic formula, W5 YaRN/MRoPE semantic rewrite, model-family/MM preprocessing, local-attention edit, or GPU execution | `ACTIVE` | 2026-07-11 (W5 merged as `d07dd1c`; port the exact low/mid/high frequency bands and boundary goldens on its shared cache/apply foundation while the serving campaign retains dgx) |
