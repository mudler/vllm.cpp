...
runner/finalizer is CPU-green **45/45**. Clean `d82d282` passed both model gates
and all c2 legs, then c16 packed r1 returned 96/96 HTTP 500 and the driver
exited without a marker. The root is preserved and resources are clean/free.
qkvz, exact-grid and 35B performance stay excluded until a fresh run closes.
...
|---|---|---|---|---|
| 1 | `KERNEL-GEMM-BF16` + `KERNEL-GDN-PACKED-DECODE` for `SERVE-GATE-ONLINE` | `3f256ab` remains 55/124; correctness/structure close at `f344dec` and `7ff713e` + `24cea4f`; `d82d282` failed incomplete at first c16 timed batch | capture the discarded server exception, root-cause/repair test-first, then execute a fresh-SHA one-lock **40+8 median + gated-paired** series (c2 TTFT-family pooled/50%-sanity and excluded from the gated paired set) before qkvz | `GATING` |
