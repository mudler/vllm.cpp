...
| `CLAIM-NVFP4-SMALL-M-3` | `KERNEL-GEMM-NVFP4-W4A4` (`W3`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; immutable W3-A evidence `~/work/vllm.cpp-nvfp4-small-m/71f1e894d0c5e496607d08cfe9089a9944128271/w3`; disposable W3-B stage `~/work/vllm.cpp-nvfp4-small-m/precommit-w3b-195b475`; exact online-gate root | `codex/nvfp4-small-m` | W3 only: A mirrors production pip-vLLM/FlashInfer's three-warmup, stream-sync, 1-ms GPU-delay, ten-repeat eager event selection; B adds actual-W4A4-gated shared-loader max-token/all-bucket in-memory warmup, completion stats, capture-ready reuse, diagnostic lazy fallback and `VT_FP4_PRE_SERVE_WARMUP=0`; C adds separately gated optional versioned device/CUDA/CUTLASS/model-shape persistence with atomic load/save/stale/collision rejection. No new tactic, model topology, HTTP, scheduler, GDN or device-residency changes. Same-binary selection A/B, paired trace, then exact 27B under one uncontended lock | `ACTIVE` | 2026-07-12 (W3-A strict-failed. W3-B implementation stage passes CPU 3/3, exact/legacy CUDA 14/14 + 26,819 each, 27B 235/235 + 16/16, and server ordering with 80/80 profiles/zero lazy misses. Evidence hashes `96afc6ed…de401` / `2264a306…7e9e`. Clean pushed-SHA sanitizer, repeated selection/component, trace and oracle gates remain pending. Pip-vLLM disables its colliding file cache, so persistence cannot own the speed denominator) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + FP4 repair | Exact clean `b5c6e4f` 27B checkpoint is binding below floor. W2 improves every concurrency and wins c16/c32 throughput, but c1-c8, TPOT/ITL and host memory remain red. W3-A strict-fails. W3-B's shared-loader all-bucket implementation is disposable-stage green at 80/80 profiles, zero lazy misses and 16/16 correctness; no immutable speed evidence exists yet | freeze pushed W3-B, run focused sanitizer/model/server plus repeated plan-stability/component and paired trace, gate optional W3-C persistence separately, repeat exact 27B until every performance and memory axis passes, then run 35B | `ACTIVE` |
