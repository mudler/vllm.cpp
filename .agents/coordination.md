...
| `CLAIM-BACKEND-ABI-VT-SPIKE-1` | `BACKEND-ABI-VT` | `backend-abi-vt-spike` takeover agent (recovered prior draft after predecessor rate limit) | isolated worktree `/home/mudler/_git/vllm.cpp-backend-abi-spike`; no remote dir or GPU work | `codex/backend-abi-vt-spike` → `main` | Docs only: new `.agents/specs/dropin-kernel-abi.md`; `BACKEND-ABI-VT` row in `.agents/backend-matrix.md`; C1 convention/reference only in `.agents/kernel-matrix.md`; `ROAD-V1-C1` rows in `.agents/roadmap_v1.md`; this claim/handoff row; `.agents/state.md` append. No implementation, test, build, ledger, README, or GPU ownership | `ACTIVE` | 2026-07-10 (claim repaired before recovering the predecessor's uncommitted spec into this worktree) |
| `CLAIM-MODEL-FACTORY-SPIKE-1` | `MODEL-FACTORY-registry` | `codex` (handoff recovery of stopped C2 spike) | local main, preserving stopped agent's draft | `main` | `.agents/specs/model-factory-registry.md`; `model-matrix.md` factory row; `roadmap_v1.md` C2 row; `coordination.md` claim/handoff; `state.md` append. Docs-only; no GPU | `SPIKE` | 2026-07-10 (recovered complete draft; validating anchors and spike contract before READY) |
