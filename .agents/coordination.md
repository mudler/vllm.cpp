...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid/trace `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; W3-F immutable evidence `~/work/vllm.cpp-nvfp4-device-alpha/7517af4f983fe322ac88ce2d9869e1441b7be3fd/evidence`; W3-G immutable DGX root `~/work/vllm.cpp-fa2-decode/ae9e8ff0576badabdda7289beeacaa1041c55d21`; W3-H local `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; H1a `~/work/vllm.cpp-executed-path-refresh/5d8af792…`; H1b `~/work/vllm.cpp-executed-path-refresh-frozen-h1b/5d8af792…`; H1c `~/work/vllm.cpp-executed-path-refresh-h1c/d1f8e332…`; H1d immutable root pending | current checkpoint `codex/nvfp4-small-m` | online harness/evidence/matrices plus completed W3-G ratio-6 FA2 gates and [W3-H normal-producer spike](specs/nvfp4-bf16-producer-vectorization.md). H1 owns target-build/dispatch preflight, exact-workload paired ours/vLLM traces, lossless validation and residual re-ranking; implementation is prohibited until valid H1d. Host-weight, fused producer, GDN changes, scheduler and 35B performance excluded. ONE `flock` around the complete series | `ACTIVE` | 2026-07-13 (W3-G remains without speed credit. H1a/H1b are `VOID`. Immutable `d1f8e33` H1c is doubly `VOID`: possible event loss stopped before r2/r3/vLLM, and configure/compile/kernel evidence proves CUTLASS was disabled and **146,661 naive + 10,944 WMMA** FP4 GEMMs ran instead. H1d must fail before GPU unless exact CUTLASS dependency, `VT_CUTLASS_NVFP4`, binary and runtime dispatch provenance pass, then harden trace semantics. No implementation, ratio, exact grid or 35B performance exists. GPU, `/tmp/gpu` and serving ports are idle; `3f256ab` remains 55/124) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + W3-H executed-path refresh | `3f256ab` remains 55/124; W3-C reproduction control is green, while W3-E/W3-F/W3-G strict-fail and earn no speed credit. W3-H H1a/H1b/H1c are `VOID`; H1c lost events and was a CUTLASS-disabled naive/WMMA fallback build. Implementation remains prohibited; GPU/lock/ports are idle | hard-gate the exact external CUTLASS tree, compile definition, binary linkage and runtime target family before GPU work; harden lossless collection and exact plan/workload/graph/capture/vLLM semantics; then run immutable H1d at cache-off input-1,024/output-128 with the exact read-only 64-plan fixture. Implement only the highest losslessly verified lever; no exact grid/35B performance until a component passes every axis | `ACTIVE` |
