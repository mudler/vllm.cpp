...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local `/home/mudler/_git/vllm.cpp-serve-gate`; binding grid `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; W3-D evidence as recorded; W3-E staging `~/work/vllm.cpp-nvfp4-direct-sf-evidence` | `codex/serve-gate-harness` (merged `b0acec2`; current checkpoint on `codex/nvfp4-small-m`) | `tools/bench/online_gate*`, profiler/summarizer/driver contracts, exact DGX evidence, matrices/roadmap/ledger/state. ONE `flock` around each complete model A/B+trace series | `ACTIVE` | 2026-07-13 (`3f256ab` remains 55/124. W3-E implementation/correctness/sanitizer/trace pass: direct/fallback 27B 235/235+16/16, 35B inert 315/315, and 624 activation swizzles removed. c2/c16 immutable A/B precedes any exact rerun; 35B performance held) |
...
| `CLAIM-NVFP4-SMALL-M-3` | `KERNEL-GEMM-NVFP4-W4A4` (`W3`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; binding grid as above; mutable W3-E build/safety/trace under `~/work/vllm.cpp-nvfp4-direct-sf-{src,build-cuda,evidence}` | `codex/nvfp4-small-m` | W3 owns FP4 tactic/topology repair. W3-E scope is explicit linear/direct-swizzled producer layouts, padded zero contract, true-W4A4 CUTLASS dispatch, ported tests and `VT_FP4_DIRECT_SF=0` per [spike](specs/nvfp4-direct-swizzled-scales.md). No other lever stacks into its A/B | `ACTIVE` | 2026-07-13 (W3-E code, CPU/CUDA byte/GEMM tests, focused sanitizer, direct/fallback 27B, 35B inertness and paired structural trace pass. Standalone swizzles are **208 direct / 832 fallback**, removing 624 activation launches. c2/c16 unprofiled A/B and speed/memory disposition remain pending. Do not run 35B performance) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + FP4 repair | Binding immutable `3f256ab` remains 55/124. W3-E implementation/correctness/sanitizer/trace pass and remove 624 activation swizzles; no unprofiled performance number exists. GPU/ports/lock idle | commit/push the W3-E checkpoint, then run immutable c2/c16 AB/BA/AB over all timing+memory axes. Exact rerun only if the component accepts. Modernize/delete only trace-proven paths; 35B performance only after full 27B closure | `ACTIVE` |
