...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; active worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; finalized c2 root `~/work/vllm.cpp-executed-path-c2/179a0fc2afc1c33b63d14de8e50d3fde976c7356`; W1 safety root `~/work/vllm.cpp-gdn-ba/immutable-581d335fec2e5a96d9ccbb38c1ec001c39ac1789`; completed structural root `~/work/vllm.cpp-gdn-ba-trace/0091cd192d9a6baa2197a4f3bdb0561bd859baf5` | current checkpoint `codex/nvfp4-small-m` | Online-gate execution and harness work. `CLAIM-GDN-BA-ROUNDING-1` leads packed-decode code and its c2/c16 component; this claim retains the binding grid. qkvz code, loader-memory repair, exact grid, and 35B performance are excluded. Any A/B or trace series uses one `flock` | `ACTIVE` | 2026-07-14 (BA safety/structure/projection remain closed. BF16 27B is 233/235; the packed oracle now selects the exact q/k/beta fix at `0/1`. Immutable replay, implementation and c2/c16 remain open. Binding 55/124, no speed credit) |
| `CLAIM-GDN-BA-ROUNDING-1` | `KERNEL-GEMM-BF16` (`W1C`), `KERNEL-GDN-PACKED-DECODE` (`W1D`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-gdn-ba-rounding`; DGX roots `~/work/vllm.cpp-gdn-ba-rounding/f9252943d1e96dbfa43e3b8f2d06dec1aa5f20d3` and `~/work/vllm.cpp-gdn-ba-rounding/diagnostic-w1c` | `codex/gdn-ba-rounding-w1c` | Leads `ProjectGdnBA` output dtype plus the exact pure-non-spec packed-decode op in `include/vt/ops.h`, `src/vt/{ops.cpp,cpu/cpu_ops.cpp,cuda/cuda_gdn.cu}`, registrations, `qwen3_5.cpp`, focused oracle/tests, 35B inertness, node trace and c2/c16 component. This is the explicit overlap lead over `CLAIM-PR3` for pure decode/model dispatch and over `CLAIM-SERVE-GATE-1` for the component. Prefill/mixed/spec GDN, qkvz, host-weight repair, binding-grid execution and 35B performance are excluded. One `flock /tmp/gpu` covers every GPU correctness or A/B series | `ACTIVE` | 2026-07-14 (projection/inertness remain closed. Official packed oracle identifies F32 in-kernel q/k normalization plus dtype-rounded beta; mutable local replay is current `306/7552`, beta-only `308/6558`, full semantics `0/1`. Commit/push and immutable replay precede production implementation; no speed credit) |
...
boundary is now identified and owned by `KERNEL-GDN-PACKED-DECODE`. Immutable
replay, production semantics and the component result remain open.
qkvz, exact-grid and 35B work stay excluded until those gates close.

## Handoff queue

| Priority | Row/block | Dependency | Next handoff | State |
|---|---|---|---|---|
| 1 | `KERNEL-GEMM-BF16` + `KERNEL-GDN-PACKED-DECODE` for `SERVE-GATE-ONLINE` | `3f256ab` remains 55/124; `581d335`/`0091cd1`/`f925294` close BA safety, structure and projection/inertness; the packed oracle proves the remaining 233/235 failure is q/k/beta consumer semantics | commit and immutably replay W1D0, port the full pure-decode op, restore 235/235, then complete c2/c16 before qkvz | `GATING` |
