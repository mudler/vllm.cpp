...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local `/home/mudler/_git/vllm.cpp-serve-gate`; evidence root `~/work/vllm.cpp-online-gate` | `codex/serve-gate-harness` (merged `b0acec2`; corrected trace/gate contract active) | `tools/bench/online_gate*`, profiler/summarizer/driver contracts, exact DGX evidence, matrices/roadmap/ledger/state. ONE `flock` around each complete model A/B+trace series | `ACTIVE` | 2026-07-12 (`b5c6e4f` is historical vLLM 0.24.0 evidence. `3cc490c` was stopped and is VOID at 28/36 groups, 1,602/2,016 requests, four returns and no trace. The canonical v0.25.0/FlashInfer 0.6.13 oracle is now validated/active after exact package/import/CLI, lock-held real 27B production-graph and server health/completion smoke; v0.24 rollback is preserved. Commit the migrated harness contract, then start a new SHA-bound 27B grid/trace. No current ratio binds; 35B held) |
...
| `CLAIM-NVFP4-SMALL-M-3` | `KERNEL-GEMM-NVFP4-W4A4` (`W3`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; immutable W3-A evidence `~/work/vllm.cpp-nvfp4-small-m/71f1e894d0c5e496607d08cfe9089a9944128271/w3`; immutable W3-B evidence `~/work/vllm.cpp-nvfp4-small-m/d7cdf66db0cfcc53d68d49613623ec6cd3807641/w3b`; void attempt `~/work/vllm.cpp-online-gate/evidence/3cc490cfa6314e81a69451c5f175b071e7970506` | `codex/nvfp4-small-m` | W3 owns v0.25.0 re-trace and the highest-ranked executed FP4 topology/selection repair. A/B correctness and structural evidence remain; C persistence stays separately optional. Do not implement direct-swizzled scales, resident alpha or another topology change until the new paired trace proves it is on the remaining path | `ACTIVE` | 2026-07-12 (`3cc490c` is VOID at 28/36 groups after the 0.24.0/0.25.0 denominator drift was proven. No trace or partial rate binds. v0.25.0 validation/activation plus real 27B offline/server smoke are complete; commit the contract, then run the new exact grid and paired trace before changing code. 35B held) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + FP4 repair | No current denominator binds. `b5c6e4f` is historical vLLM 0.24.0 evidence; `3cc490c` is VOID at 28/36 groups after the FlashInfer 0.6.12/0.6.13 drift was proven. The canonical v0.25.0 oracle is validated/active and rollback-safe; GPU/ports/lock are idle | commit the migrated v0.25.0 harness contract, then run the exact 27B c1-c32 grid and paired traces. Modernize/delete only trace-proven paths, repair every red 27B axis, gate W3-C separately, and run 35B only after full 27B closure | `ACTIVE` |
