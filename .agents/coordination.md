...
| `CLAIM-C5-CHUNKED-KV-1` | `KV-CHUNKED-LOCAL-SPEC` | root | local `/home/mudler/_git/vllm.cpp-c5-chunked-kv` | `codex/c5-chunked-kv-w3` | W3 only: chunked-local slices in `include/src/vllm/v1/kv_cache_interface.*`, `kv_cache_spec_registry.*`, `core/single_type_kv_cache_manager.*`, `core/kv_cache_utils.*`, `core/kv_cache_coordinator.*`; corresponding interface/registry/manager/utils/coordinator tests and C5 matrix/roadmap/inventory/ledger/state rows. No sliding-window attention edits, W4 metadata/backend, RoPE, model-family, connector/offload, DCP/PCP, or GPU work | `ACTIVE` | 2026-07-11 (claim starts from merged W1 registry shape and accepted W3 spike; port concrete chunked-local spec, fixed-chunk prefix/recycling policy, exact admission cap, hybrid-disabled fallback, and deterministic CPU properties without touching the serving GPU campaign) |
...
| 8 | C3/C5/C7/C8 engine/API work | C3/C5/C6 specs accepted; C7/C8 leaf specs missing | C3 M-mtp-0 remains `GATING` on its GPU recipe. C5 W1/W2 are implemented and `GATING`; W3 `KV-CHUNKED-LOCAL-SPEC` is `ACTIVE` under `CLAIM-C5-CHUNKED-KV-1` for CPU-only spec/manager/property work. Restore the oracle and run W2 G4/G6-G9 after serving releases dgx; W4 follows W3 and `ATTN-YARN` remains claimable. Then spike C7/C8 | `ACTIVE` |
