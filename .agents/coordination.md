...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; active worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; finalized c2 root `~/work/vllm.cpp-executed-path-c2/179a0fc2afc1c33b63d14de8e50d3fde976c7356`; W1 safety root `~/work/vllm.cpp-gdn-ba/immutable-581d335fec2e5a96d9ccbb38c1ec001c39ac1789`; completed structural root `~/work/vllm.cpp-gdn-ba-trace/0091cd192d9a6baa2197a4f3bdb0561bd859baf5` | current checkpoint `codex/nvfp4-small-m` | Online-gate execution and harness work. `CLAIM-GDN-BA-ROUNDING-1` leads packed-decode code and its c2/c16 component; this claim retains the binding grid. qkvz code, loader-memory repair, exact grid, and 35B performance are excluded. Any A/B or trace series uses one `flock` | `ACTIVE` | 2026-07-14 (W1D2 packed dispatch is mutable G2-green at default+rollback 27B **235/235**, 35B/GGUF inertness and safety. Immutable G2 plus c2/c16 remain open. Binding 55/124, no speed credit) |
| `CLAIM-GDN-BA-ROUNDING-1` | `KERNEL-GEMM-BF16` (`W1C`), `KERNEL-GDN-PACKED-DECODE` (`W1D`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-gdn-ba-rounding`; immutable G1 root `~/work/vllm.cpp-gdn-packed-decode/9ad8fb76940e68737d2a13ad8ddd97d649bb577c`; mutable G2 root `~/work/vllm.cpp-gdn-packed-decode/w1d2-preflight/evidence-w1d2-final-mutable-20260714-postreview` | `codex/gdn-ba-rounding-w1c` | Leads `ProjectGdnBA` output dtype plus exact packed decode, model dispatch/rollback, independent Mamba cache dtypes, metadata preflight, focused oracle/tests, 35B inertness, node trace and c2/c16 component. Prefill/mixed/spec GDN, qkvz, host-weight repair, binding-grid execution and 35B performance are excluded. One `flock /tmp/gpu` covers every GPU correctness or A/B series | `ACTIVE` | 2026-07-14 (W1D2 implemented; final mutable default+rollback 27B **235/235**, CUDA GDN **43/43**, 35B **315/315**, GGUF **14/14 + 14/14**, and strict memcheck pass. Clean-SHA G2 is next; speed credit remains absent) |
...
model semantics are mutable G2-green. Immutable G2, trace and the component
result remain open. qkvz, exact-grid and 35B performance stay excluded until
those gates close.
...
|---|---|---|---|---|
| 1 | `KERNEL-GEMM-BF16` + `KERNEL-GDN-PACKED-DECODE` for `SERVE-GATE-ONLINE` | `3f256ab` remains 55/124; W1D2 is implemented and mutable G2-green | commit/push and repeat immutable G2, then complete paired node traces plus c2/c16 before qkvz | `GATING` |
