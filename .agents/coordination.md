...
| `CLAIM-NVFP4-SMALL-M-3` | `KERNEL-GEMM-NVFP4-W4A4` (`W3`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; next immutable evidence under `~/work/vllm.cpp-nvfp4-small-m/<pushed-sha>/w3` plus the exact online-gate root | `codex/nvfp4-small-m` | W3 only: mirror pinned vLLM/FlashInfer pre-serve all-bucket warmup, event/graph timing and tactic selection; versioned device/CUDA/CUTLASS/model-shape cache with atomic load/save/stale rejection; stable selected-plan evidence and lazy diagnostic fallback. No new tactic, model topology, HTTP, scheduler, GDN or device-residency changes. Same-binary selection A/B, paired trace, then exact 27B under one uncontended lock | `ACTIVE` | 2026-07-12 (claimed before code edits from accepted spike; W2 trace shows vLLM selects 128x32x256 Stream-K/static while ours predominantly selects 128x128x128/256x128x128) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + FP4 repair | Exact clean `b5c6e4f` 27B checkpoint is binding below floor. W2 improves every concurrency and wins c16/c32 throughput, but c1-c8, TPOT/ITL and host memory remain red; paired traces localize the next mismatch to runtime tactic selection | implement/gate W3 pre-serve all-bucket warmup, selection and versioned plan cache, then repeat exact 27B until every performance and memory axis passes; only then run 35B | `ACTIVE` |
