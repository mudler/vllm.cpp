...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local `/home/mudler/_git/vllm.cpp-serve-gate`; binding grid `~/work/vllm.cpp-online-gate/evidence/9cc71918dbdc10f014c02feb9bab1d00963a16fe`; accepted node trace `~/work/vllm.cpp-online-trace-node/evidence/def5f752896036d9b35841a278578fd812f75a0d`; W3-D mutable staging `~/work/vllm.cpp-packed-qkv-staging` | `codex/serve-gate-harness` (merged `b0acec2`; current checkpoint on `codex/nvfp4-small-m`) | `tools/bench/online_gate*`, profiler/summarizer/driver contracts, exact DGX evidence, matrices/roadmap/ledger/state. ONE `flock` around each complete model A/B+trace series | `ACTIVE` | 2026-07-12 (`9cc7191` remains binding at 54/124. W3-D staging default/fallback correctness and focused sanitizer pass; no speed number binds. Commit/push, clean-build the immutable SHA and run the packed/split same-binary 27B A/B, then re-trace/repeat the grid; keep 35B held) |
...
| `CLAIM-NVFP4-SMALL-M-3` | `KERNEL-GEMM-NVFP4-W4A4` (`W3`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; immutable W3-A/B and binding/trace evidence as above; W3-D mutable staging `~/work/vllm.cpp-packed-qkv-staging` | `codex/nvfp4-small-m` | W3 owns the highest-ranked executed FP4 topology/selection repair. C persistence stays optional; D owns only packed full-attention QKV, its max-shard scalar/resident/row-strided consumers, ported tests and `VT_FP4_MERGED_QKV=0` fallback | `ACTIVE` | 2026-07-12 (W3-D implementation and staging correctness/safety are green: packed/logical exact, strided CUDA suites, zero sanitizer errors, default/split 235/235 +16/16 each. Default/split prewarm is 64/80 profiles. Performance and immutable evidence remain pending: push, clean-build, run same-binary A/B and re-trace; do not stack other levers or run 35B) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + FP4 repair | Binding immutable `9cc7191` v0.25 27B grid is complete: all 124 axes bind, 54 pass and 70 fail. W3-D packed QKV is implemented and mutable-staging correctness/safety plus both real-model arms pass; no performance result binds. GPU/port/lock exit idle | commit/push W3-D, clean-build the immutable SHA, run the packed/split same-binary 27B A/B, then re-trace/re-rank and repeat the exact grid. Modernize/delete only trace-proven paths and run 35B only after full 27B closure | `ACTIVE` |
