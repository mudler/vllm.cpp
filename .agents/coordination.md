...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + `KV-DEVICE-RESIDENCY` | Historical offline denominators and every prior online/paired campaign are non-binding. Exact `ed6247d` vLLM trace is retained as diagnostic; W0 cache/default/closed-loop repair is being merged | finish W0 CPU/record checkpoint; claim `KV-DEVICE-RESIDENCY`; recapture corrected 27B pair, implement device-backed caches then indexed state I/O with same-binary A/Bs; rerun exact direct-library and online 27B before 35B | `ACTIVE` |
