...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + HTTP/FP4 repair | Exact `a531e05` 27B checkpoint is binding below floor; c32 HTTP capacity and small-M FP4 dispatch are reproduced. The fixed HTTP implementation is CPU/sanitizer-green | push the HTTP checkpoint, run fixed/legacy AB/BA/AB c32 plus fresh exact 27B, then write/commit the FP4 spike and port buckets/tactics as separate iterations. Repeat 27B until every axis passes; only then run 35B | `ACTIVE` |
