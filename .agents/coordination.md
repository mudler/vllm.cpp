...
bootstrap failure and exact-build/gate `e1acb75` profiler-topology failure cannot
be reused;
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + W3-H executed-path refresh | `3f256ab` remains 55/124; W3-E/W3-F/W3-G strict-fail. H1a/H1b/H1c and clean attempts through `e1acb75` are `VOID`; the latest exact build and 27B gate pass, then no trace workload/report follows because of the repaired profiler topology invariant. GPU/lock/ports are idle | execute the clean pushed repair in a fresh immutable root with plan-first exact build plus recorded `nsys -> nsys-launcher -> target-session` ancestry and separately owned cleanup. Require correctness plus three zero-exit unique Nsight sessions, lossless direct graph-child correlation, identical resources and exact plans before implementing any lever. No exact grid/35B performance until a later component passes every axis | `ACTIVE` |
