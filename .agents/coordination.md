...
| `CLAIM-HOST-WEIGHT-RESIDENCY-1` | `ENG-HOST-WEIGHT-RESIDENCY` | Codex:gpt-5 | local `/home/rich/c/vllm.cpp`; local RTX evidence under `/tmp/qwen35-memory-*` and `/tmp/qwen35-host-residency-*` | `local-blackwell-environment` | `.agents/specs/discrete-cuda-host-weight-residency.md`; `.agents/specs/safetensors-bounded-page-residency.md`; safetensors reader, dense loader/forward/model-registry files and focused tests; matrix-count invariant in `scripts/check-agent-record.py`; matching matrix/roadmap/README/BENCHMARKS/ledger/state rows. One `/tmp/gpu` lock around each complete local A/B series; no Spark/GB10 access or system configuration changes | `ACTIVE` | 2026-07-12 (W4 page discard implemented default-on with same-binary opt-out. CPU 105/105, native-sm_120 default 5/5 and opt-out 2/2 pass. Peak PSS/load-time/throughput versus OFF and fresh vLLM remain pending; no improved number yet) |
