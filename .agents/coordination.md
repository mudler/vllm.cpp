...
| `CLAIM-LOAD-WINDOWED-1` | `LOAD-SAFETENSORS` | root | isolated worktree `/home/mudler/_git/vllm.cpp/.claude/worktrees/agent-a9185fadaff48c325`; DGX A/B root `~/work/vllm.cpp-windowed-load/<sha>` | `main` (checkpoint pushes) | Windowed source-page release during load: `safetensors_reader.{h,cpp}` release primitive/gate/test-seam, `qwen3_5_weights.cpp` + `qwen3_5_dense_weights.cpp` copy-helper instrumentation, `tests/vllm/test_safetensors.cpp`, and the record surfaces (README, docs/BENCHMARKS.md, engine-matrix `LOAD-SAFETENSORS`, roadmap memory note, state, parity-ledger). EXCLUDES `src/vllm/v1/worker/gpu/runner.cpp`, the GDN slot-lifecycle region of `qwen3_5.cpp` (owned by `CLAIM-GDN-BA-ROUNDING-1`), and `scripts/tools` c16-component surfaces | `ACTIVE` | 2026-07-15 (spike [safetensors-windowed-load.md](specs/safetensors-windowed-load.md) written; page-lifetime proven copied-then-dead for every 27B/35B copy helper, zero referenced-live ranges. Implementing test-first; DGX VmHWM A/B disposition PENDING behind the code checkpoint) |
