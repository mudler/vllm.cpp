...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; active worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; accepted H1d root `~/work/vllm.cpp-executed-path-refresh-h1d/c498a4131af7e6cf0ac678841212af80f4f12d53`; W3-I immutable root `~/work/vllm.cpp-nvfp4-fused/15c6b8933d982019aa8965d218deb0eb1d9dc3f4-r2`; superseded roots remain in state/ledger only | current checkpoint `codex/nvfp4-small-m` | online harness/evidence/matrices, completed [W3-I fused-producer](specs/nvfp4-fused-silu-producer.md) I1 classification, and the read-only post-I1 cross-stack residual scan. Any W3-H2/I2+, host-memory, GDN, scheduler, exact-grid, or 35B implementation remains excluded until the scan selects a lever and the claim/spec is updated. ONE `flock` around any complete GPU series | `ACTIVE` | 2026-07-14 (W3-I1 structure passes but the complete component strict-fails **27/40 timing + 3/8 memory**, c2/c16 **1.002457× / 0.999771×**. Candidate remains off; `3f256ab` remains 55/124. Cross-stack re-ranking is active) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` post-W3-I scan | `3f256ab` remains 55/124. Clean W3-I1 `15c6b89` passes structural gates but complete component-fails **27/40 timing + 3/8 memory**; c2/c16 totals are **1.002457× / 0.999771×** and GPU/lock/ports are idle | compare vLLM v0.25.0 and ours across low-concurrency engine/host overhead, host-memory ownership, and executed-kernel residuals; adversarially rank gain÷effort, then write/update the spike and claim before implementation. No exact grid or 35B performance before an isolated component passes | `ACTIVE` |
