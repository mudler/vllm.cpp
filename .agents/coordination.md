...
| `CLAIM-BACKEND-ABI-VT-SPIKE-1` | `BACKEND-ABI-VT` | `backend-abi-vt-spike` takeover agent (recovered prior draft after predecessor rate limit) | isolated worktree `/home/mudler/_git/vllm.cpp-backend-abi-spike`; no remote dir or GPU work | `codex/backend-abi-vt-spike` → `main` | Docs only: new `.agents/specs/dropin-kernel-abi.md`; `BACKEND-ABI-VT` row in `.agents/backend-matrix.md`; C1 convention/reference only in `.agents/kernel-matrix.md`; `ROAD-V1-C1` rows in `.agents/roadmap_v1.md`; `.agents/backends.md` M0.6/drop-in decision lines only; this claim/handoff row; `.agents/state.md` append. No implementation, test, build, ledger, README, porting-inventory, or GPU ownership | `ACTIVE` | 2026-07-10 (scope repaired before recovering the predecessor's uncommitted spec into this worktree) |
