...
| `CLAIM-KV-DEVICE-1` | `KV-DEVICE-RESIDENCY` | root | main worktree `/home/mudler/_git/vllm.cpp`; baseline evidence `~/work/vllm.cpp-online-gate/evidence/f065fce18027b52a178b246f490b091b9d9e07a3`; W0 evidence `~/work/vllm.cpp-kv-device/7d29e0cd69709f6f96ebbf86a320f8885d7362d1` | `main` | W0 move-only backend cache ownership/device residency + pointer/lifecycle tests, then W1 indexed GDN state I/O. Primary files `include/vllm/v1/worker/gpu/runner.h`, `src/vllm/v1/worker/gpu/runner.cpp`, `src/vllm/model_executor/models/qwen3_5.cpp`, vt CUDA op/tests, owning spec/matrices/README/BENCHMARKS/ledger/state. Every GPU A/B/trace series holds one uncontended lock | `ACTIVE` | 2026-07-11 (pushed `7d29e0c` W0 source/CPU/CI green. Exact f065 pre-W0 baseline complete below floor; its trace records 66,096 H2D + 66,094 D2H state rows and 20.809/20.806 GiB state bytes. Clean CUDA 13.0.88/sm_121a build 100%; default and `VT_DEVICE_KV_CACHE=0` 35B+27B pointer/token gates pass 2/2 each, log SHA `051a83cb…2f66`. Sanitizer/lifecycle/memory/trace/A-B remain open; no speed claim) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + `KV-DEVICE-RESIDENCY` | Exact f065 27B before-state is complete and below floor; historical mismatched campaigns remain void. Pushed `7d29e0c` W0 is CPU/CI-green and passes clean sm_121a default/fallback 35B+27B pointer/token gates | finish W0 sanitizer/lifecycle/memory/corrected-trace and repeated interleaved A/B; then W1 indexed state I/O and exact direct/online 27B parity before 35B. Later roadmap blocks remain behind this priority-zero gate | `ACTIVE` |
