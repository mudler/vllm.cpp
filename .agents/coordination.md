...
| `CLAIM-HOST-WEIGHT-RESIDENCY-1` | `ENG-HOST-WEIGHT-RESIDENCY` | Codex:gpt-5 | local `/home/rich/c/vllm.cpp`; local RTX evidence under `/tmp/qwen35-memory-*` and `/tmp/qwen35-host-residency-*` | `local-blackwell-environment` | `.agents/specs/discrete-cuda-host-weight-residency.md`; `qwen3_5_weights.h`; dense loader/forward/model-registry files and focused tests; matrix-count invariant in `scripts/check-agent-record.py`; matching matrix/roadmap/README/BENCHMARKS/ledger/state rows. One `/tmp/gpu` lock around each complete local A/B series; no Spark/GB10 access or system configuration changes | `ACTIVE` | 2026-07-11 (W1-W3 accepted at `6c30657`: steady PSS 0.752 vs 4.097 GiB vLLM, VRAM 11,689 vs 12,924 MiB, ON +1.74% vs OFF and 0.9754x vLLM. Peak PSS remains 15.55 vs 6.69 GiB; W4 streaming/direct-device follow-on must be spiked before implementation. CPU 105/105 and focused CUDA-build 4/4 pass) |
