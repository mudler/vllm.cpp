...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; active worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; accepted H1d root `~/work/vllm.cpp-executed-path-refresh-h1d/c498a4131af7e6cf0ac678841212af80f4f12d53`; W3-I immutable root `~/work/vllm.cpp-nvfp4-fused/15c6b8933d982019aa8965d218deb0eb1d9dc3f4-r2`; async-credit evidence root to be created by the pending run | current checkpoint `codex/nvfp4-small-m` | online harness/evidence/matrices and the selected diagnostic-only vLLM async ON/OFF c2 timing + Torch traces, including `tools/bench/profile_vllm_online_gate.py` and `tests/tools/test_online_gate_trace.py`. `ENG-ASYNC-SCHED`, loader/memory, RMSNorm, W3-H2/I2+, exact-grid, and 35B implementation remain excluded until the control selects a lever and this claim is updated. ONE `flock` around the complete async series | `ACTIVE` | 2026-07-14 (scan complete: c2 TPOT is 6.1% slower while TTFT passes; async credit is `PENDING`. Host PSS/RSS root is a 22.920 GiB CPU weight mirror. W3-I remains off; `3f256ab` remains 55/124) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` async-credit control | `3f256ab` remains 55/124; scan complete. c2 TPOT is 114.841 vs 108.274 ms while ours TTFT passes; exact mode-forcing profiler control and unit contract are prepared | run vLLM v0.25.0 async ON/OFF on the binding c2 corpus with paired timing and Torch traces under one lock. If credit explains 4–6%, update claim and activate W3; if neutral, capture low-batch kernels and map RMSNorm partitions. No exact grid or 35B performance yet | `ACTIVE` |
