...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local `/home/mudler/_git/vllm.cpp-serve-gate`; closed diagnostic DGX dir `~/work/vllm.cpp-latency`; binding evidence root `~/work/vllm.cpp-online-gate` | `codex/serve-gate-harness` (merged `b0acec2`; execution hardening active) | committed `tools/bench/online_gate*`, `tools/bench/profile_vllm_online_gate.py`, `tools/bench/summarize_torch_kernels.py`, `scripts/dgx-online-serving.sh`, `tests/tools/test_online_gate_*`; rootless verified cache-eviction helper `tools/bench/drop_file_cache.py`; explicit server scheduling-capacity CLI in `examples/server/main.cpp` + help test/README; DGX current-main build/harness/results; engine matrix, roadmap, ledger/state. ONE `flock` around each complete model A/B+trace series | `ACTIVE` | 2026-07-11 (`8289cbd`: regenerated 27B model/cache/live-SSE and ours rep1 c1-c16 pass. c32 completed 192/192, but request 158 delivered 128 choice frames whose decoded text re-tokenized to 126 because the requested final usage frame is absent. Exact-token validation stopped before vLLM; cleanup/lock release passed. `CLAIM-SERVE-STREAM-USAGE-1` owns the protocol dependency; merge it, regenerate, then restart 27B followed by 35B) |
| `CLAIM-SERVE-STREAM-USAGE-1` | `SERVE-STREAM-USAGE` | root, surfaced by fail-closed `SERVE-GATE-ONLINE` c32 execution | local `/home/mudler/_git/vllm.cpp-serve-stream-usage`; no separate DGX build until CPU gates pass | `codex/serve-stream-usage` | `.agents/specs/stream-options.md`; request/response protocol, serving usage selection and completion/chat SSE paths under `include/vllm/entrypoints/openai/` + `src/vllm/entrypoints/openai/`; server force-usage CLI; exact ported protocol/serving/API tests; owning matrix/roadmap/ledger/state. GPU handoff is a regenerated two-model online gate only after merge | `SPIKE` | 2026-07-11 (27B c32 completed 192/192 requests and emitted 128 choice frames for request 158, but our missing requested usage frame forced pinned vLLM's client to re-tokenize text as 126; the whole arm failed closed and released the lock) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` | All partial campaigns are invalid; clean `8289cbd` 27B ours rep1 reached c32, where missing requested native-ID usage made one 128-frame response re-tokenize to 126 | `CLAIM-SERVE-STREAM-USAGE-1` spikes and implements final/continuous/force usage first. Then merge, regenerate the commit-bound corpus/evidence, and restart 27B followed by 35B under new whole-model locks | `ACTIVE` |
