...
|---|---|---|---|---|---|---|---|
| `CLAIM-PR3` | `KERNEL-GDN-AOT-BF16`, `KERNEL-GDN-SCRATCH` | root takeover of stopped `validate_pr3` / `complete_pr3` stream | primary recovery tree `/home/mudler/_git/vllm.cpp-pr3-validate`; 27B default/component integration in `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; DGX evidence `~/work/vllm.cpp-noPy` plus `~/work/vllm.cpp-nvfp4-small-m/debug/gdn-out-bf16-c16-ab-20260712` | recovery branch integrated into `main` by `a767188`; current checkpoint on `codex/nvfp4-small-m` | PR #3 files and rows `KERNEL-GDN-AOT-BF16`+`KERNEL-GDN-SCRATCH`; 27B-only `GdnOutDType` default/f32 override in `qwen3_5.cpp`; ledger/inventory/roadmap evidence. No 35B default change. GPU lock: residual trace/pool classification remains separate from FP4 W3 | `ACTIVE` | 2026-07-12 (vendored BF16 H32/H48 AOT/safety evidence and native 16/16 correctness remain green. Binding `9cc7191` has c16 total at 1.028782× but mean TPOT/ITL at 0.988851×. Accepted `def5f75` node tracing sees the executed GDN family but ranks the exact packed-QKV launch gap first; GDN re-ranking waits for W3-D. All 35B paths keep f32) |
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local `/home/mudler/_git/vllm.cpp-serve-gate`; binding grid `~/work/vllm.cpp-online-gate/evidence/9cc71918dbdc10f014c02feb9bab1d00963a16fe`; accepted node trace source/build/evidence `~/work/vllm.cpp-online-trace-node/checkpoints/def5f752896036d9b35841a278578fd812f75a0d` / `~/work/vllm.cpp-online-trace-node/evidence/def5f752896036d9b35841a278578fd812f75a0d` | `codex/serve-gate-harness` (merged `b0acec2`; current checkpoint on `codex/nvfp4-small-m`) | `tools/bench/online_gate*`, profiler/summarizer/driver contracts, exact DGX evidence, matrices/roadmap/ledger/state. ONE `flock` around each complete model A/B+trace series | `ACTIVE` | 2026-07-12 (`9cc7191` grid remains binding at 54/124. `def5f75` passes the replacement node contract with 2,315,412 graph-child rows and selects W3-D packed QKV from the ~240 versus 208 FP4 GEMMs/step gap. Run its same-binary 27B A/B, then repeat the grid; keep 35B held) |
...
| `CLAIM-NVFP4-SMALL-M-3` | `KERNEL-GEMM-NVFP4-W4A4` (`W3`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; immutable W3-A evidence `~/work/vllm.cpp-nvfp4-small-m/71f1e894d0c5e496607d08cfe9089a9944128271/w3`; immutable W3-B evidence `~/work/vllm.cpp-nvfp4-small-m/d7cdf66db0cfcc53d68d49613623ec6cd3807641/w3b`; binding grid `~/work/vllm.cpp-online-gate/evidence/9cc71918dbdc10f014c02feb9bab1d00963a16fe`; accepted node trace `~/work/vllm.cpp-online-trace-node/evidence/def5f752896036d9b35841a278578fd812f75a0d` | `codex/nvfp4-small-m` | W3 owns the highest-ranked executed FP4 topology/selection repair. A/B correctness and structural evidence remain; C persistence stays optional; D owns only the full-attention packed-QKV resident/one-GEMM/split-view path, its ported tests and `VT_FP4_MERGED_QKV=0` fallback | `ACTIVE` | 2026-07-12 (`def5f75` node tracing proves the current three-GEMM Q/K/V topology adds exactly 32 FP4 launches per step across 16 full-attention layers. Implement and gate W3-D, run its same-binary 27B A/B, then re-trace/re-rank; do not stack direct-swizzle/alpha work and keep 35B held) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + FP4 repair | Binding immutable `9cc7191` v0.25 27B grid is complete: all 124 axes bind, 54 pass and 70 fail. Accepted `def5f75` node tracing exposes 2,315,412 local graph-child rows and proves our separate Q/K/V topology executes ~240 FP4 GEMMs/step versus vLLM's exact 208. GPU/port/lock exit idle | implement the trace-selected W3-D packed-QKV path with its fallback and ported tests, run the same-binary 27B A/B, then re-trace/re-rank and repeat the exact grid. Modernize/delete only trace-proven paths and run 35B only after full 27B closure | `ACTIVE` |
