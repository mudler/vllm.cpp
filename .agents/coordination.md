...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid/trace `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; W3-F immutable evidence `~/work/vllm.cpp-nvfp4-device-alpha/7517af4f983fe322ac88ce2d9869e1441b7be3fd/evidence`; W3-G local `/home/mudler/_git/vllm.cpp-nvfp4-small-m`, future DGX root `~/work/vllm.cpp-fa2-decode/<sha>` | current checkpoint `codex/nvfp4-small-m` | online harness/evidence/matrices plus W3-G ratio-6 FA2 split-KV decode adapter, exact fallback, model dtype dispatch, scratch lifecycle and ported attention tests per [spike](specs/fa2-gqa-split-kv-decode.md). FP4 producer, host-weight, GDN, scheduler and 35B performance excluded. ONE `flock` around a complete model A/B+trace | `ACTIVE` | 2026-07-13 (W3-G1--G3 source/tests implemented: direct swap strides, exact heuristic, fixed per-shape queue-owned scratch, cast-free model/dispatch toggle, upstream fallback/ratio/capture/lifecycle vectors. CUDA-off build/focused CTest pass; clean sm_121a and all CUDA/sanitizer/model/trace/performance evidence PENDING. `3f256ab` remains 55/124; no exact grid/35B performance) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + `KERNEL-ATTN-FA2` W3-G | `3f256ab` remains 55/124; W3-C reproduction control is green, while W3-E and W3-F strict-fail. The completed trace/source/dependency scan selects ratio-6 FA2 split-KV decode and its spike is accepted; GPU/lock/port are idle | implement capture-safe BF16 FA2 decode plus exact `VT_FA2_DECODE=0` fallback and port upstream paged-decode tests; then close immutable CUDA/model/trace and every-axis c2/c16 gates. No exact grid/35B performance before the component passes all axes | `ACTIVE` |
