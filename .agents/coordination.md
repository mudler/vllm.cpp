...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; active worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; finalized c2 root `~/work/vllm.cpp-executed-path-c2/179a0fc2afc1c33b63d14de8e50d3fde976c7356`; W1 safety root `~/work/vllm.cpp-gdn-ba/immutable-581d335fec2e5a96d9ccbb38c1ec001c39ac1789`; completed structural root `~/work/vllm.cpp-gdn-ba-trace/0091cd192d9a6baa2197a4f3bdb0561bd859baf5` | current checkpoint `codex/nvfp4-small-m` | Online-gate execution and harness work. `CLAIM-GDN-BA-ROUNDING-1` leads packed-decode code and its c2/c16 component; this claim retains the binding grid. qkvz code, loader-memory repair, exact grid, and 35B performance are excluded. Any A/B or trace series uses one `flock` | `ACTIVE` | 2026-07-14 (`8fbb950` W1D3 stopped before the GPU lock on inherited plan environment; deterministic repair is CPU-green **79/79**. Replacement traces plus c2/c16 remain open. Binding 55/124, no speed credit) |
| `CLAIM-GDN-BA-ROUNDING-1` | `KERNEL-GEMM-BF16` (`W1C`), `KERNEL-GDN-PACKED-DECODE` (`W1D`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-gdn-ba-rounding`; immutable G1 root `~/work/vllm.cpp-gdn-packed-decode/9ad8fb76940e68737d2a13ad8ddd97d649bb577c`; immutable G2 root `~/work/vllm.cpp-gdn-packed-decode/f344decf457a4d50c3bcae78a2903d7fe176a511/evidence-g2`; failed pre-GPU root `~/work/vllm.cpp-gdn-packed-trace/8fbb9502f8cb04a9d781f5a0fe0213953433219a`; replacement root `~/work/vllm.cpp-gdn-packed-trace/<repair-sha>` | `codex/gdn-ba-rounding-w1c` | Leads exact packed decode, model dispatch/rollback, trace and c2/c16 component. Prefill/mixed/spec GDN, qkvz, host-weight repair, binding-grid execution and 35B performance are excluded. One `flock /tmp/gpu` covers every GPU correctness or A/B series | `ACTIVE` | 2026-07-14 (harness requires 915 packed vs 963 rollback nodes, exact environment provenance, exact mode-invariant signatures after separately hashing 48 coupled BA nodes, and marker-last finalization. Deterministic plan setup passes **79/79**; push and execute replacement traces. Speed credit remains absent) |
...
is CPU-green **79/79** with exact mode-invariant signatures after separately
hashing the 48 coupled BA nodes and deterministic plan setup. The `8fbb950`
attempt stopped before the GPU lock; replacement paired trace and component results remain
open. qkvz, exact-grid and 35B performance stay excluded until
those gates close.
...
|---|---|---|---|---|
| 1 | `KERNEL-GEMM-BF16` + `KERNEL-GDN-PACKED-DECODE` for `SERVE-GATE-ONLINE` | `3f256ab` remains 55/124; clean `f344dec` closes W1D2/G2; `8fbb950` failed pre-GPU and deterministic repair passes 79/79 | push repair, execute/finalize replacement node traces, then c2/c16 before qkvz | `GATING` |
