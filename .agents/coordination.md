...
| 10 | `ENG-EXPERT-STREAM` (D4 first leaf) | spike accepted ([expert-streaming.md](specs/expert-streaming.md), `CLAIM-EXPSTREAM-SPIKE-1` released); surpass-track, low-concurrency capacity regime | claim W1 (expert cache manager + eviction, CPU-testable) then W2 (pread pool); W3 needs dgx + 35B checkpoint | `READY` |
