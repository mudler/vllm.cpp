...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local `/home/mudler/_git/vllm.cpp-serve-gate`; binding grid `~/work/vllm.cpp-online-gate/evidence/9cc71918dbdc10f014c02feb9bab1d00963a16fe`; W3-D component/trace as recorded; active exact root `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560` | `codex/serve-gate-harness` (merged `b0acec2`; current checkpoint on `codex/nvfp4-small-m`) | `tools/bench/online_gate*`, profiler/summarizer/driver contracts, exact DGX evidence, matrices/roadmap/ledger/state. ONE `flock` around each complete model A/B+trace series | `ACTIVE` | 2026-07-12 (`9cc7191` binds 54/124. W3-D post-pack launch topology passes. New exact root is ACTIVE/PENDING with validated plan/corpus and 0/36 groups; execute under one lock, keep 35B held) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + FP4 repair | Binding immutable `9cc7191` v0.25 27B grid is complete: 54/124 pass. W3-D post-pack trace closes 240→208. New exact `3f256ab` root is ACTIVE/PENDING with validated plan/corpus and 0/36 groups; GPU/lock idle | commit/push this ACTIVE checkpoint, execute/re-rank the exact grid. Modernize/delete only trace-proven paths and run 35B only after full 27B closure | `ACTIVE` |
