...
| `CLAIM-C5-CHUNKED-ATTN-1` | `ATTN-CHUNKED-LOCAL` | root | local `/home/mudler/_git/vllm.cpp-c5-chunked-attn` | `codex/c5-chunked-attn-w4` | W4 only: new chunked-local wrapper/metadata and backend-utils slices under `include/src/vllm/model_executor/layers/attention/` and `include/src/vllm/v1/attention/`, their CMake registration, exact upstream-derived metadata/reference tests, and C5 matrix/roadmap/inventory/ledger/state rows. No W3 KV edits, Llama4/model-family implementation, RoPE, connector/offload, DCP/PCP, kernel optimization or GPU execution | `ACTIVE` | 2026-07-11 (W3 is merged/CPU-gated; port virtual-batch Q/K lengths, slot/block-table transforms, cudagraph rejection, spec emission and ordinary-backend dispatch without queuing behind the serving campaign) |
...
| 8 | C3/C5/C7/C8 engine/API work | C3/C5/C6 specs accepted; C7/C8 leaf specs missing | C3 M-mtp-0 remains `GATING` on its GPU recipe. C5 W1/W2/W3 are implemented and `GATING`; W4 `ATTN-CHUNKED-LOCAL` is `ACTIVE` under `CLAIM-C5-CHUNKED-ATTN-1` for CPU/reference metadata work. Restored oracle and positive model rows gate W2/W4 runtime G4-G9; `ATTN-YARN` remains claimable. Then spike C7/C8 | `ACTIVE` |
