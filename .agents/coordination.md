...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid/trace `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; W3-F immutable evidence `~/work/vllm.cpp-nvfp4-device-alpha/7517af4f983fe322ac88ce2d9869e1441b7be3fd/evidence`; W3-G immutable DGX root `~/work/vllm.cpp-fa2-decode/ae9e8ff0576badabdda7289beeacaa1041c55d21`; W3-H local `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; H1a `~/work/vllm.cpp-executed-path-refresh/5d8af792…`; H1b `~/work/vllm.cpp-executed-path-refresh-frozen-h1b/5d8af792…`; H1c `~/work/vllm.cpp-executed-path-refresh-h1c/d1f8e332…`; H1d failed setups `~/work/vllm.cpp-executed-path-refresh-h1d/{7bae38afe8f97603fec525e8a2b8833bcba983cf,2d16c681f3fe64130a7ab9b6fc55ad7e21c0808e}`; replacement immutable root pending | current checkpoint `codex/nvfp4-small-m` | online harness/evidence/matrices plus completed W3-G ratio-6 FA2 gates and [W3-H normal-producer spike](specs/nvfp4-bf16-producer-vectorization.md). H1 owns the diagnostic-only four-replay controller, target-build/dispatch manifest, exact-workload paired ours/vLLM trace validation and residual re-ranking; W3-H2 inference-kernel implementation remains prohibited until valid H1d evidence. Host-weight, fused producer, GDN changes, scheduler and 35B performance excluded. ONE `flock` around the complete series | `ACTIVE` | 2026-07-13 (H1a/H1b/H1c are `VOID`. H1d controller/validators are CPU-gated. Clean `7bae38a` failed before compile/GPU on missing Ninja; clean `2d16c68` found Ninja but failed CUDA compiler discovery because login `PATH` omitted `nvcc` (configure SHA `378fdd7a…4c3c`). Both roots are retained/void. The next recipe pins/hashes oracle Ninja and `/usr/local/cuda-13.0/bin/nvcc` 13.0.88. No ratio, exact grid or 35B performance exists. GPU, `/tmp/gpu` and ports are idle; `3f256ab` remains 55/124) |
...
execution is next from a fully pinned-toolchain replacement SHA. The retained
`7bae38a`/`2d16c68` roots are pre-build/pre-GPU failures and cannot be reused;
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + W3-H executed-path refresh | `3f256ab` remains 55/124; W3-E/W3-F/W3-G strict-fail. H1a/H1b/H1c are `VOID`; H1d `7bae38a`/`2d16c68` are pre-build/pre-GPU environment failures from omitted Ninja/`nvcc`. Controller/schema remains CPU-gated; GPU/lock/ports are idle | execute the new clean pushed repair in a fresh immutable root with oracle Ninja and CUDA 13.0.88 `nvcc` pinned/hashed. Require three zero-exit unique Nsight sessions, lossless direct graph-child correlation, identical resources and exact plans before implementing any lever. No exact grid/35B performance until a later component passes every axis | `ACTIVE` |
