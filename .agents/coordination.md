...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover | binding grid `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; current local `/home/mudler/_git/vllm.cpp-nvfp4-small-m` | current checkpoint `codex/nvfp4-small-m` | online harness/evidence/matrices. ONE `flock` around a complete model A/B+trace | `ACTIVE` | 2026-07-13 (`3f256ab` remains 55/124. W3-C2 runtime/frozen-map correctness passes with 64/64 zero-tune plans and both 27B arms exact. Commit-bound six-process/same-plan gates next; no exact rerun/35B performance) |
...
| `CLAIM-NVFP4-SMALL-M-3` | `KERNEL-GEMM-NVFP4-W4A4` (`W3`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; oracle cache fixture under `tests/fixtures/nvfp4_flashinfer_v025_gb10/`; C2 staging evidence `~/work/vllm.cpp-nvfp4-persistent-c2-staging/evidence` | `codex/nvfp4-small-m` | W3-C owns document/import/atomicity, ready-map/lifecycle/5,000-us wiring and same-plan gates per [spike](specs/nvfp4-persistent-plan-cache.md). Other levers excluded | `ACTIVE` | 2026-07-13 (C2 implemented/correctness-gated: 64/64 loaded, zero tuning, runtime/save/memcheck and default/fallback 27B exact. C3 six-process/same-plan performance `READY`; no ratio/exact grid/35B performance) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + FP4 repair | `3f256ab` remains 55/124; W3-E failed. W3-C2 runtime/frozen-map correctness is green; GPU/ports/lock idle | run commit-bound six-process 64/64 and one frozen-map c2/c16 all-axis A/B. No exact grid/35B performance before acceptance | `ACTIVE` |
