...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local `/home/mudler/_git/vllm.cpp-serve-gate`; closed diagnostic DGX dir `~/work/vllm.cpp-latency`; binding evidence root `~/work/vllm.cpp-online-gate` | `codex/serve-gate-harness` (merged `b0acec2`; execution hardening active) | committed `tools/bench/online_gate*`, `tools/bench/profile_vllm_online_gate.py`, `tools/bench/summarize_torch_kernels.py`, `scripts/dgx-online-serving.sh`, `tests/tools/test_online_gate_*`; rootless verified cache-eviction helper `tools/bench/drop_file_cache.py`; explicit server scheduling-capacity CLI in `examples/server/main.cpp` + help test/README; DGX current-main build/harness/results; engine matrix, roadmap, ledger/state. ONE `flock` around each complete model A/B+trace series | `ACTIVE` | 2026-07-11 (`4e7aa12` exact corpora converted and 27B model gate passed, then stopped before server start on unavailable passwordless global cache drop. Rootless leaf now inventories checkpoint/corpus/client/server files, applies `POSIX_FADV_DONTNEED`, requires `mincore` zero residency, and hashes every leg/trace report into the summary. CPU tools 34/34 + CTest 1/1; warmed real 27B probe evicted 49 files / 26.55 GB from 1.20 GB resident to zero, report SHA `21bbcc75…069`. Regenerate under merged SHA, then 27B followed by 35B) |
