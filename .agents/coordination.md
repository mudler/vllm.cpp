...
| `CLAIM-C5-LLAMA3-1` | `ATTN-ROPE-LLAMA3` | root | local `/home/mudler/_git/vllm.cpp-c5-llama3` | `codex/c5-llama3-w6` | W6 only: Llama 3 typed `low_freq_factor`/`high_freq_factor` config fields, new `include/src/vllm/model_executor/layers/rotary_embedding/llama3_rope.*`, the minimal upstream `RotaryEmbedding(init_cache)` extension point plus exact factory branch in `base.{h,cpp}`, CMake/leaf tests, extensions to the generic long-context oracle dumper/runner, pinned goldens, and C5 record rows. No W7/W8 LongRoPE/dynamic formula, W5 YaRN/MRoPE semantic rewrite, model-family/MM preprocessing, local-attention edit, or GPU execution | `ACTIVE` | 2026-07-11 (W5 merged as `d07dd1c`; port the exact low/mid/high frequency bands and boundary goldens on its shared cache/apply foundation while the serving campaign retains dgx) |
...
| 8 | C3/C5/C7/C8 engine/API work | C3/C5/C6 specs accepted; C7/C8 leaf specs missing | C3 M-mtp-0 remains `GATING` on its GPU recipe. C5 W1-W5 are implemented and `GATING`; W5 retains CUDA/model/trace/every-axis debts. W6 `ATTN-ROPE-LLAMA3` is `ACTIVE` under `CLAIM-C5-LLAMA3-1`; W7/W8 follow on the shared factory/cache seam. Restored positive model rows and an idle GPU still gate runtime closure; then spike C7/C8 | `ACTIVE` |
