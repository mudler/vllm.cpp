...
| `CLAIM-NVFP4-SMALL-M-3` | `KERNEL-GEMM-NVFP4-W4A4` (`W3`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; binding grid as above; W3-E immutable component as above; v0.25 cache evidence `~/.cache/vllm/flashinfer_autotune_cache/0.6.13/121a/cbd38fe31b19a593fd4ac474a8a138a545227805f23c425119de8429f384d163/autotune_configs.json` | `codex/nvfp4-small-m` | W3-C owns only persistent/native JSON load-save, exact FlashInfer cache import, metadata/stale rejection, atomic publication, frozen read-only mode, 5,000-us miss timing, ready-map integration, ported tests and same-plan gates per [spike](specs/nvfp4-persistent-plan-cache.md). Device alpha, vectorized quant, GDN, host weights and attention are excluded | `ACTIVE` | 2026-07-13 (W3-C0 spike accepted/`READY`; implementation not started. Binding v0.25 cache SHA `b41a8ecc…677` has 64 plans; W3-E fresh processes match only 18--33/64 paired IDs. C1/C2 schema/integration then six-process 64/64 and same-plan c2/c16 are next. No speed credit/exact grid/35B performance) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + FP4 repair | Binding immutable `3f256ab` remains 55/124. W3-E strict-fails 32/40 timing +6/8 memory. Dynamic scan proves binding v0.25 loads 64 persistent plans while ours retunes with only 18--33/64 paired matches; W3-C0 is spiked/`READY`. GPU/ports/lock idle | implement C1 native/import JSON + tests, then C2 ready-map/5,000-us/lifecycle integration. Require six-process 64/64 stability and one frozen-map c2/c16 all-axis A/B before any exact grid. 35B performance remains held until full 27B closure | `ACTIVE` |
