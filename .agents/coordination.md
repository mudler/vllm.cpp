...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local `/home/mudler/_git/vllm.cpp-serve-gate`; closed diagnostic DGX dir `~/work/vllm.cpp-latency`; binding evidence root `~/work/vllm.cpp-online-gate` | `codex/serve-gate-harness` (merged `b0acec2`; execution hardening active) | committed `tools/bench/online_gate*`, `tools/bench/profile_vllm_online_gate.py`, `tools/bench/summarize_torch_kernels.py`, `scripts/dgx-online-serving.sh`, `tests/tools/test_online_gate_*`; rootless verified cache-eviction helper `tools/bench/drop_file_cache.py`; explicit server scheduling-capacity CLI in `examples/server/main.cpp` + help test/README; DGX current-main build/harness/results; engine matrix, roadmap, ledger/state. ONE `flock` around each complete model A/B+trace series | `ACTIVE` | 2026-07-11 (`54cba52`: 27B model gate, 26.55-GB zero-residency drop and 128-chunk live-SSE preflight pass; first timed client then failed before requests because oracle venv lacked pandas from the `bench` extra. Pinned CUDA requirement pandas 2.2.3 plus python-dateutil 2.9.0.post0/pytz+tzdata 2024.2 installed; manifest now rejects pandas version/file drift before build/GPU lock. No raw timing retained. Merge, regenerate SHA-bound evidence, then 27B followed by 35B) |
