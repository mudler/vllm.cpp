...
| `CLAIM-GDN-BA-ROUNDING-1` | `KERNEL-GEMM-BF16` (`W1C`), `KERNEL-GDN-PACKED-DECODE` (`W1D`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-gdn-ba-rounding`; DGX roots `~/work/vllm.cpp-gdn-ba-rounding/f9252943d1e96dbfa43e3b8f2d06dec1aa5f20d3`, `~/work/vllm.cpp-gdn-ba-rounding/diagnostic-w1c`, and `~/work/vllm.cpp-gdn-packed-decode/f18ca23691bc7e38adbf04912da92f819154379e` | `codex/gdn-ba-rounding-w1c` | Leads `ProjectGdnBA` output dtype plus the exact pure-non-spec packed-decode op in `include/vt/ops.h`, `src/vt/{ops.cpp,cpu/cpu_ops.cpp,cuda/cuda_gdn.cu}`, registrations, `qwen3_5.cpp`, focused oracle/tests, 35B inertness, node trace and c2/c16 component. This is the explicit overlap lead over `CLAIM-PR3` for pure decode/model dispatch and over `CLAIM-SERVE-GATE-1` for the component. Prefill/mixed/spec GDN, qkvz, host-weight repair, binding-grid execution and 35B performance are excluded. One `flock /tmp/gpu` covers every GPU correctness or A/B series | `ACTIVE` | 2026-07-14 (projection/inertness and W1D0 immutable G0 are closed. Clean `f18ca23` fixture is byte-identical and CUDA **10/10**, reproducing current `306/7552`, beta-only `308/6558`, full semantics `0/1`. W1D1 production op/test matrix is next; no speed credit) |
...
`f18ca23` closes immutable G0; production semantics and the component result
remain open.
...
|---|---|---|---|---|
| 1 | `KERNEL-GEMM-BF16` + `KERNEL-GDN-PACKED-DECODE` for `SERVE-GATE-ONLINE` | `3f256ab` remains 55/124; `581d335`/`0091cd1`/`f925294` close BA safety, structure and projection/inertness; clean `f18ca23` closes packed semantic G0 | port the full pure-decode op, restore 235/235, then complete c2/c16 before qkvz | `GATING` |
