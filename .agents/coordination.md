...
| `CLAIM-HOST-WEIGHT-RESIDENCY-1` | `ENG-HOST-WEIGHT-RESIDENCY` | Codex:gpt-5 | local `/home/rich/c/vllm.cpp`; local RTX evidence under `/tmp/qwen35-memory-*`, `/tmp/qwen35-host-residency-*`, `/tmp/qwen35-peak-w4-*`, `/tmp/qwen35-direct-w4-*` | `local-blackwell-environment` | `.agents/specs/discrete-cuda-host-weight-residency.md`; `.agents/specs/safetensors-bounded-page-residency.md`; `.agents/specs/dense-direct-device-loading.md`; safetensors reader, dense loader/forward/model-registry/entrypoint files and focused tests; matrix-count invariant in `scripts/check-agent-record.py`; matching matrix/roadmap/README/BENCHMARKS/ledger/state rows. One `/tmp/gpu` lock around each complete local A/B series; no Spark/GB10 access or system configuration changes | `ACTIVE` | 2026-07-12 (W4.4 launch PSS accepted: 1.855 GiB ON vs 8.168 OFF vs 7.640 vLLM; throughput 6556.86 ON vs 6556.90 OFF vs 6717.48 vLLM. CPU 105/105; native-sm_120 default 5/5 + control 3/3. Memory axis closed locally; correctness/performance parity remain) |
