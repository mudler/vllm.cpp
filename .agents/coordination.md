...
| `CLAIM-C5-SW-KV-1` | `KV-SLIDING-WINDOW-SPEC` | root (`resume-roadmap`) | local `/home/mudler/_git/vllm.cpp-c5-sw-kv` (CPU-only build; no GPU ownership) | `codex/c5-sw-kv-w1` -> `main` | W1 only: sliding-window classes/cases in `kv_cache_interface.*`, `single_type_kv_cache_manager.*`, narrow coordinator/KV-utils registry and hybrid-conversion slices, existing KV test files, and this row's matrix/roadmap/feature/inventory/ledger/state/coordination lines. No attention kernels, chunked-local classes, RoPE, model code, or GPU execution | `ACTIVE` | 2026-07-10 (claimed after the recovery merge; implement complete pinned K4/K5/K7/K8 semantics and port W1 tests before any support claim) |
...
| 8 | C3/C5/C7/C8 engine/API work | C3/C5/C6 specs accepted; C7/C8 leaf specs missing | C3 M-mtp-0 remains `GATING` on its two-checkpoint GPU recipe. C5 W1 `KV-SLIDING-WINDOW-SPEC` is active under `CLAIM-C5-SW-KV-1`; chunked-local KV and `ATTN-YARN` remain claimable. Then spike C7/C8 | `ACTIVE` |
