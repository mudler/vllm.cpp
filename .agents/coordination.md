...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local `/home/mudler/_git/vllm.cpp-serve-gate`; closed diagnostic DGX dir `~/work/vllm.cpp-latency`; binding evidence root `~/work/vllm.cpp-online-gate` | `codex/serve-gate-harness` (merged `b0acec2`; corrected trace/gate contract active) | `tools/bench/online_gate*`, closed-loop `profile_vllm_online_gate.py`, `summarize_torch_kernels.py`, `scripts/dgx-online-serving.sh`, `tests/tools/test_online_gate_*`; cache-eviction helper; explicit server scheduling/cache CLI; DGX exact build/harness/results; matrix/roadmap/ledger/state. ONE `flock` around each complete model A/B+trace series | `ACTIVE` | 2026-07-11 (`ed6247d` exact c16/48 vLLM trace captured; kernel-summary SHA-256 `8213…df64`. Audit invalidated historical direct denominators: temperature 0/1 on both models, plus 27B token budget 2048/8192. Old paired trace also differed in cache policy, admission, max-seqs, and model length. Pushed `f065fce` ports hybrid-default cache-off/no-prefix coordination and repairs closed-loop trace validation. Its exact corrected 27B baseline is active under one whole-series lock; model gate passed. Trace scan identifies ACTIVE `KV-DEVICE-RESIDENCY` as top lever: exactly 255.375 MiB state traffic/direction at d11) |
| `CLAIM-KV-DEVICE-1` | `KV-DEVICE-RESIDENCY` | root | main worktree `/home/mudler/_git/vllm.cpp`; baseline evidence `~/work/vllm.cpp-online-gate/evidence/f065fce18027b52a178b246f490b091b9d9e07a3` | `main` | W0 move-only backend cache ownership/device residency + pointer/lifecycle tests, then W1 indexed GDN state I/O. Primary files `include/vllm/v1/worker/gpu/runner.h`, `src/vllm/v1/worker/gpu/runner.cpp`, `src/vllm/model_executor/models/qwen3_5.cpp`, vt CUDA op/tests, owning spec/matrices/README/BENCHMARKS/ledger/state. No GPU work until the f065 serving baseline releases its whole-series lock; subsequent A/B holds one uncontended lock | `ACTIVE` | 2026-07-11 (W0 source implemented: stable backend owners, stream zeroing, host fallback, real-model CUDA pointer assertions; focused CPU build + 4/4 pass. Pushed f065 exact 27B baseline now holds `/tmp/gpu`, model gate passed, first interleaved ladder active. W0 CUDA build/runtime/A-B waits for release) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + `KV-DEVICE-RESIDENCY` | Historical denominators/campaigns are non-binding. Pushed f065 corrected 27B baseline is lock-held and active; model gate passed. Device-residency W0 is implemented and focused CPU-green, with no GPU result yet | finish/record f065 baseline; merge the GPU-open W0 checkpoint; run exact GB10 default/fallback pointer/correctness/sanitizer/memory/trace/A-B gates; then W1 indexed state I/O and exact direct/online 27B before 35B | `ACTIVE` |
