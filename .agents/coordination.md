...
schema-v2 validators are implemented and CPU-gated. `219f4f2` proves the FIFO
shutdown path but exposes the continuous-range sampler gap; the accepted spike
now requires four isolated repeated ranges and implementation is pending. The
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + W3-H executed-path refresh | `3f256ab` remains 55/124; W3-E/W3-F/W3-G strict-fail. H1a/H1b/H1c and H1d attempts through `219f4f2` are `VOID`; the latest proves FIFO/zero-exit and exact graph children but captures three eager sampler/input gaps and emits severity-2 possible loss. GPU/lock/ports are idle | implement and CPU-gate four synchronized single-replay ranges with Nsight `repeat:4`; then execute a fresh immutable root. Require correctness plus three unique zero-eager/lossless sessions, direct graph-child correlation, identical resources and exact plans before implementing any lever. No exact grid/35B performance until a later component passes every axis | `ACTIVE` |
