...
| 2 | `SERVE-ASYNC-LLM` block (`ENG-CORE-BUSY-LOOP`, `ENG-ASYNC-SCHED`, `ENG-PRIORITY-SCHED`) | joint spike accepted ([async-serving.md](specs/async-serving.md)); BLOCKS the `SERVE-GATE-ONLINE` latency axes | W1 landed and `GATING` (`CLAIM-BUSY-LOOP-1` released): CPU suites green, GPU G1/G4 deferred while `CLAIM-SERVE-GATE-1` holds the GPU — next GPU-idle window runs both greedy gates + the offline A/B for W1; W2 (AsyncLLM + real SSE) is claimable NOW on the merged queue split — unblocks order 0; W3 overlap mirrors vLLM's default; W4 priority separable | `READY` |
