...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local `/home/mudler/_git/vllm.cpp-serve-gate`; binding grid `~/work/vllm.cpp-online-gate/evidence/9cc71918dbdc10f014c02feb9bab1d00963a16fe`; W3-D component `~/work/vllm.cpp-packed-qkv/3f256abdbb558e162bf8a2196284deb119648560/w3d/component-ab`; post-pack trace `~/work/vllm.cpp-online-trace-node/evidence/3f256abdbb558e162bf8a2196284deb119648560` | `codex/serve-gate-harness` (merged `b0acec2`; current checkpoint on `codex/nvfp4-small-m`) | `tools/bench/online_gate*`, profiler/summarizer/driver contracts, exact DGX evidence, matrices/roadmap/ledger/state. ONE `flock` around each complete model A/B+trace series | `ACTIVE` | 2026-07-12 (`9cc7191` remains binding at 54/124. W3-D c16 is 1.005049× mean-positive but strict-fails 14/20 timing +2/4 memory. Post-pack trace passes and closes 240→208 at 208.192 vs 208 FP4 GEMMs/forward. Repeat the exact grid; keep 35B held) |
...
| `CLAIM-NVFP4-SMALL-M-3` | `KERNEL-GEMM-NVFP4-W4A4` (`W3`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; immutable W3-A/B and binding evidence as above; W3-D component and post-pack trace roots listed in `CLAIM-SERVE-GATE-1` | `codex/nvfp4-small-m` | W3 owns the highest-ranked executed FP4 topology/selection repair. C persistence stays optional; D owns only packed full-attention QKV, its max-shard scalar/resident/row-strided consumers, ported tests and `VT_FP4_MERGED_QKV=0` fallback | `ACTIVE` | 2026-07-12 (W3-D correctness/A-B/trace complete: c16 1.005049×, strict 14/20 timing +2/4 memory; post-pack **296,674/1,425 = 208.192** vs vLLM 208 FP4 GEMMs/forward. Structural target passes; exact grid next. Do not stack another lever or run 35B) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + FP4 repair | Binding immutable `9cc7191` v0.25 27B grid is complete: all 124 axes bind, 54 pass and 70 fail. W3-D c16 is 1.005049× mean-positive but strict-fails at 14/20 timing +2/4 memory; its post-pack node trace closes the 32-launch gap at 208.192 vs 208 FP4 GEMMs/forward. GPU/port/lock exit idle | commit/push this trace checkpoint, then repeat/re-rank the exact grid. Modernize/delete only trace-proven paths and run 35B only after full 27B closure | `ACTIVE` |
