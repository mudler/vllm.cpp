...
schema-v2 validators are implemented and CPU-gated. The claim continues to own
only this diagnostic harness and its resulting paired traces; immutable H1d GPU
execution is next from an exact-path replacement SHA. The retained
`7bae38a`/`2d16c68` pre-build failures, invalid-config `5f8fab1` correctness
failure, pre-plan `d063f20` artifact-order failure and pre-build `b1c7eb6`
bootstrap failure cannot be reused;
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + W3-H executed-path refresh | `3f256ab` remains 55/124; W3-E/W3-F/W3-G strict-fail. H1a/H1b/H1c are `VOID`; H1d `7bae38a`/`2d16c68` are pre-build failures, `5f8fab1` is an invalid no-Triton-AOT/`Release` correctness failure before capture, `d063f20` is a pre-plan artifact-order failure, and `b1c7eb6` is a pre-build bootstrap failure. GPU/lock/ports are idle | execute the clean pushed repair in a fresh immutable root: generate the plan first in empty evidence, seed the corpus second, configure `RelWithDebInfo` with Triton-AOT/FA2/CUTLASS, oracle Ninja and CUDA 13.0.88, then let the driver own the recorded target build before executable validation. Require correctness plus three zero-exit unique Nsight sessions, lossless direct graph-child correlation, identical resources and exact plans before implementing any lever. No exact grid/35B performance until a later component passes every axis | `ACTIVE` |
