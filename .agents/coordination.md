...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local `/home/mudler/_git/vllm.cpp-serve-gate`; closed diagnostic DGX dir `~/work/vllm.cpp-latency`; binding evidence root `~/work/vllm.cpp-online-gate` | `codex/serve-gate-harness` (merged `b0acec2`; profiler/output-contract diagnosis active) | committed `tools/bench/online_gate*`, `tools/bench/profile_vllm_online_gate.py`, `tools/bench/summarize_torch_kernels.py`, `scripts/dgx-online-serving.sh`, `tests/tools/test_online_gate_*`; rootless verified cache-eviction helper `tools/bench/drop_file_cache.py`; explicit server scheduling-capacity CLI in `examples/server/main.cpp` + help test/README; DGX current-main build/harness/results; engine matrix, roadmap, ledger/state. ONE `flock` around each complete model A/B+trace series | `ACTIVE` | 2026-07-11 (`d4ddeb1` proved Ninja/PATH fixed and captured a 143-MiB vLLM trace, then failed closed because FP4 measured output digests differed from warmup. The 31d grid has 4/2,016 exact paired texts despite its passed model gate/exact counts. Output equality is now recorded diagnostically; merge, recapture, then rank/optimize the trace differences. GPU idle/unlocked) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` | All partial campaigns are invalid. `31d053f` completed the repeated 27B grid; `d4ddeb1` captured the vLLM trace after fixing Ninja/PATH but exposed an over-strong FP4 output-equality postcondition | merge the digest/exact-match diagnostic contract; recapture the diagnostic paired trace; rank and implement the top parity lever; then restart the full 27B gate before 35B | `ACTIVE` |
