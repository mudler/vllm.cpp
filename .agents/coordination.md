...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local `/home/mudler/_git/vllm.cpp-serve-gate`; closed diagnostic DGX dir `~/work/vllm.cpp-latency`; binding evidence root `~/work/vllm.cpp-online-gate` | `codex/serve-gate-harness` (merged `b0acec2`; execution hardening active) | committed `tools/bench/online_gate*`, `tools/bench/profile_vllm_online_gate.py`, `tools/bench/summarize_torch_kernels.py`, `scripts/dgx-online-serving.sh`, `tests/tools/test_online_gate_*`; rootless verified cache-eviction helper `tools/bench/drop_file_cache.py`; explicit server scheduling-capacity CLI in `examples/server/main.cpp` + help test/README; DGX current-main build/harness/results; engine matrix, roadmap, ledger/state. ONE `flock` around each complete model A/B+trace series | `ACTIVE` | 2026-07-11 (`8289cbd` remains void at c32 missing usage. The native-ID protocol implementation is now CPU/sanitizer-gated and its implementation claim is released; regenerate every commit-bound artifact, then restart 27B followed by 35B. No prior ratio is reusable) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` | All partial campaigns are invalid; clean `8289cbd` stopped at missing requested native usage. The host-side dependency now passes CPU and sanitizer gates with its implementation claim released | regenerate the commit-bound corpus/evidence and restart 27B followed by 35B under new whole-model locks | `ACTIVE` |
