...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + FP4 repair | `3f256ab` remains 55/124; W3-C reproduction control is green and W3-E fails at 39/40 timing + 1/8 memory. W3-F implements the 208-local-versus-zero-vLLM device-alpha mismatch; immutable CUDA/operator/memcheck/model/node-trace gates pass and the DGX lock is held by its strict component | finish the one-lock frozen-plan c2/c16 device-versus-host-alpha component. Run no exact grid/35B performance unless every component axis passes | `ACTIVE` |
