...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; active worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; finalized c2 root `~/work/vllm.cpp-executed-path-c2/179a0fc2afc1c33b63d14de8e50d3fde976c7356`; packed trace root `~/work/vllm.cpp-gdn-packed-trace/7ff713e377457130db4ed15929133d1b463aff96` | current checkpoint `codex/nvfp4-small-m` | Online-gate execution and harness work. `CLAIM-GDN-BA-ROUNDING-1` leads packed-decode code and its c2/c16 component; this claim retains the binding grid. qkvz code, loader-memory repair, exact grid, and 35B performance are excluded. Any A/B or trace series uses one `flock` | `ACTIVE` | 2026-07-14 (packed structure is accepted. Component attempt `593996d` failed pre-GPU after build **154/154** because required `--profile-control off` was omitted; no measured evidence. Binding stays 55/124 with no speed credit) |
| `CLAIM-GDN-BA-ROUNDING-1` | `KERNEL-GEMM-BF16` (`W1C`), `KERNEL-GDN-PACKED-DECODE` (`W1D`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-gdn-ba-rounding`; immutable G2 root `~/work/vllm.cpp-gdn-packed-decode/f344decf457a4d50c3bcae78a2903d7fe176a511/evidence-g2`; accepted trace root `~/work/vllm.cpp-gdn-packed-trace/7ff713e377457130db4ed15929133d1b463aff96`; failed pre-GPU component root `~/work/vllm.cpp-gdn-packed-component/593996d7a40ad323834a96eaa542c943142f788e` | `codex/gdn-ba-rounding-w1c` | Leads exact packed decode, model dispatch/rollback, trace and c2/c16 component. Prefill/mixed/spec GDN, qkvz, host-weight repair, binding-grid execution and 35B performance are excluded. One `flock /tmp/gpu` covers every GPU correctness or A/B series | `ACTIVE` | 2026-07-14 (runner/finalizer CPU **44/44**; clean attempt built **154/154** then failed before GPU ownership because its `record-execution` call omitted `--profile-control off`. Next session adds a test-first contract, repairs, pushes, and uses a new SHA-owned root. Speed credit remains absent) |
...
remaining packed component checkpoint. Its `593996d` attempt failed before the
lock on a missing required `--profile-control off`; repair/test and a fresh root
come first. qkvz, exact-grid and 35B performance stay excluded until that gate
closes.
...
|---|---|---|---|---|
| 1 | `KERNEL-GEMM-BF16` + `KERNEL-GDN-PACKED-DECODE` for `SERVE-GATE-ONLINE` | `3f256ab` remains 55/124; correctness/structure close at `f344dec` and `7ff713e` + `24cea4f`; `593996d` built **154/154** then failed pre-GPU on missing required `--profile-control off` | test-first repair the driver, push, then execute/finalize a fresh-SHA one-lock c2/c16 **40+8** series before qkvz | `GATING` |
