...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + FP4 repair | `3f256ab` remains 55/124; W3-C reproduction control is green and W3-E fails at 39/40 timing + 1/8 memory. W3-F implements the 208-local-versus-zero-vLLM device-alpha mismatch with local CPU/sanitizer gates green; GPU/ports/lock are idle | publish W3-F, then run immutable CUDA/operator/sanitizer/model/node-trace and frozen-plan c2/c16 gates. No exact grid/35B performance before an every-axis component passes | `ACTIVE` |
