...
| 8 | C3/C5/C7/C8 engine/API work | existing feature rows | C3 M-mtp-0 CPU implementation is complete and its claim is released; `SPEC-MTP` + both Qwen3.5 MTP model rows are `GATING` on the exact two-checkpoint recipe in the latest [state log](state.md), queued behind `CLAIM-SERVE-GATE-1`. C5/C7/C8 spikes remain open | `GATING` |
