...
|---|---|---|---|---|---|---|---|
| `CLAIM-PR3` | `KERNEL-GDN-AOT-BF16`, `KERNEL-GDN-SCRATCH` | root takeover of stopped `validate_pr3` / `complete_pr3` stream | primary recovery tree `/home/mudler/_git/vllm.cpp-pr3-validate`; 27B default/component integration in `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; DGX evidence `~/work/vllm.cpp-noPy` plus `~/work/vllm.cpp-nvfp4-small-m/debug/gdn-out-bf16-c16-ab-20260712` | recovery branch integrated into `main` by `a767188`; current checkpoint on `codex/nvfp4-small-m` | PR #3 files and rows `KERNEL-GDN-AOT-BF16`+`KERNEL-GDN-SCRATCH`; 27B-only `GdnOutDType` default/f32 override in `qwen3_5.cpp`; ledger/inventory/roadmap evidence. No 35B default change. GPU lock: residual trace/pool classification remains separate from FP4 W3 | `ACTIVE` | 2026-07-12 (vendored BF16 H32/H48 AOT/safety evidence and native 16/16 correctness remain green. Its vLLM 0.24.0 component/grid ratios are historical after oracle drift; re-rank from the v0.25.0 paired trace. All 35B paths keep f32) |
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local `/home/mudler/_git/vllm.cpp-serve-gate`; immutable source/build `~/work/vllm.cpp-online-gate/checkpoints/9cc71918dbdc10f014c02feb9bab1d00963a16fe`; evidence `~/work/vllm.cpp-online-gate/evidence/9cc71918dbdc10f014c02feb9bab1d00963a16fe` | `codex/serve-gate-harness` (merged `b0acec2`; execution source `9cc7191`) | `tools/bench/online_gate*`, profiler/summarizer/driver contracts, exact DGX evidence, matrices/roadmap/ledger/state. ONE `flock` around each complete model A/B+trace series | `ACTIVE` | 2026-07-12 (`b5c6e4f` is historical and `3cc490c` is VOID. Canonical v0.25.0/FlashInfer 0.6.13 is active. Clean exact `9cc7191` source, fresh sm_121a build, oracle fingerprint and deterministic source/vLLM corpora are prepared/hashed. A first metadata-only recorder command failed before output for missing repo module path; corrected module invocation passes. GPU/port/lock idle; no model gate or timed request yet. Start the one-lock 27B series; no current ratio binds and 35B is held) |
...
| `CLAIM-NVFP4-SMALL-M-3` | `KERNEL-GEMM-NVFP4-W4A4` (`W3`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; immutable W3-A evidence `~/work/vllm.cpp-nvfp4-small-m/71f1e894d0c5e496607d08cfe9089a9944128271/w3`; immutable W3-B evidence `~/work/vllm.cpp-nvfp4-small-m/d7cdf66db0cfcc53d68d49613623ec6cd3807641/w3b`; active v0.25 campaign `~/work/vllm.cpp-online-gate/evidence/9cc71918dbdc10f014c02feb9bab1d00963a16fe` | `codex/nvfp4-small-m` | W3 owns v0.25.0 re-trace and the highest-ranked executed FP4 topology/selection repair. A/B correctness and structural evidence remain; C persistence stays separately optional. Do not implement direct-swizzled scales, resident alpha or another topology change until the new paired trace proves it is on the remaining path | `ACTIVE` | 2026-07-12 (`3cc490c` is VOID and no rate binds. Immutable `9cc7191` exact source/build/oracle/corpus preflight is complete; the one-lock 27B model gate/grid/trace is active but unexecuted. Change no topology until its trace; 35B held) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + FP4 repair | No current denominator binds. `b5c6e4f` is historical and `3cc490c` is VOID. Canonical v0.25.0 is active. Immutable `9cc7191` source/build/oracle/corpus preflight passes; its model gate, 36 timed groups, six returns and paired traces remain unexecuted. GPU/port/lock were idle at preflight handoff | run the exact `9cc7191` 27B c1-c32 series and paired traces under its single lock. Modernize/delete only trace-proven paths, repair every red 27B axis, gate W3-C separately, and run 35B only after full 27B closure | `ACTIVE` |
