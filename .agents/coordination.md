...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid/trace `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; W3-F immutable evidence `~/work/vllm.cpp-nvfp4-device-alpha/7517af4f983fe322ac88ce2d9869e1441b7be3fd/evidence`; W3-G immutable DGX root `~/work/vllm.cpp-fa2-decode/ae9e8ff0576badabdda7289beeacaa1041c55d21`; W3-H local `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; H1a `~/work/vllm.cpp-executed-path-refresh/5d8af792…`; H1b `~/work/vllm.cpp-executed-path-refresh-frozen-h1b/5d8af792…`; H1c `~/work/vllm.cpp-executed-path-refresh-h1c/d1f8e332…`; H1d immutable root pending | current checkpoint `codex/nvfp4-small-m` | online harness/evidence/matrices plus completed W3-G ratio-6 FA2 gates and [W3-H normal-producer spike](specs/nvfp4-bf16-producer-vectorization.md). H1 owns the diagnostic-only four-replay controller, target-build/dispatch manifest, exact-workload paired ours/vLLM trace validation and residual re-ranking; W3-H2 inference-kernel implementation remains prohibited until valid H1d evidence. Host-weight, fused producer, GDN changes, scheduler and 35B performance excluded. ONE `flock` around the complete series | `ACTIVE` | 2026-07-13 (W3-G remains without speed credit. H1a/H1b/H1c are `VOID`; H1c both lost events and ran a CUTLASS-disabled naive/WMMA fallback. H1d's SIGUSR2/four-replay controller, exact CUTLASS/configure/compile/binary manifest and schema-v2 ours/vLLM validators are implemented and CPU-gated: focused tests **25/25**, full CUDA-off Release build, initial parallel CTest **104/106** with both failures passing serially **2/2**. Immutable target-path H1d execution is pending. No ratio, exact grid or 35B performance exists. GPU, `/tmp/gpu` and serving ports are idle; `3f256ab` remains 55/124) |
...
execution is next, while no W3-H2 inference-kernel code is authorized before
three valid target-path captures re-rank the residual.

## Handoff queue

| Priority | Row/block | Dependency | Next handoff | State |
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + W3-H executed-path refresh | `3f256ab` remains 55/124; W3-C reproduction control is green, while W3-E/W3-F/W3-G strict-fail and earn no speed credit. W3-H H1a/H1b/H1c are `VOID`; H1c lost events and was a CUTLASS-disabled naive/WMMA fallback build. H1d's trace-only controller/build manifest/schema-v2 validation is CPU-gated; GPU/lock/ports are idle | execute the clean, pushed H1d checkpoint as three immutable captures at cache-off input-1,024/output-128 with the exact read-only 64-plan fixture. Accept only exact CUTLASS build/dispatch provenance, zero-exit unique Nsight sessions, lossless direct graph-child correlation and identical resource structure; implement only the highest verified lever. No exact grid/35B performance until a later component passes every axis | `ACTIVE` |
