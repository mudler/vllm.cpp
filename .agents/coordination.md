...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; active worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; accepted H1d root `~/work/vllm.cpp-executed-path-refresh-h1d/c498a4131af7e6cf0ac678841212af80f4f12d53`; W3-I root `~/work/vllm.cpp-nvfp4-fused/15c6b8933d982019aa8965d218deb0eb1d9dc3f4-r2`; current VOID async root `~/work/vllm-async-credit/9b1774c014880a0039545ea1be0fa01426cbd900`; fresh root follows shape-neutral summary correction | current checkpoint `codex/nvfp4-small-m` | online harness/evidence/matrices and diagnostic-only vLLM async ON/OFF c2 timing + Torch traces. `ENG-ASYNC-SCHED`, loader/memory, RMSNorm, W3-H2/I2+, exact-grid, and 35B implementation remain excluded until the control selects a lever. ONE `flock` around the complete async series | `ACTIVE` | 2026-07-14 (`9b1774c` VOID: six timing legs and ON capture complete, provisional ON/OFF total **1.000464×**, but H1d exact-count validation stopped before shape-neutral summary/OFF/marker; cleanup pass. Fresh no-model-key run pending; binding 55/124) |
...
Default-off W3-I1 is structurally accepted at clean `15c6b89`, but its
complete component strict-fails **30/48** and keeps the exact grid blocked.
The claim therefore returns to the standing cross-stack scan across engine,
host memory, and executed kernels. W3-H2, W3-I2+, the exact grid, and 35B
performance remain prohibited until a new spike/claim explicitly selects the
next isolated lever. Superseded attempt roots remain in the append-only record,
not this live control plane.

## Handoff queue

| Priority | Row/block | Dependency | Next handoff | State |
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` async-credit control | `3f256ab` remains 55/124; `9b1774c` completed six timing legs at provisional **1.000464×** ON/OFF total and ON capture but is VOID because H1d exact-count validation stopped before shape-neutral summary/OFF | create a fresh root and repeat all six timing legs + two traces under one lock, summarizing c2 without `--model-key`. If the provisional neutral result repeats, map low-batch RMSNorm/generated partitions; only complete 4–6% credit may activate W3 for speed | `ACTIVE` |
| 2 | `SERVE-ASYNC-LLM` block (`ENG-CORE-BUSY-LOOP`, `SERVE-ASYNC-LLM`, `ENG-ASYNC-SCHED`, `ENG-PRIORITY-SCHED`) | joint spike accepted ([async-serving.md](specs/async-serving.md)); W1/W2/W4 implemented/`GATING`, W3 `READY` | diagnostic mode control is under row 1 only. Do not claim W3 implementation until its measured credit is known and the claim explicitly adds `ENG-ASYNC-SCHED` | `GATING` |
