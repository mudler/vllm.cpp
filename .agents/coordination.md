...
| `CLAIM-KV-DEVICE-1` | `KV-DEVICE-RESIDENCY` | root | main worktree `/home/mudler/_git/vllm.cpp`; baseline evidence `~/work/vllm.cpp-online-gate/evidence/f065fce18027b52a178b246f490b091b9d9e07a3`; W0 evidence `~/work/vllm.cpp-kv-device/7d29e0cd69709f6f96ebbf86a320f8885d7362d1/{w0-ab-27,w0-sanitizer,w0-sanitizer-access,w0-sanitizer-leaks-35}` | `main` | W0 move-only backend cache ownership/device residency + pointer/lifecycle tests, then W1 indexed GDN state I/O. Primary files `include/vllm/v1/worker/gpu/runner.h`, `src/vllm/v1/worker/gpu/runner.cpp`, `src/vllm/model_executor/models/qwen3_5.cpp`, vt CUDA op/tests, owning spec/matrices/README/BENCHMARKS/ledger/state. Every GPU A/B/trace series holds one uncontended lock | `ACTIVE` | 2026-07-11 (W0 A/B 785.49/769.15 tok/s = 1.021239×, 20/20 axes; paired trace 1.022188×, state rows H2D/D2H→D2D with 160,232 copy calls. Both-model access memcheck passes 234/234 + 315/315 with 0 errors. Full leak-check fails inherited pools: 47,290,056 B/101 dense allocations and 36,822,413,188 B/1,236 MoE allocations; W0 caches are absent. W1 is next; teardown remains required before row closure. No fresh vLLM parity claim) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + `KV-DEVICE-RESIDENCY` | Exact f065 27B before-state is below floor. W0 is CPU/CI/pointer/model/lifecycle/A-B/trace/access green at +2.1239%; inherited pools fail zero-leak. Trace retains ~160k D2D row-copy calls | implement and independently A/B W1 indexed state I/O; then exact direct/online 27B parity before 35B. Repair inherited pool teardown before row closure. Later roadmap blocks remain behind this priority-zero gate | `ACTIVE` |
