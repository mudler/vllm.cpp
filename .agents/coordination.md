...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover of stopped `serve-gate-online` stream | local `/home/mudler/_git/vllm.cpp-serve-gate`; DGX `~/work/vllm.cpp-latency` (pre-W2 build @ 83010c7) + a fresh current-main gate directory after the diagnostic lock releases | `codex/serve-gate-harness` | committed `tools/bench/online_gate*`, `tools/bench/profile_vllm_online_gate.py`, `tools/bench/summarize_torch_kernels.py`, `scripts/dgx-online-serving.sh`, `tests/tools/test_online_gate_*`; explicit server scheduling-capacity CLI in `examples/server/main.cpp` + help test/README; DGX current-main build/harness/results; engine matrix, roadmap, ledger/state. ONE `flock` around each complete model A/B+trace series | `ACTIVE` | 2026-07-11 (fail-closed harness implemented and CPU-gated: 16/16 focused, 32/32 full tools, relevant CTest 2/2, clean server build, compile/ShellCheck clean. It refreshes targets from a clean exact HEAD, hashes the pip-vLLM 0.24.0 runtime/client modules, and fixes identical c32 scheduler capacity/token budgets on both arms. Pre-W2 27B is diagnostic-complete; pre-W2 35B local is void after 1/6 c1 + surfaced illegal-memory-access abort; its vLLM arm continues diagnostically. Next: current-main DGX build, sanitizer reproduction if needed, then W2 G1/G3-G6) |

## Handoff queue

| Priority | Row/block | Dependency | Next handoff | State |
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` | First campaign invalid: vLLM startup failure + ours-35B aborts | root recovered `CLAIM-SERVE-GATE-1`; committed harness hardening is CPU-complete and fail-closed through exact corpus/output, resources and paired traces. Preserve the still-running pre-W2 vLLM diagnostic, then build current main, reproduce 35B under sanitizer if necessary, and run W2 G1/G3-G6 under new whole-model locks | `ACTIVE` |
