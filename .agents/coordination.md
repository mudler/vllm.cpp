...
| 7 | C2/C4 models + quantizations | accepted comprehensive inventories | `MODEL-FACTORY-registry` spike accepted and READY ([model-factory-registry.md](specs/model-factory-registry.md)); claim the registry implementation, then Llama dense. `QUANT-GGUF-CPU-THREADPOOL` W1-W3 is implemented and `GATING`: 1/3/20 determinism + TSAN pass, but the mandatory B4 1-vs-20 speed/RSS series needs an exclusive idle 20-core host (recovered host had unowned inference load). Claim released; exact rerun is in its leaf spec. Keep-quant loader + CIQ GEMM remain next after that checkpoint | `GATING` |
