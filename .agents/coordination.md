...
| `CLAIM-HOST-WEIGHT-RESIDENCY-1` | `ENG-HOST-WEIGHT-RESIDENCY` | Codex:gpt-5 | local `/home/rich/c/vllm.cpp`; local RTX evidence under `/tmp/qwen35-memory-*`, `/tmp/qwen35-host-residency-*`, `/tmp/qwen35-peak-w4-*` | `local-blackwell-environment` | `.agents/specs/discrete-cuda-host-weight-residency.md`; `.agents/specs/safetensors-bounded-page-residency.md`; `.agents/specs/dense-direct-device-loading.md`; safetensors reader, dense loader/forward/model-registry/entrypoint files and focused tests; matrix-count invariant in `scripts/check-agent-record.py`; matching matrix/roadmap/README/BENCHMARKS/ledger/state rows. One `/tmp/gpu` lock around each complete local A/B series; no Spark/GB10 access or system configuration changes | `ACTIVE` | 2026-07-12 (W4.4 implemented: one preselected dense queue, strict eligibility, per-layer stage/sync/release, `VT_DIRECT_DEVICE_LOAD=0` control. CPU focused 4/4; native-sm_120 default 5/5 + control 3/3. Real-model peak/timing pending) |
