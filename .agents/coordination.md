...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE` | root takeover | binding grid `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; current C3 evidence `~/work/vllm.cpp-nvfp4-persistent/d211b8f80fff831a712f0bfafa4f65f1abe1892d/evidence`; local `/home/mudler/_git/vllm.cpp-nvfp4-small-m` | current checkpoint `codex/nvfp4-small-m` | online harness/evidence/matrices. ONE `flock` around a complete model A/B+trace | `ACTIVE` | 2026-07-13 (`3f256ab` remains 55/124. W3-C six-process 64/64 stability passes. C3R proves 6/6 x 128-token direct/fallback equality under identical sequential batch shape and 0/6 sequential-vs-c2 for both ours and production-default vLLM; old cross-run predicate void/reclassified. Corrected C3 ready; no exact grid/35B performance) |
...
| `CLAIM-NVFP4-SMALL-M-3` | `KERNEL-GEMM-NVFP4-W4A4` (`W3`) | root | isolated worktree `/home/mudler/_git/vllm.cpp-nvfp4-small-m`; oracle cache fixture under `tests/fixtures/nvfp4_flashinfer_v025_gb10/`; C2 staging `~/work/vllm.cpp-nvfp4-persistent-c2-staging/evidence`; immutable C3/C3R `~/work/vllm.cpp-nvfp4-persistent/d211b8f80fff831a712f0bfafa4f65f1abe1892d/evidence` | `codex/nvfp4-small-m` | W3-C owns document/import/atomicity, ready-map/lifecycle/5,000-us wiring, corrected same-plan gates and C3 execution per [spike](specs/nvfp4-persistent-plan-cache.md). Other levers excluded | `ACTIVE` | 2026-07-13 (C2/six-process stability pass. C3R summary `a1c500b3…41de`: sequential direct/fallback 6/6 x 128 equal; each arm and vLLM default 0/6 sequential-vs-c2. Independent-run equality reclassified; stopped performance remains void. Corrected C3 `READY`; no ratio/exact grid/35B performance) |
...
| `CLAIM-HOST-WEIGHT-RESIDENCY-1` | `ENG-HOST-WEIGHT-RESIDENCY` | Codex:gpt-5 | local `/home/rich/c/vllm.cpp`; local RTX evidence under `/tmp/qwen35-memory-*`, `/tmp/qwen35-host-residency-*`, `/tmp/qwen35-peak-w4-*`, `/tmp/qwen35-direct-*` | `local-blackwell-environment` | `.agents/specs/discrete-cuda-host-weight-residency.md`; `.agents/specs/safetensors-bounded-page-residency.md`; `.agents/specs/dense-direct-device-loading.md`; safetensors reader, dense loader/forward/model-registry/entrypoint files and focused tests; matrix-count invariant in `scripts/check-agent-record.py`; matching matrix/roadmap/README/BENCHMARKS/ledger/state rows. One `/tmp/gpu` lock around each complete local A/B series; no Spark/GB10 access or system configuration changes | `ACTIVE` | 2026-07-12 (`80f370f` post-rebase campaign diagnostic: peak PSS 1.847/8.168/6.883 GiB ON/OFF/vLLM; throughput 6607.68/6605.42/6717.32; mean TTFT 658/659/904 ms. ON/OFF and same-mode output vary while vLLM is stable, so correctness fails and no metric is promoted. CPU focused 4/4, full 104/105 + async isolated 3/3; CUDA 5/5 + 3/3) |

## Handoff queue

| Priority | Row/block | Dependency | Next handoff | State |
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + FP4 repair | `3f256ab` remains 55/124; W3-E failed. W3-C runtime plus six-process 64/64 stability is green; C3R reclassifies the invalid cross-run batch-invariance predicate with controlled local/vLLM execution; GPU/ports/lock idle | rerun the corrected frozen-plan c2/c16 C3 component, requiring both model gates, controlled 6/6 proof, exact counts, lifecycle, 64/64 map, zero tuning/misses and all 40 timing + 8 memory axes. No exact grid/35B performance before acceptance | `ACTIVE` |
