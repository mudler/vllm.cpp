...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid/trace `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; W3-F immutable evidence `~/work/vllm.cpp-nvfp4-device-alpha/7517af4f983fe322ac88ce2d9869e1441b7be3fd/evidence`; W3-G local `/home/mudler/_git/vllm.cpp-nvfp4-small-m`, immutable DGX root `~/work/vllm.cpp-fa2-decode/ae9e8ff0576badabdda7289beeacaa1041c55d21` | current checkpoint `codex/nvfp4-small-m` | online harness/evidence/matrices plus W3-G ratio-6 FA2 split-KV decode adapter, exact fallback, model dtype dispatch, scratch lifecycle and ported attention tests per [spike](specs/fa2-gqa-split-kv-decode.md). FP4 producer, host-weight, GDN, scheduler and 35B performance excluded. ONE `flock` around a complete model A/B+trace or component series | `ACTIVE` | 2026-07-13 (W3-G1--G4 source and immutable sm_121a/operator/zero-error-zero-leak memcheck/27B default+fallback/35B correctness/paired-node-trace gates pass at `ae9e8ff`. Trace switch is exact and FP4 topology/plans match; short-prompt timing is negative/non-binding. Frozen input-1,024 c2/c16 40+8 component started `2026-07-13T10:44:49Z` under the full-series lock; both model arms passed and partial legs are non-binding. `3f256ab` remains 55/124; no exact grid/35B performance) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + `KERNEL-ATTN-FA2` W3-G | `3f256ab` remains 55/124; W3-C reproduction control is green, while W3-E and W3-F strict-fail. Immutable `ae9e8ff` closes W3-G source, CUDA/operator/sanitizer/model/inertness and paired structural trace gates. The frozen c2/c16 component owns `/tmp/gpu`; both model arms passed and partial legs remain non-binding | finish/classify the active input-1,024 c2/c16 AB/BA/AB **40 timing + 8 memory** component. No exact grid/35B performance unless every component axis passes | `ACTIVE` |
