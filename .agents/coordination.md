...
| 8 | C3/C5/C7/C8 engine/API work | C3/C5/C6 specs accepted; C7/C8 leaf specs missing | C3 M-mtp-0 CPU implementation is complete and its claim is released; `SPEC-MTP` + both Qwen3.5 MTP model rows are `GATING` on the exact two-checkpoint recipe in the latest [state log](state.md), queued behind `CLAIM-SERVE-GATE-1`. C5 is `READY` via [the joint spike](specs/sliding-local-yarn-long-context.md); next claim its dependency-free KV leaves or `ATTN-YARN`, then spike C7/C8 | `GATING` |
