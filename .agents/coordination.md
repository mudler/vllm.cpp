...
| `CLAIM-SERVE-GATE-1` | `SERVE-GATE-ONLINE`, `KERNEL-GEMM-NVFP4-W4A4`, `KERNEL-ATTN-FA2` | root takeover | binding grid/trace `~/work/vllm.cpp-online-gate/evidence/3f256abdbb558e162bf8a2196284deb119648560`; W3-F immutable evidence `~/work/vllm.cpp-nvfp4-device-alpha/7517af4f983fe322ac88ce2d9869e1441b7be3fd/evidence`; W3-G immutable DGX root `~/work/vllm.cpp-fa2-decode/ae9e8ff0576badabdda7289beeacaa1041c55d21`; W3-H local `/home/mudler/_git/vllm.cpp-nvfp4-small-m`, pending immutable DGX root `~/work/vllm.cpp-executed-path-refresh/<SHA>` | current checkpoint `codex/nvfp4-small-m` | online harness/evidence/matrices plus completed W3-G ratio-6 FA2 gates and [W3-H normal-producer spike](specs/nvfp4-bf16-producer-vectorization.md). H1 owns only a fresh exact-workload paired ours/vLLM trace and residual re-ranking; implementation is prohibited until H1. Host-weight, fused producer, GDN changes, scheduler and 35B performance excluded. ONE `flock` around the complete paired profile series | `ACTIVE` | 2026-07-13 (W3-G strict-fails **35/40 timing + 5/8 memory** and earns no speed credit. W3-H H0 is complete/`READY`: current decode-only slices show **0.627934/0.343538 ms per forward** local/vLLM, but this is cross-profiler diagnostic evidence and the prior combined vector experiment was neutral. H1 fresh exact current-source tracing is `PENDING`; no code, accepted ratio, exact grid or 35B performance exists. GPU, `/tmp/gpu` and serving ports are idle; `3f256ab` remains 55/124) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + W3-H executed-path refresh | `3f256ab` remains 55/124; W3-C reproduction control is green, while W3-E/W3-F/W3-G strict-fail and earn no speed credit. W3-H H0 now inventories the complete normal BF16→FP4 source/SASS/history/test/gate chain and is `READY / trace-first`; implementation remains prohibited. GPU/lock/ports are idle | freeze/push H0, then run W3-H1's exact cache-off input-1,024/output-128 current ours/vLLM paired trace and re-rank every hot family. Implement only if it remains the highest verified lever; no exact grid/35B performance until a component passes every axis | `ACTIVE` |
