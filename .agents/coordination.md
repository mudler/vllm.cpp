...
| `CLAIM-HOST-WEIGHT-RESIDENCY-1` | `ENG-HOST-WEIGHT-RESIDENCY` | Codex:gpt-5 | local `/home/rich/c/vllm.cpp`; local RTX evidence under `/tmp/qwen35-memory-*`, `/tmp/qwen35-host-residency-*`, `/tmp/qwen35-peak-w4-*` | `local-blackwell-environment` | `.agents/specs/discrete-cuda-host-weight-residency.md`; `.agents/specs/safetensors-bounded-page-residency.md`; safetensors reader, dense loader/forward/model-registry files and focused tests; matrix-count invariant in `scripts/check-agent-record.py`; matching matrix/roadmap/README/BENCHMARKS/ledger/state rows. One `/tmp/gpu` lock around each complete local A/B series; no Spark/GB10 access or system configuration changes | `ACTIVE` | 2026-07-12 (W4.3 accepted at `a077d72`: peak 8.168 ON vs 15.562 OFF vs 7.695 GiB vLLM; steady 0.754 vs 4.099; VRAM 11,682 vs 12,924 MiB; ON +0.02% vs OFF and 0.9755x vLLM. CPU 105/105, CUDA default 5/5 + opt-out 2/2. W4.4 direct-device spike next) |
