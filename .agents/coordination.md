...
| `CLAIM-HOST-WEIGHT-RESIDENCY-1` | `ENG-HOST-WEIGHT-RESIDENCY` | Codex:gpt-5 | local `/home/rich/c/vllm.cpp`; local RTX evidence under `/tmp/qwen35-memory-*`, `/tmp/qwen35-host-residency-*`, `/tmp/qwen35-peak-w4-*`, `/tmp/qwen35-direct-*` | `local-blackwell-environment` | `.agents/specs/discrete-cuda-host-weight-residency.md`; `.agents/specs/safetensors-bounded-page-residency.md`; `.agents/specs/dense-direct-device-loading.md`; safetensors reader, dense loader/forward/model-registry/entrypoint files and focused tests; matrix-count invariant in `scripts/check-agent-record.py`; matching matrix/roadmap/README/BENCHMARKS/ledger/state rows. One `/tmp/gpu` lock around each complete local A/B series; no Spark/GB10 access or system configuration changes | `ACTIVE` | 2026-07-12 (post-W2 rebase fix: broad dense predicate made 4B hit the 27B BF16 GDN dtype contract; native-NVFP4 predicate now preserves 27B and restores 4B f32. CUDA rebuild, explicit-f32 and corrected-default real-model smoke pass; full immutable campaign pending. Prior `80f370f` metrics remain diagnostic) |
