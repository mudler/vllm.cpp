...
| `CLAIM-C5-SW-ATTN-1` | `ATTN-SLIDING-WINDOW` | root | local `/home/mudler/_git/vllm.cpp-c5-sw-attn` | `codex/c5-sw-attn-w2` | W2 only: `include/vt/ops.h`; `src/vt/{ops.cpp,cpu/cpu_paged_attn.cpp,cuda/cuda_paged_attn.cu,cuda/cuda_flash_attn_fa2.cu}`; narrow generic attention/backend/config seams under `include/src/vllm/model_executor/layers/attention/`, `include/src/vllm/v1/attention/`, and `include/src/vllm/transformers_utils/hf_config.*`; corresponding CMake and paged-attention/backend/config tests; C5 matrix/roadmap/inventory/ledger/state rows. No chunked-local, RoPE, model-family implementation, connector/offload, DCP/PCP, or GPU execution while `CLAIM-SERVE-GATE-1` owns dgx | `ACTIVE` | 2026-07-11 (claim established from accepted W2 spike; implementation starts with upstream-derived operator vectors and one backend-neutral `(left,right)` semantic propagated through CPU, portable CUDA and vendored FA2; GPU/model/oracle gates remain a later handoff) |
...
| 8 | C3/C5/C7/C8 engine/API work | C3/C5/C6 specs accepted; C7/C8 leaf specs missing | C3 M-mtp-0 remains `GATING` on its two-checkpoint GPU recipe. C5 W1 `KV-SLIDING-WINDOW-SPEC` is implemented and `GATING`; W2 `ATTN-SLIDING-WINDOW` is `ACTIVE` under `CLAIM-C5-SW-ATTN-1` for generic CPU/CUDA/FA2 semantics and tests, without GPU contention. Restore the pinned oracle and run W2 model/trace/performance gates after serving releases dgx. Chunked-local KV and `ATTN-YARN` remain claimable. Then spike C7/C8 | `ACTIVE` |
