...
| `CLAIM-SERVE-HTTP-POOL-1` | `SERVE-ASYNC-LLM` (W2 HTTP-capacity repair) | root | planned isolated worktree `/home/mudler/_git/vllm.cpp-serve-http-pool`; DGX evidence under `~/work/vllm.cpp-online-gate` or a commit-bound component-A/B sibling | `codex/serve-http-pool` | `src/vllm/entrypoints/openai/api_server.{h,cpp}`, `examples/server/main.cpp` only if an explicit worker setting is required, `tests/vllm/entrypoints/openai/test_api_server.cpp`, async-serving spec + owning README/BENCHMARKS/matrix/roadmap/ledger/state rows. No scheduler/kernel edits. Exact c32/full-ladder series holds one uncontended lock | `ACTIVE` | 2026-07-11 (claim established from `a531e05` evidence: cpp-httplib initial pool 19 on 20 CPUs; c32 r1/r3 each leave one accepted socket at Recv-Q 2,131/no bytes sent for 205–207 s while 31 streams progress. Implement deterministic stream capacity + lifecycle regression before GPU A/B) |
...
|---|---|---|---|---|
| 1 | `SERVE-GATE-ONLINE` + HTTP/FP4 repair | Exact `a531e05` 27B checkpoint is binding below floor; c32 HTTP capacity and small-M FP4 cache/tactic mismatch are reproduced. W0/W1 component wins and teardown debt remain context | `CLAIM-SERVE-HTTP-POOL-1` repairs/tests c32 transport first. Then write/commit the specific FP4 dependency-chain spike, port hybrid buckets/single-flight, and expand tactics as separately measured iterations. Repeat exact 27B until every axis passes; only then run 35B. Later roadmap blocks remain behind this priority-zero gate | `ACTIVE` |
| 2 | `SERVE-ASYNC-LLM` block (`ENG-CORE-BUSY-LOOP`, `SERVE-ASYNC-LLM`, `ENG-ASYNC-SCHED`, `ENG-PRIORITY-SCHED`) | joint spike accepted ([async-serving.md](specs/async-serving.md)); exact c32 evidence confirms W2 transport capacity is not closed | W1/W2/W4 host behavior remains CPU/TSan-green. The active bounded W2 repair adds deterministic persistent-stream worker capacity and tests, then closes G3 at c32/full ladder. W3 overlap stays READY and must not be mixed into the transport A/B | `GATING` |
