...
- Historical offline vLLM throughput baselines are non-binding after the
  sampling/token-budget audit. Exact pushed-`4e1d8ca` 27B online evidence is
  ...
  legacy series does not reproduce the historical rare tail. FP4 small-M
  key/tactic mismatch is the active next repair. Repeat exact 27B under one
  whole-series GPU lock after each FP4 iteration before starting exact 35B.
