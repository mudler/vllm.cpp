...
- Binding immutable `3f256ab` vLLM v0.25.0 evidence completes the exact
  ...
  strict-fail and earn no speed credit. W3-H schema-v5 `c498a413` passes exact
  build/correctness, three local sessions, all 12 lossless reports and the
  paired vLLM trace; it remains VOID pending final-status revalidation because
  diagnostic output repeatability was incorrectly fatal. Revalidate under the
  corrected CPU-green contract and close all 69 failed axes before 35B
  performance.
