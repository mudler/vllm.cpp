...
| GDN packed pure decode | `envs.py::VLLM_ENABLE_FLA_PACKED_RECURRENT_DECODE`, `fla/ops/fused_recurrent.py` | `ACTIVE` T0 | `KERNEL-GDN-PACKED-DECODE` mirrors vLLM's default pure non-spec branch across FP16/BF16/F32. Clean `f344dec` closes correctness and `7ff713e` + `24cea4f` close structure. The repaired component runner has an exact `--profile-control off` contract and passes focused **45/45**, all tools **127/127**. Fresh-SHA **40+8** DGX execution remains open with no speed credit | [gdn-packed-decode.md](specs/gdn-packed-decode.md) |
