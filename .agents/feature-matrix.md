...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | all three native CUDA slices retain support/correctness closure on GB10; performance remains separately `ACTIVE`. Immutable `9cc7191` binds at **54/124 axes** and `def5f75` selected packed QKV. Clean `3f256ab` W3-D packed/split arms pass 235/235 + 16/16; the c16 component is **1.005049×** mean-positive but strict-fails at **14/20 timing + 2/4 memory**. `VT_FP4_MERGED_QKV=0` retains the split path. Support/correctness stays closed while performance awaits the node re-trace and exact grid; 35B remains held ([small-M spike](specs/nvfp4-small-m-dispatch.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
