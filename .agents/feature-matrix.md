...
| GDN packed pure decode | `envs.py::VLLM_ENABLE_FLA_PACKED_RECURRENT_DECODE`, `fla/ops/fused_recurrent.py` | `ACTIVE` T0 | New execution row `KERNEL-GDN-PACKED-DECODE` mirrors vLLM's default pure non-spec branch across FP16/BF16/F32. The official oracle proves our BF16 downstream mismatch requires both F32 in-kernel q/k normalization and dtype-rounded beta: current `306/7552`, beta-only `308/6558`, full semantics `0/1` output/state differences. Immutable replay, portable/CUDA implementation, 235/235 and c2/c16 remain open | [gdn-packed-decode.md](specs/gdn-packed-decode.md) |
...
| **Async/overlap scheduling** | `v1/core/sched/async_scheduler.py` | ☐ T1 (**promoted**), spike accepted | vLLM's default-ON depth-2 path remains an unmet mirror obligation. Accepted `3812d8` c2 control completes six timing legs and two shape-neutral traces: total credit only **+0.215%**, TPOT ~1.0% better, TTFT ~16.0% worse, and ON traced GPU time ~0.2% higher. `ENG-ASYNC-SCHED` remains `READY`, unowned and uncredited as later parity work; it leaves the immediate speed path | [async-serving.md](specs/async-serving.md) |
...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | support/correctness stays closed; performance remains `ACTIVE` at `3f256ab` **55/124**. Finalized `179a0fc` proves the FP4 tactic family matches. The separate merged-GDN W1C has exact BA projection/inertness but BF16 27B remains 233/235; the new [packed-decode spike](specs/gdn-packed-decode.md) closes the first downstream divergence and is the active non-quantized dependency. Immutable replay and component performance remain open, with no quantization speed credit | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
...
| Feature | Upstream | Status | Notes | Spec |
|---|---|---|---|---|
| safetensors container + model-specific loading | `model_loader/default_loader.py`, `models/utils.py` | `PARTIAL` T0 | Qwen-specific resolvers work, but exact 27B accounting finds a persistent **22.920 GiB** CPU tensor mirror and overlapping source-page residency during load. Generic mapping and direct-to-final-device streaming remain absent | `planned: specs/safetensors-loader.md` |
