...
| GDN packed pure decode | `envs.py::VLLM_ENABLE_FLA_PACKED_RECURRENT_DECODE`, `fla/ops/fused_recurrent.py` | `ACTIVE` T0 | `KERNEL-GDN-PACKED-DECODE` mirrors vLLM's default pure non-spec branch across FP16/BF16/F32. Clean `f344dec` closes W1D2/G2. Clean `7ff713e` completes all 24 exact packed/rollback raw ranges and both steady-window oracle traces. The only new launch fingerprints are inspected generated-RMSNorm 48/50-register distributions; exact regression coverage and dry finalization pass `complete-structural`. The pushed marker and c2/c16 remain open with no speed credit | [gdn-packed-decode.md](specs/gdn-packed-decode.md) |
