...
| Device-resident attention KV + indexed GDN state I/O | `v1/worker/gpu/attn_utils.py`, `model_executor/layers/mamba/gdn/qwen_gdn_linear_attn.py` | `READY` T0 | trace-grounded row `KV-DEVICE-RESIDENCY`: current CUDA caches are host vectors; W0-W2 move ownership device-side and eliminate mixed-prefill row copies before exact 27B→35B gates | [device-resident-kv-gdn-state.md](specs/device-resident-kv-gdn-state.md) |
