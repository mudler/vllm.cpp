...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | support/correctness stays closed; performance remains `ACTIVE` at `3f256ab` **55/124**. W3-C2 wires native/imported plans into CUDA startup with source identity, loaded priority, frozen misses, 5,000-us timing and completed-only publication. Exact 64/64 zero-tune import, lifecycle/memcheck and both frozen 27B arms pass; no performance result or support expansion exists. Six-process/same-plan/35B performance remain pending ([persistent-cache spike](specs/nvfp4-persistent-plan-cache.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
