...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | support/correctness stays closed; performance remains `ACTIVE` at `3f256ab` **55/124**. Finalized `179a0fc` proves the FP4 tactic family matches. The separate [merged-GDN spike](specs/gdn-merged-input-projections.md) has W1C `ACTIVE`: finalized `0091cd1` is `complete-structural`, while the exact five-shape vLLM BF16 oracle and default-off local/35B/GGUF tests are implemented. Oracle 3×, synthetic GGUF 97/97 and CPU parity 123/123 pass; local CUDA/model/hardware gates and component performance remain open, with no quantization speed credit | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
