...
| **Async/overlap scheduling** | `v1/core/sched/async_scheduler.py` | ☐ T1 (**promoted**), spike accepted | vLLM's default-ON depth-2 path is the only unmeasured structural candidate large enough for the c2 gap. Explicit modes are CPU-tested; `2ec6dda` is VOID after successful ON-r1 because validation stopped before OFF, and a fresh corrected control is `PENDING`. `ENG-ASYNC-SCHED` remains `READY`, unowned, and uncredited | [async-serving.md](specs/async-serving.md) |
