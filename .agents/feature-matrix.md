...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | all three native CUDA slices retain support/correctness closure on GB10; performance remains separately `ACTIVE`. Immutable `3f256ab` binds at **55/124 axes**. W3-E direct swizzled activation scales is implemented with explicit linear/direct layouts and `VT_FP4_DIRECT_SF=0`; CPU/CUDA byte parity, focused sanitizer, direct/fallback 27B **235/235 + 16/16**, and 35B inertness **315/315** pass. Paired tracing removes all **624** hot activation swizzles. Its completed c2/c16 component gains **1.002108× / 1.006222×** mean total throughput but strict-fails **32/40 timing + 6/8 memory**, so it receives no speed credit and no exact rerun. Support/correctness stays closed while 69 binding performance axes and 35B performance remain held ([small-M spike](specs/nvfp4-small-m-dispatch.md), [direct-scale spike](specs/nvfp4-direct-swizzled-scales.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
