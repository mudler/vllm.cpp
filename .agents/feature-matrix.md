...
| llama3 / longrope / dynamic-NTK scaling | same | `GATING` T1–T2 | umbrella split into three independently claimable engine leaves. W6 Llama 3 typed config/factory and exact low/mid/high formula (including equal factors) pass three pinned-source f32/bf16 NeoX/GPT-J fixtures plus CPU/sanitizers; CUDA apply, Llama-3.1 e2e and G9 remain gated. LongRoPE and dynamic-NTK remain `READY` | [sliding-local-yarn-long-context.md](specs/sliding-local-yarn-long-context.md) |
