...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | all three native CUDA slices retain support/correctness closure on GB10; performance remains separately `ACTIVE`. Immutable `9cc7191` completed the v0.25.0/FlashInfer 0.6.13 27B cache-off gate and paired traces: all 124 axes bind, **54 pass / 70 fail**. c16/c32 total throughput passes, low-concurrency totals and decode latency remain red, and host PSS/RSS fail. No support closure is revoked, no topology repair is selected without the executed kernel diff, and 35B remains held ([small-M spike](specs/nvfp4-small-m-dispatch.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
