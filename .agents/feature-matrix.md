...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | support/correctness stays closed; performance remains `ACTIVE` at `3f256ab` **55/124**. W3-C reproduction control is complete; W3-E/W3-F/W3-G strict-fail and earn no speed credit. The complete [W3-H normal-producer spike](specs/nvfp4-bf16-producer-vectorization.md) is **ACTIVE / trace-first**: H1a/H1b are `VOID`; immutable `d1f8e33` H1c is doubly `VOID` from possible event loss and a CUTLASS-disabled build. Its trace ran **146,661 naive + 10,944 WMMA** FP4 GEMMs, not the target path, so retained producer timing remains diagnostic only. H1d must preflight exact target-build/dispatch provenance before GPU work, then enforce lossless collection plus semantic plan/workload/graph validation. Implementation remains prohibited and quant support state is unchanged ([persistent-cache spike](specs/nvfp4-persistent-plan-cache.md), [device-alpha spike](specs/nvfp4-device-alpha.md), [FA2 decode spike](specs/fa2-gqa-split-kv-decode.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
