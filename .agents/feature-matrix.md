...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | support/correctness stays closed; performance remains `ACTIVE` at `3f256ab` **55/124**. W3-C reproduction control is complete; W3-E/W3-F/W3-G strict-fail and earn no speed credit. The complete [W3-H normal-producer spike](specs/nvfp4-bf16-producer-vectorization.md) is **ACTIVE / trace-first**: H1a/H1b/H1c and H1d attempts through `a96e899` are `VOID`. The latest exact build/gate and four replays pass, then SIGTERM yields Nsight exit 143 before validation. H1d now preflights exact build/ancestry and a diagnostic-only synchronous SIGUSR1 graceful stop before enforcing zero-exit lossless plan/workload/graph validation. Implementation remains prohibited and quant support state is unchanged ([persistent-cache spike](specs/nvfp4-persistent-plan-cache.md), [device-alpha spike](specs/nvfp4-device-alpha.md), [FA2 decode spike](specs/fa2-gqa-split-kv-decode.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
