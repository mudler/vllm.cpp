...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | support/correctness stays closed; performance remains `ACTIVE` at `3f256ab` **55/124**. Finalized `179a0fc` proves the FP4 tactic family matches; no quantization lever or speed credit follows. The separate [merged-GDN spike](specs/gdn-merged-input-projections.md) has BA-only W1 `GATING`: pushed `581d335` is core-correctness/safety green and finalized `0091cd1` is `complete-structural` at exact merged 963/145 versus split 1,011/193 with `benchmark_binding=false`. BF16 output, GGUF/legacy inertness and component performance remain open | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
