...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | support/correctness stays closed; performance remains `ACTIVE` at `3f256ab` **55/124**. W3-C reproduction control is complete and W3-E strict-fails at **39/40 timing + 1/8 memory**. W3-F model-owned device alpha addresses the verified 208-local-versus-zero-vLLM scalar-stage mismatch: tensor ABI/validation, direct CUDA pointer, individual/merged residency, exact fallback and upstream-shaped tests are implemented. CPU Release 103/103 and focused Release/ASan+UBSan/TSan 17/17 + 911/911 pass; CUDA safety/model/node-trace/frozen-plan A/B remain pending. Exact grid and 35B performance remain open ([persistent-cache spike](specs/nvfp4-persistent-plan-cache.md), [device-alpha spike](specs/nvfp4-device-alpha.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
