...
| Chunked-local attention wrapper/backend | `model_executor/layers/attention/chunked_local_attention.py` | `READY` T1 | W4 is the next dependency-ready leaf: virtual-batch metadata over the selected ordinary backend; W3 chunked-local KV is implemented/`GATING` | [sliding-local-yarn-long-context.md](specs/sliding-local-yarn-long-context.md) |
