...
| CUDA graphs (decode capture/replay, no torch) | `config/compilation.py::cudagraph_mode` | `PARTIAL` T0 | Qwen-specific capture; W3-G adds source-level FA2 per-shape stable scratch plus capture/replay/capacity/lifecycle tests, but their sm_121a runtime evidence is pending. Explicit 35B assertion, generic modes and direct 27B evidence remain open | [FA2 decode spike](specs/fa2-gqa-split-kv-decode.md); `planned: specs/cuda-graphs.md` |
| FA2 paged GQA pure-decode split-KV | `v1/attention/backends/flash_attn.py`; dependency `flash_api.cpp` | `ACTIVE` T0 | Exact 27B ratio-6 BF16/D256 adapter, swap strides, split heuristic, queue-owned graph-stable scratch, model/dispatcher toggle and ported operator tests are present. CUDA-off compile passes; sm_121a/operator/model/trace/performance evidence is pending. Ratio-8 and other modes intentionally fall back | [FA2 decode spike](specs/fa2-gqa-split-kv-decode.md) |
...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | support/correctness stays closed; performance remains `ACTIVE` at `3f256ab` **55/124**. W3-C reproduction control is complete and W3-E strict-fails at **39/40 timing + 1/8 memory**. W3-F immutable `7517af4` passes CUDA/memcheck/model and zero-versus-3,536-stage trace gates with identical FP4 topology/plans, but its component strict-fails at **27/40 timing + 3/8 memory**. W3-G attention source/tests now exist with only a CUDA-off checkpoint; no quant behavior, speed credit, exact grid or 35B performance claim follows. Vectorized normal FP4 production remains the second, unstacked lever ([persistent-cache spike](specs/nvfp4-persistent-plan-cache.md), [device-alpha spike](specs/nvfp4-device-alpha.md), [FA2 decode spike](specs/fa2-gqa-split-kv-decode.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
