...
| SlidingWindowSpec + ChunkedLocalAttentionSpec | `v1/kv_cache_interface.py` | `PARTIAL` T1 | `KV-SLIDING-WINDOW-SPEC` W1 is implemented and `GATING`; `KV-CHUNKED-LOCAL-SPEC` W3 is `ACTIVE` under `CLAIM-C5-CHUNKED-KV-1` for its concrete spec/manager/registry/hybrid-conversion semantics and ported CPU properties | [sliding-local-yarn-long-context.md](specs/sliding-local-yarn-long-context.md) |
