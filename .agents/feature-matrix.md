...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | support/correctness stays closed; performance remains `ACTIVE` at `3f256ab` **55/124**. W3-C reproduction control passes; W3-E/W3-F/W3-G earn no speed credit. W3-H is **ACTIVE / trace-first**: clean schema-v4 `b9beccd` passes build/gate/plans/clients/FIFO/zero-exit and four complete zero-eager reports, but fails closed on pinned Nsight's possible-loss capture-range diagnostic before sessions 2/3 or vLLM and is VOID. Calibrate that diagnostic under exact event/completion/topology reconciliation, then rerun all 12 reports. W3-H2 remains prohibited and quant support is unchanged | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
