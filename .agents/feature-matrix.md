...
| GDN packed pure decode | `envs.py::VLLM_ENABLE_FLA_PACKED_RECURRENT_DECODE`, `fla/ops/fused_recurrent.py` | `ACTIVE` T0 | `KERNEL-GDN-PACKED-DECODE` mirrors vLLM's default pure non-spec branch across FP16/BF16/F32. Clean `f344dec` closes W1D2/G2. Clean `7ff713e`, finalized by pushed `24cea4f`, closes W1D3 structural evidence as marker-last `complete-structural`: all 24 packed/rollback ranges and both steady-window oracle traces are exact. c2/c16 remains open with no speed credit | [gdn-packed-decode.md](specs/gdn-packed-decode.md) |
