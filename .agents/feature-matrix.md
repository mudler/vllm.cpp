...
| GDN packed pure decode | `envs.py::VLLM_ENABLE_FLA_PACKED_RECURRENT_DECODE`, `fla/ops/fused_recurrent.py` | `ACTIVE` T0 | `KERNEL-GDN-PACKED-DECODE` mirrors vLLM's default pure non-spec branch across FP16/BF16/F32. Clean `f344dec` closes W1D2/G2. Clean `4804ee4` validated eight packed ranges, then failed/voided on a shared-log marker-prefix race before the complete dual-arm trace. The prefix/cardinality/suffix parser and 48-node coupled-BA isolation pass **82/82** tools. Fresh trace and c2/c16 remain open with no speed credit | [gdn-packed-decode.md](specs/gdn-packed-decode.md) |
