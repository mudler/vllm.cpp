...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | support/correctness stays closed; performance remains `ACTIVE` at `3f256ab` **55/124**. W3-C reproduction control is complete; W3-E/W3-F/W3-G strict-fail and earn no speed credit. The complete [W3-H normal-producer spike](specs/nvfp4-bf16-producer-vectorization.md) is **ACTIVE / trace-first**: H1a/H1b/H1c are `VOID`; clean `5f8fab1` is also void because its H1d recipe omitted the binding Triton-AOT GDN path, used `Release`, and failed correctness identically with profile control on/off before capture. H1d now preflights `RelWithDebInfo`, vendored Triton-AOT, FA2, CUTLASS and pinned tools before GPU work, then enforces lossless plan/workload/graph validation. Implementation remains prohibited and quant support state is unchanged ([persistent-cache spike](specs/nvfp4-persistent-plan-cache.md), [device-alpha spike](specs/nvfp4-device-alpha.md), [FA2 decode spike](specs/fa2-gqa-split-kv-decode.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
