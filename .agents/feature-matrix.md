...
| GDN packed pure decode | `envs.py::VLLM_ENABLE_FLA_PACKED_RECURRENT_DECODE`, `fla/ops/fused_recurrent.py` | `ACTIVE` T0 | `KERNEL-GDN-PACKED-DECODE` mirrors vLLM's default pure non-spec branch across FP16/BF16/F32. W1D2 dispatch, independent state dtypes, metadata preflight and rollback are implemented; mutable default+rollback 27B **235/235**, 35B/GGUF inertness and CUDA safety pass. Immutable G2 and trace/c2/c16 remain open | [gdn-packed-decode.md](specs/gdn-packed-decode.md) |
...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | support/correctness stays closed; performance remains `ACTIVE` at `3f256ab` **55/124**. FP4 tactics match, and W1D2 packed recurrence restores mutable 27B **235/235** in default+rollback arms with 35B/GGUF inertness. Immutable G2/component evidence remains open; no quantization speed credit follows | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
