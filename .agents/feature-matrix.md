...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | all three native CUDA slices retain support/correctness closure on GB10; performance remains separately `ACTIVE`. W2's 32 SM12 tactics, merged CT scaling and fused SiLU→NVFP4 path pass focused/native gates. Clean `b5c6e4f` remains the below-floor 27B denominator. W3-A's +0.816% component strict-fails. W3-B pre-serve all-bucket code is immutable build/correctness/safety-green at clean `d7cdf66`: 80/80 profiles, zero lazy misses, 16/16 correctness and zero memcheck errors; repeated selection/component/trace/oracle performance remains pending, so this does not change support or performance state ([small-M spike](specs/nvfp4-small-m-dispatch.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
