...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | all three native CUDA slices retain their support/correctness closure on GB10; exact production-serving performance remains separately active under `KERNEL-GEMM-NVFP4-W4A4` and `SERVE-GATE-ONLINE`. W2 ports all 32 SM12 tactics, merged CT gate/up max-divisor/one-alpha processing and the traced one-input SiLU→NVFP4 producer. CPU/CUDA/sanitizer/native-27B gates pass; the fused component is +1.521% but misses strict timing/memory axes, and clean pushed-SHA vLLM closure remains ([small-M spike](specs/nvfp4-small-m-dispatch.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
