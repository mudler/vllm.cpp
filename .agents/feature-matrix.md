...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | support/correctness stays closed; performance remains `ACTIVE` at `3f256ab` **55/124**. W3-C runtime/cache gates, six-process stability and the corrected 12-leg component use one byte-identical 64/64 native map with zero tuning/misses and fixed 27B gates. C3R proves direct/fallback 6/6 x 128-token equality under identical sequential batch shape and shows the earlier independent-run 2/6 condition was an invalid default-mode batch-invariance predicate. The corrected W3-E component gains 1.004483×/1.005044× c2/c16 mean throughput but strict-fails at **39/40 timing + 1/8 memory**; stopped partial performance stays void. Exact grid and 35B performance remain open ([persistent-cache spike](specs/nvfp4-persistent-plan-cache.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
