...
| Feature | Upstream | Status | Notes | Spec |
|---|---|---|---|---|
| safetensors container + model-specific loading | `model_loader/default_loader.py`, `models/utils.py` | `PARTIAL` T0 | Qwen-specific resolvers work. Windowed release removes source-copy coexistence; additive `LOAD-SAFETENSORS-DIRECT-DENSE` implements layer-bounded same-queue staging/host release for plain-BF16 Qwen3.5 discrete CUDA. Real 4B CPU/AOT CUDA gates pass and corrected ON/OFF/vLLM are **6155.10/6064.06/6730.46 tok/s**, ON=OFF 128/128, peak PSS **2.405/8.571/7.569 GiB**. The 0.9316x previous-AOT regression is profile-attributed: sync GPU time **23.307 vs 21.960 s**, dominated by missing H=32 packed Triton AOT (current hand 234.2 us/call vs previous Triton 47.7); async is secondary. Next is a current-ABI H=32 specialization with token-exact A/B. Generic mapping, quantized/MoE direct streaming, current-v0.25 oracle/sanitizer, strict VRAM and external 27B/35B remain open | [plain-BF16 direct-load spike](specs/qwen35-plain-bf16-direct-load.md); [profile](../docs/bench-evidence/qwen35-4b-aot-regression-profile-20260721.md) |
