...
| Chunked-local attention wrapper/backend | `model_executor/layers/attention/chunked_local_attention.py` | `ACTIVE` T1 | W4 virtual-batch metadata, block-table transforms and underlying-backend dispatch are claimed by `CLAIM-C5-CHUNKED-ATTN-1`; W3 chunked-local KV is implemented/`GATING` | [sliding-local-yarn-long-context.md](specs/sliding-local-yarn-long-context.md) |
