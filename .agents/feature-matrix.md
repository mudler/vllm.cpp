...
|---|---|---|---|---|---|
| `MODEL-GATE-QWEN35` | Qwen3.5/3.6 dense + MoE wrappers | `models/registry.py:556-560` | `PARTIAL` | 27B/35B **text submodels** pass token/perf gates; local plain-BF16 4B now casts its F32 checkpoint GDN norm to model BF16 and is stable 6/6 on the natural corpus, but remains diagnostic at 15/16 versus vLLM and exact-1024 stress remains variable; both upstream IDs are multimodal wrappers and their ViT/merger paths are absent | model matrix + existing Qwen specs |
