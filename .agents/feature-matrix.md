...
| FA2 paged GQA pure-decode split-KV | `v1/attention/backends/flash_attn.py`; dependency `flash_api.cpp` | `ACTIVE` T0 | Exact 27B ratio-6 BF16/D256 adapter, swap strides, split heuristic, queue-owned graph-stable scratch, model/dispatcher toggle and ported tests pass immutable sm_121a operator/memcheck/model/trace gates at `ae9e8ff`. Default/fallback switch **240 main+combine / 0 old** versus **0 combine / 240 old**. Short-prompt kernel time is performance-negative and non-binding; c2/c16 every-axis A/B is active under one complete-series lock, with both model arms passed and no partial result accepted. Ratio-8 and other modes intentionally fall back | [FA2 decode spike](specs/fa2-gqa-split-kv-decode.md) |
...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | support/correctness stays closed; performance remains `ACTIVE` at `3f256ab` **55/124**. W3-C reproduction control is complete and W3-E strict-fails at **39/40 timing + 1/8 memory**. W3-F strict-fails at **27/40 timing + 3/8 memory**. W3-G immutable `ae9e8ff` passes CUDA/memcheck/model/trace with both arms retaining identical **3,536 FP4 GEMMs**, **3,536 producers**, eight kernel names and 64 plans; therefore it changes no quant behavior or support state. Its attention component is active with no partial result accepted, so no speed credit, exact grid or 35B performance claim follows. Vectorized normal FP4 production remains the second, unstacked lever ([persistent-cache spike](specs/nvfp4-persistent-plan-cache.md), [device-alpha spike](specs/nvfp4-device-alpha.md), [FA2 decode spike](specs/fa2-gqa-split-kv-decode.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
