...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | support/correctness stays closed; performance remains `ACTIVE` at `3f256ab` **55/124**. W3-C reproduction control is complete; W3-E/W3-F/W3-G strict-fail and earn no speed credit. The complete [W3-H normal-producer spike](specs/nvfp4-bf16-producer-vectorization.md) is **ACTIVE / trace-first**: H1a/H1b/H1c and clean attempts through `e1acb75` are `VOID`. The latest exact build and 27B gate pass, then profiling stops before workload/report on a repaired flat-process-group invariant. H1d now preflights exact build provenance and the actual Nsight launcher/target-session ancestry with owned cleanup before enforcing lossless plan/workload/graph validation. Implementation remains prohibited and quant support state is unchanged ([persistent-cache spike](specs/nvfp4-persistent-plan-cache.md), [device-alpha spike](specs/nvfp4-device-alpha.md), [FA2 decode spike](specs/fa2-gqa-split-kv-decode.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
