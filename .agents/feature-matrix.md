...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | support/correctness stays closed; performance remains `ACTIVE` at `3f256ab` **55/124**. [W3-I](specs/nvfp4-fused-silu-producer.md) stays default-off. Finalized `179a0fc` proves all 12 local and 1,522 steady oracle B=2 windows use 128 Stream-K + 80 static-persistent FP4 calls; diagnostic FP4 time is already non-positive. Status `9e0143fa…7b57` is `complete-diagnostic`; no support or binding-performance state changes | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
