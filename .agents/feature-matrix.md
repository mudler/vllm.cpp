...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | all three native CUDA slices retain support/correctness closure on GB10; performance remains separately `ACTIVE`. W2's 32 SM12 tactics, merged CT scaling and fused SiLU→NVFP4 path pass focused/native gates. Clean `b5c6e4f` improves the exact 27B ladder to 0.9933/0.9520/0.9657/0.9760/1.0213/1.0218×, but trace-grounded runtime selection and every-axis parity remain open in W3 ([small-M spike](specs/nvfp4-small-m-dispatch.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
