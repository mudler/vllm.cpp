...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | support/correctness stays closed; performance remains `ACTIVE` at `3f256ab` **55/124**. W3-C reproduction control is complete; W3-E and W3-F strict-fail. W3-G immutable `ae9e8ff` passes CUDA/memcheck/model/trace with both arms retaining identical **3,536 FP4 GEMMs**, **3,536 producers**, eight kernel names and 64 plans, so it changes no quant behavior or support state. Its attention component reaches **1.017668×/1.006548×** mean total throughput but strict-fails **35/40 timing + 5/8 memory** and earns no speed credit. The complete [W3-H normal-producer spike](specs/nvfp4-bf16-producer-vectorization.md) is **ACTIVE / trace-first**: H1a/H1b are `VOID`; immutable `d1f8e33` H1c is also `VOID` after capture 1/3 emitted possible CUDA-event loss and stopped before r2/r3/vLLM, with no parseable frozen-plan lifecycle. Retained normal-producer timing remains diagnostic only. H1d lossless collection plus semantic plan/workload/graph validation is pending, and implementation remains prohibited ([persistent-cache spike](specs/nvfp4-persistent-plan-cache.md), [device-alpha spike](specs/nvfp4-device-alpha.md), [FA2 decode spike](specs/fa2-gqa-split-kv-decode.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
