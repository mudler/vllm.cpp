...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | support/correctness stays closed; performance remains `ACTIVE` at `3f256ab` **55/124**. W3-C1 implements/gates the CUDA-free native/imported plan document, stale/collision validation, merge, atomic replace and frozen/path options: Release/ASan+UBSan/TSan **6/6 + 174/174**, exact 64/64 oracle import, full CPU **103/103**. It is not wired to quant dispatch, so no speed/support expansion or new benchmark exists. C2/runtime/same-plan/35B performance remain pending ([persistent-cache spike](specs/nvfp4-persistent-plan-cache.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
