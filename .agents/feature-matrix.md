...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | all three native CUDA slices retain support/correctness closure on GB10; performance remains separately `ACTIVE`. W2's 32 SM12 tactics, merged CT scaling and fused SiLU→NVFP4 path pass focused/native gates. Clean `b5c6e4f` remains the below-floor 27B denominator. W3-B pre-serve all-bucket code is immutable build/correctness/safety-green at clean `d7cdf66`; its component strict-fails but first-use improves. Corrected trace attempt 2 is lifecycle-clean at **432/432** retained requests and records prewarm/lazy/vLLM FP4 sums **110.623/114.229/109.932 s** with the narrow pair **70.333/70.986 s** prewarm/vLLM. This closes the original tactic-family mismatch, but profiled normalized mean TPOT remains **0.9673x** and no trace rate binds. Exact W3-B c1-c32 performance remains ([small-M spike](specs/nvfp4-small-m-dispatch.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
