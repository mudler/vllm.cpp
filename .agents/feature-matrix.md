...
| Device-resident attention KV + indexed GDN state I/O | `v1/worker/gpu/attn_utils.py`, `model_executor/layers/mamba/gdn/qwen_gdn_linear_attn.py` | `ACTIVE` T0 | W0 replaces CUDA host-vector cache ownership with stable backend allocations and retains `VT_DEVICE_KV_CACHE=0`; CPU focused gates pass, CUDA pointer/model/lifecycle/A-B gates are open. W1-W2 eliminate indexed mixed-prefill row copies | [device-resident-kv-gdn-state.md](specs/device-resident-kv-gdn-state.md) |
