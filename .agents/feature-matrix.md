...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | all three native CUDA slices retain support/correctness closure on GB10; performance remains separately `ACTIVE`. Immutable `9cc7191` completed the v0.25.0 27B cache-off grid: all 124 axes bind, **54 pass / 70 fail**. Its first ours trace used whole-graph granularity and omitted child kernels, so node-level paired recapture precedes any topology selection. No support closure is revoked and 35B remains held ([small-M spike](specs/nvfp4-small-m-dispatch.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
