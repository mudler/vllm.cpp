...
| **Async/overlap scheduling** | `v1/core/sched/async_scheduler.py` | ☐ T1 (**promoted**), spike accepted | vLLM's default-ON depth-2 path remains an unmet mirror obligation. Explicit modes and direct-script bootstrap are CPU-tested; `9b1774c` is VOID after six timing legs and ON capture because H1d exact-count validation stopped before OFF, with provisional total credit only **+0.05%**. Shape-neutral aggregation is verified and a fresh complete control is `PENDING`; `ENG-ASYNC-SCHED` remains `READY`, unowned, and uncredited | [async-serving.md](specs/async-serving.md) |
...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | support/correctness stays closed; performance remains `ACTIVE` at `3f256ab` **55/124**. Clean [W3-I](specs/nvfp4-fused-silu-producer.md) `15c6b89` is structurally green but its complete component fails **27/40 timing + 3/8 memory**, with c2/c16 total ratios **1.002457× / 0.999771×**. Candidate stays default-off; quant support and binding performance are unchanged | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
