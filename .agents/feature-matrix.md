...
| AsyncLLM-equivalent streaming engine API | `v1/engine/async_llm.py` | 🚧 **T0 `ACTIVE`** | W2 implements per-request coalescing collectors, output handler, concurrent generate/abort, live completion/chat SSE and disconnect abort; CPU/TSan gates pass. Exact `a531e05` GB10 evidence confirms c32 HTTP delivery under-capacity: two repetitions strand one accepted unread socket for 205–207 s while 31 streams progress. `CLAIM-SERVE-HTTP-POOL-1` owns the capacity/test repair before the remaining token/performance/memory gates | [async-serving.md](specs/async-serving.md) |
