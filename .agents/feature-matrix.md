...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | support/correctness stays closed; performance remains `ACTIVE` at `3f256ab` **55/124**. W3-C reproduction control is complete and W3-E strict-fails at **39/40 timing + 1/8 memory**. The next verified execution mismatch is W3-F model-owned device alpha: ours launches 208 scalar stages/forward, vLLM zero. Its spike is `READY`; implementation, ported tests, safety, node trace and frozen-plan A/B remain pending. Exact grid and 35B performance remain open ([persistent-cache spike](specs/nvfp4-persistent-plan-cache.md), [device-alpha spike](specs/nvfp4-device-alpha.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
