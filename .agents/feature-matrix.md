...
| Device-resident attention KV + indexed GDN state I/O | `v1/worker/gpu/attn_utils.py`, `model_executor/layers/mamba/gdn/qwen_gdn_linear_attn.py` | `ACTIVE` T0 | W0 stable allocations/fallback pass CPU/CI, clean sm_121a, default/fallback 35B+27B pointer/token, lifecycle, trace and repeated c16/48 A/B at 1.021239×/20-of-20. W1 persistent metadata + indexed BF16↔F32 gather/scatter passes focused CPU/sanitizer/CUDA/model/turnover gates and a separate 1.006246×/20-of-20 A/B; trace collapses async copies 163,540→7,508 and D2D calls 142,717→1,231. Exact `a531e05` oracle evidence remains below floor, but direct-c16 retuning and c32 socket tracing localize higher-priority FP4/HTTP defects; W2 stays scoped pending the repaired residual. Inherited pools still fail zero-leak | [device-resident-kv-gdn-state.md](specs/device-resident-kv-gdn-state.md) |
...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | all three native CUDA slices retain their support/correctness closure on GB10; current exact production-serving performance is separately reopened under `KERNEL-GEMM-NVFP4-W4A4` and `SERVE-GATE-ONLINE` (accepted [small-M spike](specs/nvfp4-small-m-dispatch.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
...
| AsyncLLM-equivalent streaming engine API | `v1/engine/async_llm.py` | 🚧 **T0 `ACTIVE`** | W2 live SSE/concurrent abort behavior remains CPU/TSan-green. Its confirmed c32 HTTP defect now has fixed `max_num_seqs + 4` workers, a same-binary legacy toggle, and a 32-persistent-client/control-path regression stable 100× plus ASan+UBSan/TSan. Exact fixed-vs-legacy c32/full-ladder GPU evidence remains before `DONE` | [async-serving.md](specs/async-serving.md) |
