...
| Device-resident attention KV + indexed GDN state I/O | `v1/worker/gpu/attn_utils.py`, `model_executor/layers/mamba/gdn/qwen_gdn_linear_attn.py` | `ACTIVE` T0 | W0 replaces CUDA host-vector cache ownership with stable backend allocations and retains `VT_DEVICE_KV_CACHE=0`; CPU/CI, clean sm_121a build, and default/fallback 35B+27B pointer/token gates pass. Exact pre-W0 trace records 20.809/20.806 GiB H2D/D2H state rows; sanitizer/lifecycle/trace/memory/A-B gates are open. W1-W2 collapse the remaining indexed row loop | [device-resident-kv-gdn-state.md](specs/device-resident-kv-gdn-state.md) |
