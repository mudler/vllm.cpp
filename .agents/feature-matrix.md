...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | support/correctness stays closed; performance remains `ACTIVE` at `3f256ab` **55/124**. [W3-I](specs/nvfp4-fused-silu-producer.md) stays default-off. Oracle and the void first local c2 session both resolve 128 Stream-K + 80 static-persistent FP4 calls/window; the local run stopped after one four-range session on a stale c16 validator assertion. Batch-aware full retry/final status remains pending; no support or performance state changes | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
