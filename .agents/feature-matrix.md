...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | support/correctness stays closed; performance remains `ACTIVE` at `3f256ab` **55/124**. W3-C reproduction control is complete and W3-E strict-fails at **39/40 timing + 1/8 memory**. W3-F strict-fails at **27/40 timing + 3/8 memory**. W3-G immutable `ae9e8ff` passes CUDA/memcheck/model/trace with both arms retaining identical **3,536 FP4 GEMMs**, **3,536 producers**, eight kernel names and 64 plans; therefore it changes no quant behavior or support state. Its attention component is pending, so no speed credit, exact grid or 35B performance claim follows. Vectorized normal FP4 production remains the second, unstacked lever ([persistent-cache spike](specs/nvfp4-persistent-plan-cache.md), [device-alpha spike](specs/nvfp4-device-alpha.md), [FA2 decode spike](specs/fa2-gqa-split-kv-decode.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
