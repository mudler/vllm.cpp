...
| GDN packed pure decode | `envs.py::VLLM_ENABLE_FLA_PACKED_RECURRENT_DECODE`, `fla/ops/fused_recurrent.py` | `ACTIVE` T0 | New execution row `KERNEL-GDN-PACKED-DECODE` mirrors vLLM's default pure non-spec branch across FP16/BF16/F32. Clean `f18ca23` regenerates the official fixture byte-for-byte and passes CUDA **10/10**, proving the BF16 mismatch requires both F32 in-kernel q/k normalization and dtype-rounded beta: current `306/7552`, beta-only `308/6558`, full semantics `0/1`. Portable/CUDA implementation, 235/235 and c2/c16 remain open | [gdn-packed-decode.md](specs/gdn-packed-decode.md) |
...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | support/correctness stays closed; performance remains `ACTIVE` at `3f256ab` **55/124**. Finalized `179a0fc` proves the FP4 tactic family matches. The separate merged-GDN W1C has exact BA projection/inertness but BF16 27B remains 233/235; [packed-decode](specs/gdn-packed-decode.md) closes immutable semantic G0 and is the active non-quantized implementation dependency. Component performance remains open, with no quantization speed credit | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
