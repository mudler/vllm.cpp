...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | support/correctness stays closed; performance remains `ACTIVE` at `3f256ab` **55/124**. W3-C reproduction control is complete; W3-E/W3-F/W3-G strict-fail and earn no speed credit. The complete [W3-H normal-producer spike](specs/nvfp4-bf16-producer-vectorization.md) is **ACTIVE / trace-first**: H1a/H1b/H1c and H1d attempts through `b2c940c` are `VOID`. The latest exact build/gate, clients, frozen plans, FIFO removal and zero-exit profiler pass; four reports each contain one complete 1,107-kernel replay and zero eager CUDA rows, but all emit severity-2 possible loss and schema v3 expects one combined report. Schema v4 now implements and CPU-gates three sessions x four indexed reports plus synchronous generation/device flush while zero-eager/lossless validation remains strict; fresh DGX evidence is pending. W3-H2 inference implementation remains prohibited and quant support state is unchanged ([persistent-cache spike](specs/nvfp4-persistent-plan-cache.md), [device-alpha spike](specs/nvfp4-device-alpha.md), [FA2 decode spike](specs/fa2-gqa-split-kv-decode.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
