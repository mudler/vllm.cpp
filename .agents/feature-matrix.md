...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | all three native CUDA slices retain support/correctness closure on GB10; performance remains separately `ACTIVE`. W2's 32 SM12 tactics, merged CT scaling and fused SiLU→NVFP4 path pass focused/native gates. Clean `b5c6e4f` remains the below-floor 27B denominator. W3-A's +0.816% component strict-fails. W3-B pre-serve all-bucket code is now staged in the shared loader and disposable-GPU green at 80/80 profiles, zero lazy misses and 16/16 correctness; immutable safety/selection/component/trace/oracle gates remain pending, so this does not change support or performance state ([small-M spike](specs/nvfp4-small-m-dispatch.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
