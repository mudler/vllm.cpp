...
| GDN packed pure decode | `envs.py::VLLM_ENABLE_FLA_PACKED_RECURRENT_DECODE`, `fla/ops/fused_recurrent.py` | `ACTIVE` T0 | `KERNEL-GDN-PACKED-DECODE` mirrors vLLM's default pure non-spec branch across FP16/BF16/F32. Clean `f344dec` closes correctness and `7ff713e` + `24cea4f` close structure. The CPU-green **44/44** component runner's first clean attempt (`593996d`) built **154/154** then failed before GPU ownership because required `--profile-control off` was missing. Repair/test plus the fresh-SHA **40+8** run remain open with no speed credit | [gdn-packed-decode.md](specs/gdn-packed-decode.md) |
