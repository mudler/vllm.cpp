...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | all three native CUDA slices retain support/correctness closure on GB10; performance remains separately `ACTIVE`. Immutable `3f256ab` binds at **55/124 axes**. W3-E removes all **624** hot activation swizzles, but its c2/c16 component strict-fails **32/40 timing + 6/8 memory**. The binding v0.25 oracle loads/hits 64 persistent FP4 plans while ours retunes and matches only 18--33/64 paired IDs. W3-C is spiked/`READY` for JSON persistence, exact FlashInfer import, frozen read-only benchmarking, stale rejection and 5,000-us miss timing; no code, speed credit or new benchmark exists yet. Support/correctness stays closed while 69 binding performance axes and 35B performance remain held ([small-M spike](specs/nvfp4-small-m-dispatch.md), [direct-scale spike](specs/nvfp4-direct-swizzled-scales.md), [persistent-cache spike](specs/nvfp4-persistent-plan-cache.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
