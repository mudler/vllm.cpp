...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | support/correctness stays closed; performance remains `ACTIVE` at `3f256ab` **55/124**. W3-C reproduction control is complete and W3-E strict-fails at **39/40 timing + 1/8 memory**. W3-F immutable `7517af4` passes CUDA/memcheck/model and zero-versus-3,536-stage trace gates with identical FP4 topology/plans, but its completed c2/c16 component is only **1.001967×/1.000144×** and strict-fails at **27/40 timing + 3/8 memory**. No speed credit/exact grid follows. The completed scan moves the next component to W3-G FA2 ratio-6 decode and ranks vectorized normal FP4 production second; no new quant behavior or 35B performance claim follows ([persistent-cache spike](specs/nvfp4-persistent-plan-cache.md), [device-alpha spike](specs/nvfp4-device-alpha.md), [FA2 decode spike](specs/fa2-gqa-split-kv-decode.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
