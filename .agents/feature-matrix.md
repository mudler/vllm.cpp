...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | support/correctness stays closed; performance remains `ACTIVE` at `3f256ab` **55/124**. W3-C reproduction control is complete and W3-E strict-fails at **39/40 timing + 1/8 memory**. W3-F model-owned device alpha addresses the verified 208-local-versus-zero-vLLM scalar-stage mismatch. Immutable `7517af4` passes focused CUDA 33/33, strict no-pool memcheck 22/22 + 26,884/26,884 with zero errors/leaks, both 27B 235/235 + 16/16 arms, 35B correctness-only 315/315 and paired zero-versus-3,536-stage node traces with identical FP4 topology/plans. Its strict frozen-plan c2/c16 A/B is active; partial legs do not bind. Exact grid and 35B performance remain open ([persistent-cache spike](specs/nvfp4-persistent-plan-cache.md), [device-alpha spike](specs/nvfp4-device-alpha.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
