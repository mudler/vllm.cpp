...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | all three native CUDA slices retain support/correctness closure on GB10; performance remains separately `ACTIVE`. Immutable `3f256ab` binds at **55/124 axes**. Its W3-D packed/split arms pass 235/235 + 16/16; the c16 component is **1.005049×** mean-positive but strict-fails at **14/20 timing + 2/4 memory**. Post-pack tracing closes **208.192 vs vLLM 208 FP4 GEMMs/forward**; `VT_FP4_MERGED_QKV=0` retains split. Generated code shows norm and FP4 quant remain separate. W3-E direct swizzled activation scales is `READY` from the local-only 320,099-launch/1.239-s standalone swizzle family; code/tests/A-B are pending. Support/correctness stays closed while 69 performance axes and 35B remain held ([small-M spike](specs/nvfp4-small-m-dispatch.md), [direct-scale spike](specs/nvfp4-direct-swizzled-scales.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
