...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | support/correctness stays closed; performance remains `ACTIVE` at `3f256ab` **55/124**. W3-C reproduction control passes; W3-E/W3-F/W3-G earn no speed credit. W3-H is **ACTIVE / trace-first**: schema-v5 `c498a413` completes all three local sessions, 12/12 lossless reports and the paired vLLM trace, but is VOID pending corrected final-status revalidation and residual ranking. W3-H2 remains prohibited and quant support is unchanged | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
