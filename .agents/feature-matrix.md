...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | support/correctness stays closed; performance remains `ACTIVE` at `3f256ab` **55/124**. W3-C runtime/cache gates and six-process stability pass: immutable `d211b8f` loads one byte-identical 64/64 native map with zero tuning/misses and passes both 27B arms. The first same-plan c2 direct/fallback pair fails long-output equality at **2/6**, so C3 stops and all partial performance is void; fixed-plan localization, exact grid and 35B performance remain open ([persistent-cache spike](specs/nvfp4-persistent-plan-cache.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
