...
| `stream_options` / `include_usage` | `engine/protocol.py`, completion/chat `protocol.py` + `serving.py`, `serve/utils/api_utils.py` | 🚧 **SPIKE** T1 | complete final/continuous/force-mode contract accepted for `SERVE-STREAM-USAGE`; `8289cbd` online c32 surfaced the native-ID requirement when one 128-frame response re-tokenized to 126 without the requested usage frame | [stream-options.md](specs/stream-options.md) |
