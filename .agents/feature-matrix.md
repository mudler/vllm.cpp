...
| **Async/overlap scheduling** | `v1/core/sched/async_scheduler.py` | ☐ T1 (**promoted**), spike accepted | vLLM's default-ON depth-2 path is the only unmeasured structural candidate large enough for the c2 decode gap. Explicit oracle `on`/`off` profiling is implemented and CPU-tested; GPU timing/trace is `PENDING`. `ENG-ASYNC-SCHED` remains `READY`, unowned, and uncredited until that control completes | [async-serving.md](specs/async-serving.md) |
