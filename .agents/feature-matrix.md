...
| Device-resident attention KV + indexed GDN state I/O | `v1/worker/gpu/attn_utils.py`, `model_executor/layers/mamba/gdn/qwen_gdn_linear_attn.py` | `ACTIVE` T0 | W0 stable allocations/fallback pass CPU/CI, clean sm_121a, default/fallback 35B+27B pointer/token, lifecycle, trace and repeated c16/48 A/B gates: 1.021239× and 20/20 axes. State rows are D2D but 160,232 copies remain. Both model paths are sanitizer access-clean; inherited 47.29 MB/36.82 GB process-lifetime pools fail zero-leak, with W0 caches absent. W1-W2 collapse the row loop, then the oracle gate reruns | [device-resident-kv-gdn-state.md](specs/device-resident-kv-gdn-state.md) |
