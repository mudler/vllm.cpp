...
| **Async/overlap scheduling** | `v1/core/sched/async_scheduler.py` | ☐ T1 (**promoted**), spike accepted | vLLM's default-ON depth-2 path remains an unmet mirror obligation. Explicit modes and direct-script bootstrap are CPU-tested; `b8681ac` is VOID after six timing legs because both traces are absent, with provisional total credit only **+0.13%**. A fresh complete control is `PENDING`; `ENG-ASYNC-SCHED` remains `READY`, unowned, and uncredited | [async-serving.md](specs/async-serving.md) |
