...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | all three native CUDA slices retain their support/correctness closure on GB10; current exact production-serving performance is separately reopened under active `KERNEL-GEMM-NVFP4-W4A4` W1 and `SERVE-GATE-ONLINE`. W1 exact buckets/key/single-flight passes pushed-`c8807b0` exact/legacy capture, focused memcheck and both 27B model arms; component/trace/oracle performance remains ([small-M spike](specs/nvfp4-small-m-dispatch.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
