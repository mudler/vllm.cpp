...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | all three native CUDA slices retain support/correctness closure on GB10; performance remains separately `ACTIVE`. Immutable `9cc7191` binds at **54/124 axes** and `def5f75` selected packed QKV. W3-D staging now passes exact packed/logical output, packed-view CUDA, zero-error sanitizer and both 16/16 model arms; `VT_FP4_MERGED_QKV=0` retains the split path. No speed credit is inferred; clean A/B is next and 35B remains held ([small-M spike](specs/nvfp4-small-m-dispatch.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
