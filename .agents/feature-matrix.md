...
| GDN packed pure decode | `envs.py::VLLM_ENABLE_FLA_PACKED_RECURRENT_DECODE`, `fla/ops/fused_recurrent.py` | `ACTIVE` T0 | `KERNEL-GDN-PACKED-DECODE` mirrors vLLM's default pure non-spec branch across FP16/BF16/F32. Clean `f18ca23` closes semantic G0 at `0/1`. W1D1 public CPU/CUDA op, upstream 3-dtype × 2-stride matrix, local slot ABI, capture/canaries and strict memcheck are implemented and mutable-green; clean pushed-SHA G1, W1D2 model dispatch/235/235 and c2/c16 remain open | [gdn-packed-decode.md](specs/gdn-packed-decode.md) |
