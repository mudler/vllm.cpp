...
| SlidingWindowSpec + ChunkedLocalAttentionSpec | `v1/kv_cache_interface.py` | `READY` T1 | accepted joint spike; claim `KV-SLIDING-WINDOW-SPEC` and `KV-CHUNKED-LOCAL-SPEC` separately | [sliding-local-yarn-long-context.md](specs/sliding-local-yarn-long-context.md) |
...
| Feature | Upstream | Status | Notes | Spec |
|---|---|---|---|---|
| YaRN rope scaling | `layers/rotary_embedding/` | `READY` T1 | includes plain YaRN and its `mrope_section` branch; audited Qwen3.6 gates use **default interleaved MRoPE, not YaRN**, so they are regression-only | [sliding-local-yarn-long-context.md](specs/sliding-local-yarn-long-context.md) |
| llama3 / longrope / dynamic-NTK scaling | same | `READY` T1–T2 | umbrella split into three independently claimable engine leaves; direct upstream numerical tests are sparse, so the accepted spec requires pinned-vLLM oracle goldens per formula/boundary | [sliding-local-yarn-long-context.md](specs/sliding-local-yarn-long-context.md) |
| Sliding-window attention backend | `v1/attention/backends/` | `READY` T1 | pairs with the sliding-window KV leaf; sm_121 default dispatch resolves pinned FA2, with FlashInfer fallback coverage and nsys verification | [sliding-local-yarn-long-context.md](specs/sliding-local-yarn-long-context.md) |
| Chunked-local attention wrapper/backend | `model_executor/layers/attention/chunked_local_attention.py` | `READY` T1 | virtual-batch metadata over the selected ordinary backend; pairs with the chunked-local KV leaf | [sliding-local-yarn-long-context.md](specs/sliding-local-yarn-long-context.md) |
