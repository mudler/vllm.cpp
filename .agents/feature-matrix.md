...
| llama3 / longrope / dynamic-NTK scaling | same | `ACTIVE` T1–T2 | umbrella split into three independently claimable engine leaves. W6 `ATTN-ROPE-LLAMA3` is claimed by `CLAIM-C5-LLAMA3-1`; LongRoPE and dynamic-NTK remain `READY`. Direct upstream numerical tests are sparse, so the accepted spec requires pinned-vLLM oracle goldens per formula/boundary | [sliding-local-yarn-long-context.md](specs/sliding-local-yarn-long-context.md) |
