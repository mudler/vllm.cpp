...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | all three native CUDA slices retain support/correctness closure on GB10; performance remains separately `ACTIVE`. W2/W3 focused/native gates and the corrected old-oracle structural trace remain green. `b5c6e4f` is historical and `3cc490c` is VOID. Canonical vLLM 0.25.0/FlashInfer 0.6.13 is active; immutable `9cc7191` clean source, fresh sm_121a build, oracle fingerprint and exact c1-c32 corpora are prepared/hashed. Its locked 27B model gate/grid/trace remains unexecuted, so no ratio binds and no further topology repair is selected; 35B remains held ([small-M spike](specs/nvfp4-small-m-dispatch.md)) | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
