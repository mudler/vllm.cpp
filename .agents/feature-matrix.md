...
|---|---|---|---|---|
| `QUANT-CUDA-GATES` | NVFP4 W4A16, NVFP4 W4A4, gate-specific FP8 W8A8 | `DONE` | support/correctness stays closed; performance remains `ACTIVE` at `3f256ab` **55/124**. W3-C reproduction control passes; W3-E/W3-F/W3-G earn no speed credit. W3-H is **ACTIVE / trace-first**: schema-v5 `a7f67c7` closes one complete four-report group but is VOID on a repaired stop-marker log-forwarding race in session 2. A fresh immutable 12-report DGX run is pending. W3-H2 remains prohibited and quant support is unchanged | quant matrix §2 + [coverage spike](specs/quantization-coverage.md) |
