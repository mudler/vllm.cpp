...
| **Async/overlap scheduling** | `v1/core/sched/async_scheduler.py` | ☐ T1 (**promoted**), spike accepted | vLLM's default-ON depth-2 path is the only unmeasured structural candidate large enough for the c2 decode gap. Explicit modes are CPU-tested; first `4d85ead` GPU setup is VOID before requests on missing EngineCore `ninja`, and the fresh PATH-repaired control is `PENDING`. `ENG-ASYNC-SCHED` remains `READY`, unowned, and uncredited | [async-serving.md](specs/async-serving.md) |
