...
**STOP FINDING 2026-09-26 (the replay-port fix session):** the two increments are confirmed missing and correctly mirrored
(qwen3_5.cpp:12397/:12625), and the fix works mechanically -- replays serve (8/4 served), boundary resets stop, TPOT -2.2% --
but the FIRST SERVED REPLAY DESYNCS THE DEVICE (all subsequent tokens 0, deterministic across two legs; sha256 a79a66bc...).
The dense replay arm has NEVER served: the boundary bug masked it. The defect: the dense capture EXCLUDES the embedding
(region = DenseForwardLayers, qwen3_5.cpp:12526), so every replay step runs DenseEmbedInto EAGERLY while the trace is live
(fresh dids alloc + EmbeddingKernel device alloc + eager ttnn::embedding + shadow copy) -- the documented replay-corrupting
class (tenstorrent_paged.cpp:2361-2364, #2469). The repair: port qwen3.cpp's proven host-free replay pattern (embedding
INSIDE the captured region + persistent ids via WarmDecodeIds, qwen3.cpp:953-955) to the dense driver; the increments land on
top of it. The suite has zero DenseDecodeGraph coverage (the finding isolated by gate 4). Evidence: /tmp/fix-replay/ (red,
green, green2, suite logs, token JSONs); no commit -- the fix session stopped per its contract.
