...
`no contact 28m1s` when this record was written. A build-only job
(`DO_COALESCE=0 ARCH_LIST=12.1a`) stays queued on `dgx:gpu0` and writes to
`/mnt/nas_share/rc/exllamav3-gateability/logs/`, so the confirmation lands
whenever the box returns. It deliberately skips the 99 GiB TP1 coalesce, because
the build verdict already rules out the run that would consume it.
