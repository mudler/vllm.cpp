...
| Paged attention for full-attn layers on sm_121 (bf16, GQA 16/2, partial RoPE) — FlashInfer-class performance is the bar; strategy in §9 | ref: `v1/attention/backends/{flashinfer,triton_attn,flash_attn}.py` | T0 🚧 on GB10 gate workloads (CPU/CUDA parity + full 27B/35B paged-engine greedy correctness gates pass. Immutable `3f256ab` is the binding vLLM v0.25.0 27B cache-off result: model gate, 36/36 groups, 2,016 requests and six returns pass; all 124 axes bind, **55 pass / 69 fail**. Median total ratios c1→c32 are 0.993504/0.954464/0.966438/0.980678/1.027889/1.039417×, with 5/4/5/4/17/18 of 20 performance axes; memory is 2/4 with host PSS/RSS red. Node-level paired attribution completed; W3-E/W3-F/W3-G strict-failed, and W3-H's exact-target refresh remains pending after void attempts through FIFO/zero-exit `219f4f2`. That trace contains exact graph children but also three eager sampler/input gaps and severity-2 possible loss; four isolated `repeat:4` ranges plus schema-v3 four-start validation are implemented/CPU-gated. No attention speed credit or 35B performance run follows); broader attention/backend/SM coverage lives in `kernel-matrix.md` and remains open |
...
|---|---|---|
| **NVFP4 gate slices** — ModelOpt W4A16 experts (35B) + compressed-tensors W4A4 dense (27B) | `quantization/modelopt.py`, `compressed_tensors/` | **T0 ✅ correctness/support; performance W3 ACTIVE** — existing CUDA paths remain gated; `3f256ab` binds at **55/124**. W3-C installs exact plans before warmup and completes frozen-plan reproduction control. W3-E/W3-F/W3-G strict-fail and earn no credit. W3-H is **ACTIVE / trace-first**: H1a/H1b/H1c and H1d attempts through `219f4f2` are `VOID`; the latest exact build/gate, four launches, FIFO removal and zero-exit profiler pass, but one continuous range captures eager sampler/input work and emits severity-2 possible loss. H1d now implements four isolated `repeat:4` ranges and schema-v3 four-start proof, and still fails unless plan/build/ancestry, FIFO lifecycle/removal, zero exit, zero eager rows and lossless plan/workload/graph validation all pass. Fresh DGX evidence is pending; no exact grid/35B performance is authorized. See [W3-C spike](specs/nvfp4-persistent-plan-cache.md), [W3-F spike](specs/nvfp4-device-alpha.md), [W3-G spike](specs/fa2-gqa-split-kv-decode.md) and [W3-H spike](specs/nvfp4-bf16-producer-vectorization.md) |
