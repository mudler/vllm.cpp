...
| YaRN + long-context RoPE factory/cache/apply | `model_executor/layers/rotary_embedding/{__init__,base,yarn_scaling_rope,mrope}.py` | T1 🚧 W5 `ATTN-YARN` `GATING`: typed modern/legacy parameters, memoized f32/bf16 cache/factory, shared NeoX/GPT-J supplied-cache apply, plain + 1-D/3-axis contiguous/interleaved `mrope_section` branches, and six pinned-source CPU oracle fixtures pass with sanitizers; CUDA compile/runtime, positive-model e2e, trace and every-axis gates remain. Llama3 is `ACTIVE` under `CLAIM-C5-LLAMA3-1`; LongRoPE/dynamic formulas remain separate READY leaves |
