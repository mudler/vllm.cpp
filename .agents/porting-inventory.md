...
|---|---|---|
| **NVFP4 gate slices** — ModelOpt W4A16 experts (35B) + compressed-tensors W4A4 dense (27B) | `quantization/modelopt.py`, `compressed_tensors/` | **T0 ✅ correctness/support; performance W3 ACTIVE** — existing CUDA paths remain gated; `3f256ab` binds at **55/124** and W3-E strict-fails. W3-C installs exact plans before warmup and completes frozen-plan reproduction control. The next executed difference is W3-F device alpha: local steady trace has 208 scalar-stage launches/forward, while vLLM/FlashInfer passes one model-owned F32 device pointer and traces zero. Its API/residency/fallback/tests/safety/trace/A-B contract is `READY`; no runtime/result exists. No exact grid/35B performance follows. See [W3-C spike](specs/nvfp4-persistent-plan-cache.md) and [W3-F spike](specs/nvfp4-device-alpha.md) |
