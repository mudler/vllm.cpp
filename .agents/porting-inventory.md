...
|---|---|---|
| **NVFP4 gate slices** — ModelOpt W4A16 experts (35B) + compressed-tensors W4A4 dense (27B) | `quantization/modelopt.py`, `compressed_tensors/` | **T0 ✅ correctness/support; performance W3 ACTIVE** — existing CUDA paths remain gated; `3f256ab` binds at **55/124**. W3-C installs exact plans before warmup and completes frozen-plan reproduction control. W3-E/W3-F/W3-G strict-fail and earn no credit. W3-H is **ACTIVE / trace-first**: H1a/H1b/H1c are `VOID`; `5f8fab1` failed correctness on an invalid build path and `d063f20` failed pre-plan on corpus/plan ordering. H1d now fails before GPU work unless plan-first ordering, `RelWithDebInfo`, vendored Triton-AOT, FA2, exact CUTLASS and pinned tools all pass, then enforces lossless plan/workload/graph validation. No exact grid/35B performance is authorized. See [W3-C spike](specs/nvfp4-persistent-plan-cache.md), [W3-F spike](specs/nvfp4-device-alpha.md), [W3-G spike](specs/fa2-gqa-split-kv-decode.md) and [W3-H spike](specs/nvfp4-bf16-producer-vectorization.md) |
