...
|---|---|---|
| **NVFP4 gate slices** — ModelOpt W4A16 experts (35B) + compressed-tensors W4A4 dense (27B) | `quantization/modelopt.py`, `compressed_tensors/` | **T0 ✅ correctness/support; performance ACTIVE** — `3f256ab` binds at **55/124**. Finalized `179a0fc` proves the FP4 tactic family already matches. Selected `KERNEL-GEMM-BF16` W1C is `ACTIVE`: finalized `0091cd1` closes exact 963/145 versus 1,011/193 structure. W1C implements a default-off BF16-output arm, exact vLLM M=1/2/4/16/32 oracle, and native/legacy-35B plus GGUF zero-selection tests; oracle 3×, synthetic GGUF 97/97 and CPU parity 123/123 pass. Local CUDA/model/hardware gates, component evidence, qkvz, exact grid and 35B performance remain unauthorized; `benchmark_binding=false` |
