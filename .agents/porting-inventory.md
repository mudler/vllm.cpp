...
| YaRN + long-context RoPE factory/cache/apply | `model_executor/layers/rotary_embedding/{__init__,base,yarn_scaling_rope,mrope}.py` | T1 🚧 W5 `ATTN-YARN` `ACTIVE` under `CLAIM-C5-YARN-1`: typed YaRN parameters, shared supplied-cache apply seam, plain + `mrope_section` branches and pinned oracle goldens; Llama3/LongRoPE/dynamic formulas remain separate READY leaves |
