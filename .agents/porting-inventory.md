...
| GDN packed pure non-spec decode (default-on FP16/BF16/F32 path) | `layers/fla/ops/fused_recurrent.py:255-478`; Qwen dispatch `qwen_gdn_linear_attn.py:1286-1298,1644-1695` | T0 🚧 `KERNEL-GDN-PACKED-DECODE` `ACTIVE`: official oracle proves F32 in-kernel q/k normalization plus dtype-rounded beta, with mutable local replay `306/7552 -> 0/1`. Immutable replay, production op, 235/235, trace and c2/c16 remain pending |
...
|---|---|---|
| **NVFP4 gate slices** — ModelOpt W4A16 experts (35B) + compressed-tensors W4A4 dense (27B) | `quantization/modelopt.py`, `compressed_tensors/` | **T0 ✅ correctness/support; performance ACTIVE** — `3f256ab` binds at **55/124**. `179a0fc` proves FP4 tactics already match; `0091cd1` closes BA structure and `f925294` closes exact projection/inertness. BF16 27B remains 233/235. The first divergence is now the independently owned packed GDN decode path: full semantics reproduce at `0/1`, but immutable replay, implementation, component evidence, qkvz, exact grid and 35B performance remain unauthorized; `benchmark_binding=false` |
