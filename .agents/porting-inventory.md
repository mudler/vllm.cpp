...
| Paged attention for full-attn layers on sm_121 (bf16, GQA 16/2, partial RoPE) — FlashInfer-class performance is the bar; strategy in §9 | ref: `v1/attention/backends/{flashinfer,triton_attn,flash_attn}.py` | T0 🚧 on GB10 gate workloads (CPU/CUDA parity + full 27B/35B paged-engine greedy correctness gates pass. Exact pushed-`bce2627` 27B online evidence is valid but below the every-axis floor: median total ratios c1→c32 are 0.9680/0.9317/0.9403/0.9516/0.9944/1.0073×, with 4/4/5/4/4/12 of 20 performance and 2/4 memory axes. W0/W1 device-residency component A/Bs remain 1.021239×/1.006246× with structural copy reduction; inherited pools fail zero-leak. HTTP is healthy/neutral. FP4 W2 now ports the confirmed 32-tactic/merged-scaling/fused-quant chain and passes focused correctness/sanitizer gates; its component is positive but below strict timing/memory closure, so the clean pushed-SHA exact 27B rerun still precedes device-residency W2 re-ranking and any 35B run); broader attention/backend/SM coverage lives in `kernel-matrix.md` and remains open |
...
|---|---|---|
| **NVFP4 gate slices** — ModelOpt W4A16 experts (35B) + compressed-tensors W4A4 dense (27B) | `quantization/modelopt.py`, `compressed_tensors/` | **T0 ✅ correctness/support; performance W2 ACTIVE** — both native CUDA paths and full gate workloads pass on GB10. W2 now includes distinct hybrid buckets, complete key/single-flight/capture handling, all 32 SM12 static/Stream-K and swap-AB tactics, merged gate/up resident data with maximum logical-shard CT divisors/one alpha, and the traced fused one-input SiLU→NVFP4 producer. CPU/CUDA/sanitizer/dense/paged gates pass in shipping and fusion-fallback modes. The fused c16 component is +1.521% but only 17/20 timing and 0/4 sampled-memory axes pass; a +2.084%/20-of-20 paired trace owns structural launch/time attribution only. Pushed `bce2627` remains the below-floor vLLM denominator until the clean W2 SHA reruns the full c1-c32 campaign. The accepted [small-M dispatch spike](specs/nvfp4-small-m-dispatch.md) keeps iterations independent; generic quant-config/backend breadth remains in `quantization-matrix.md` |
