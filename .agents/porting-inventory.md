...
|---|---|---|
| **NVFP4 gate slices** — ModelOpt W4A16 experts (35B) + compressed-tensors W4A4 dense (27B) | `quantization/modelopt.py`, `compressed_tensors/` | **T0 ✅ correctness/support; performance W3 ACTIVE** — both native CUDA paths and full gate workloads pass on GB10. W2 includes distinct hybrid buckets, complete key/single-flight/capture handling, all 32 SM12 static/Stream-K and swap-AB tactics, merged gate/up resident data with maximum logical-shard CT divisors/one alpha, and fused one-input SiLU→NVFP4. CPU/CUDA/sanitizer/dense/paged gates pass. Clean `b5c6e4f` completes W2's binding classification at 0.9933/0.9520/0.9657/0.9760/1.0213/1.0218× total throughput with 4/4/5/4/17/14 timing and 2/4 memory axes. The paired trace proves the 128x32x256 family exists locally but our runtime selection differs from vLLM, so W3 pre-serve warmup/timing/persistence is active. The accepted [small-M dispatch spike](specs/nvfp4-small-m-dispatch.md) keeps iterations independent; generic quant-config/backend breadth remains in `quantization-matrix.md` |
