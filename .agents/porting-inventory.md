...
|---|---|---|
| **NVFP4 gate slices** — ModelOpt W4A16 experts (35B) + compressed-tensors W4A4 dense (27B) | `quantization/modelopt.py`, `compressed_tensors/` | **T0 ✅ correctness/support; performance W2 ACTIVE** — both native CUDA paths and full gate workloads pass on GB10. W1 distinct hybrid buckets, complete key, failure-safe single-flight and capture-miss rejection retain all Release/TSan/CUDA/memcheck/model gates. Its complete exact/legacy component series is positive at c8/c32 but fails strict c16/memory no-regression; pushed `bce2627` improves the binding oracle grid yet leaves c1-c16 throughput and latency axes open. The fresh trace confirms four local wide tactics omit the heavily used 128x32x256 Stream-K/static pair and the rest of the 32-tactic family now owned by W2. The accepted [small-M dispatch spike](specs/nvfp4-small-m-dispatch.md) keeps iterations independent; generic quant-config/backend breadth remains in `quantization-matrix.md` |
