...
| ✅ `75caf38` Specs: `FullAttentionSpec`, `MambaSpec` (GDN state), `KVCacheGroupSpec`, `KVCacheConfig` — `page_size_bytes` byte-exact vs upstream. `ChunkedLocalAttentionSpec`/`MLAAttentionSpec` remain deferred | `v1/kv_cache_interface.py` | T0 |
| Mamba/GDN prefix-cache retention (`mamba_cache_mode` align, PR #45845) | `v1/core/` | T1 |
| 🚧 `SlidingWindowSpec` + registry-backed `SlidingWindowManager` W1 implemented: exact unquantized page/admission math, right-to-left prefix/EAGLE/alignment/retention policy, whole-page recycling, no cascade and hybrid-disabled full-allocation conversion; CPU/property/sanitizer gates green, feature-positive attention/model/oracle/performance gates pending (`GATING`). `ChunkedLocalAttentionSpec` remains READY | `v1/kv_cache_interface.py`, `v1/kv_cache_spec_registry.py`, `v1/core/single_type_kv_cache_manager.py`, `v1/core/kv_cache_utils.py` | T1 |
...
| Sliding-window attention | `model_executor/layers/attention/attention.py`; `v1/attention/backends/flash_attn.py`; pinned FA2 local mask chain | T1 🚧 W2 `ACTIVE` (`CLAIM-C5-SW-ATTN-1`): generic semantic window + CPU/portable-CUDA/FA2 operator port; model/oracle/trace/perf gates pending |
