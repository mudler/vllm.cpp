...
| GDN packed pure non-spec decode (default-on FP16/BF16/F32 path) | `layers/fla/ops/fused_recurrent.py:255-478`; Qwen dispatch `qwen_gdn_linear_attn.py:1286-1298,1644-1695` | T0 🚧 `KERNEL-GDN-PACKED-DECODE` `ACTIVE`: W1D2 exact dispatch, rollback, independent state dtypes, metadata preflight and graph safety are implemented. Mutable default+rollback 27B **235/235**, 35B/GGUF inertness, CUDA GDN **43/43** and focused memcheck pass. Immutable G2, trace and c2/c16 remain pending; no speed credit |
...
|---|---|---|
| **NVFP4 gate slices** — ModelOpt W4A16 experts (35B) + compressed-tensors W4A4 dense (27B) | `quantization/modelopt.py`, `compressed_tensors/` | **T0 ✅ correctness/support; performance ACTIVE** — `3f256ab` binds at **55/124**. FP4 tactics and BA topology/projection are matched. W1D2 packed recurrence restores mutable default+rollback 27B **235/235** with 35B/GGUF inertness; immutable G2, component evidence, qkvz, exact grid and 35B performance remain unauthorized; `benchmark_binding=false` |
