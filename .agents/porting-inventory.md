...
|---|---|---|
| **NVFP4 gate slices** — ModelOpt W4A16 experts (35B) + compressed-tensors W4A4 dense (27B) | `quantization/modelopt.py`, `compressed_tensors/` | **T0 ✅ correctness/support; performance ACTIVE** — `3f256ab` binds at **55/124**. Finalized `179a0fc` proves the FP4 tactic family already matches at 128 Stream-K + 80 static-persistent calls. The selected residual is now accepted `KERNEL-GEMM-BF16`: **193 vs 97** GDN projection GEMMs, with merged BA first and qkvz second. Implementation, exact-grid and 35B performance remain unauthorized/pending |
