...
| Chunked-local attention wrapper | `model_executor/layers/attention/chunked_local_attention.py`; `v1/attention/backends/utils.py` | T1 🚧 W4 `ACTIVE` under `CLAIM-C5-CHUNKED-ATTN-1`: virtual-batch metadata, block-table transforms, spec emission and ordinary-backend dispatch; W3 KV bookkeeping is implemented/`GATING`, while Llama4/model and GPU gates remain separate |
