...
|---|---|---|
| **NVFP4 gate slices** — ModelOpt W4A16 experts (35B) + compressed-tensors W4A4 dense (27B) | `quantization/modelopt.py`, `compressed_tensors/` | **T0 ✅ correctness/support; performance W3 ACTIVE** — `3f256ab` binds at **55/124**. W3-C frozen-plan control passes; W3-E/W3-F/W3-G earn no credit. W3-H is trace-first: clean schema-v4 `b9beccd` passes build/gate/plans/clients/FIFO/zero-exit and four complete zero-eager reports, but strict validation rejects pinned Nsight's possible-loss capture-range diagnostic before sessions 2/3 or vLLM; the run is VOID. Calibrate it under exact event/completion/topology rules, then rerun all 12 reports. No exact grid or 35B performance is authorized. See [W3-H spike](specs/nvfp4-bf16-producer-vectorization.md) |
