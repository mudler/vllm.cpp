...
| YaRN + long-context RoPE factory/cache/apply | `model_executor/layers/rotary_embedding/{__init__,base,yarn_scaling_rope,mrope,llama3_rope,phi3_long_rope_scaled_rope}.py` | T1 🚧 W5 `ATTN-YARN` + W6 `ATTN-ROPE-LLAMA3` + W7 `ATTN-ROPE-LONGROPE` `GATING`: typed modern/legacy parameters, memoized f32/bf16 cache/factory, shared supplied-cache apply, YaRN/MRoPE, exact Llama 3 bands, and Phi-3 globally selected short/long factor caches pass twelve pinned-source CPU oracle fixtures with sanitizers. CUDA compile/runtime, positive-model e2e, trace and every-axis gates remain; dynamic-NTK is the remaining `READY` formula leaf |
