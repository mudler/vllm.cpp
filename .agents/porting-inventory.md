...
| GDN packed pure non-spec decode (default-on FP16/BF16/F32 path) | `layers/fla/ops/fused_recurrent.py:255-478`; Qwen dispatch `qwen_gdn_linear_attn.py:1286-1298,1644-1695` | T0 🚧 `KERNEL-GDN-PACKED-DECODE` `ACTIVE`: clean `f344dec` closes W1D2/G2 for exact dispatch, rollback and safety. Clean `4804ee4` validated eight packed ranges, then failed/voided on a shared-log marker-prefix race before the complete dual-arm trace. The prefix/cardinality/suffix parser plus 48-node coupled-BA isolation pass **82/82** tools. Fresh trace and c2/c16 remain pending with no speed credit |
...
|---|---|---|
| **NVFP4 gate slices** — ModelOpt W4A16 experts (35B) + compressed-tensors W4A4 dense (27B) | `quantization/modelopt.py`, `compressed_tensors/` | **T0 ✅ correctness/support; performance ACTIVE** — `3f256ab` binds at **55/124**. FP4 tactics and BA topology/projection are matched. Clean `f344dec` closes W1D2/G2 for default+rollback 27B **235/235** with 35B/GGUF inertness; component evidence, qkvz, exact grid and 35B performance remain unauthorized; `benchmark_binding=false` |
