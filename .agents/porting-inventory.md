...
| Paged attention for full-attn layers on sm_121 (bf16, GQA 16/2, partial RoPE) — FlashInfer-class performance is the bar; strategy in §9 | ref: `v1/attention/backends/{flashinfer,triton_attn,flash_attn}.py` | T0 🚧 on GB10 gate workloads (CPU/CUDA parity + full 27B/35B paged-engine greedy correctness gates pass. Immutable `9cc7191` is the binding vLLM v0.25.0 27B cache-off result: model gate, 36/36 groups, 2,016 requests, six returns and paired traces pass; all 124 axes bind, **54 pass / 70 fail**. Median total ratios c1→c32 are 0.9901/0.9491/0.9633/0.9770/1.0288/1.0467×, with 4/4/5/4/17/18 of 20 performance axes; memory is 2/4 with host PSS/RSS red. W0/W1 device-residency component A/Bs remain 1.021239×/1.006246× with structural copy reduction; inherited pools fail zero-leak. HTTP is healthy/neutral. The completed paired trace now drives the next concrete repair before any 35B run); broader attention/backend/SM coverage lives in `kernel-matrix.md` and remains open |
...
|---|---|---|
| **NVFP4 gate slices** — ModelOpt W4A16 experts (35B) + compressed-tensors W4A4 dense (27B) | `quantization/modelopt.py`, `compressed_tensors/` | **T0 ✅ correctness/support; performance W3 ACTIVE** — both native CUDA paths and full gate workloads pass on GB10. W2 includes distinct hybrid buckets, complete key/single-flight/capture handling, all 32 SM12 tactics, merged gate/up CT semantics, and fused one-input SiLU→NVFP4. W3-A delayed timing and W3-B pre-serve all-bucket warmup are correctness/safety-gated; W3-B's repeated component is **1.000293×** but strict-fails **15/20 timing + 2/4 memory**, keeps only **20/80** tactic IDs stable and therefore earns no speed credit. Its corrected old-oracle trace closes the original wide-tactic structural mismatch. Immutable `9cc7191` now supplies the binding v0.25.0/FlashInfer 0.6.13 27B grid/trace: **54/124 axes pass, 70 fail**, with c16/c32 total throughput green but low-concurrency decode and host PSS/RSS red. No topology repair is selected without diffing that executed trace; W3-C remains optional. The accepted [small-M dispatch spike](specs/nvfp4-small-m-dispatch.md) keeps iterations independent; generic quant-config/backend breadth remains in `quantization-matrix.md` |
