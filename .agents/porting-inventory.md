...
| Async scheduling (overlap schedule with execution) | `v1/core/sched/async_scheduler.py` | T1 `READY` (W3; vLLM default-ON mirror obligation, distinct from W2's asynchronous frontend). The post-W3-I scan selects an exact c2 oracle ON/OFF timing + trace before implementation; profiler control is CPU-green, GPU result `PENDING`, no speed credit |
...
|---|---|---|
| **NVFP4 gate slices** — ModelOpt W4A16 experts (35B) + compressed-tensors W4A4 dense (27B) | `quantization/modelopt.py`, `compressed_tensors/` | **T0 ✅ correctness/support; performance W3 ACTIVE** — `3f256ab` binds at **55/124**. W3-I remains default-off after its 30/48 component failure. The completed scan selects a c2 async-scheduler oracle control before any further FP4 implementation; exact grid and 35B performance remain unauthorized |
