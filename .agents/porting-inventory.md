...
| 🚧 `SlidingWindowSpec` + registry-backed `SlidingWindowManager` W1 implemented/`GATING`; `ChunkedLocalAttentionSpec` W3 is `ACTIVE` under `CLAIM-C5-CHUNKED-KV-1` for concrete sizing/grouping, manager prefix/recycling policy, admission cap, hybrid-disabled conversion and ported CPU properties | `v1/kv_cache_interface.py`, `v1/kv_cache_spec_registry.py`, `v1/core/single_type_kv_cache_manager.py`, `v1/core/kv_cache_utils.py` | T1 |
