...
|---|---|---|
| **NVFP4 gate slices** — ModelOpt W4A16 experts (35B) + compressed-tensors W4A4 dense (27B) | `quantization/modelopt.py`, `compressed_tensors/` | **T0 ✅ correctness/support; performance W3 ACTIVE** — `3f256ab` binds at **55/124**. W3-I remains default-off and async is neutral. Complete raw `179a0fc` proves all 12 local and 1,522 steady oracle B=2 windows resolve 208 GEMMs as 128 Stream-K + 80 static-persistent calls; diagnostic FP4 time is non-positive. Durable final status is pending, so no lever, exact grid or 35B performance is authorized |
