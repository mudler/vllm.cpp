...
|---|---|---|
| **NVFP4 gate slices** — ModelOpt W4A16 experts (35B) + compressed-tensors W4A4 dense (27B) | `quantization/modelopt.py`, `compressed_tensors/` | **T0 ✅ correctness/support; performance W3 ACTIVE** — existing CUDA paths remain gated; `3f256ab` binds at **55/124** and W3-E strict-fails. W3-C installs exact plans before warmup and completes frozen-plan reproduction control. W3-F targets 208 local scalar stages/forward versus zero in vLLM/FlashInfer and implements the device-alpha API/validation, direct CUTLASS pointer, individual/merged model residency, exact fallback and ported invalid/equality/capture/forced-tactic/packed-QKV tests. Immutable `7517af4` passes focused CUDA 33/33, strict no-pool memcheck 22/22 + 26,884/26,884, both 27B arms, 35B correctness-only inertness and paired zero-versus-3,536-stage trace with identical FP4 topology/plans. Its strict c2/c16 component is active under one lock; partial legs do not bind. No exact grid/35B performance follows. See [W3-C spike](specs/nvfp4-persistent-plan-cache.md) and [W3-F spike](specs/nvfp4-device-alpha.md) |
