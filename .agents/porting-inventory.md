...
| Item | Upstream | Tier |
|---|---|---|
| Basic `/v1/completions`, `/v1/chat/completions` SSE transport, `/v1/models`, `/health`, `/version` | `entrypoints/openai/` | T0 **partial** `23d9f2c` + W2 `GATING`: production completion/chat streams pull live incremental frames from AsyncLLM, concurrent requests share one scheduler, and disconnect abort is wired. Exact c32 evidence now proves the transport lacks deterministic worker capacity under 32 persistent streams; that repair is claimed. `/health` is liveness-only, while `/metrics`, complete protocol fields, logprobs payload and remaining GB10 gates stay open |
