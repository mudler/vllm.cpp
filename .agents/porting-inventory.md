...
| Paged attention for full-attn layers on sm_121 (bf16, GQA 16/2, partial RoPE) — FlashInfer-class performance is the bar; strategy in §9 | ref: `v1/attention/backends/{flashinfer,triton_attn,flash_attn}.py` | T0 🚧 on GB10 gate workloads (CPU/CUDA parity + full 27B/35B paged-engine greedy correctness gates pass. Exact pushed-`4e1d8ca` 27B online evidence is valid but below the every-axis floor: median total ratios c1→c32 are 0.9661/0.9274/0.9378/0.9466/0.9808/0.9910×. W0/W1 device-residency component A/Bs remain 1.021239×/1.006246× with structural copy reduction; inherited pools fail zero-leak. HTTP is healthy/neutral and FP4 small-M W1 now passes its first pushed focused GB10 op gate, while final capture/memcheck/model/performance gates remain; device-residency W2 stays scoped until that residual is remeasured and 35B waits); broader attention/backend/SM coverage lives in `kernel-matrix.md` and remains open |
...
|---|---|---|
| **NVFP4 gate slices** — ModelOpt W4A16 experts (35B) + compressed-tensors W4A4 dense (27B) | `quantization/modelopt.py`, `compressed_tensors/` | **T0 ✅ correctness/support; performance W1 ACTIVE** — both native CUDA paths and full gate workloads pass on GB10. W1 now defaults to distinct FlashInfer hybrid M buckets with a complete key, single-flight failure-safe tuning and capture-miss rejection; Release 100/100, TSan 615/615 and pushed-`1a802ac` default/legacy focused sm_121a runs pass. The expanded real capture case is staging-green only; immutable capture/memcheck/model/performance gates remain. Four local wide tactics still omit the traced narrow/static/Stream-K family reserved for separate W2. The accepted [small-M dispatch spike](specs/nvfp4-small-m-dispatch.md) keeps both iterations independent; generic quant-config/backend breadth remains in `quantization-matrix.md` |
