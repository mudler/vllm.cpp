...
|---|---|---|
| **NVFP4 gate slices** — ModelOpt W4A16 experts (35B) + compressed-tensors W4A4 dense (27B) | `quantization/modelopt.py`, `compressed_tensors/` | **T0 ✅ correctness/support; performance W3 ACTIVE** — `3f256ab` binds at **55/124**. W3-I remains default-off and async is neutral. Oracle and void first local c2 session both resolve 208 GEMMs/window as 128 Stream-K + 80 static-persistent calls; the local series stopped after one session on a stale c16 graph-size assertion. Batch-aware complete retry/final status is pending, so no lever, exact grid or 35B performance is authorized |
