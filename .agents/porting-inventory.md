...
|---|---|---|
| **NVFP4 gate slices** — ModelOpt W4A16 experts (35B) + compressed-tensors W4A4 dense (27B) | `quantization/modelopt.py`, `compressed_tensors/` | **T0 ✅ correctness/support; performance ACTIVE** — `3f256ab` binds at **55/124**. Finalized `179a0fc` proves the FP4 tactic family already matches at 128 Stream-K + 80 static-persistent calls. Selected `KERNEL-GEMM-BF16` W1 BA is implemented/`GATING`: clean pushed `581d335` closes one-owner F32-output core correctness/safety; BF16 output fails. Finalized `0091cd1` is `complete-structural` at exact merged 963/145 versus split 1,011/193 with 48 BF16-only removals and `benchmark_binding=false`. GGUF/legacy inertness, BF16 rounding, component evidence, qkvz, exact grid and 35B performance remain unauthorized |
