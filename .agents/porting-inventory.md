...
| Paged attention for full-attn layers on sm_121 (bf16, GQA 16/2, partial RoPE) — FlashInfer-class performance is the bar; strategy in §9 | ref: `v1/attention/backends/{flashinfer,triton_attn,flash_attn}.py` | T0 🚧 on GB10 gate workloads (CPU/CUDA parity + full 27B/35B paged-engine greedy correctness gates pass. Immutable `9cc7191` is the binding vLLM v0.25.0 27B cache-off result: model gate, 36/36 groups, 2,016 requests and six returns pass; all 124 axes bind, **54 pass / 70 fail**. Median total ratios c1→c32 are 0.9901/0.9491/0.9633/0.9770/1.0288/1.0467×, with 4/4/5/4/17/18 of 20 performance axes; memory is 2/4 with host PSS/RSS red. The first ours trace recorded whole graph activities but zero graph-node kernels; node-level paired recapture is pending before any attention/FP4/GDN attribution or 35B run); broader attention/backend/SM coverage lives in `kernel-matrix.md` and remains open |
...
|---|---|---|
| **NVFP4 gate slices** — ModelOpt W4A16 experts (35B) + compressed-tensors W4A4 dense (27B) | `quantization/modelopt.py`, `compressed_tensors/` | **T0 ✅ correctness/support; performance W3 ACTIVE** — both native CUDA paths and full gate workloads pass on GB10. W2 owns hybrid buckets, complete process-local key/single-flight/capture handling, all 32 SM12 tactics, merged gate/up CT semantics and fused SiLU→NVFP4. W3-D closes packed-QKV topology at 208.192 versus vLLM 208 FP4 GEMMs/forward. W3-E removes 624 activation swizzles and gains **1.002108×/1.006222×** c2/c16 mean total throughput, but strict-fails **32/40 timing + 6/8 memory**. Immutable `3f256ab` binds the v0.25.0/FlashInfer 0.6.13 27B gate at **55/124 axes pass, 69 fail**. Its oracle log proves a persistent 64-plan FP4 hit; ours retuned every W3-E process, only 18--33/64 paired IDs match and five of six output hashes differ. W3-C is now [spiked/`READY`](specs/nvfp4-persistent-plan-cache.md) for exact/native JSON persistence, frozen benchmarking, stale rejection and 5,000-us miss timing. Implementation and all W3-C benchmarks are `PENDING`; no speed credit/exact grid/35B performance is claimed. The [small-M dispatch spike](specs/nvfp4-small-m-dispatch.md) keeps iterations independent; generic breadth remains in `quantization-matrix.md` |
