...
| Paged attention for full-attn layers on sm_121 (bf16, GQA 16/2, partial RoPE) — FlashInfer-class performance is the bar; strategy in §9 | ref: `v1/attention/backends/{flashinfer,triton_attn,flash_attn}.py` | T0 🚧 on GB10 gate workloads (CPU/CUDA parity + full 27B/35B paged-engine greedy correctness gates pass. Exact pushed-`a531e05` 27B online evidence is valid but below the every-axis floor: median total ratios c1→c32 are 0.9679/0.9338/0.9482/0.9547/1.0028/0.9268×. W0/W1 device-residency component A/Bs remain 1.021239×/1.006246× with structural copy reduction; inherited pools fail zero-leak. c32 HTTP capacity and FP4 small-M bucketing/tactics are confirmed first repairs, so W2 stays scoped until that residual is remeasured; 35B waits); broader attention/backend/SM coverage lives in `kernel-matrix.md` and remains open |
...
|---|---|---|
| **NVFP4 gate slices** — ModelOpt W4A16 experts (35B) + compressed-tensors W4A4 dense (27B) | `quantization/modelopt.py`, `compressed_tensors/` | **T0 ✅ correctness/support; performance repair READY** — both native CUDA paths and full gate workloads pass on GB10. The exact online audit reopened W4A4 runtime selection: M=1/2/4/8/16 currently alias and four local wide tactics omit the traced FlashInfer narrow/static/Stream-K family. The accepted [small-M dispatch spike](specs/nvfp4-small-m-dispatch.md) separates bucket/single-flight and 32-tactic iterations; generic quant-config/backend breadth remains in `quantization-matrix.md` |
...
| Item | Upstream | Tier |
|---|---|---|
| Basic `/v1/completions`, `/v1/chat/completions` SSE transport, `/v1/models`, `/health`, `/version` | `entrypoints/openai/` | T0 **partial** `23d9f2c` + W2 `ACTIVE`: live AsyncLLM streams, disconnect abort and concurrent scheduling remain; deterministic fixed delivery capacity for all `max_num_seqs` streams plus four control workers is implemented/CPU-gated, with exact GPU A/B pending. `/health` is liveness-only; `/metrics`, full fields, logprobs and remaining GB10 gates stay open |
