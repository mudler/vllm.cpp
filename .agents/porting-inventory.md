...
|---|---|---|
| **NVFP4 gate slices** — ModelOpt W4A16 experts (35B) + compressed-tensors W4A4 dense (27B) | `quantization/modelopt.py`, `compressed_tensors/` | **T0 ✅ correctness/support; performance W3 ACTIVE** — existing CUDA paths remain gated; `3f256ab` binds at **55/124**. W3-C installs exact plans before warmup and completes frozen-plan reproduction control. W3-E/W3-F/W3-G strict-fail and earn no credit. W3-H is **ACTIVE / trace-first**: H1a/H1b are `VOID`; immutable `d1f8e33` H1c is doubly `VOID`. Nsight reported possible event loss, and configure SHA `7d510aa4…ed5d` proves the detached build disabled CUTLASS; its **146,661 naive + 10,944 WMMA** FP4 calls never exercised the frozen target path. Retained **0.638331/0.342777 ms per forward** values are diagnostic only. H1d must fail before GPU work unless exact CUTLASS build/dispatch provenance passes, then harden lossless collection plus exact plan/workload/graph validation. No exact grid/35B performance is authorized. See [W3-C spike](specs/nvfp4-persistent-plan-cache.md), [W3-F spike](specs/nvfp4-device-alpha.md), [W3-G spike](specs/fa2-gqa-split-kv-decode.md) and [W3-H spike](specs/nvfp4-bf16-producer-vectorization.md) |
