...
|---|---|---|
| **NVFP4 gate slices** — ModelOpt W4A16 experts (35B) + compressed-tensors W4A4 dense (27B) | `quantization/modelopt.py`, `compressed_tensors/` | **T0 ✅ correctness/support; performance W3 ACTIVE** — existing CUDA paths remain gated; `3f256ab` binds at **55/124**. W3-C installs exact plans before warmup and completes frozen-plan reproduction control. W3-E/W3-F/W3-G strict-fail and earn no credit. W3-H is **ACTIVE / trace-first**: H1a/H1b/H1c and H1d attempts through `3c1d7b7` are `VOID`; the latest exact build/gate and four-replay window pass but SIGUSR1 terminates the target and yields profiler exit 138 before validation. H1d now fails unless plan/build/ancestry plus diagnostic-only FIFO graceful stop/removal and zero exit pass, then enforces lossless plan/workload/graph validation. No exact grid/35B performance is authorized. See [W3-C spike](specs/nvfp4-persistent-plan-cache.md), [W3-F spike](specs/nvfp4-device-alpha.md), [W3-G spike](specs/fa2-gqa-split-kv-decode.md) and [W3-H spike](specs/nvfp4-bf16-producer-vectorization.md) |
