...
| YaRN + long-context RoPE factory/cache/apply | `model_executor/layers/rotary_embedding/{__init__,base,yarn_scaling_rope,mrope,llama3_rope}.py` | T1 🚧 W5 `ATTN-YARN` + W6 `ATTN-ROPE-LLAMA3` `GATING`: typed modern/legacy parameters, memoized f32/bf16 cache/factory, shared NeoX/GPT-J supplied-cache apply, plain + 1-D/3-axis `mrope_section` YaRN, and exact Llama 3 unchanged/smoothed/scaled bands pass nine pinned-source CPU oracle fixtures with sanitizers. CUDA compile/runtime, positive-model e2e, trace and every-axis gates remain; LongRoPE/dynamic formulas are separate READY leaves |
