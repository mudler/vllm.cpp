...
|---|---|---|
| **NVFP4 gate slices** — ModelOpt W4A16 experts (35B) + compressed-tensors W4A4 dense (27B) | `quantization/modelopt.py`, `compressed_tensors/` | **T0 ✅ correctness/support; performance W3 ACTIVE** — existing CUDA paths remain gated; `3f256ab` binds at **55/124** and W3-E strict-fails. W3-C installs exact plans before warmup and completes frozen-plan reproduction control. W3-F targets 208 local scalar stages/forward versus zero in vLLM/FlashInfer and now implements the device-alpha API/validation, direct CUTLASS pointer, individual/merged model residency, exact fallback and ported invalid/equality/capture/forced-tactic/packed-QKV tests. CPU Release 103/103 and focused Release/ASan+UBSan/TSan 17/17 + 911/911 pass; CUDA/safety/model/trace/A-B results remain pending. No exact grid/35B performance follows. See [W3-C spike](specs/nvfp4-persistent-plan-cache.md) and [W3-F spike](specs/nvfp4-device-alpha.md) |
