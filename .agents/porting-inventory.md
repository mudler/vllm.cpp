...
|---|---|---|
| **NVFP4 gate slices** — ModelOpt W4A16 experts (35B) + compressed-tensors W4A4 dense (27B) | `quantization/modelopt.py`, `compressed_tensors/` | **T0 ✅ correctness/support; performance W3 ACTIVE** — `3f256ab` binds at **55/124**. W3-C frozen-plan control passes; W3-E/W3-F/W3-G earn no credit. W3-H is trace-first: schema-v4 `b9beccd` remains VOID after four complete zero-eager reports. The committed probe isolates its warning to pinned Nsight's capture boundary, and schema-v5 exact version/event/completion/topology/zero-eager/cross-report reconciliation is CPU-green. Execute a fresh immutable 12-report DGX run. No exact grid or 35B performance is authorized. See [W3-H spike](specs/nvfp4-bf16-producer-vectorization.md) |
