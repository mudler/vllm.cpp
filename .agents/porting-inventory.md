...
|---|---|---|
| **NVFP4 gate slices** — ModelOpt W4A16 experts (35B) + compressed-tensors W4A4 dense (27B) | `quantization/modelopt.py`, `compressed_tensors/` | **T0 ✅ correctness/support; performance W3 ACTIVE** — `3f256ab` binds at **55/124**. W3-I remains default-off and async is neutral. Accepted oracle c2 topology resolves 208 GEMMs/window as 128 Stream-K + 80 static-persistent calls; the local B=2 paired capture/final status is pending, so no lever, exact grid or 35B performance is authorized |
