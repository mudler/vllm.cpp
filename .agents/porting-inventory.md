...
|---|---|---|
| **NVFP4 gate slices** — ModelOpt W4A16 experts (35B) + compressed-tensors W4A4 dense (27B) | `quantization/modelopt.py`, `compressed_tensors/` | **T0 ✅ correctness/support; performance W3 ACTIVE** — existing CUDA paths remain gated; `3f256ab` binds at **55/124**. W3-C installs exact plans before warmup and completes frozen-plan reproduction control. W3-E and W3-F strict-fail. Separate W3-G FA2 ratio-6 decode passes correctness/structure and reaches **1.017668×/1.006548×** c2/c16 mean total throughput, but strict-fails **35/40 timing + 5/8 memory** and earns no credit. W3-H is **ACTIVE / trace-first**: H1a/H1b are `VOID`; immutable `d1f8e33` H1c is also `VOID` after capture 1/3 completed 48/48 requests but emitted a severity-2 possible-event-loss diagnostic, stopped before r2/r3/vLLM and retained no frozen-plan records. Retained **0.638331/0.342777 ms per forward** normal-producer values are diagnostic only. H1d must harden lossless collection plus exact plan/workload/graph validation before any implementation/performance gate. No exact grid/35B performance is authorized. See [W3-C spike](specs/nvfp4-persistent-plan-cache.md), [W3-F spike](specs/nvfp4-device-alpha.md), [W3-G spike](specs/fa2-gqa-split-kv-decode.md) and [W3-H spike](specs/nvfp4-bf16-producer-vectorization.md) |
