...
|---|---|---|---|---|---|---|---|
| `QUANT-GGUF-COMPUTE` | Quantized `vt::` storage plus direct GGUF compute kernels; bf16 expansion does not count | llama.cpp `ggml/src/ggml*.c` tensor traits and backend kernels at `237ad9b` | current loader expands at [qwen3_5_gguf_weights.cpp:105](../src/vllm/model_executor/models/qwen3_5_gguf_weights.cpp#L105); direct path absent | dequant units and same-file APEX gates cover only materialization; CPU perf floor vs same-file llama.cpp measured — decode 54–75×, prefill ≈1,480×, peak RSS 2.7× behind ([ledger B4 row](parity-ledger.md#L290)) | [coverage spike](specs/quantization-coverage.md); claim-sized leaf specs required | `INVENTORIED` | - |
