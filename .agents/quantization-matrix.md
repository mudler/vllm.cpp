...
| `QUANT-NVFP4-CT-W4A4` | compressed-tensors NVFP4 | W4/A4 | CUDA sm121a, 27B dense | Y | Y | Y | Y | Y | `DONE` | [weights](../src/vllm/model_executor/models/qwen3_5_dense_weights.cpp#L106), [merged/tactic dispatch](../src/vllm/model_executor/models/qwen3_5.cpp#L3112), [fused activation quant](../src/vt/cuda/cuda_matmul_nvfp4.cu#L1137), [pre-serve warmup](../src/vllm/entrypoints/model_loader.cpp#L211), [kernel tests](../tests/vt/test_ops_nvfp4_fp4.cpp#L882), [27B gate](../tests/parity/test_qwen27_paged_engine.cpp#L116), [support closure](parity-ledger.md#L284). Support/correctness remains closed and clean `b5c6e4f` stays below the separate serving floor. W3-A's +0.816% c16 component strict-fails. W3-B implements actual-W4A4-gated all-bucket pre-serve tuning with a diagnostic lazy fallback; clean `d7cdf66` passes exact/legacy focused, 235/235 + 16/16 model, 24,586/24,586 zero-error memcheck and HTTP-ordering checks at 80/80 profiles/zero misses. Its repeated prewarm/lazy component is **1.000293×**, strict-fails **15/20 timing + 2/4 memory**, and leaves **20/80** prewarm tactic IDs stable. First-use improves. Trace attempt 1 is VOID after prewarm-only 144/144 because mutable client outputs changed cache inventory 50→58; no trace conclusion counts. Corrected paired trace and exact-oracle performance remain; no support or performance state changes | [W4A4 notes](specs/qwen27b-w4a4-notes.md); phase 1 `4fef786`, accepted path `64ad26b`, MVP closure `83010c7`; [small-M performance spike](specs/nvfp4-small-m-dispatch.md) | `83010c7` |
