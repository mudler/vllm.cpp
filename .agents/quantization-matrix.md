...
| `QUANT-NVFP4-CT-W4A16` | compressed-tensors NVFP4 `nvfp4-pack-quantized` (**A16 / weight-only**) | W4/A16 | CUDA sm121a, Qwen3-32B dense | Y | Y | Y | part | - | `ACTIVE` | NEW dispatcher [dense_nvfp4_gemm.h](../include/vllm/model_executor/models/dense_nvfp4_gemm.h) (`MatmulNvfp4W4A16D`, resident Marlin repack, fused merged `gate_up`, `Nvfp4W4A16Stats` counters) reusing the ALREADY-VALIDATED vendored Marlin W4A16 GEMM `vt::MoeGroupedGemmNvfp4Marlin` + [marlin_repack.h](../include/vt/cuda/marlin_repack.h) with **ZERO new kernel code**; append-only loader [dense_weight_loaders.h](../include/vllm/model_executor/models/dense_weight_loaders.h) (`LoadCtNvfp4W4A16`/`LoadMergedCtNvfp4W4A16`/`IsCtNvfp4Projection`); probe + fp4 fields in [qwen3_weights.cpp](../src/vllm/model_executor/models/qwen3_weights.cpp) / [qwen3.h](../include/vllm/model_executor/models/qwen3.h); dispatch in [qwen3.cpp](../src/vllm/model_executor/models/qwen3.cpp) + [dense_attn_block.h](../include/vllm/model_executor/models/dense_attn_block.h). Tests: [loader gate](../tests/vllm/models/test_qwen3_32b_nvfp4a16_load.cpp) (1603 tensors mapped, BF16 arms provably empty, `alpha==0`, zero `input_global_scale`, merged `scale2 == 1/max(divisors)`), [forward doctest](../tests/vllm/models/test_qwen3_forward.cpp) (synthetic NVFP4 vs BF16-on-dequantized, **max abs delta-logit = 0, bit-identical**), [SACRED strict gate](../tests/vllm/models/test_qwen3_32b_nvfp4a16_paged_engine.cpp) + goldens [qwen3_32b_nvfp4a16_greedy](../tests/parity/goldens/qwen3_32b_nvfp4a16_greedy). ON-DISK (measured): `weight_packed` U8 [N,K/2] + `weight_scale` F8_E4M3 [N,K/16] + `weight_global_scale` F32 DIVISOR, and NO `input_global_scale` — that absence is what selects A16. vLLM has NO separate W4A16 scheme class: `compressed_tensors.py:696-698` returns `CompressedTensorsW4A4Fp4(use_a16=True)`, and `kernels/linear/__init__.py:879-881` then FORCES Marlin, bypassing the capability registry. OBSERVED on sm_121: `Using MarlinNvFp4LinearKernel for NVFP4 GEMM`. Gate selection MEASURED: vLLM's own greedy DETERMINISTIC (K=5, 0 multi-valued cells) so the bar is STRICT token-exact — and we score **4/6 prompts / 67/96 tokens: `E` is GATING, NOT passed, and the gate was NOT loosened** (path proven exercised: `marlin_gemms=18432`, `fused_gate_up=6144`, `fallback_gemms=0`; every prefill argmax exact; `VT_NVFP4_MARLIN=0` A/B scores 3/6 with the SAME divergences ⇒ residual invariant across both quantized GEMMs, hypothesis = pre-existing dense-forward bf16 drift, UNPROVEN pending the teacher-forcing isolation). Loader gate **5144/5144**; CUDA `-Werror` 0-warn; memcheck **0 errors**; REGRESSION 27B 235/235 + 35B 315/315 + Coder 138/138 + dense 664/664 + OPT 36/36 ALL UNCHANGED. `P` is `-` — SPEED explicitly PENDING | [NVFP4A16 spike](specs/sweep-qwen3-32b-nvfp4a16.md); breadth-sweep rank 3 ([plan](specs/breadth-sweep-plan.md) B.3) | `CLAIM-QUANT-NVFP4-CT-W4A16` |
