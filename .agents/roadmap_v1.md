...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact production-vLLM performance closure, online latency, and e2e/nightlies | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | Historical mismatched campaigns remain void. Exact pushed-`f065fce` 27B cache-off/closed-loop baseline is complete and binding as a gap: c1→c32 throughput is 0.9616/0.9258/0.9364/0.9450/0.9775/0.9722×, only 3–5/20 timing and 2/4 memory axes pass, model/trace/lifecycle contracts pass, and CV is <0.7%. Its trace attributes 20.809 GiB H2D + 20.806 GiB D2H to GDN state rows across three repetitions. `KV-DEVICE-RESIDENCY` W0 is merged/CPU/CI-green at `7d29e0c`; its clean CUDA 13.0.88/sm_121a build plus default/fallback 35B+27B pointer/token gates pass 2/2 each. No sanitizer, trace, memory, or performance result is claimed yet. Nightly remains unspiked | `GATING` | complete W0 sanitizer, lifecycle, memory, corrected trace and repeated interleaved A/B gates; then W1 indexed state I/O and exact direct-library/online 27B parity, followed by 35B every-axis parity. This priority-zero closure blocks promotion of later roadmap tracks; then spike `SERVE-E2E-NIGHTLY` |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 active and below floor: exact f065 27B baseline is complete at 0.9258–0.9775× throughput with 3–5/20 timing and 2/4 memory axes passing. Model, lifecycle and corrected trace contracts pass; the trace grounds device residency as the first repair. Pushed `7d29e0c` W0 now passes its clean GB10 build and default/fallback 35B+27B pointer/token gates; speed remains unmeasured. Close W0 sanitizer/trace/A-B and indexed state I/O, then exact 27B direct/online parity before 35B; later roadmap work is blocked |
