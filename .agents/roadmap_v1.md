...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | MVP operational closure: online latency and e2e/nightlies | [`SERVE-GATE-ONLINE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | online-gate harness `b0acec2` fixes exact corpus/pip oracle/build/resources/traces and matched c32 scheduler budgets; CPU gates 16/16 focused, 32/32 full tools, CTest 2/2. Both pre-W2 campaigns are closed/invalid: second 27B diagnostic only; second 35B ours aborted after 1/6 c1 and vLLM later hung >60m in default c16/np96 rep1. Hang evidence is hashed; owned campaign terminated and GPU lock released. W2 live SSE implemented/CPU-gated; nightly unspiked | `GATING` | deploy/build current W2 now that the lock is free, reproduce the 35B fault under sanitizer if present, then run both-model G1/G3-G6 campaigns with hashed paired traces under new whole-series locks. Only post-W2 evidence can close the row; then spike `SERVE-E2E-NIGHTLY` |
| 1 | `ROAD-V1-C1` | Drop-in kernel ABI + complete kernel-family parity | [`BACKEND-ABI-VT`](backend-matrix.md), [kernel matrix](kernel-matrix.md) | exhaustive kernel/dependency inventory and [raw-pointer adapter ABI](specs/dropin-kernel-abi.md) accepted; additive W0 implemented and CPU 94/94. `CLAIM-BACKEND-ABI-W0-GPU-1` now owns the fresh CUDA cross-build/runtime gate after GCC13 exposed one test-macro parse blocker; scalar-forwarder/backend-shim debts remain explicit | `PARTIAL` | repair the syntax-only build blocker, finish W0 GPU/cross-build gates alongside the serving window, then migrate and independently gate one kernel family at a time |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 root takeover active: fail-closed harness merged at `b0acec2` (16 focused / 32 full CPU tests, CTest 2/2). Both pre-W2 campaigns are terminal/invalid: second 27B diagnostic only; second 35B ours aborted after 1/6 c1, while vLLM completed greedy c1-c32 + default c1 then hung >60m in default c16/np96 rep1. Snapshot/hash manifest: `latres2/diagnostic-vllm35-hang-20260711T0734CEST`; owned PGID terminated and `/tmp/gpu` released. W2 has genuine live SSE and is CPU/TSan-gated; fresh current-main GPU evidence owns closure |
