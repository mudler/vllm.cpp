...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 active: `31d053f` completed the full repeated 27B timing/memory grid and showed reproducible 0.926–0.961× throughput plus broad latency gaps without showing a direct-library regression, but its vLLM trace failed at missing profiler `PATH`/`ninja`; the lock released, so the campaign is void. The preflight/path repair is CPU-gated. Recapture the diagnostic paired trace, localize the full online-chain difference, then run a fresh optimized 27B gate before 35B. No existing online ratio is binding |
