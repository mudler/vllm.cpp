...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact performance closure against the faster applicable vLLM v0.25.0/SGLang floor before broader roadmap implementation | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG-PREFIX`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-MAMBA-ALIGN`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-ASYNC-LLM`](engine-matrix.md), [`KERNEL-GEMM-NVFP4-W4A4`](kernel-matrix.md), [`KERNEL-GDN-AOT-BF16`](kernel-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | v0.25.0 target `702f481` is audited and the canonical DGX oracle is active at vLLM 0.25.0 + FlashInfer 0.6.13. Immutable `9cc7191` completed its 27B correctness gate, all 36 cache-off groups/2,016 requests and six returns under one lock. All 124 axes bind, but only 54 pass: total ratios c1→c32 are 0.9901/0.9491/0.9633/0.9770/1.0288/1.0467×, and host PSS/RSS normalize to 0.5855/0.5934×. The result is stable and **FAILED/open**. Replacement `def5f75` selected packed QKV from the ~240-versus-208 launch gap. W3-D implements that path; immutable `3f256ab` packed/split correctness passes 235/235 + 16/16 each. Its c16/96 means are 812.231/808.150 tok/s = 1.005049×, but strict component acceptance fails at 14/20 timing + 2/4 memory. This does not change the binding 54/124 vLLM result and 35B remains held. The SGLang DGX claim remains source-audited and non-binding | `GATING` | re-trace `3f256ab` to prove 240→208 launches, repeat the exact grid and continue the trace-driven repair loop until all 124 axes pass; next execute PX1 plus `KV-MAMBA-ALIGN` PX2 for SGLang shared-prefix parity, and run 35B only after every applicable 27B cache-off/cache-on axis passes |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `9cc7191` v0.25.0 27B re-baseline complete and **FAILED/open**: 12/12 performance groups, 2/2 memory groups and 124/124 axes bind; 54 pass and 70 fail. Lifecycle proofs pass. W3-D packed QKV is implemented and staging correctness/safety plus both 16/16 arms pass; its clean pushed-SHA A/B is next and no speed credit exists yet. Close every 27B axis before 35B and keep optional W3-C plus teardown debt separate |
