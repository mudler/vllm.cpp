...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact performance closure against the faster applicable vLLM v0.25.0/SGLang floor before broader roadmap implementation | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG-PREFIX`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-MAMBA-ALIGN`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-ASYNC-LLM`](engine-matrix.md), [`KERNEL-GEMM-NVFP4-W4A4`](kernel-matrix.md), [`KERNEL-GDN-AOT-BF16`](kernel-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | v0.25.0 target `702f481` is audited and immutable `3f256ab` binds at **55/124 axes pass, 69 fail**. W3-D closes 208-GEMM topology. W3-C installs exact plans before warmup and completes reproduction control. W3-E gains **1.004483×/1.005044×** c2/c16 mean throughput but strict-fails at **39/40 timing + 1/8 memory**. W3-F mirrors vLLM's device alpha; immutable `7517af4` passes CUDA/operator/memcheck/model/trace gates and removes **3,536** scalar stages while preserving FP4 topology/plans. Its completed frozen-plan c2/c16 means are only **1.001967×/1.000144×** and strict-fail at **27/40 timing + 3/8 memory** across 12 legs/612 requests. Neither lever earns speed credit; the exact grid remains blocked and a fresh multi-lens executed-path scan is active. SGLang remains source-audited/non-binding | `GATING` | close the fresh vLLM/ours trace/source/dependency scan, rank and spike one bounded executed lever, then require its every-axis component before the exact grid. Close all 124 axes before 35B performance, then execute the distinct SGLang prefix gate |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. W3-C frozen-map control passes; W3-E gains 1.004483×/1.005044× c2/c16 but strict-fails at 39/40 timing + 1/8 memory. W3-F device alpha passes immutable CUDA/operator/memcheck/model/trace gates and removes all scalar stages, but its completed c2/c16 component is only 1.001967×/1.000144× and strict-fails at 27/40 timing + 3/8 memory. The exact grid remains blocked and a fresh executed-path scan is active. Close every 27B axis before 35B performance; keep teardown debt separate |
