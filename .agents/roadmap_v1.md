...
`GATING` BA-only W1 implementation. Its one-owner F32-output merge passes
merged/split 27B correctness, packed-view safety and 35B inertness; the
upstream-dtype BF16-output arm fails the known near-tie, and immutable
structure/performance remain pending. qkvz stays excluded. Trace durations earn
no speed credit. Host PSS/RSS separately retains a **22.920 GiB** CPU weight
mirror plus source mmap residency.
