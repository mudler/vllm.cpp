...
| 6 | `ROAD-V1-C6` | Priority and async/overlap scheduling + AsyncLLM streaming serving — RE-PRIORITIZED 2026-07-10: `SERVE-ASYNC-LLM` re-promoted T1→T0 because it now BLOCKS order 0 (fake-SSE finding) and async scheduling is vLLM's default at the pin (B3) | [engine matrix](engine-matrix.md), [coverage view §1](feature-matrix.md#1-engine-core--scheduling), [async-serving spike](specs/async-serving.md) | W1 `ENG-CORE-BUSY-LOOP` + W4 `ENG-PRIORITY-SCHED` implemented and `GATING` (W1: EngineCoreProc busy loop + queue split + InprocClient; W4: PriorityRequestQueue + priority preemption + policy config + `priority` plumbing, default stays FCFS — CPU suites green, GPU G1 deferred to the next GPU-idle window — campaign holds the GPU); `SERVE-ASYNC-LLM`, `ENG-ASYNC-SCHED` READY as W2-W3 claim-sized leaves | `PARTIAL` | claim W2 (`SERVE-ASYNC-LLM`) on the merged queue split — W2 unblocks the `ROAD-V1-A` latency axes; run W1/W4's GPU G1 when the GPU frees; W3 async-overlap mirrors vLLM's default |
...
| 13 | `ROAD-V1-D4` | LoRA, KV/offload, wider model zoo | [engine matrix](engine-matrix.md), [model matrix](model-matrix.md) | expert-streaming-from-disk spike accepted ([specs/expert-streaming.md](specs/expert-streaming.md): `ENG-EXPERT-STREAM` READY — surpass-track NVMe expert paging, low-concurrency capacity regime only, math + dgx NVMe measured; mirror floor `ENG-WEIGHT-OFFLOAD` inventoried); LoRA/KV-offload/model-zoo unspiked | `PARTIAL` | `ENG-EXPERT-STREAM` W1 (CPU-testable) claimable now; rest after T1 dependencies close |
...
| D4 | LoRA, KV/offload breadth, and wider model zoo | 🚧 T2 — first leaf spiked: [expert streaming from disk](specs/expert-streaming.md) (`ENG-EXPERT-STREAM` READY, surpass-track); LoRA/KV-offload/zoo still ☐ |
