...
and exact structure/performance remain pending. The mode-aware c2 harness is
locally green at 68/68 tools tests and fails closed on merged 963/145 versus
split 1,011/193 topology without reinterpreting old evidence; pushed-SHA GPU
execution remains pending. qkvz stays excluded and no trace duration earns
speed credit. Host PSS/RSS separately retains a **22.920 GiB** CPU weight mirror
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. Finalized `179a0fc` selects `KERNEL-GEMM-BF16`; BA code is `GATING` on exact rounding/trace/component evidence before qkvz. Repair the 22.920 GiB host mirror and close every 27B axis before 35B performance |
