...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. W3-E strict-fails its component. W3-C runtime publication and six-process frozen-map stability pass, but C3 same-plan long-output equality fails **2/6** at the first c2 pair; partial rates are void and the exact grid is blocked on numerical localization. Close every 27B axis before 35B performance; keep teardown debt separate |
