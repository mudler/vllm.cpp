...
| 6 | `ROAD-V1-C6` | Priority and async/overlap scheduling + AsyncLLM streaming serving — RE-PRIORITIZED 2026-07-10: `SERVE-ASYNC-LLM` re-promoted T1→T0 because it blocks order 0 and async scheduling is vLLM's default at the pin (B3) | [engine matrix](engine-matrix.md), [coverage view §1](feature-matrix.md#1-engine-core--scheduling), [async-serving spike](specs/async-serving.md) | W1 `ENG-CORE-BUSY-LOOP`, W2 `SERVE-ASYNC-LLM`, W4 `ENG-PRIORITY-SCHED` implemented/`GATING`; **W3 `ENG-ASYNC-SCHED` host-side machinery + runner device-input half LANDED + CPU-gated 2026-07-16 (`ACTIVE`, `CLAIM-ASYNC-SCHED-W3`):** `AsyncScheduler` placeholders + depth-2 `step_with_batch_queue` + `async_scheduling` config resolution + `VT_ASYNC_SCHED` rollback; PLUS the runner `combine_sampled_and_draft_tokens` device-input path (rebuild decode input ids from GPU-resident-analog `last_sampled_tokens`, `VT_ASYNC_RUNNER` default OFF), CPU ctest 110/110, sync path byte-identical, RED→GREEN. Remaining DGX-gated: the sampler-output non-blocking D2H (copy-stream + `vt::Backend` event/pinned) + flip `runner_supports_async` ON — together they capture the ~3.25 ms/step idle; prior `3812d8` control freezes the c2 speed budget neutral (+0.215%) | `PARTIAL` | land the W3 sampler-output D2H half + flip runner_supports_async, then DGX both-arms A/B; execute order-0 low-batch kernel mapping alongside |
...
| C6 | Priority + async/overlap scheduling + AsyncLLM streaming serving | 🚧 joint spike written: [async-serving.md](specs/async-serving.md); W1/W2/W4 are implemented and `GATING`. W2 fixed capacity is CPU/sanitizer-green and GPU-classified healthy/steady-state-neutral; broader every-axis parity remains. **W3 host-side + runner device-input half LANDED + CPU-gated (2026-07-16, `ACTIVE`)** — `combine_sampled_and_draft_tokens` device-input path (default OFF); the sampler-output D2H half + `runner_supports_async` flip stay DGX-gated. Order-0 FP4 closure precedes W3 default-ON overlap; [feature matrix §1](feature-matrix.md#1-engine-core--scheduling) |
