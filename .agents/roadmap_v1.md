...
implemented and CPU-gated; fresh immutable evidence is pending. `3f256ab` still
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. W3-C frozen-map control passes; W3-E/W3-F/W3-G strict-fail and earn no speed credit. [W3-H normal BF16→FP4 vectorized I/O](specs/nvfp4-bf16-producer-vectorization.md) is **ACTIVE / trace-first**: H1a/H1b/H1c and H1d attempts through exact-build/gate/FIFO-zero-exit `219f4f2` are `VOID`. H1d's repeated single-replay range repair and schema-v3 validator are implemented/CPU-gated; a fresh immutable DGX series is next and every new performance ratio remains prohibited. Close every 27B axis before 35B performance; keep host-memory/teardown debt separate |
