...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact performance closure against the faster applicable vLLM v0.25.0/SGLang floor before broader roadmap implementation | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG-PREFIX`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-MAMBA-ALIGN`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-ASYNC-LLM`](engine-matrix.md), [`KERNEL-GEMM-NVFP4-W4A4`](kernel-matrix.md), [`KERNEL-GDN-AOT-BF16`](kernel-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | v0.25.0 target `702f481` is audited and the canonical DGX oracle is validated/active at vLLM 0.25.0 + FlashInfer 0.6.13; exact package/import/CLI plus lock-held 27B production-graph and server smoke pass, with v0.24 rollback preserved. The last complete grid (`b5c6e4f`) is historical and `3cc490c` is VOID at 28/36 groups, so no ratio binds. The SGLang DGX claim is source-audited: its original 10--40x gap was cache-on versus cache-off; the residual reported 25--45% cache-on lead uses 35B only, vLLM 0.23.1, mismatched BF16/FP8 KV and memory policies, MTP, one/two runs and incomplete axes, so no number is accepted. vLLM v0.25 supports explicit Qwen hybrid `mamba_cache_mode=align` although the hybrid default is off. Cache-neutral and deterministic 64k/256k shared-prefix workloads now own distinct backend rows; exact tokens/dtypes/capacity/hits/no-eviction/full axes/traces are mandatory and the faster equivalent reference binds per axis. 35B and later blocks remain held | `GATING` | commit the migrated v0.25 oracle contract and close the exact 27B cache-off grid/trace, modernizing only trace-proven paths; then execute PX1 and the dedicated `KV-MAMBA-ALIGN` spike/PX2 for the SGLang v0.5.15 shared-prefix row, close 27B cache-on, and run 35B only after every applicable 27B cache-off/cache-on axis passes |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 v0.25.0 re-baseline active. Old-oracle `b5c6e4f` is historical and `3cc490c` is VOID at 28/36 groups after the FlashInfer 0.6.12/0.6.13 drift was proven. No current ratio binds. Validate/activate v0.25.0, re-trace and run the full 27B grid; close every 27B axis before 35B and keep optional W3-C plus teardown debt separate |
