...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact performance closure against the faster applicable vLLM v0.25.0/SGLang floor before broader roadmap implementation | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG-PREFIX`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-MAMBA-ALIGN`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-ASYNC-LLM`](engine-matrix.md), [`KERNEL-GEMM-NVFP4-W4A4`](kernel-matrix.md), [`KERNEL-GDN-AOT-BF16`](kernel-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | v0.25.0 target `702f481` is audited and immutable `3f256ab` binds at **55/124 axes pass, 69 fail**. W3-D closes 208-GEMM topology. W3-C installs exact plans before warmup; six fresh processes plus the corrected 12-leg component use byte-identical **64/64** maps with zero tuning/misses and fixed model gates. C3R proves direct/fallback **6/6 x 128-token equality** under identical sequential batch shape and reproduces default-mode batch-shape dependence in both ours and vLLM. W3-E gains **1.004483×/1.005044×** c2/c16 mean throughput under that control but strict-fails at **39/40 timing + 1/8 memory**. W3-F owns the next executed mismatch: **208.123 local `SetScalar` launches/forward versus zero in vLLM**. Its implementation and local gates pass; immutable `7517af4` then passes focused CUDA **33/33**, strict no-pool memcheck **22/22 + 26,884/26,884** with zero errors/leaks, both 27B **235/235 + 16/16** arms, 35B correctness-only inertness **315/315**, and paired node traces with **0 versus 3,536 `SetScalar`** while FP4 topology/plans stay identical. Its frozen-plan c2/c16 component is active under one lock and has no binding partial value. SGLang remains source-audited/non-binding | `GATING` | finish W3-F's strict c2/c16 component against `VT_FP4_DEVICE_ALPHA=0`; run the exact grid only if all 40 timing and 8 memory axes pass. Close all 124 axes before 35B performance, then execute the distinct SGLang prefix gate |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. W3-C runtime publication, six-process stability and the corrected 12-leg frozen-map control pass with identical 64/64 plans and zero tuning/misses; C3R proves same-shape sequential direct/fallback 6/6 x 128-token equality and shows the prior 2/6 independent-run check was an invalid batch-invariance predicate. W3-E gains 1.004483×/1.005044× c2/c16 mean throughput under that control but strict-fails at 39/40 timing + 1/8 memory, so the exact grid remains blocked. W3-F device alpha is implemented/`GATING`: immutable CUDA/operator/memcheck, both 27B arms, 35B correctness-only inertness and paired zero-versus-3,536-stage trace pass with identical FP4 topology/plans. Its strict frozen-plan c2/c16 A/B is active and partial legs do not bind. Close every 27B axis before 35B performance; keep teardown debt separate |
