...
| 2 | `ROAD-V1-C2` | Model families: Llama/Qwen3/Mistral, MoE, Qwen3-Next | [model matrix](model-matrix.md), [`ENG-HOST-WEIGHT-RESIDENCY`](engine-matrix.md) | current pin has 353 static IDs; v0.25.0 adds three sync-target rows, yielding 356 after pin advance. Current Qwen wrappers and the type-erased factory remain partial/`GATING`. Post-W3 `829883d` completes all 18 local arms: peak PSS is 1.768 GiB and throughput 0.9837x vLLM, but output varies at 121-125/128. The first repair closes the odd-tail BF16 CUDA test (76/76) and casts the plain GDN norm to model BF16; natural output is stable 6/6 but still 15/16 versus vLLM, while exact-1024 stress remains variable. P99 traces show deterministic initial-fill cost, not random stalls | `PARTIAL` | restore repeated 128/128 project output first by continuing from the localized GDN prefill recurrence discrepancy; then trace and mirror vLLM's compiled/piecewise mixed-prefill path before async overlap, and do not infer GB10 results; after performance closure and target pin advancement, run the two-model factory no-regression handoff |
