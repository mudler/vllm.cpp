...
**Current order-0 substage (2026-07-13):** schema-v5 `c498a413` passes exact
build and 27B correctness, three independent 48/48 + 16/16 local sessions, all
12 lossless report exports/validations/summaries, and the paired vLLM trace.
Final status creation incorrectly makes diagnostic production-default output
repeatability fatal, so the complete root remains `VOID` pending revalidation.
The corrected contract is CPU-green. `3f256ab` remains 55/124, and W3-H2, the
exact grid, 35B performance, and later portfolio stay blocked.
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. W3-C frozen-map control passes; W3-E/W3-F/W3-G strict-fail and earn no speed credit. [W3-H normal BF16→FP4 vectorized I/O](specs/nvfp4-bf16-producer-vectorization.md) is **ACTIVE / trace-first**: `c498a413` completes all three local sessions, 12/12 lossless reports, and the paired vLLM trace but remains VOID pending corrected final-status revalidation and residual ranking. No new ratio exists. Close every 27B axis before 35B performance and keep host-memory/teardown debt separate |
