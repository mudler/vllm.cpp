...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact production-vLLM performance closure, online latency, and e2e/nightlies | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-ASYNC-LLM`](engine-matrix.md), [`KERNEL-GEMM-NVFP4-W4A4`](kernel-matrix.md), [`KERNEL-GDN-AOT-BF16`](kernel-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | Clean pushed-`b5c6e4f` remains below the all-axis floor: c1→c32 median totals **0.9933/0.9520/0.9657/0.9760/1.0213/1.0218×**, **4/4/5/4/17/14 of 20** performance and **2/4** memory axes. W3-B at clean `d7cdf66` is immutable build/correctness/safety-green and first-use-positive, but its component strict-fails. Corrected trace attempt 2 now succeeds under one lock with **432/432** retained requests, unchanged 49-file inventories and three memory returns. Prewarm/lazy/vLLM FP4 sums are **110.623/114.229/109.932 s**; prewarm removes 480 retained delay launches, is within **0.63%** of vLLM, and matches the narrow pair at **70.333/70.986 s** and **218,434/220,465** calls. The original wide-tactic structural mismatch is therefore closed. Profiled total throughput is **1.0082x** but normalized mean TPOT is only **0.9673x**; node-traced rates do not bind. The correctness-required 27B BF16 GDN default remains and 35B keeps f32. W0/W1 device-residency wins and inherited pool debt remain; 35B/nightly are deferred | `GATING` | run the fresh clean W3-B exact c1-c32 27B grid. If any axis remains red, drive the highest-ranked remaining executed-kernel gap, repeat until every 27B axis passes, gate optional W3-C separately, then run 35B, repair teardown, and spike `SERVE-E2E-NIGHTLY` |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 active and below floor: exact clean pushed-`b5c6e4f` 27B evidence validates 12/12 groups and all lifecycle/trace preconditions, but c1→c32 total ratios are 0.9933/0.9520/0.9657/0.9760/1.0213/1.0218×, only 4/4/5/4/17/14 of 20 performance axes and 2/4 memory axes pass. W3-B is correctness/safety-green and first-use-positive but its component remains a strict fail. Corrected trace attempt 2 completes **432/432** retained requests under one lock with unchanged inventories and three returns. Prewarm FP4 kernel time is **110.623 s** versus lazy **114.229 s** and vLLM **109.932 s**; the narrow pair matches at **70.333/70.986 s** despite a different scheduler split. Profiled normalized mean TPOT remains **0.9673x**, so no trace rate binds and `b5c6e4f` is not replaced. Run the exact W3-B grid next; the oracle-disabled W3-C file cache is separate. Close every exact 27B axis before 35B; W0/W1 device-residency wins and inherited zero-leak debt remain open; later roadmap work is blocked |
