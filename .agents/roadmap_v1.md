...
| 5 | `ROAD-V1-C5` | Sliding window, local attention, YaRN/long context | [engine matrix](engine-matrix.md), [coverage view §§2,11](feature-matrix.md#2-kv-cache--memory) | joint spike claimed; composite KV and RoPE rows split into claim-sized leaves before analysis | `SPIKE` | complete pinned vLLM/dependency dispatch and upstream-test inventory in `specs/sliding-local-yarn-long-context.md` |
...
| C5 | Sliding-window KV/attention and YaRN long-context scaling | 🚧 joint spike claimed; [feature matrix §§2,11](feature-matrix.md#2-kv-cache--memory) |
