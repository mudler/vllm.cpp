...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact performance closure against the faster applicable vLLM v0.25.0/SGLang floor before broader roadmap implementation | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG-PREFIX`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-MAMBA-ALIGN`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-ASYNC-LLM`](engine-matrix.md), [`KERNEL-GEMM-NVFP4-W4A4`](kernel-matrix.md), [`KERNEL-ATTN-FA2`](kernel-matrix.md), [`KERNEL-GDN-AOT-BF16`](kernel-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | v0.25.0 target `702f481` is audited and immutable `3f256ab` binds at **55/124 axes pass, 69 fail**. W3-E/W3-F strict-fail and earn no speed credit. W3-G implements and correctness-gates the selected FA2 ratio-6 decode route; immutable `ae9e8ff` passes clean sm_121a/operator/memcheck/model/paired-trace gates. Its completed c2/c16 component reaches **1.017668×/1.006548×** mean total throughput but strict-fails **35/40 timing + 5/8 memory**, so it also earns no speed credit and no exact grid/35B performance follows. All 12 legs, 612 requests, memory returns, cache drops and 64-plan checks pass. [W3-H](specs/nvfp4-bf16-producer-vectorization.md) is `ACTIVE / trace-first`: H1a/H1b are `VOID` from tactic drift and event loss. Immutable `d1f8e33` H1c is also `VOID`: its 19.20-s model gate passed and 48/48 capture-1 requests completed, but Nsight warned that not all 818,537 CUDA events might have been collected, so the driver stopped before r2/r3/vLLM; the retained log also contains no frozen-plan records. Binding speed stays unchanged and implementation remains prohibited. SGLang remains source-audited/non-binding | `GATING` | harden collector plus exact plan/workload/graph validation and execute immutable W3-H1d; then gate the highest losslessly confirmed lever. Only a 48/48 component authorizes the exact grid. Close all 124 axes before 35B performance, then execute the distinct SGLang prefix gate |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. W3-C frozen-map control passes; W3-E and W3-F strict-fail and earn no speed credit. [W3-G FA2 ratio-6 split-KV decode](specs/fa2-gqa-split-kv-decode.md) immutable `ae9e8ff` passes sm_121a/operator/sanitizer/default+fallback 27B/35B-correctness/paired-structural gates; its completed c2/c16 component reaches **1.017668×/1.006548×** mean total throughput but strict-fails **35/40 timing + 5/8 memory** and earns no credit. [W3-H normal BF16→FP4 vectorized I/O](specs/nvfp4-bf16-producer-vectorization.md) is **ACTIVE / trace-first**: H1a/H1b are `VOID`, and immutable H1c is now `VOID` after its first of three captures emitted severity-2 possible CUDA-event loss and stopped before vLLM; the same capture retained no parseable frozen-plan lifecycle. H1d collector/semantic hardening is next; implementation and every new ratio remain prohibited. Close every 27B axis before 35B performance; keep host-memory/teardown debt separate |
