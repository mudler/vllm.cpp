...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact production-vLLM performance closure, online latency, and e2e/nightlies | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-ASYNC-LLM`](engine-matrix.md), [`KERNEL-GEMM-NVFP4-W4A4`](kernel-matrix.md), [`KERNEL-GDN-AOT-BF16`](kernel-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | Exact pushed-`bce2627` 27B online evidence validates all 12 groups but is below the all-axis floor: c1→c32 median total ratios **0.9680/0.9317/0.9403/0.9516/0.9944/1.0073×**, only **4/4/5/4/4/12 of 20** performance axes and **2/4** memory axes pass. W2 now ports and correctness-gates the full 32-tactic SM12 family, merged gate/up CT max-divisor/one-alpha path and traced one-input SiLU→NVFP4 producer. Its latest unprofiled c16 fusion A/B is **1.015211×**, but only **17/20** timing and **0/4** sampled-memory axes pass; a paired trace is **1.020843×/20-of-20** and structurally removes about 8k launches, but is not the binding speed result. The correctness-required 27B BF16 GDN output default is separately **1.007989×**, 16/20 timing and 2/4 memory axes; 35B keeps f32. W0/W1 device-residency wins and inherited pool debt remain; 35B/nightly are deferred | `GATING` | commit/push the W2 + 27B-GDN checkpoint, rebuild the immutable SHA and repeat the exact c1-c32 27B vLLM campaign until every performance and memory axis passes; only then run 35B, repair teardown, and spike `SERVE-E2E-NIGHTLY` |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 active and below floor: exact pushed-`bce2627` 27B evidence validates 12/12 groups, but c1→c32 total ratios are 0.9680/0.9317/0.9403/0.9516/0.9944/1.0073×, only 4/4/5/4/4/12 of 20 performance axes and 2/4 memory axes pass. W2 from the accepted [FP4 spike](specs/nvfp4-small-m-dispatch.md) now implements all 32 tactics, merged CT gate/up scaling and fused SiLU→NVFP4 quantization; focused correctness/sanitizer gates pass. Its unprofiled fusion component is +1.521% but still 17/20 timing and 0/4 sampled-memory, while the paired trace's +2.084%/20-of-20 result is structural only. The 27B-only BF16 GDN output default restores native correctness and is +0.799%, but also remains 16/20 timing and 2/4 memory. Push/rebuild and close every exact 27B axis before running 35B; W0/W1 device-residency wins and inherited zero-leak debt remain open context; later roadmap work is blocked |
