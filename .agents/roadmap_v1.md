...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact production-vLLM performance closure, online latency, and e2e/nightlies | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | Exact pushed-`f065fce` 27B baseline remains binding below floor at 0.9258–0.9775× throughput, 3–5/20 timing and 2/4 memory axes. W0 at `7d29e0c` is CPU/CI/pointer/model/lifecycle/A-B/trace/access green at +2.1239%/20-of-20. W1 indexed state I/O is implemented and focused CPU/sanitizer/CUDA/op/model/turnover-gated; its independent c16/48 AB/BA/AB averages 781.799 vs 776.946 tok/s (**+0.6246%**, 20/20 axes, six memory returns). Its structural trace collapses async copies 163,540→7,508 and D2D calls 142,717→1,231; profiler throughput is perturbed/inverted and is not used for the ratio. Full W0 leak-check still exposes inherited pools (47.29 MB dense; 36.82 GB MoE/Marlin), while W0 caches are absent. Neither component ratio is a fresh vLLM denominator. Nightly remains unspiked | `GATING` | run fresh exact direct-library/online 27B parity now; if any axis remains below floor, implement W2 direct indexed convolution-state update and repeat. Then run 35B every-axis parity and repair inherited pool teardown before row closure. This priority-zero closure blocks later roadmap tracks; then spike `SERVE-E2E-NIGHTLY` |
...
| 6 | `ROAD-V1-C6` | Priority and async/overlap scheduling + AsyncLLM streaming serving — RE-PRIORITIZED 2026-07-10: `SERVE-ASYNC-LLM` re-promoted T1→T0 because it blocks order 0 and async scheduling is vLLM's default at the pin (B3) | [engine matrix](engine-matrix.md), [coverage view §1](feature-matrix.md#1-engine-core--scheduling), [async-serving spike](specs/async-serving.md) | W1 `ENG-CORE-BUSY-LOOP`, W2 `SERVE-ASYNC-LLM`, and W4 `ENG-PRIORITY-SCHED` implemented and `GATING`: CPU/TSan gates pass; exact matched-config GPU evidence is open. W3 `ENG-ASYNC-SCHED` remains READY. The larger `KV-DEVICE-RESIDENCY` W1 mixed-state-I/O lever is now implemented and component-gated at +0.6246%; fresh oracle evidence decides whether W2 direct convolution-state update is needed before scheduler overlap | `PARTIAL` | close order-0 fresh direct/online 27B denominators, then W1/W2/W4 GPU gates; if the residency track is at floor, proceed with W3 async overlap/default-ON mirror |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 active and below floor: exact f065 27B baseline is complete at 0.9258–0.9775× throughput with 3–5/20 timing and 2/4 memory axes passing. Pushed `7d29e0c` W0 wins a separate c16/48 A/B by +2.1239% (20/20 axes). W1 indexed state I/O wins its independent AB/BA/AB by +0.6246% (20/20) and collapses traced async copies 163,540→7,508; focused CPU/sanitizer/CUDA/model/turnover gates pass. Inherited pools still fail zero-leak. These component gains are not fresh vLLM ratios. Run exact current-SHA 27B direct/online parity now, use W2 if a gap remains, then close 35B; later roadmap work is blocked |
