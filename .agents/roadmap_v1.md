...
| 5 | `ROAD-V1-C5` | Sliding window, local attention, YaRN/long context | [engine matrix](engine-matrix.md), [coverage view §§2,11](feature-matrix.md#2-kv-cache--memory), [joint spike](specs/sliding-local-yarn-long-context.md) | accepted complete KV/manager/backend/dependency and RoPE formula/cache/apply chains; `KV-SLIDING-WINDOW-SPEC` W1 now implements registry dispatch, sizing/admission, recycling, prefix/EAGLE/retention policy and hybrid fallback with CPU G1/G2 + sanitizer evidence, and remains `GATING`; seven other leaves remain `READY` | `PARTIAL` | claim `ATTN-SLIDING-WINDOW` for operator/model-positive G4/G6-G9 and restore the pinned oracle test environment before SW closure; chunked-local KV and the `ATTN-YARN` foundation remain independently claimable |
