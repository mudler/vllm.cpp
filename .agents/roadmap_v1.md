...
execution remains pending: clean `7bae38a` and `2d16c68` failed before
...
owns the recorded target build before executable validation. `3f256ab` still
...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact performance closure against the faster applicable vLLM v0.25.0/SGLang floor before broader roadmap implementation | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG-PREFIX`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-MAMBA-ALIGN`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-ASYNC-LLM`](engine-matrix.md), [`KERNEL-GEMM-NVFP4-W4A4`](kernel-matrix.md), [`KERNEL-ATTN-FA2`](kernel-matrix.md), [`KERNEL-GDN-AOT-BF16`](kernel-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | v0.25.0 target `702f481` is audited and immutable `3f256ab` binds at **55/124 axes pass, 69 fail**. W3-E/W3-F/W3-G strict-fail and earn no speed credit. [W3-H](specs/nvfp4-bf16-producer-vectorization.md) is `ACTIVE / trace-first`; H1a/H1b/H1c are `VOID`. H1d implements the four-replay controller and exact evidence contracts. Clean `7bae38a`/`2d16c68` failed pre-build/GPU; `5f8fab1` failed correctness on an invalid no-Triton-AOT/`Release` path; `d063f20` failed pre-plan because corpus preceded dry-run; `b1c7eb6` passed exact plan/configuration but failed pre-build on the now-fixed driver bootstrap check. The repair binds plan-first ordering, `RelWithDebInfo`, Triton-AOT/FA2/CUTLASS, pinned tools and driver-owned target build before executable validation. Binding speed stays unchanged, W3-H2 remains prohibited and SGLang remains source-audited/non-binding | `GATING` | execute fresh immutable W3-H1d and accept only correctness plus three zero-exit, lossless, exact-plan captures. Only a later 48/48 component authorizes the exact grid. Close all 124 axes before 35B performance, then execute the distinct SGLang prefix gate |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. W3-C frozen-map control passes; W3-E/W3-F/W3-G strict-fail and earn no speed credit. [W3-H normal BF16→FP4 vectorized I/O](specs/nvfp4-bf16-producer-vectorization.md) is **ACTIVE / trace-first**: H1a/H1b/H1c, invalid-path `5f8fab1`, pre-plan `d063f20` and pre-build `b1c7eb6` are `VOID`. H1d's repaired plan/build/profile validation awaits a fresh immutable DGX series; every new performance ratio remains prohibited. Close every 27B axis before 35B performance; keep host-memory/teardown debt separate |
