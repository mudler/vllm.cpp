...
| 5 | `ROAD-V1-C5` | Sliding window, local attention, YaRN/long context | [engine matrix](engine-matrix.md), [coverage view §§2,11](feature-matrix.md#2-kv-cache--memory), [joint spike](specs/sliding-local-yarn-long-context.md) | accepted complete KV/manager/backend/dependency and RoPE formula/cache/apply chains; W1 sliding-window KV, W2 attention and W3 chunked-local KV are implemented and `GATING`; five other leaves remain `READY` | `GATING` | claim W4 `ATTN-CHUNKED-LOCAL` for virtual-batch metadata/backend semantics; W2/W4 G4-G9 remain gated by the serving lock, restored oracle and positive-model prerequisites |
...
| C5 | Sliding-window KV/attention and YaRN long-context scaling | 🚧 [joint spike accepted](specs/sliding-local-yarn-long-context.md): W1 sliding KV, W2 attention and W3 chunked-local KV are implemented/`GATING`; five leaves remain `READY`. W4 chunked-local attention is next; runtime/model/oracle/trace/performance gates remain open; gate Qwen3.6 checkpoints are default interleaved MRoPE regressions, with separate feature-positive e2e/oracle gates |
