...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` v0.25.0 27B re-baseline complete and **FAILED/open**: 12/12 performance groups, 2/2 memory groups and 124/124 axes bind; 55 pass and 69 fail. Lifecycle and paired-trace proofs pass. W3-D packed QKV is structurally closed; the apparent norm+FP4 trace-name lever is refuted and residual selection is open. Close every 27B axis before 35B and keep optional W3-C plus teardown debt separate |
