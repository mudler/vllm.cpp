...
Next: execute that runner under one uncontended DGX lock and disposition all
c2/c16 **40 timing + 8 memory** axes. qkvz
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. Packed structure is accepted; the hardened c2/c16 component runner is CPU-green **44/44** and DGX-pending before qkvz. Repair the 22.920 GiB host mirror before 35B performance |
