...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact production-vLLM performance closure, online latency, and e2e/nightlies | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | Exact pushed-`f065fce` 27B baseline remains binding below floor at 0.9258–0.9775× throughput, 3–5/20 timing and 2/4 memory axes. W0 at `7d29e0c` is CPU/CI/pointer/model/lifecycle/A-B/trace/access green: c16/48 averages 785.49 vs 769.15 tok/s (+2.1239%), wins 20/20 axes, reproduces +2.2188% under nsys, and changes state rows H2D/D2H→D2D. It still issues 160,232 copies. Full leak-check exposes inherited pools (47.29 MB dense; 36.82 GB MoE/Marlin), while W0 caches are absent; zero-leak remains open. W0's ratio is not a fresh vLLM denominator. Nightly remains unspiked | `GATING` | implement and independently A/B W1 indexed state I/O, then exact direct-library/online 27B parity followed by 35B every-axis parity; repair inherited pool teardown before row closure. This priority-zero closure blocks later roadmap tracks; then spike `SERVE-E2E-NIGHTLY` |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 active and below floor: exact f065 27B baseline is complete at 0.9258–0.9775× throughput with 3–5/20 timing and 2/4 memory axes passing. Pushed `7d29e0c` W0 wins a separate c16/48 A/B by +2.1239% (20/20 axes); trace proves state traffic moved to D2D and ~160k residual copies ground W1. Both model paths are sanitizer access-clean; inherited pools fail zero-leak. This component gain is not a fresh vLLM ratio. Implement W1, then exact 27B direct/online parity before 35B; later roadmap work is blocked |
