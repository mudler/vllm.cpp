...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact performance closure against the faster applicable vLLM v0.25.0/SGLang floor before broader roadmap implementation | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG-PREFIX`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-MAMBA-ALIGN`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-ASYNC-LLM`](engine-matrix.md), [`KERNEL-GEMM-NVFP4-W4A4`](kernel-matrix.md), [`KERNEL-ATTN-FA2`](kernel-matrix.md), [`KERNEL-GDN-AOT-BF16`](kernel-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | v0.25.0 target `702f481` is audited and immutable `3f256ab` binds at **55/124 axes pass, 69 fail**. W3-E/W3-F strict-fail and earn no speed credit. W3-G now implements the selected FA2 ratio-6 decode route: cast-free BF16 model/operator selection, direct group-swap strides, exact split heuristic, graph-stable per-shape scratch/queue cleanup, fallback and ported ratio/capture/lifecycle tests. CUDA-off build/test is green; clean sm_121a compile and every CUDA/model/sanitizer/trace/runtime axis remain `PENDING`. The diagnostic trace remains local **22,893 / 8,793.238 ms** versus unequal-window vLLM split main **23,616 / 7,061.921 ms** + combine **23,488 / 123.245 ms**. Vectorized FP4 production ranks second. SGLang remains source-audited/non-binding | `GATING` | freeze/build W3-G on sm_121a and require operator/sanitizer/model/trace plus all-40-timing/all-8-memory c2/c16 axes before the exact grid. Close all 124 axes before 35B performance, then execute the distinct SGLang prefix gate |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. W3-C frozen-map control passes; W3-E and W3-F strict-fail and earn no speed credit. [W3-G FA2 ratio-6 split-KV decode](specs/fa2-gqa-split-kv-decode.md) source and ported tests now exist with a green CUDA-off compile checkpoint; immutable sm_121a/operator/sanitizer/model/trace evidence and every-axis component remain `PENDING`. The exact grid stays blocked. Close every 27B axis before 35B performance; keep host-memory/teardown debt separate |
