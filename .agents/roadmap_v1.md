...
Mutable replay is current `306/7552`, beta-only `308/6558`, and both semantics
`0/1` output/state BF16 differences. Immutable replay and the full packed port
now precede c2/c16; qkvz stays excluded and no diagnostic earns speed credit.
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. BA safety/structure/projection are closed, but BF16 end-to-end is 233/235. The packed-decode oracle selects the full upstream pure-decode recurrence; immutable replay, implementation, 235/235 and c2/c16 precede qkvz. Repair the 22.920 GiB host mirror before 35B performance |
