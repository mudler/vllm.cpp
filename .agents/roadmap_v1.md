...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | MVP operational closure: online latency and e2e/nightlies | [`SERVE-GATE-ONLINE`](engine-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | exact validator/corpus/oracle/resources/traces/c32 contracts are CPU-green. Clean `8289cbd` 27B ours rep1 passed model/cache/live-SSE and c1-c16; c32 completed 192/192 with zero errors, but one 128-frame response re-tokenized to 126 because our server ignored the client's requested final usage frame. The gate failed closed before vLLM and released the lock. Complete stream-usage spike now owns native-ID final/continuous/force modes; all partial attempts remain invalid; nightly unspiked | `GATING` | commit the `SERVE-STREAM-USAGE` spike, implement/CPU-gate it, merge, regenerate SHA-bound corpora, then run 27B and 35B G1/G3-G6 under separate whole-model locks. Only complete evidence can close the online row; then spike `SERVE-E2E-NIGHTLY` |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 active: tools 34/34, CTest 1/1. All partial campaigns remain invalid. `8289cbd` 27B ours rep1 passed through c16; c32 completed all 192 requests but one 128-frame output re-tokenized to 126 because requested `include_usage` was absent. Fail-closed stop/cleanup passed. `SERVE-STREAM-USAGE` is now the explicit implementation dependency before regenerated current-main evidence |
