...
| 5 | `ROAD-V1-C5` | Sliding window, local attention, YaRN/long context | [engine matrix](engine-matrix.md), [coverage view §§2,11](feature-matrix.md#2-kv-cache--memory), [joint spike](specs/sliding-local-yarn-long-context.md) | accepted complete KV/manager/backend/dependency and RoPE formula/cache/apply chains; W1-W3 are implemented and `GATING`; W4 `ATTN-CHUNKED-LOCAL` is `ACTIVE` under `CLAIM-C5-CHUNKED-ATTN-1`; four RoPE leaves remain `READY` | `ACTIVE` | implement W4 virtual-batch metadata, block-table transforms, underlying-backend dispatch and ported CPU/reference properties without touching the occupied serving GPU |
...
| C5 | Sliding-window KV/attention and YaRN long-context scaling | 🚧 [joint spike accepted](specs/sliding-local-yarn-long-context.md): W1-W3 are implemented/`GATING`; W4 chunked-local attention is `ACTIVE`; four RoPE leaves remain `READY`. Runtime/model/oracle/trace/performance gates remain open; gate Qwen3.6 checkpoints are default interleaved MRoPE regressions, with separate feature-positive e2e/oracle gates |
