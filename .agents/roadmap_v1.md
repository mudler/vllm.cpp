...
preflight and `VT_GDN_PACKED_DECODE=0` rollback. Clean pushed `f344dec` closes
immutable W1D2/G2: gates pass
...
The W1D3 fail-closed trace harness is now implemented and CPU-gated: explicit
packed/rollback toggle provenance, 915/963-node contracts, one-lock ordering
exact mode-invariant signature equality after isolating the 48 coupled BA
projection nodes and completion-marker-last finalization pass the full
**79/79** tool suite. The first clean `8fbb950` attempt stopped before the GPU
lock because the driver inherited an absent H1d plan environment; deterministic
source/evidence-derived setup is implemented and a replacement trace is pending.
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. Clean `f344dec` closes W1D2/G2; paired trace/c2/c16 precedes qkvz. Repair the 22.920 GiB host mirror before 35B performance |
