...
**Current order-0 substage (2026-07-13):** clean schema-v4 `b9beccd` passes its
exact build, 27B correctness, frozen plans, clients, FIFO/zero-exit, and four
complete zero-eager graph reports, but fails closed on pinned Nsight's
possible-loss capture-range diagnostic before sessions 2/3 or vLLM. It is
`VOID` and changes no ratio. Calibrate that tool behavior with a minimal graph,
retain exact event/topology reconciliation, then rerun the 12-report gate from
a new SHA/root. W3-H2, the exact 27B grid, 35B performance, and the later
portfolio remain blocked. Attempt chronology lives only in the append-only
state/ledger record.
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. W3-C frozen-map control passes; W3-E/W3-F/W3-G strict-fail and earn no speed credit. [W3-H normal BF16→FP4 vectorized I/O](specs/nvfp4-bf16-producer-vectorization.md) is **ACTIVE / trace-first**: schema-v4 `b9beccd` is `VOID` on an unresolved Nsight capture-range diagnostic after four structurally complete reports. Calibrate, repair, and rerun all 12 reports; no new ratio exists. Close every 27B axis before 35B performance and keep host-memory/teardown debt separate |
