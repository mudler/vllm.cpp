...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact production-vLLM performance closure, online latency, and e2e/nightlies | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | Historical offline denominators and the `31d053f` online/old paired trace are non-binding for the recorded configuration mismatches. Pushed `f065fce` repairs cache policy and closed-loop trace equivalence; its exact cache-off 27B baseline is now running under one DGX series lock and its model gate passed. `KV-DEVICE-RESIDENCY` is claimed ACTIVE: W0 replaces CUDA host-vector cache owners with stable backend allocations and a fallback; focused CPU build/tests pass, while every CUDA/pointer/model/lifecycle/A-B gate remains open. Pure b16 decode was nearly tied; mixed prefill/decode state residency remains the top traced lever. Nightly remains unspiked | `GATING` | finish and record the f065 27B baseline; merge W0 as an explicitly GPU-open checkpoint; run GB10 default/fallback pointer, correctness, sanitizer, memory, trace and repeated A/B gates; then W1 indexed state I/O and fresh exact direct-library/online 27B→35B closure. Only complete passing evidence closes performance/serving rows; then spike `SERVE-E2E-NIGHTLY` |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 active: no historical ratio is binding. Pushed `f065fce` is executing the first exact cache-off/closed-loop 27B baseline under one lock; its model gate passed. In parallel, claimed `KV-DEVICE-RESIDENCY` W0 is CPU-gated but has no CUDA result. Finish the baseline, gate W0 default/fallback on GB10, then run indexed state I/O and exact 27B direct/online gates before 35B |
