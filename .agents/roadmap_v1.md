...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact production-vLLM performance closure, online latency, and e2e/nightlies | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-ASYNC-LLM`](engine-matrix.md), [`KERNEL-GEMM-NVFP4-W4A4`](kernel-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | Exact pushed-`a531e05` 27B online evidence validates all 12 groups but is below the all-axis floor: c1→c32 median total ratios **0.9679/0.9338/0.9482/0.9547/1.0028/0.9268×**, only **4/2/5/3/10/8 of 20** performance axes and **2/4** memory axes pass. The reproduced c32 unread-socket defect now has a fixed `max_num_seqs + 4` worker implementation, a legacy same-binary toggle and a 100×/sanitizer-green 32-client socket regression; its exact GPU A/B/oracle outcome remains pending. Direct-c16 traces separately prove FP4 M=1/2/4/8/16 key aliasing and match vLLM TPOT after retuning; vLLM also selects absent 128×32×256 Stream-K/static-persistent tactics. The full dependency-chain [FP4 repair spike](specs/nvfp4-small-m-dispatch.md) is accepted: exact buckets/single-flight and the 32-tactic family are separate measured iterations. W0/W1 wins and inherited pool debt remain; 35B/nightly are deferred | `GATING` | finish and classify the fixed HTTP campaign; then claim FP4 W1 exact buckets/single-flight, measure it, and only then port/measure the full tactic family. Repeat 27B after every iteration until every axis passes, then re-rank residuals, run 35B, repair teardown, and spike `SERVE-E2E-NIGHTLY` |
...
| 6 | `ROAD-V1-C6` | Priority and async/overlap scheduling + AsyncLLM streaming serving — RE-PRIORITIZED 2026-07-10: `SERVE-ASYNC-LLM` re-promoted T1→T0 because it blocks order 0 and async scheduling is vLLM's default at the pin (B3) | [engine matrix](engine-matrix.md), [coverage view §1](feature-matrix.md#1-engine-core--scheduling), [async-serving spike](specs/async-serving.md) | W1 `ENG-CORE-BUSY-LOOP` and W4 `ENG-PRIORITY-SCHED` are implemented/`GATING`; W2 `SERVE-ASYNC-LLM` is `ACTIVE`. Its confirmed cpp-httplib c32 defect now has capacity-derived fixed workers, a diagnostic legacy arm, a 32-persistent-client/control-path regression stable 100×, and ASan+UBSan/TSan passes. Exact GPU evidence is open. W3 `ENG-ASYNC-SCHED` remains READY | `PARTIAL` | run W2 fixed-vs-legacy c32 and full-ladder GPU gates; then finish order-0 FP4/27B/35B closure before W3 async overlap/default-ON mirror |
...
| C6 | Priority + async/overlap scheduling + AsyncLLM streaming serving | 🚧 joint spike written: [async-serving.md](specs/async-serving.md); W1/W4 implemented and `GATING`; W2 is `ACTIVE` with the fixed-capacity HTTP implementation CPU/sanitizer-green and exact GPU A/B pending. Order-0 closure precedes W3 default-ON overlap; [feature matrix §1](feature-matrix.md#1-engine-core--scheduling) |
