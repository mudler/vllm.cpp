...
| 5 | `ROAD-V1-C5` | Sliding window, local attention, YaRN/long context | [engine matrix](engine-matrix.md), [coverage view §§2,11](feature-matrix.md#2-kv-cache--memory), [joint spike](specs/sliding-local-yarn-long-context.md) | accepted complete KV/manager/backend/dependency and RoPE formula/cache/apply chains; `KV-SLIDING-WINDOW-SPEC` W1 and `ATTN-SLIDING-WINDOW` W2 are implemented and `GATING`; W2 is CPU/sanitizer green and sm_121a compile-checked but has no runtime/model/oracle/performance claim; six other leaves remain `READY` | `GATING` | after the serving lock releases, run W2 G4 and restore the pinned oracle/model-positive G6-G9 handoff; meanwhile the independent chunked-local KV and `ATTN-YARN` leaves remain claimable |
...
| C5 | Sliding-window KV/attention and YaRN long-context scaling | 🚧 [joint spike accepted](specs/sliding-local-yarn-long-context.md): W1 KV and W2 attention are implemented/`GATING`, and six leaves remain `READY`; W2 CPU/sanitizer + sm_121a compile gates pass, while runtime/model/oracle/trace/performance remain open; gate Qwen3.6 checkpoints are default interleaved MRoPE regressions, with separate feature-positive e2e/oracle gates |
