...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact performance closure against the faster applicable vLLM v0.25.0/SGLang floor before broader roadmap implementation | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG-PREFIX`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-MAMBA-ALIGN`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-ASYNC-LLM`](engine-matrix.md), [`KERNEL-GEMM-NVFP4-W4A4`](kernel-matrix.md), [`KERNEL-GDN-AOT-BF16`](kernel-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | v0.25.0 target `702f481` is audited and the canonical DGX oracle is active. Immutable `3f256ab` completed all 36 27B groups, 2,016 requests, six returns and paired trace; **55/124 axes pass and 69 fail**. Total ratios c1→c32 are **0.993504/0.954464/0.966438/0.980678/1.027889/1.039417×**. W3-D closes topology at 208.192 vs 208 FP4 GEMMs/forward but strict-fails its component. W3-E removes 624 activation swizzles and gains **1.002108× / 1.006222×** c2/c16 mean total throughput but strict-fails **32/40 timing + 6/8 memory**. The post-failure scan corrects the v0.24 cache assumption: binding v0.25 loads/hits a persistent 64-plan FlashInfer cache, while W3-E fresh processes match only 18--33/64 paired IDs. [W3-C](specs/nvfp4-persistent-plan-cache.md) is now `READY` for native/imported JSON persistence, frozen-map benchmarking and current 5,000-us miss timing; it has no code, speed credit or new exact grid yet. Binding parity remains 55/124, and the SGLang DGX claim remains source-audited/non-binding | `GATING` | implement/gate W3-C, require six-process 64/64 plan stability and a same-plan c2/c16 all-axis component before another exact grid; close all 124 cache-off axes before 35B performance, then execute PX1 plus `KV-MAMBA-ALIGN` PX2 for the distinct SGLang shared-prefix gate |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` v0.25.0 27B re-baseline complete and **FAILED/open**: 12/12 performance groups, 2/2 memory groups and 124/124 axes bind; 55 pass and 69 fail. W3-E removes 624 activation swizzles and gains 1.002108×/1.006222× c2/c16 mean total throughput, but strict-fails **32/40 timing + 6/8 memory**. W3-C persistent/frozen plan parity is now spiked/`READY` because v0.25 actually loads 64 cached tactics and ours is unstable across fresh processes; implementation and same-plan gates are pending, with no speed credit or conditional exact grid. Close every 27B axis before 35B performance; keep teardown debt separate |
