...
| 5 | `ROAD-V1-C5` | Sliding window, local attention, YaRN/long context | [engine matrix](engine-matrix.md), [coverage view §§2,11](feature-matrix.md#2-kv-cache--memory), [joint spike](specs/sliding-local-yarn-long-context.md) | accepted complete KV/manager/backend/dependency and RoPE formula/cache/apply chains; W1-W7 are implemented and `GATING`; W5-W7 carry twelve pinned-source YaRN/MRoPE/Llama 3/Phi-3 LongRoPE fixtures plus CPU/sanitizer checkpoints; W8 `ATTN-ROPE-DYNAMIC-NTK` is active under `CLAIM-C5-DYNAMIC-1` | `ACTIVE` | complete both pinned W8 factor/alpha modes, precedence/error cases and oracle goldens without consuming the serving campaign's GPU; retain W1-W7 model/CUDA/trace/every-axis handoffs |
...
| C5 | Sliding-window KV/attention and YaRN long-context scaling | 🚧 [joint spike accepted](specs/sliding-local-yarn-long-context.md): W1-W7 are implemented/`GATING`; W5-W7 typed YaRN/MRoPE/Llama 3/Phi-3 LongRoPE cache construction passes twelve pinned-source CPU oracle fixtures and sanitizers; W8 dynamic-NTK is `ACTIVE`. CUDA compile/runtime and all model/trace/every-axis gates remain open; gate Qwen3.6 checkpoints are default interleaved MRoPE regressions, with separate feature-positive e2e gates |
