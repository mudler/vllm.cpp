...
`b8681ac` is `VOID`: all six timing legs completed and
provisionally put async ON/OFF total throughput at **1.001345×**, but direct
trace startup lacked the source root on `sys.path`, so neither required trace
exists. The bootstrap repair and a fresh complete run are `PENDING`. W3 remains
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. Current async root `b8681ac` is VOID after six timing legs because trace bootstrap failed; its provisional **1.001345×** total ratio earns no credit. Host PSS/RSS is a 22.920 GiB CPU mirror. Run the bootstrap-repaired complete diagnostic, gate the selected lever, repair direct-device loading, then close every 27B axis before 35B performance |
