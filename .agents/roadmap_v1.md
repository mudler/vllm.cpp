...
launches. Pushed `24cea4f` adds only those exact fingerprints, binds the
validator hash, revalidates the immutable root and writes accepted marker-last
`complete-structural` evidence.

Next: run the c2/c16 **40 timing + 8 memory** component. qkvz
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. Clean `7ff713e`, finalized by `24cea4f`, closes W1D3 structural evidence; c2/c16 remains before qkvz. Repair the 22.920 GiB host mirror before 35B performance |
