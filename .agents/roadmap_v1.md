...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | MVP operational closure: online latency and e2e/nightlies | [`SERVE-GATE-ONLINE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | online-gate spike accepted; first campaign invalid/incomplete; 35B exact-shape sanitizer diagnostic clean 3/3; second pre-W2 27B same-lock diagnostic completed both arms rc=0, but its 35B local arm completed 1/6 c1 requests then aborted with a surfaced CUDA illegal-memory-access error and is void; its vLLM arm continues only diagnostically; W2 live SSE implemented/CPU-gated; nightly unspiked | `GATING` | finish preserving the diagnostic vLLM arm, land the committed fail-closed harness, then build current W2 and run both-model G1/G3-G6 every-axis online campaigns under new whole-series locks; only post-W2 evidence can close TTFT/TPOT/ITL. Then spike `SERVE-E2E-NIGHTLY` |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 root takeover active: first campaign remains invalid diagnostic evidence; 35B exact-shape sanitizer diagnostic survived 3/3 with zero sanitizer reports; the complete 27B ours→vLLM series in `~/work/vllm.cpp-latency/latres2` is retained only as a pre-W2 diagnostic. Its second 35B local arm is void after completing 1/6 c1 requests and aborting with a surfaced CUDA illegal-memory-access error; the vLLM arm continues only to preserve diagnostics. `SERVE-ASYNC-LLM` has genuine live SSE and is CPU/TSan-gated; a committed fail-closed harness plus a fresh post-W2 both-model campaign own closure |
