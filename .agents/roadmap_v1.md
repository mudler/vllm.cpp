...
replay controller, pre-GPU target-build manifest and schema-v2 ours/vLLM
validators are implemented and CPU-gated. They bind unique Nsight sessions,
zero exit status, exact runtime-child correlation, the frozen plan/client
contracts and 1,476 clean vLLM decode windows. The immutable three-capture DGX
execution remains pending: clean `7bae38a` failed before build/GPU because the
DGX login `PATH` omitted Ninja, and the corrected recipe now pins the oracle
venv executable by absolute path. `3f256ab` still binds at 55/124 and no speed
credit follows.
...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact performance closure against the faster applicable vLLM v0.25.0/SGLang floor before broader roadmap implementation | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG-PREFIX`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-MAMBA-ALIGN`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-ASYNC-LLM`](engine-matrix.md), [`KERNEL-GEMM-NVFP4-W4A4`](kernel-matrix.md), [`KERNEL-ATTN-FA2`](kernel-matrix.md), [`KERNEL-GDN-AOT-BF16`](kernel-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | v0.25.0 target `702f481` is audited and immutable `3f256ab` binds at **55/124 axes pass, 69 fail**. W3-E/W3-F/W3-G strict-fail and earn no speed credit. [W3-H](specs/nvfp4-bf16-producer-vectorization.md) is `ACTIVE / trace-first`; H1a/H1b/H1c are `VOID`, with H1c both lossy and CUTLASS-disabled. H1d now implements the diagnostic-only four-replay controller, exact CUTLASS/configure/compile/binary manifest, unique-session/lossless SQLite contract, parsed frozen-plan/client evidence and clean vLLM decode-window validation. Focused tests **25/25** and a CUDA-off Release build pass. Clean `7bae38a` then failed before build/GPU solely because the login `PATH` omitted Ninja; the corrected pinned-Ninja immutable execution is pending. Binding speed stays unchanged, W3-H2 remains prohibited and SGLang remains source-audited/non-binding | `GATING` | execute immutable W3-H1d and accept only three zero-exit, lossless, exact-plan captures. Only a later 48/48 component authorizes the exact grid. Close all 124 axes before 35B performance, then execute the distinct SGLang prefix gate |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. W3-C frozen-map control passes; W3-E/W3-F/W3-G strict-fail and earn no speed credit. [W3-H normal BF16→FP4 vectorized I/O](specs/nvfp4-bf16-producer-vectorization.md) is **ACTIVE / trace-first**: H1a/H1b/H1c are `VOID`. H1d's build/profile controller and schema-v2 exact evidence validation are implemented and CPU-gated; its immutable DGX series is pending and every new performance ratio remains prohibited. Close every 27B axis before 35B performance; keep host-memory/teardown debt separate |
