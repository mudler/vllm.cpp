...
128 Stream-K + 80 static-persistent FP4 GEMMs. Committed finalizer `fe28003`
records `complete-diagnostic` and ranks the residual as BF16 CUTLASS GEMMs
(193 vs 97 launches, +2.864630 ms median) then RMSNorm/generated partitions
(177 vs 177, +1.810237 ms); FP4 GEMM time is already non-positive. The marker
is durable (status `9e0143fa…7b57`). Cross-profiler times earn no speed credit;
the exact launch structure selects the BF16-GEMM whole-chain spike, not code.
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. Accepted async `3812d8` is neutral. Finalized `179a0fc` c2 mapping is `complete-diagnostic`; no cross-profiler ratio supersedes the grid. Spike/gate BF16-GEMM launch parity, repair the 22.920 GiB host mirror, then close every 27B axis before 35B performance |
