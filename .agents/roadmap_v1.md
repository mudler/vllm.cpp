...
**Current order-0 substage (2026-07-13):** schema-v5 `a7f67c7` passes its exact
build, 27B correctness, and all four session-1 exports/validations/summaries.
Session 2 emits four reports and its exact stop marker, but the immediate
driver check races asynchronous Nsight log forwarding before export. The root
is `VOID`; the bounded liveness-aware wait is CPU-green and a fresh immutable
12-report DGX run is next. `3f256ab` remains 55/124, and W3-H2, the exact grid,
35B performance, and later portfolio stay blocked.
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. W3-C frozen-map control passes; W3-E/W3-F/W3-G strict-fail and earn no speed credit. [W3-H normal BF16→FP4 vectorized I/O](specs/nvfp4-bf16-producer-vectorization.md) is **ACTIVE / trace-first**: `a7f67c7` proves one complete four-report schema-v5 group but is VOID on a repaired stop-marker forwarding race in session 2. Fresh 12-report DGX evidence is next and no new ratio exists. Close every 27B axis before 35B performance and keep host-memory/teardown debt separate |
