...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | MVP operational closure: online latency and e2e/nightlies | [`SERVE-GATE-ONLINE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | online-gate spike accepted; first campaign invalid/incomplete; 35B exact-shape sanitizer diagnostic clean 3/3; second pre-W2 27B same-lock ours→vLLM baseline completed all points with both arms rc=0, while its 35B series waits behind queued PR3 work; W2 live SSE implemented/CPU-gated; nightly unspiked | `GATING` | preserve/finish the 35B pre-W2 baseline after queued PR3 work, then build current W2 in the first GPU-idle window and run both-model G1/G3-G6 every-axis online campaigns; only that post-W2 evidence can close TTFT/TPOT/ITL. Then spike `SERVE-E2E-NIGHTLY` |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 root takeover active: first campaign remains invalid diagnostic evidence; 35B exact-shape sanitizer diagnostic survived 3/3 with zero sanitizer reports; the complete 27B ours→vLLM series in `~/work/vllm.cpp-latency/latres2` is retained as a pre-W2 baseline and its 35B continuation waits behind queued PR3 work. `SERVE-ASYNC-LLM` now has genuine live SSE and is CPU/TSan-gated, so true TTFT/TPOT/ITL closure moves to the required post-W2 both-model GPU campaign after this baseline finishes |
