...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact production-vLLM performance closure, online latency, and e2e/nightlies | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-ASYNC-LLM`](engine-matrix.md), [`KERNEL-GEMM-NVFP4-W4A4`](kernel-matrix.md), [`KERNEL-GDN-AOT-BF16`](kernel-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | Clean pushed-`b5c6e4f` 27B online evidence validates all 12 groups, 2,016/2,016 requests, six returns, model gate and traces, but is below the all-axis floor: c1→c32 median total ratios **0.9933/0.9520/0.9657/0.9760/1.0213/1.0218×**, only **4/4/5/4/17/14 of 20** performance axes and **2/4** memory axes pass. W2 improves every point and wins c16/c32 throughput, but c1-c8 and mean TPOT/ITL remain red. Its paired trace shows vLLM's 128x32x256 Stream-K/static pair at about 25.1% of captured kernel time while our planner predominantly selects 128x128x128/256x128x128 despite all 32 tactics being present. Runtime grounding confirms pip-vLLM pre-tunes in memory with a 1-ms GPU-delayed eager event window and disables its colliding file cache. W3-A stages that timing mirror with a W2 fallback; clean GB10 gates/A-B remain pending. The correctness-required 27B BF16 GDN output default remains; 35B keeps f32. W0/W1 device-residency wins and inherited pool debt remain; 35B/nightly are deferred | `GATING` | clean-build and same-binary gate W3-A delayed/off selection, then implement W3-B pre-serve all-bucket in-memory warmup; gate optional collision-complete W3-C persistence separately, prove the oracle-selected narrow pair and repeat exact 27B until every axis passes; only then run 35B, repair teardown, and spike `SERVE-E2E-NIGHTLY` |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 active and below floor: exact clean pushed-`b5c6e4f` 27B evidence validates 12/12 groups and all lifecycle/trace preconditions, but c1→c32 total ratios are 0.9933/0.9520/0.9657/0.9760/1.0213/1.0218×, only 4/4/5/4/17/14 of 20 performance axes and 2/4 memory axes pass. W2's 32 tactics, merged scaling and fused activation quantization materially improve the grid, while the trace localizes the residual to runtime selection: vLLM resolves 128x32x256 Stream-K/static and ours predominantly chooses wide tactics. W3-A's exact production delayed-event timing mirror is staged/GPU-pending with a same-binary off arm; W3-B pre-serve in-memory bucket tuning follows. The oracle disables its file cache, so W3-C persistence is separate. Close every exact 27B axis before 35B; W0/W1 device-residency wins and inherited zero-leak debt remain open; later roadmap work is blocked |
