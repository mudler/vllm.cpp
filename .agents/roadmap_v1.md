...
that gap. Current root `2ec6dda` is `VOID` after ON-r1 only: 6/6 requests
passed, then post-run validation rejected vLLM's six empty error strings before
an OFF arm. The fresh validator repair is `PENDING`. W3 remains `READY`, not credited or claimed for implementation.
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. Current async root `2ec6dda` is VOID after ON-r1 only; fresh validator repair is pending and the one-arm number earns no credit. Host PSS/RSS is a 22.920 GiB CPU mirror. Run the repaired diagnostic, gate the selected lever, repair direct-device loading, then close every 27B axis before 35B performance |
