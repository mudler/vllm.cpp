...
| 10 | `ROAD-V1-D1` | NVIDIA target fan-out, ROCm, MLX, Vulkan, XPU, ANE | [backend matrix](backend-matrix.md), [backends strategy](backends.md) | 13 CUDA targets, component rules, platforms and native floors inventoried; SGLang preflight P1 CPU harness/dry-run is `ACTIVE` under `CLAIM-BACKEND-SGLANG-PREFLIGHT-1`, while image/model P2 and binding `BACKEND-GATE-CUDA-SGLANG` evidence remain open | `PARTIAL` | complete and CPU-gate the deterministic corpus/client/summary/memory/dry-run harness without touching dgx; provision only after the active benchmark window, then run exact-checkpoint P2 |
