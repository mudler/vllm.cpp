...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 v0.25.0 re-baseline active. Old-oracle `b5c6e4f` is historical and `3cc490c` is VOID. Immutable `9cc7191` source/build/oracle/exact-corpus preflight is complete; its model gate, full 27B grid and paired traces are active but unexecuted, so no ratio binds. Close every 27B axis before 35B and keep optional W3-C plus teardown debt separate |
