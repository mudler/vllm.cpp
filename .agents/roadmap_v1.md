...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | MVP operational closure: online latency and e2e/nightlies | [`SERVE-GATE-ONLINE`](engine-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | `31d053f` completed all three 27B paired c1–c32 ladders with 2,016 exact-count requests and six memory-return checks. Mean throughput ratios were 0.959/0.926/0.934/0.937/0.961/0.956×, with most latency axes also below floor. Ours remained within +0.14%/-0.63% of its accepted offline c16/c32 checkpoints while vLLM's changed online recipe ran +4.92%/+4.65%, so the responsible online-chain layer is not yet localized. `d4ddeb1` captured the vLLM trace after the Ninja repair but failed its over-strong FP4 digest-equality postcondition; the 31d grid had only 4/2,016 exact text matches despite a passing model gate and exact native counts. The contract now records output repeatability diagnostically behind those correctness preconditions. Nightly remains unspiked | `GATING` | merge the output-contract repair, recapture the trace, distinguish scheduling/config/frontend effects from model execution, mirror the highest-value differences, then rerun the full 27B every-axis gate before 35B. Only complete passing evidence can close both serving rows; then spike `SERVE-E2E-NIGHTLY` |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 active: `31d053f` completed the repeated 27B grid and showed 0.926–0.961× online throughput without a direct-library regression. `d4ddeb1` then captured the vLLM trace but rejected legitimate FP4 output-repeatability drift; exact-match counts are now diagnostic behind the model/count gates. Merge and recapture the trace, localize the online-chain difference, then run a fresh optimized 27B gate before 35B. No existing online ratio is binding |
