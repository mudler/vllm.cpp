...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | MVP operational closure: online latency and e2e/nightlies | [`SERVE-GATE-ONLINE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | online-gate harness `b0acec2` fixes exact corpus/pip oracle/build/resources/traces and matched c32 scheduler budgets; CPU gates 16/16 focused, 32/32 full tools, CTest 2/2. Fresh sm_121a build and both model tests pass; two no-GPU preflights caught the corpus package invocation and ambient-interpreter dependency, now pinned to the vLLM environment's Python module command. Both pre-W2 campaigns remain closed/invalid; W2 live SSE is CPU-gated; nightly unspiked | `GATING` | merge the final preparation repair, regenerate the SHA-bound plan/corpora, then run 27B and 35B G1/G3-G6 campaigns with hashed paired traces under separate whole-model locks. Only post-W2 evidence can close the row; then spike `SERVE-E2E-NIGHTLY` |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 root takeover active: fail-closed harness merged at `b0acec2` (16 focused / 32 full CPU tests, CTest 2/2). Both pre-W2 campaigns are terminal/invalid. Fresh sm_121a build and 35B/27B model tests pass; two no-GPU preflights exposed the clean-shell package and ambient-Python dependency defects before measurement, and corpus generation is now tied to the pinned oracle Python. W2 has genuine live SSE and is CPU/TSan-gated; regenerated current-main GPU evidence owns closure |
