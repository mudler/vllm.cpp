...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact production-vLLM performance closure, online latency, and e2e/nightlies | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-ASYNC-LLM`](engine-matrix.md), [`KERNEL-GEMM-NVFP4-W4A4`](kernel-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | Exact pushed-`4e1d8ca` 27B online evidence validates all 12 groups but is below the all-axis floor: c1→c32 median total ratios **0.9661/0.9274/0.9378/0.9466/0.9808/0.9910×**, only **0/2/5/3/3/5 of 20** performance axes and **2/4** memory axes pass. The fixed HTTP pool is healthy/steady-state-neutral. W1 implements exact hybrid buckets, complete key, single-flight publication and capture-miss rejection behind `VT_FP4_EXACT_BUCKETS=0`; Release 100/100, TSan 615/615 and pushed-`1a802ac` default/legacy sm_121a focused runs pass. Its expanded real capture case is staging-green but still needs the immutable capture/memcheck/model/performance gates. Direct-c16/oracle traces remain the causal before-state and expose the separate absent 32-tactic family reserved for W2. W0/W1 device-residency wins and inherited pool debt remain; 35B/nightly are deferred | `GATING` | push the capture-test checkpoint and run its immutable sm_121a/capture/memcheck/model gates, then W1 component A/B + trace + exact 27B oracle campaign. Only after that classification separately port/measure the full tactic family; repeat until every axis passes, then run 35B, repair teardown, and spike `SERVE-E2E-NIGHTLY` |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 active and below floor: exact pushed-`4e1d8ca` 27B evidence validates 12/12 groups, but c1→c32 total ratios are 0.9661/0.9274/0.9378/0.9466/0.9808/0.9910×, only 0/2/5/3/3/5 of 20 performance axes and 2/4 memory axes pass. The fixed HTTP series is healthy and steady-state-neutral. W1 from the accepted [FP4 spike](specs/nvfp4-small-m-dispatch.md) now implements exact cache identity/single-flight/capture-miss handling and is CPU/TSan-green; GPU and performance gates remain. Direct c16 retuning matches vLLM mean TPOT and the oracle trace contains absent narrow static-persistent/Stream-K tactics reserved for separate W2. Re-rank residual projection/attention/scheduler gaps and close every 27B axis before running 35B. W0/W1 component wins and inherited zero-leak debt remain open context; later roadmap work is blocked |
