...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact production-vLLM performance closure, online latency, and e2e/nightlies | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-ASYNC-LLM`](engine-matrix.md), [`KERNEL-GEMM-NVFP4-W4A4`](kernel-matrix.md), [`KERNEL-GDN-AOT-BF16`](kernel-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | Clean pushed-`b5c6e4f` remains below the all-axis floor: c1→c32 median totals **0.9933/0.9520/0.9657/0.9760/1.0213/1.0218×**, **4/4/5/4/17/14 of 20** performance and **2/4** memory axes. W2 improves every point but the trace shows vLLM's 128x32x256 Stream-K/static pair where local selection had been wide. W3-A `71f1e89` mirrors pip-vLLM's 1-ms delayed timing and is correctness/safety green. Its c16 delayed/off means are **809.932/803.379 tok/s = 1.008156×**, but strict acceptance fails at **13/20 timing** and **2/4 memory**; only **5/35** delayed keys retain one tactic ID across all fresh processes. No vLLM denominator changes. W3-B at clean `d7cdf66` is immutable build/correctness/safety-green: a 2,048-token request materializes 80/80 profiles (16 buckets × five real shapes) before HTTP readiness, with zero lazy misses, 16/16 correctness and 24,586/24,586 memcheck assertions. Its repeated prewarm/lazy c16 component is steady-state neutral at **808.457/808.220 tok/s = 1.000293×** and strict-fails **15/20 timing, 2/4 memory**. Only **20/80** prewarmed tactic IDs are stable, although first-use first-chunk latency improves from **5.662 s to 0.779 s**. Shipping prewarm stays without speed credit. Trace attempt 1 is **VOID**: prewarm completed 144/144, but mutable client results changed cache inventory 50→58 and stopped lifecycle validation before lazy/vLLM; no kernel result is claimed. Corrected paired trace and exact-oracle performance remain open. The correctness-required 27B BF16 GDN default remains and 35B keeps f32. W0/W1 device-residency wins and inherited pool debt remain; 35B/nightly are deferred | `GATING` | rerun W3-B's corrected corpus-only-root paired trace, select the next execution-grounded lever, then repeat exact 27B until every axis passes. Gate optional W3-C persistence separately, then run 35B, repair teardown, and spike `SERVE-E2E-NIGHTLY` |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 active and below floor: exact clean pushed-`b5c6e4f` 27B evidence validates 12/12 groups and all lifecycle/trace preconditions, but c1→c32 total ratios are 0.9933/0.9520/0.9657/0.9760/1.0213/1.0218×, only 4/4/5/4/17/14 of 20 performance axes and 2/4 memory axes pass. W2's trace localizes the residual to runtime selection. W3-A `71f1e89` is correctness/safety green and +0.816% in mean c16 total throughput, but strict-fails 13/20 timing and 2/4 memory with only 5/35 delayed keys stable. W3-B's shared-loader all-bucket implementation is immutable build/correctness/safety-green at `d7cdf66`: 80/80 profiles, completion-before-listening, zero lazy misses, 16/16 correctness and zero memcheck errors. Its repeated prewarm/lazy component is only **1.000293×**, strict-fails **15/20 timing and 2/4 memory**, and leaves **20/80** tactic IDs stable; first-use improves, so shipping prewarm stays without speed credit. Trace attempt 1 is VOID after prewarm-only 144/144 because its cache inventory changed 50→58; lazy/vLLM did not run and no trace result counts. Corrected paired trace and exact-oracle performance remain open, and the oracle-disabled W3-C file cache is separate. Close every exact 27B axis before 35B; W0/W1 device-residency wins and inherited zero-leak debt remain open; later roadmap work is blocked |
