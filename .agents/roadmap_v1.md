...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | MVP operational closure: online latency and e2e/nightlies | [`SERVE-GATE-ONLINE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | online-gate spike accepted; committed fail-closed exact-corpus/pip-oracle/build/stream/memory/thermal/trace/every-axis harness fixes identical c32 scheduler capacity and per-model token budgets on both arms and is CPU-gated 16/16 focused, 32/32 full tools, CTest 2/2; first campaign invalid; second pre-W2 27B diagnostic complete, but its 35B local arm completed 1/6 c1 then aborted with a surfaced CUDA illegal-memory-access error and is void while vLLM diagnostics continue; W2 live SSE implemented/CPU-gated; nightly unspiked | `GATING` | finish preserving the diagnostic vLLM arm, deploy/build current W2, reproduce the 35B fault under sanitizer if present, then run both-model G1/G3-G6 campaigns with hashed paired traces under new whole-series locks. Only post-W2 evidence can close the row; then spike `SERVE-E2E-NIGHTLY` |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 root takeover active: the committed harness now freezes exact corpora, pins/hashes the pip-vLLM 0.24.0 runtime, refreshes targets from a clean exact HEAD, fixes identical c32 sequence capacity and model-specific token budgets, interleaves three ours/vLLM repetitions, captures load+runtime memory/thermal/memory-return and fully hashed ours-nsys/vLLM-torch trace workloads, and cannot pass on missing/hash-drifted/below-floor evidence (16 focused / 32 full CPU tests, CTest 2/2). The first campaign remains invalid; second 27B is pre-W2 diagnostic only; second 35B local is void after 1/6 c1 + CUDA abort while its vLLM diagnostic arm continues. W2 has genuine live SSE and is CPU/TSan-gated; fresh current-main GPU evidence owns closure |
