...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact production-vLLM performance closure, online latency, and e2e/nightlies | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-ASYNC-LLM`](engine-matrix.md), [`KERNEL-GEMM-NVFP4-W4A4`](kernel-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | Exact pushed-`4e1d8ca` 27B online evidence validates all 12 groups but is below the all-axis floor: c1→c32 median total ratios **0.9661/0.9274/0.9378/0.9466/0.9808/0.9910×**, only **0/2/5/3/3/5 of 20** performance axes and **2/4** memory axes pass. The fixed `max_num_seqs + 4` worker pool completes all fresh c32 legs without an unread socket; its separate fixed/legacy c32 AB/BA/AB is steady-state-neutral at **0.999764×**, 8/20 axes, with all 1,152 requests/six memory returns and no sampled legacy stall. Direct-c16 traces still prove FP4 M=1/2/4/8/16 key aliasing and match vLLM TPOT after retuning; vLLM also selects absent 128×32×256 Stream-K/static-persistent tactics. The accepted [FP4 repair spike](specs/nvfp4-small-m-dispatch.md) is now claimed for W1 exact buckets/single-flight; the 32-tactic family remains a separate measured W2. W0/W1 wins and inherited pool debt remain; 35B/nightly are deferred | `GATING` | implement and gate FP4 W1, repeat the exact 27B oracle campaign, then separately port/measure the full tactic family. Repeat 27B after every iteration until every axis passes, then re-rank residuals, run 35B, repair teardown, and spike `SERVE-E2E-NIGHTLY` |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 active and below floor: exact pushed-`4e1d8ca` 27B evidence validates 12/12 groups, but c1→c32 total ratios are 0.9661/0.9274/0.9378/0.9466/0.9808/0.9910×, only 0/2/5/3/3/5 of 20 performance axes and 2/4 memory axes pass. The fixed HTTP series is healthy and steady-state-neutral, so the confirmed next repair is small-M FP4 cache identity/single-flight; direct c16 retuning matches vLLM mean TPOT and the oracle trace contains absent narrow static-persistent/Stream-K tactics. W1 from the accepted [FP4 spike](specs/nvfp4-small-m-dispatch.md) is active; the traced 32-tactic surface remains a separate W2 with its own A/B and oracle rerun. Re-rank residual projection/attention/scheduler gaps and close every 27B axis before running 35B. W0/W1 component wins and inherited zero-leak debt remain open context; later roadmap work is blocked |
