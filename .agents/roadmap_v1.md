...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | MVP operational closure: online latency and e2e/nightlies | [`SERVE-GATE-ONLINE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | online harness fixes exact corpus/oracle/resources/traces/c32 budgets; online-focused 18/18, full tools 34/34, CTest tools 1/1. Fresh sm_121a model gates pass; exact `4e7aa12` corpora were generated and its 27B gate passed before the root-only global-cache assumption stopped execution pre-server. Rootless replacement hashes the complete file inventory, applies `DONTNEED`, requires `mincore` zero residency, and passed a real 49-file/26.55-GB probe. Both pre-W2 campaigns remain invalid; nightly unspiked | `GATING` | merge the cache-evidence repair, regenerate the SHA-bound corpora, then run 27B and 35B G1/G3-G6 campaigns with hashed paired traces under separate whole-model locks. Only post-W2 evidence can close the row; then spike `SERVE-E2E-NIGHTLY` |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 active: harness CPU tools 34/34 and CTest 1/1. Both pre-W2 campaigns remain invalid. Fresh model gates pass; `4e7aa12` generated both exact corpora and passed the 27B gate, then stopped before server start because global cache dropping needs sudo. The rootless replacement proves zero resident pages over a hashed complete file inventory (real 26.55-GB probe passed). W2 is CPU/TSan-gated; regenerated current-main GPU evidence owns closure |
