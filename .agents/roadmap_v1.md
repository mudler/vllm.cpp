...
scheduler is the only unmeasured structural difference large enough to explain
that gap. The first exact async ON/OFF attempt (`4d85ead`) is `VOID` before
requests because the spawned EngineCore could not find `ninja`; the fresh
venv/CUDA-PATH repair is `PENDING`. W3 remains `READY`, not credited or claimed for implementation.
...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact performance closure against the faster applicable vLLM v0.25.0/SGLang floor before broader roadmap implementation | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG-PREFIX`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-MAMBA-ALIGN`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-ASYNC-LLM`](engine-matrix.md), [`KERNEL-GEMM-NVFP4-W4A4`](kernel-matrix.md), [`KERNEL-ATTN-FA2`](kernel-matrix.md), [`KERNEL-GDN-AOT-BF16`](kernel-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | v0.25.0 target `702f481` is audited and `3f256ab` binds at **55/124**. The residual scan selects a c2 async ON/OFF oracle control; first `4d85ead` setup is VOID before requests on missing EngineCore `ninja`, and the fresh PATH-repaired run is pending. W3-I remains off/no-credit; host PSS/RSS is grounded in a **22.920 GiB** retained CPU mirror. No SGLang or newer binding result exists | `GATING` | run the fresh uncontended PATH-repaired c2 async control; promote W3 only if its measured credit explains the gap, otherwise capture low-batch kernels. Then gate the selected lever and repair direct-device weight ownership before the exact grid |
...
| 6 | `ROAD-V1-C6` | Priority and async/overlap scheduling + AsyncLLM streaming serving — RE-PRIORITIZED 2026-07-10: `SERVE-ASYNC-LLM` re-promoted T1→T0 because it blocks order 0 and async scheduling is vLLM's default at the pin (B3) | [engine matrix](engine-matrix.md), [coverage view §1](feature-matrix.md#1-engine-core--scheduling), [async-serving spike](specs/async-serving.md) | W1 `ENG-CORE-BUSY-LOOP`, W2 `SERVE-ASYNC-LLM`, and W4 `ENG-PRIORITY-SCHED` are implemented/`GATING`; W3 `ENG-ASYNC-SCHED` remains `READY`. Explicit oracle modes are implemented, but first `4d85ead` is VOID before requests on missing EngineCore `ninja`; fresh PATH-repaired timing + trace credit is `PENDING`, so no W3 implementation claim exists | `PARTIAL` | execute the repaired order-0 async control. If positive at the size needed, claim/port W3 with graph-compatible copy events and GPU-resident sampled-token state; otherwise keep W3 as parity work and return the speed track to low-batch kernel mapping |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. First c2 async control root `4d85ead` is VOID before requests on missing EngineCore `ninja`; fresh venv/CUDA-PATH repair is pending, not speed credit. Host PSS/RSS is grounded in a 22.920 GiB retained CPU mirror. Run the repaired diagnostic, gate the selected lever, repair direct-device loading, then close every 27B axis before 35B performance |
