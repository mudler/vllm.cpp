...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact performance closure against the faster applicable vLLM v0.25.0/SGLang floor before broader roadmap implementation | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG-PREFIX`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-MAMBA-ALIGN`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-ASYNC-LLM`](engine-matrix.md), [`KERNEL-GEMM-NVFP4-W4A4`](kernel-matrix.md), [`KERNEL-GDN-AOT-BF16`](kernel-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | v0.25.0 target `702f481` is audited and the canonical DGX oracle is active. Immutable `9cc7191` completed all 36 27B groups and six returns but only **54/124 axes** pass. W3-D c16 is 1.005049× mean-positive but strict-fails at 14/20 timing +2/4 memory; its post-pack trace closes the topology at **208.192 versus vLLM 208 FP4 GEMMs/forward**. A fresh `3f256ab` exact root is now **ACTIVE/PENDING** with validated plan/corpus only: 0/36 groups, 0/2,016 requests, 0/6 returns and no trace. Thus `9cc7191` still binds and 35B remains held. The SGLang DGX claim remains source-audited and non-binding | `GATING` | execute the one-lock exact grid, re-rank the post-pack residual and continue the trace-driven repair loop until all 124 axes pass; next execute PX1 plus `KV-MAMBA-ALIGN` PX2 for SGLang shared-prefix parity, and run 35B only after every applicable 27B cache-off/cache-on axis passes |
