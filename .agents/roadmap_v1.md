...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | MVP operational closure: online latency and e2e/nightlies | [`SERVE-GATE-ONLINE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | online-gate spike accepted; first campaign invalid/incomplete; 35B exact-shape sanitizer diagnostic clean 3/3; second pre-W2 27B same-lock ours→vLLM baseline active; W2 live SSE implemented/CPU-gated; nightly unspiked | `GATING` | preserve/finish the running pre-W2 baseline, then build W2 in the first GPU-idle window and run both-model G1/G3-G6 every-axis online campaigns; only that post-W2 evidence can close TTFT/TPOT/ITL. Then spike `SERVE-E2E-NIGHTLY` |
| 1 | `ROAD-V1-C1` | Drop-in kernel ABI + complete kernel-family parity | [`BACKEND-ABI-VT`](backend-matrix.md), [kernel matrix](kernel-matrix.md) | exhaustive kernel/dependency inventory and [raw-pointer adapter ABI](specs/dropin-kernel-abi.md) accepted; additive W0 implemented and CPU 94/94, with CUDA/cross-build + scalar-forwarder/backend-shim debts explicit | `PARTIAL` | finish W0 GPU/cross-build gates after the serving lock releases, then migrate and independently gate one kernel family at a time |
| 2 | `ROAD-V1-C2` | Model families: Llama/Qwen3/Mistral, MoE, Qwen3-Next | [model matrix](model-matrix.md) | all 353 static architecture IDs inventoried; current Qwen wrappers are partial; factory leaf spiked ([model-factory-registry.md](specs/model-factory-registry.md)) — `MODEL-FACTORY-registry` READY | `PARTIAL` | claim `MODEL-FACTORY-registry` (central registry + reject-unknown + live `IsDenseArch` replacement), then spike/claim Llama dense as the first family |
| 3 | `ROAD-V1-C3` | MTP k=1 + GDN speculative path, then DFlash | [engine matrix](engine-matrix.md), [coverage view §8](feature-matrix.md#8-speculative-decoding) | MTP and DFlash specs written; M-mtp-0 dense/MoE safetensors loaders + standalone forward and ported CPU tests complete; oracle tool/runner locally validated | `ACTIVE` | run M-mtp-0 exact head parity on both gate checkpoints in the first GPU window after `CLAIM-SERVE-GATE-1`, then claim M-mtp-1 scheduler/rejection/GDN-spec integration |
...
| 5 | `ROAD-V1-C5` | Sliding window, local attention, YaRN/long context | [engine matrix](engine-matrix.md), [coverage view §§2,11](feature-matrix.md#2-kv-cache--memory), [joint spike](specs/sliding-local-yarn-long-context.md) | accepted: complete KV/manager/backend/dependency and RoPE formula/cache/apply chains; 8 claimable leaves, upstream-test inventory, model/hardware dependencies and every-axis gates fixed | `READY` | claim the dependency-free KV spec leaves and `ATTN-YARN` foundation; attention e2e follows its KV leaf, and Llama3/LongRoPE/dynamic leaves parallelize after the common RoPE API |
| 6 | `ROAD-V1-C6` | Priority and async/overlap scheduling + AsyncLLM streaming serving — RE-PRIORITIZED 2026-07-10: `SERVE-ASYNC-LLM` re-promoted T1→T0 because it blocks order 0 and async scheduling is vLLM's default at the pin (B3) | [engine matrix](engine-matrix.md), [coverage view §1](feature-matrix.md#1-engine-core--scheduling), [async-serving spike](specs/async-serving.md) | W1 `ENG-CORE-BUSY-LOOP`, W2 `SERVE-ASYNC-LLM`, and W4 `ENG-PRIORITY-SCHED` implemented and `GATING`: W2 full CPU 94/94 + focused TSan 5/5; all GPU gates deferred while the baseline campaign owns dgx. W3 `ENG-ASYNC-SCHED` remains READY | `PARTIAL` | first GPU-idle window: W1 G1/G4 + W2 G1/G3/G4/G6 + W4 G1, using post-W2 binaries and fresh vLLM denominators; then implement W3 async overlap/default-ON mirror |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 root takeover active: first campaign remains invalid diagnostic evidence; 35B exact-shape sanitizer diagnostic survived 3/3 with zero sanitizer reports; the 27B ours→vLLM series in `~/work/vllm.cpp-latency/latres2` is retained as a pre-W2 baseline. `SERVE-ASYNC-LLM` now has genuine live SSE and is CPU/TSan-gated, so true TTFT/TPOT/ITL closure moves to the required post-W2 both-model GPU campaign once the current lock releases |
...
| C5 | Sliding-window KV/attention and YaRN long-context scaling | 🟢 [joint spike accepted](specs/sliding-local-yarn-long-context.md): 8 claimable leaves `READY`; gate Qwen3.6 checkpoints are default interleaved MRoPE regressions, with separate feature-positive e2e/oracle gates |
| C6 | Priority + async/overlap scheduling + AsyncLLM streaming serving | 🚧 joint spike written: [async-serving.md](specs/async-serving.md); W1/W2/W4 implemented and `GATING`, with W2 live SSE removing the structural fake-streaming blocker; post-W2 GB10 online gates and W3 default-ON overlap remain; [feature matrix §1](feature-matrix.md#1-engine-core--scheduling) |
