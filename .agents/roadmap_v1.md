...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | MVP operational closure: online latency and e2e/nightlies | [`SERVE-GATE-ONLINE`](engine-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | exact corpus/oracle/resources/traces/c32 contracts are CPU-green. `SERVE-STREAM-USAGE` is CPU/sanitizer-gated and the interrupted `a40a9e3` 27B run proved exact native counts across 1,488 retained requests. Two full paired ladders plus ours rep3 through c16 completed, but the validator then rejected 12 upstream-legal producer-ahead merged DELTAs; the result is void. The inter-chunk contract is corrected and regression-tested. No online ratio is claimed; nightly remains unspiked | `GATING` | merge the validator checkpoint, regenerate SHA-bound corpora/evidence, then run 27B and 35B G1/G3-G6 under separate whole-model locks. Only complete evidence can close both serving rows; then spike `SERVE-E2E-NIGHTLY` |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 active: focused client 8/8. All partial campaigns remain invalid. `a40a9e3` proved native counts but stopped after c16-r3 because the validator rejected pinned-vLLM's legal producer-ahead DELTA merge (12/96 requests retained 126 inter-chunk intervals for 128 native tokens). The contract now accepts merged deltas while rejecting extra intervals and count drift; regenerate current-main evidence, then rerun 27B followed by 35B. No existing online ratio is binding |
