...
| 5 | `ROAD-V1-C5` | Sliding window, local attention, YaRN/long context | [engine matrix](engine-matrix.md), [coverage view §§2,11](feature-matrix.md#2-kv-cache--memory), [joint spike](specs/sliding-local-yarn-long-context.md) | accepted complete KV/manager/backend/dependency and RoPE formula/cache/apply chains; `KV-SLIDING-WINDOW-SPEC` W1 is implemented and `GATING`; `ATTN-SLIDING-WINDOW` W2 is `ACTIVE` under `CLAIM-C5-SW-ATTN-1`; six other leaves remain `READY` | `ACTIVE` | complete W2 generic window plumbing and upstream-derived CPU/CUDA/FA2 operator gates without contending with the active serving campaign; then restore the pinned oracle and run model-positive G4/G6-G9 after dgx is released |
...
| C5 | Sliding-window KV/attention and YaRN long-context scaling | 🚧 [joint spike accepted](specs/sliding-local-yarn-long-context.md): W1 KV is implemented/`GATING`, W2 attention is `ACTIVE`, and six leaves remain `READY`; gate Qwen3.6 checkpoints are default interleaved MRoPE regressions, with separate feature-positive e2e/oracle gates |
