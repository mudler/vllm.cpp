...
**Current order-0 substage (2026-07-13):** schema-v5 `b8c8086` passes its exact
build, 27B correctness, frozen plans, clients, shutdown, and first report's
event/completion/topology reconciliation. It then fails closed because the
secondary kernel-summary command omitted the model contract, before sessions
2/3 or vLLM. The root is `VOID`; the CLI/helper/driver/reconstruction repair is
CPU-green and a fresh immutable 12-report DGX run is next. `3f256ab` remains
55/124, and W3-H2, the exact grid, 35B performance, and later portfolio stay
blocked.
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. W3-C frozen-map control passes; W3-E/W3-F/W3-G strict-fail and earn no speed credit. [W3-H normal BF16→FP4 vectorized I/O](specs/nvfp4-bf16-producer-vectorization.md) is **ACTIVE / trace-first**: `b8c8086` proves report-1 schema-v5 reconciliation but is VOID because the secondary summary omitted the model key. Its repair is CPU-green; fresh 12-report DGX evidence is next and no new ratio exists. Close every 27B axis before 35B performance and keep host-memory/teardown debt separate |
