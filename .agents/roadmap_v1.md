...
The post-W3-I cross-stack scan is complete. At c2 ours has better TTFT but
**6.1% slower TPOT** (114.841 vs 108.274 ms); vLLM's default-ON, depth-2 async
scheduler remains the selected structural control for that gap. Current root
`9b1774c` is `VOID`: all six timing legs completed and provisionally put async
ON/OFF total throughput at **1.000464×**. ON trace capture also completed, but
the driver applied the 48-prompt H1d exact-count validator to the six-prompt c2
trace and stopped before OFF. Shape-neutral aggregation and a fresh complete
run are `PENDING`. W3 remains
`READY`, not credited or claimed for implementation.
Separately, exact loader accounting identifies a persistent **22.920 GiB** CPU
weight mirror plus load-time mmap residency as the host PSS/RSS root cause.
...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact performance closure against the faster applicable vLLM v0.25.0/SGLang floor before broader roadmap implementation | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG-PREFIX`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-MAMBA-ALIGN`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-ASYNC-LLM`](engine-matrix.md), [`KERNEL-GEMM-NVFP4-W4A4`](kernel-matrix.md), [`KERNEL-ATTN-FA2`](kernel-matrix.md), [`KERNEL-GDN-AOT-BF16`](kernel-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | v0.25.0 target `702f481` is audited and `3f256ab` binds at **55/124**. Current async root `9b1774c` completed six timing legs at provisional **1.000464×** ON/OFF total and captured ON, but is VOID because the H1d exact-count validator stopped before shape-neutral summary/OFF; corrected rerun pending. W3-I remains off/no-credit; host PSS/RSS is a **22.920 GiB** CPU mirror. No SGLang or newer binding result exists | `GATING` | run the fresh uncontended shape-neutral c2 async control; if the provisional neutral result repeats, capture and map low-batch kernels. Then gate the selected lever and repair direct-device weight ownership before the exact grid |
...
| 6 | `ROAD-V1-C6` | Priority and async/overlap scheduling + AsyncLLM streaming serving — RE-PRIORITIZED 2026-07-10: `SERVE-ASYNC-LLM` re-promoted T1→T0 because it blocks order 0 and async scheduling is vLLM's default at the pin (B3) | [engine matrix](engine-matrix.md), [coverage view §1](feature-matrix.md#1-engine-core--scheduling), [async-serving spike](specs/async-serving.md) | W1 `ENG-CORE-BUSY-LOOP`, W2 `SERVE-ASYNC-LLM`, and W4 `ENG-PRIORITY-SCHED` are implemented/`GATING`; W3 remains `READY`. Current `9b1774c` control is VOID despite six timing legs and ON capture because H1d exact-count validation stopped before OFF; provisional total credit is only **+0.05%**, shape-neutral aggregation is verified, and no W3 claim exists | `PARTIAL` | execute the corrected order-0 async control. If its provisional neutral result repeats, keep W3 as parity work and return the speed track to low-batch kernel mapping; only a complete positive result can promote W3 for speed |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. Current async root `9b1774c` is VOID after six timing legs and ON capture because H1d exact-count validation stopped before OFF; its provisional **1.000464×** total ratio earns no credit. Host PSS/RSS is a 22.920 GiB CPU mirror. Run the shape-neutral complete diagnostic, gate the selected lever, repair direct-device loading, then close every 27B axis before 35B performance |
