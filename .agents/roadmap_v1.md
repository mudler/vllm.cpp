...
| 5 | `ROAD-V1-C5` | Sliding window, local attention, YaRN/long context | [engine matrix](engine-matrix.md), [coverage view §§2,11](feature-matrix.md#2-kv-cache--memory), [joint spike](specs/sliding-local-yarn-long-context.md) | accepted complete KV/manager/backend/dependency and RoPE formula/cache/apply chains; W1-W4 local-attention leaves are implemented and `GATING`; four RoPE leaves remain `READY` | `GATING` | claim W5 `ATTN-YARN` for the typed RoPE factory/cache/apply foundation; W2/W4 runtime G4-G9 remain gated by the occupied serving GPU, restored oracle and positive-model prerequisites |
...
| C5 | Sliding-window KV/attention and YaRN long-context scaling | 🚧 [joint spike accepted](specs/sliding-local-yarn-long-context.md): W1-W4 local-attention leaves are implemented/`GATING`; four RoPE leaves remain `READY`, with W5 `ATTN-YARN` next. Runtime/model/oracle/trace/performance gates remain open; gate Qwen3.6 checkpoints are default interleaved MRoPE regressions, with separate feature-positive e2e/oracle gates |
