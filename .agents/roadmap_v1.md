...
CUTLASS and both exact tools before GPU work. `3f256ab` still binds at 55/124
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. W3-C frozen-map control passes; W3-E/W3-F/W3-G strict-fail and earn no speed credit. [W3-H normal BF16→FP4 vectorized I/O](specs/nvfp4-bf16-producer-vectorization.md) is **ACTIVE / trace-first**: H1a/H1b/H1c and the invalid-config `5f8fab1` attempt are `VOID`. H1d's repaired build/profile controller and schema-v2 exact-path validation await a fresh immutable DGX series; every new performance ratio remains prohibited. Close every 27B axis before 35B performance; keep host-memory/teardown debt separate |
