...
| 5 | `ROAD-V1-C5` | Sliding window, local attention, YaRN/long context | [engine matrix](engine-matrix.md), [coverage view §§2,11](feature-matrix.md#2-kv-cache--memory), [joint spike](specs/sliding-local-yarn-long-context.md) | accepted complete KV/manager/backend/dependency and RoPE formula/cache/apply chains; W1-W5 are implemented and `GATING`, including W5's six pinned-source YaRN/MRoPE oracle fixtures and CPU/sanitizer checkpoint; W6 `ATTN-ROPE-LLAMA3` is `ACTIVE` under `CLAIM-C5-LLAMA3-1`; W7-W8 remain `READY` | `ACTIVE` | port only the Llama 3 low/mid/high frequency formula, typed fields, factory case, tests and pinned oracle boundary goldens against W5's cache/apply seam |
...
| C5 | Sliding-window KV/attention and YaRN long-context scaling | 🚧 [joint spike accepted](specs/sliding-local-yarn-long-context.md): W1-W5 are implemented/`GATING`; W5 typed YaRN/MRoPE cache/apply passes six pinned-source CPU oracle fixtures and sanitizers; W6 Llama 3 is `ACTIVE`, W7-W8 remain `READY`. W5 CUDA compile/runtime and all model/trace/every-axis gates remain open; gate Qwen3.6 checkpoints are default interleaved MRoPE regressions, with separate feature-positive e2e gates |
