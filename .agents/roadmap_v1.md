...
production command and requires exactly one `--profile-control off`. It still
contributes no model, timing, memory, or speed result. Next: push this repair,
then execute that runner under one uncontended DGX lock and
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. Packed structure is accepted; the component invocation defect is test-first repaired at focused **45/45** and all-tools **127/127**. Push and execute a fresh-SHA run before qkvz; repair the 22.920 GiB host mirror before 35B performance |
