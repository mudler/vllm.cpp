...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | MVP operational closure: online latency and e2e/nightlies | [`SERVE-GATE-ONLINE`](engine-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | exact validator/corpus/oracle/resources/traces/c32 contracts are CPU-green. Clean `8289cbd` 27B ours rep1 remains void after its missing native usage frame. `SERVE-STREAM-USAGE` now implements completion/chat native-ID final+continuous frames, validation and force mode; CPU 105/105, focused 63 cases/658 assertions, API repeat 100/100, ASan+UBSan 3/3 and TSan 1/1 pass. No new GPU ratio is claimed; nightly remains unspiked | `GATING` | regenerate SHA-bound corpora/evidence, then run 27B and 35B G1/G3-G6 under separate whole-model locks. Only complete evidence can close both serving rows; then spike `SERVE-E2E-NIGHTLY` |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 active: tools 34/34, CTest 1/1. All partial campaigns remain invalid. `8289cbd` stopped fail-closed at the missing-usage c32 case. The dependency is now host-implemented and CPU/sanitizer-gated under `SERVE-STREAM-USAGE`; merge it, regenerate current-main evidence, then rerun 27B followed by 35B. No existing online ratio is binding |
