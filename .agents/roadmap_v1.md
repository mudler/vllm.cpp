...
| 6 | `ROAD-V1-C6` | Priority and async/overlap scheduling + AsyncLLM streaming serving — RE-PRIORITIZED 2026-07-10: `SERVE-ASYNC-LLM` re-promoted T1→T0 because it blocks order 0 and async scheduling is vLLM's default at the pin (B3) | [engine matrix](engine-matrix.md), [coverage view §1](feature-matrix.md#1-engine-core--scheduling), [async-serving spike](specs/async-serving.md) | W1 `ENG-CORE-BUSY-LOOP`, W2 `SERVE-ASYNC-LLM`, W4 `ENG-PRIORITY-SCHED` implemented/`GATING`; **W3 `ENG-ASYNC-SCHED` host-side machinery LANDED + CPU-gated 2026-07-16 (`ACTIVE`, `CLAIM-ASYNC-SCHED-W3`):** `AsyncScheduler` placeholders + depth-2 `step_with_batch_queue` + `async_scheduling` config resolution + `VT_ASYNC_SCHED` rollback, CPU ctest 108/108, sync path byte-identical. The runner device-input leaf (captures the ~3.25 ms/step idle) is the remaining DGX-gated piece; prior `3812d8` control freezes the c2 speed budget neutral (+0.215%) | `PARTIAL` | run the W3 runner-side leaf + DGX both-arms A/B; execute order-0 low-batch kernel mapping alongside |
