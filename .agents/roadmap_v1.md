...
**Current order-0 substage (2026-07-13):** schema-v5 `c498a413` is accepted
diagnostic evidence under validator `7112864` and status SHA
`84d15970…6e66`. It passes exact build/correctness provenance, all 12 lossless
local reports, and the paired vLLM trace. Fused SiLU→FP4 is the largest positive
mapped residual in 12/12 reports, displacing normal-producer W3-H2. `3f256ab`
remains 55/124. The [W3-I fused-producer spike](specs/nvfp4-fused-silu-producer.md)
is `ACTIVE`: the default-off I1 packed implementation passes dirty-root
CPU/CUDA/operator/sanitizer, both 27B oracle arms, 35B correctness and SASS
preflight. Its immutable paired trace and 48-axis component, exact grid, 35B
performance, and later portfolio remain open in that order.
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. W3-C frozen-map control passes; W3-E/W3-F/W3-G strict-fail and earn no speed credit. [W3-H](specs/nvfp4-bf16-producer-vectorization.md) H1d is accepted at status `84d15970…6e66`; its 12/12 ranking displaces normal H2. [W3-I](specs/nvfp4-fused-silu-producer.md) I1 is implemented/default-off and preflight-green, with immutable trace and performance still pending. Strictly gate I1, then close every 27B axis before 35B performance while keeping host-memory/teardown debt separate |
