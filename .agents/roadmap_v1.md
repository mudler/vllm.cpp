...
| 6 | `ROAD-V1-C6` | Priority and async/overlap scheduling + AsyncLLM streaming serving — RE-PRIORITIZED 2026-07-10: `SERVE-ASYNC-LLM` re-promoted T1→T0 because it now BLOCKS order 0 (fake-SSE finding) and async scheduling is vLLM's default at the pin (B3) | [engine matrix](engine-matrix.md), [coverage view §1](feature-matrix.md#1-engine-core--scheduling), [async-serving spike](specs/async-serving.md) | joint spike accepted: `SERVE-ASYNC-LLM`, `ENG-CORE-BUSY-LOOP`, `ENG-ASYNC-SCHED`, `ENG-PRIORITY-SCHED` all READY with W1-W4 claim-sized leaves | `READY` | claim W1 (`ENG-CORE-BUSY-LOOP`) then W2 (`SERVE-ASYNC-LLM`) — W2 unblocks the `ROAD-V1-A` latency axes; W4 priority is separable |
