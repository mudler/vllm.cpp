...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | MVP operational closure: online latency and e2e/nightlies | [`SERVE-GATE-ONLINE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | online harness fixes exact corpus/oracle/resources/traces/c32 budgets; online-focused 18/18, full tools 34/34, CTest tools 1/1. `54cba52` 27B model/cache/live-SSE gates pass, then the first client stopped pre-request because pip-vLLM lacked its pandas bench dependency. Exact pandas 2.2.3 and pinned transitive set are installed; future manifests reject pandas version/file drift before GPU work. Both pre-W2 campaigns and this partial attempt remain invalid; nightly unspiked | `GATING` | merge the oracle bench-dependency preflight, regenerate the SHA-bound corpora, then run 27B and 35B G1/G3-G6 campaigns under separate whole-model locks. Only complete post-W2 evidence can close the row; then spike `SERVE-E2E-NIGHTLY` |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 active: tools 34/34, CTest 1/1. Both pre-W2 campaigns remain invalid. `54cba52` passed 27B model/cache/live-SSE, then its first client stopped pre-request because the oracle lacked pandas. Exact pinned pandas is installed and its runtime/distribution files join the fail-closed manifest before GPU acquisition. W2 is CPU/TSan-gated; regenerated current-main evidence owns closure |
