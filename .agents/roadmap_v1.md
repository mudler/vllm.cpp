...
`KERNEL-GEMM-BF16` `READY`; implement and gate BA first, then qkvz. Trace
durations earn no speed credit. Host PSS/RSS separately retains a **22.920
GiB** CPU weight mirror plus source mmap residency.
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. Finalized `179a0fc` selects accepted `KERNEL-GEMM-BF16`: gate merged BA then qkvz, repair the 22.920 GiB host mirror, and close every 27B axis before 35B performance |
