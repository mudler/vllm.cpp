...
| 5 | `ROAD-V1-C5` | Sliding window, local attention, YaRN/long context | [engine matrix](engine-matrix.md), [coverage view §§2,11](feature-matrix.md#2-kv-cache--memory), [joint spike](specs/sliding-local-yarn-long-context.md) | accepted complete KV/manager/backend/dependency and RoPE formula/cache/apply chains; W1-W4 local-attention leaves are implemented and `GATING`; W5 `ATTN-YARN` is `ACTIVE` under `CLAIM-C5-YARN-1`; three dependent RoPE leaves remain `READY` | `ACTIVE` | port the typed YaRN config/factory/cache/apply foundation, including its `mrope_section` branch and pinned oracle goldens, without touching W6-W8 formula leaves |
...
| C5 | Sliding-window KV/attention and YaRN long-context scaling | 🚧 [joint spike accepted](specs/sliding-local-yarn-long-context.md): W1-W4 local-attention leaves are implemented/`GATING`; W5 YaRN is `ACTIVE`; W6-W8 remain `READY`. Runtime/model/oracle/trace/performance gates remain open; gate Qwen3.6 checkpoints are default interleaved MRoPE regressions, with separate feature-positive e2e/oracle gates |
