...
The diagnostic-only repair now consumes SIGUSR1 synchronously, calls the
server's graceful `stop()` and requires its exact lifecycle plus profiler exit
zero. `3f256ab` still binds at 55/124 and no speed credit follows.
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. W3-C frozen-map control passes; W3-E/W3-F/W3-G strict-fail and earn no speed credit. [W3-H normal BF16→FP4 vectorized I/O](specs/nvfp4-bf16-producer-vectorization.md) is **ACTIVE / trace-first**: H1a/H1b/H1c and H1d attempts through exact-build/gate/four-replay `a96e899` are `VOID`. H1d's repaired plan/build/profile-ancestry plus graceful-stop validation awaits a fresh immutable DGX series; every new performance ratio remains prohibited. Close every 27B axis before 35B performance; keep host-memory/teardown debt separate |
