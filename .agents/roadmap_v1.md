...
The selected work is now exact low-batch RMSNorm/generated-partition and FP4-
tactic mapping. The accepted oracle trace fixes c2 at **1,524 clean B=2
windows / 1,160 kernels each**, including seven generated partition families
and the 128 Stream-K + 80 static-persistent FP4 split. The trace-only local
observer is now batch-2 capable. The first local attempt passed correctness and
captured four identical lossless 1,011-kernel ranges, then failed closed because
the validator still asserted the c16 1,107-kernel contract. Its batch-aware
repair is staged and the complete retry remains **PENDING**, so no implementation
leaf is selected yet.
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. Accepted async `3812d8` is neutral at **1.002153×** total and earns no credit. Host PSS/RSS is a 22.920 GiB CPU mirror. Map/gate the low-batch kernel residual, repair direct-device loading, then close every 27B axis before 35B performance |
