...
| 5 | `ROAD-V1-C5` | Sliding window, local attention, YaRN/long context | [engine matrix](engine-matrix.md), [coverage view §§2,11](feature-matrix.md#2-kv-cache--memory), [joint spike](specs/sliding-local-yarn-long-context.md) | accepted complete KV/manager/backend/dependency and RoPE formula/cache/apply chains; W1-W6 are implemented and `GATING`; W5+W6 now carry nine pinned-source YaRN/MRoPE/Llama 3 fixtures plus CPU/sanitizer checkpoints; W7-W8 remain `READY` | `PARTIAL` | claim W7 `ATTN-ROPE-LONGROPE` for global short/long cache selection, validation and oracle goldens; retain W1-W6 model/CUDA/trace/every-axis handoffs |
...
| C5 | Sliding-window KV/attention and YaRN long-context scaling | 🚧 [joint spike accepted](specs/sliding-local-yarn-long-context.md): W1-W6 are implemented/`GATING`; W5+W6 typed YaRN/MRoPE/Llama 3 cache construction passes nine pinned-source CPU oracle fixtures and sanitizers; W7-W8 remain `READY`. CUDA compile/runtime and all model/trace/every-axis gates remain open; gate Qwen3.6 checkpoints are default interleaved MRoPE regressions, with separate feature-positive e2e gates |
