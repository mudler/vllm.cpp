...
Clean [W3-I](specs/nvfp4-fused-silu-producer.md) `15c6b89` structurally ports
the vLLM packed fused producer, but its complete exact-fixture component fails
at **27/40 timing + 3/8 memory**. Mean c2/c16 total-throughput ratios are
**1.002457× / 0.999771×**; all 612 requests and control gates pass. The
candidate stays default-off, earns no speed credit, and does not authorize the
exact grid. Order 0 now returns to the mandated cross-stack residual scan to
select and spike the next isolated lever before any new implementation.
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. W3-C control passes; W3-E/F/G/I1 earn no speed credit. W3-I1 is structure-green but component-fails **27/40 timing + 3/8 memory** and remains default-off. Re-run the cross-stack scan, gate the selected lever, then close every 27B axis before 35B performance while keeping host-memory/teardown debt separate |
