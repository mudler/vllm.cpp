...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact performance closure against the faster applicable vLLM v0.25.0/SGLang floor before broader roadmap implementation | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG-PREFIX`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-MAMBA-ALIGN`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-ASYNC-LLM`](engine-matrix.md), [`KERNEL-GEMM-NVFP4-W4A4`](kernel-matrix.md), [`KERNEL-ATTN-FA2`](kernel-matrix.md), [`KERNEL-GDN-AOT-BF16`](kernel-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | v0.25.0 target `702f481` is audited and immutable `3f256ab` binds at **55/124 axes pass, 69 fail**. W3-E/W3-F strict-fail and earn no speed credit. W3-G implements the selected FA2 ratio-6 decode route. Immutable `ae9e8ff` passes clean sm_121a compile, **20/20 + 454,323** operator assertions, zero-error/zero-leak memcheck, both 27B arms, correctness-only 35B and paired structural trace. Default/fallback switch exactly between **240 FA2 main+combine / 0 old** and **0 decode combine / 240 old** with identical 3,536 FP4 GEMMs/producers and 64 plans. The short correctness trace is performance-negative (**3.246400/1.395488 ms**) and non-binding. The c2/c16 **40+8** component is `ACTIVE` from `ae9e8ff` under one uninterrupted lock; both model arms passed and partial legs remain non-binding. Binding trace diagnostics remain local **22,893 / 8,793.238 ms** versus unequal-window vLLM split main **23,616 / 7,061.921 ms** + combine **23,488 / 123.245 ms**. Vectorized FP4 production ranks second. SGLang remains source-audited/non-binding | `GATING` | finish/classify W3-G's active frozen input-1,024 c2/c16 component; only 48/48 authorizes the exact grid. Close all 124 axes before 35B performance, then execute the distinct SGLang prefix gate |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. W3-C frozen-map control passes; W3-E and W3-F strict-fail and earn no speed credit. [W3-G FA2 ratio-6 split-KV decode](specs/fa2-gqa-split-kv-decode.md) immutable `ae9e8ff` passes sm_121a/operator/sanitizer/default+fallback 27B/35B-correctness/paired-structural gates; its short trace is performance-negative and non-binding. The c2/c16 every-axis component is `ACTIVE` under one lock with both model arms green and no partial result; the exact grid remains blocked. Close every 27B axis before 35B performance; keep host-memory/teardown debt separate |
