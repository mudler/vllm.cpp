...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact production-vLLM performance closure, online latency, and e2e/nightlies | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-ASYNC-LLM`](engine-matrix.md), [`KERNEL-GEMM-NVFP4-W4A4`](kernel-matrix.md), [`KERNEL-GDN-AOT-BF16`](kernel-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | Clean pushed-`b5c6e4f` remains below the all-axis floor: c1→c32 median totals **0.9933/0.9520/0.9657/0.9760/1.0213/1.0218×**, **4/4/5/4/17/14 of 20** performance and **2/4** memory axes. W3-B at clean `d7cdf66` is immutable build/correctness/safety-green and first-use-positive, but its component strict-fails. Corrected trace attempt 2 closes the original tactic-family structural mismatch. The replacement clean pushed-`3cc490c` exact 27B campaign is now **ACTIVE** under one uncontended model-wide lock: the commit-bound model/corpus/cache preconditions pass, ours repetition 1 retained c1-c32 plus memory return, and vLLM repetition 1 is in progress. No partial rate binds and `b5c6e4f` remains the denominator. The correctness-required 27B BF16 GDN default remains and 35B keeps f32. W0/W1 device-residency wins and inherited pool debt remain; 35B/nightly are deferred | `GATING` | finish all 36 timed groups, six returns and the paired trace for `3cc490c`; produce the canonical 27-only summary. If any axis remains red, drive the highest-ranked remaining executed-kernel gap, repeat until every 27B axis passes, gate optional W3-C separately, then run 35B, repair teardown, and spike `SERVE-E2E-NIGHTLY` |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 active and below floor: exact clean pushed-`b5c6e4f` 27B evidence remains binding at 0.9933/0.9520/0.9657/0.9760/1.0213/1.0218× totals, 4/4/5/4/17/14 of 20 performance axes and 2/4 memory axes. The corrected W3-B trace closes the original FP4 tactic-family mismatch. Clean pushed `3cc490c` is now running the replacement exact c1-c32 campaign under one lock; model/corpus/cache preconditions, ours repetition 1 and its memory return pass, while vLLM repetition 1 is active. No partial ratio is binding. Finish the complete grid and trace, close every exact 27B axis before 35B, and keep W3-C plus inherited zero-leak debt separate |
