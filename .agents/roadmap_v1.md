...
and explicit 35B/GGUF zero-selection tests. The oracle is stable 3×, synthetic
GGUF is 97/97 and CPU parity is 123/123; local CUDA/model and real hardware
inertness remain pending. The c2/c16 component remains pending, qkvz stays excluded
and no trace duration earns speed credit. Host PSS/RSS separately retains a
...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact performance closure against the faster applicable vLLM v0.25.0/SGLang floor before broader roadmap implementation | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG-PREFIX`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-MAMBA-ALIGN`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-ASYNC-LLM`](engine-matrix.md), [`KERNEL-GEMM-BF16`](kernel-matrix.md), [`KERNEL-GEMM-NVFP4-W4A4`](kernel-matrix.md), [`KERNEL-ATTN-FA2`](kernel-matrix.md), [`KERNEL-GDN-AOT-BF16`](kernel-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | v0.25.0 target `702f481` is audited and `3f256ab` binds at **55/124**. Finalized `179a0fc` proves matching FP4 tactics and exact **193 vs 97** BF16 projection launches. Pushed `581d335` closes W1 BA core correctness/safety; finalized `0091cd1` closes 963/145 versus 1,011/193 structure. W1C's exact five-shape vLLM BF16 oracle is stable 3× and its default-off local arm plus 35B/GGUF selection tests are implemented; local CUDA/model/hardware gates and component performance remain open. `benchmark_binding=false`; qkvz, host-memory repair, SGLang and a newer binding result remain open | `GATING` | validate W1C projection and native/legacy-35B/GGUF inertness, then c2/c16 component disposition before qkvz |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. Finalized `179a0fc` selects `KERNEL-GEMM-BF16`; BA code is `GATING`. Finalized `0091cd1` closes exact BA structure with `benchmark_binding=false`. Close GGUF/legacy inertness, BF16 rounding and c2/c16 component evidence before qkvz; repair the 22.920 GiB host mirror before 35B performance |
