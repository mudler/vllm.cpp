...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact performance closure against the faster applicable vLLM v0.25.0/SGLang floor before broader roadmap implementation | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG-PREFIX`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-MAMBA-ALIGN`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-ASYNC-LLM`](engine-matrix.md), [`KERNEL-GEMM-NVFP4-W4A4`](kernel-matrix.md), [`KERNEL-EW-NORM-QUANT`](kernel-matrix.md), [`KERNEL-GDN-AOT-BF16`](kernel-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | v0.25.0 target `702f481` is audited and the canonical DGX oracle is active. Immutable `3f256ab` completed all 36 27B groups, 2,016 requests, six returns and the paired trace; **55/124 axes pass and 69 fail**. Total ratios c1→c32 are **0.993504/0.954464/0.966438/0.980678/1.027889/1.039417×**. W3-D c16 remains 1.005049× mean-positive but strict-fails at 14/20 timing +2/4 memory; its post-pack topology is closed at **208.192 versus vLLM 208 FP4 GEMMs/forward**. The exact trace now selects `KERNEL-EW-NORM-QUANT`: vLLM executes **127,040 = 80/forward** fused Add+RMSNorm+FP4-quant launches while ours remains split. No fusion speedup or 35B result is claimed. The SGLang DGX claim remains source-audited and non-binding | `GATING` | spike the full vLLM/FlashInfer/Inductor norm+FP4 chain, port tests, run a same-binary 27B A/B and repeat the exact grid until all 124 axes pass; next execute PX1 plus `KV-MAMBA-ALIGN` PX2 for SGLang shared-prefix parity, and run 35B only after every applicable 27B cache-off/cache-on axis passes |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` v0.25.0 27B re-baseline complete and **FAILED/open**: 12/12 performance groups, 2/2 memory groups and 124/124 axes bind; 55 pass and 69 fail. Lifecycle and paired-trace proofs pass. W3-D packed QKV is structurally closed; `KERNEL-EW-NORM-QUANT` is the next trace-selected spike/A-B. Close every 27B axis before 35B and keep optional W3-C plus teardown debt separate |
