...
| 5 | `ROAD-V1-C5` | Sliding window, local attention, YaRN/long context | [engine matrix](engine-matrix.md), [coverage view §§2,11](feature-matrix.md#2-kv-cache--memory), [joint spike](specs/sliding-local-yarn-long-context.md) | accepted complete KV/manager/backend/dependency and RoPE formula/cache/apply chains; W1 sliding-window KV and W2 attention are implemented and `GATING`; W3 `KV-CHUNKED-LOCAL-SPEC` is `ACTIVE` under `CLAIM-C5-CHUNKED-KV-1`; five other leaves remain `READY` | `ACTIVE` | implement W3 concrete spec/manager/registry/hybrid conversion plus the ported CPU allocation/chunk-boundary properties; W2 G4/G6-G9 stays queued behind the serving lock and model/oracle prerequisites |
...
| C5 | Sliding-window KV/attention and YaRN long-context scaling | 🚧 [joint spike accepted](specs/sliding-local-yarn-long-context.md): W1 KV and W2 attention are implemented/`GATING`; W3 chunked-local KV is `ACTIVE`; five leaves remain `READY`. W2 runtime/model/oracle/trace/performance gates remain open; gate Qwen3.6 checkpoints are default interleaved MRoPE regressions, with separate feature-positive e2e/oracle gates |
