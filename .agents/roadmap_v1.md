...
remains 55/124; the fused-producer spike/component, exact grid, 35B performance,
and later portfolio remain open in that order.
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. W3-C frozen-map control passes; W3-E/W3-F/W3-G strict-fail and earn no speed credit. [W3-H normal BF16→FP4 vectorized I/O](specs/nvfp4-bf16-producer-vectorization.md) H1d is accepted at status `84d15970…6e66`; its 12/12 ranking displaces normal H2 with the larger fused-producer residual. No new ratio exists. Spike/gate fused production, then close every 27B axis before 35B performance while keeping host-memory/teardown debt separate |
