...
**45/45** and all tool tests pass **127/127**. It binds both corpus manifests and every partition, exactly
...
The pre-GPU failure is repaired: a test-first contract parses the real
production command and requires exactly one `--profile-control off`. Clean
`d82d282` completed both model gates and all c2 legs, then failed incomplete at
c16 packed r1: preflight and 16 warmups passed, but all 96 timed requests
returned HTTP 500 and no terminal marker was sealed. Partial legs contribute no
timing, memory, or speed result. Next: capture the discarded server exception,
diagnose/repair test-first, and rerun all **40+8 / 144 paired** axes from a new
SHA/root. qkvz stays excluded. Host PSS/RSS separately retains a
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. Packed structure is accepted; clean `d82d282` failed incomplete at c16 packed r1 with 96/96 HTTP 500 responses and no terminal marker. Capture/repair/rerun before qkvz; repair the 22.920 GiB host mirror before 35B performance |
