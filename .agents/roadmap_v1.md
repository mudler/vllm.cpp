...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact production-vLLM performance closure, online latency, and e2e/nightlies | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | Historical offline denominators are non-binding because sampling differed on both models and the 27B token budget differed. The `31d053f` online grid is also diagnostic only (0.926–0.961×): its old trace repeated cache-hit prompts on ours while vLLM caching was off, and admission/max-seqs/model-length differed. `ed6247d` captured a complete exact 48-prompt vLLM trace. W0 now ports the no-prefix coordinator, makes hybrid defaults cache-off, exposes explicit CLI policy, and repairs closed-loop trace equivalence. Pure b16 decode is nearly tied; mixed prefill/decode plus host-backed KV/GDN state traffic is the leading traced lever. Nightly remains unspiked | `GATING` | merge and CPU-gate W0; recapture the corrected 27B trace pair; move persistent KV/GDN caches device-side and A/B indexed state I/O; rerun exact matched-config direct-library plus online 27B, then 35B. Only complete passing evidence closes performance/serving rows; then spike `SERVE-E2E-NIGHTLY` |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 active: `31d053f` completed the repeated 27B grid and showed 0.926–0.961× diagnostic online throughput, but neither it nor the historical offline row can establish a direct-library regression because the configurations were not exact. `ed6247d` captured the full vLLM trace; W0 repairs cache policy, closed-loop admission, max-seqs, model length, and diagnostic output handling. Recapture the corrected pair, optimize device-resident KV/GDN state traffic, then run exact 27B direct/online gates before 35B. No existing performance ratio is binding |
