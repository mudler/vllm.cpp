...
scheduler control is now closed. Accepted root `3812d8` measures async ON/OFF
total throughput at only **1.002153×**, with ~1.0% better TPOT, ~16.0% worse
TTFT and ~0.2% more aggregate traced GPU time ON. W3 remains `READY`, not
credited or claimed for implementation, and leaves the immediate speed path.
The selected work is now exact low-batch kernel mapping. Clean `179a0fc` has
completed the whole raw series: **12/12** local B=2 ranges are lossless and
topology-identical at 1,011 kernels, while the fresh oracle has **1,522** exact
B=2 windows at 1,160 kernels plus two classified B=1 drains. Both sides resolve
128 Stream-K + 80 static-persistent FP4 GEMMs. A full read-only finalizer
preflight passes and ranks the diagnostic residual as BF16 CUTLASS GEMMs
(193 vs 97 launches, +2.864630 ms median) then RMSNorm/generated partitions
(177 vs 177, +1.810237 ms); FP4 GEMM time is already non-positive. The durable
summary/manifest/marker is **PENDING**, so cross-profiler times select no
implementation leaf or speed credit yet.
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. Accepted async `3812d8` is neutral. Complete `179a0fc` raw c2 mapping awaits its durable finalizer marker; no diagnostic cross-profiler ratio supersedes the grid. Finalize/spike/gate the residual, repair the 22.920 GiB host mirror, then close every 27B axis before 35B performance |
