...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 active and below floor: exact pushed-`bce2627` 27B evidence validates 12/12 groups, but c1→c32 total ratios are 0.9680/0.9317/0.9403/0.9516/0.9944/1.0073×, only 4/4/5/4/4/12 of 20 performance axes and 2/4 memory axes pass. W1 from the accepted [FP4 spike](specs/nvfp4-small-m-dispatch.md) is fully classified but fails strict component/full closure. W2 raw tactics are green; source+nsys+layer replay promote merged gate/up and maximum logical-shard CT divisors into W2 before the next timed campaign. Close every 27B axis before running 35B; W0/W1 device-residency wins and inherited zero-leak debt remain open context; later roadmap work is blocked |
