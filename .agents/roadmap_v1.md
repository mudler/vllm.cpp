...
| 5 | `ROAD-V1-C5` | Sliding window, local attention, YaRN/long context | [engine matrix](engine-matrix.md), [coverage view §§2,11](feature-matrix.md#2-kv-cache--memory), [joint spike](specs/sliding-local-yarn-long-context.md) | accepted complete KV/manager/backend/dependency and RoPE formula/cache/apply chains; W1-W5 are implemented and `GATING`, including W5's six pinned-source YaRN/MRoPE oracle fixtures and CPU/sanitizer checkpoint; W6-W8 formula leaves remain `READY` | `PARTIAL` | claim W6 `ATTN-ROPE-LLAMA3` against the merged W5 factory/cache seam; keep W5 CUDA/model/trace/every-axis closure as an explicit gating handoff |
...
| C5 | Sliding-window KV/attention and YaRN long-context scaling | 🚧 [joint spike accepted](specs/sliding-local-yarn-long-context.md): W1-W5 are implemented/`GATING`; W5 typed YaRN/MRoPE cache/apply passes six pinned-source CPU oracle fixtures and sanitizers; W6-W8 remain `READY`. W5 CUDA compile/runtime and all model/trace/every-axis gates remain open; gate Qwen3.6 checkpoints are default interleaved MRoPE regressions, with separate feature-positive e2e gates |
