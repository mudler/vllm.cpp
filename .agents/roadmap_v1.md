...
**Current order-0 substage (2026-07-13):** a committed one-kernel probe proves
pinned Nsight 2025.3.2.474 emits the possible-loss warning at
`cudaProfilerStop` despite a complete bounded range, across repeat/sync/reset
modes; full-process tracing is clean. Schema v5 remains fail-closed but
classifies that exact warning only after pinned-version, runtime,
synchronization, collected-counter, complete model-topology, zero-eager, and
cross-report reconciliation. CPU contracts are green; a fresh immutable
12-report DGX run is next. `b9beccd` remains `VOID`, `3f256ab` remains 55/124,
and W3-H2, the exact grid, 35B performance, and later portfolio stay blocked.
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. W3-C frozen-map control passes; W3-E/W3-F/W3-G strict-fail and earn no speed credit. [W3-H normal BF16→FP4 vectorized I/O](specs/nvfp4-bf16-producer-vectorization.md) is **ACTIVE / trace-first**: the pinned-tool warning is calibrated and schema-v5 exact event/completion/topology reconciliation is CPU-green. Fresh 12-report DGX evidence is next; no new ratio exists. Close every 27B axis before 35B performance and keep host-memory/teardown debt separate |
