...
**44/44**. It binds both corpus manifests and every partition, exactly
...
The first clean DGX attempt at `593996d` built **154/154** but failed before
corpus validation or GPU ownership because the driver omitted mandatory
`record-execution --profile-control off`. It contributes no model, timing,
memory, or speed result. Next: add a test-first CLI contract, patch the driver,
push a fresh SHA, then execute that runner under one uncontended DGX lock and
disposition all c2/c16 **40 timing + 8 memory** axes. qkvz
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. Packed structure is accepted; clean `593996d` built **154/154** then failed pre-GPU because the component driver omitted required `--profile-control off`. Repair/test and rerun from a fresh SHA before qkvz; repair the 22.920 GiB host mirror before 35B performance |
