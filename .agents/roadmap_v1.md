...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. W3-C frozen-map control passes; W3-E and W3-F strict-fail and earn no speed credit. [W3-G FA2 ratio-6 split-KV decode](specs/fa2-gqa-split-kv-decode.md) immutable `ae9e8ff` passes sm_121a/operator/sanitizer/default+fallback 27B/35B-correctness/paired-structural gates; its completed c2/c16 component reaches **1.017668×/1.006548×** mean total throughput but strict-fails **35/40 timing + 5/8 memory** and earns no credit. The exact grid remains blocked while fresh executed-path scanning revalidates the next candidate. Close every 27B axis before 35B performance; keep host-memory/teardown debt separate |
