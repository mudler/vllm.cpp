...
is `ACTIVE`: clean `15c6b89` passes the default-off I1 implementation's exact
CUDA build, operator/sanitizer/model/SASS gates and paired 27B/35B traces. Its
packed body plus required zeroing reduces the comparable graph slice by
**36.68%**, but the 48-axis component, exact grid, 35B performance, and later
portfolio remain open in that order. Its first component start is `VOID` before
measurement on an older native-cache build ID; the repaired driver binds the
exact accepted v0.25 fixture.
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. W3-C control passes; W3-E/W3-F/W3-G earn no speed credit. H1d selects fused SiLU→FP4. [W3-I](specs/nvfp4-fused-silu-producer.md) I1 is structurally accepted at clean `15c6b89`; its first component start is `VOID` before timing on stale native-cache metadata, and repaired exact-fixture execution is pending. Strictly gate I1, then close every 27B axis before 35B performance while keeping host-memory/teardown debt separate |
