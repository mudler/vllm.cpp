...
Raw `0091cd1` completed all 24 local ranges at merged 963/145 versus split
1,011/193, but its first finalizer failed closed on two new vLLM
Torch/Inductor RMSNorm register-allocation signatures and wrote no marker.
Whole-chain comparison found no name, geometry, shared-memory, family or tactic
drift; the exact two-hash repair passes 69/69 tools tests and a no-write
full-chain preflight validates the exact 48-BF16-only delta. Pushed-source
re-finalization and component performance remain pending, qkvz stays excluded and
no trace duration earns speed credit. Host PSS/RSS separately retains a **22.920 GiB** CPU weight mirror
plus source mmap residency.
...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact performance closure against the faster applicable vLLM v0.25.0/SGLang floor before broader roadmap implementation | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG`](backend-matrix.md), [`BACKEND-GATE-CUDA-SGLANG-PREFIX`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-MAMBA-ALIGN`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-ASYNC-LLM`](engine-matrix.md), [`KERNEL-GEMM-BF16`](kernel-matrix.md), [`KERNEL-GEMM-NVFP4-W4A4`](kernel-matrix.md), [`KERNEL-ATTN-FA2`](kernel-matrix.md), [`KERNEL-GDN-AOT-BF16`](kernel-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | v0.25.0 target `702f481` is audited and `3f256ab` binds at **55/124**. Finalized `179a0fc` proves matching FP4 tactics and exact **193 vs 97** BF16 projection launches. Pushed `581d335` closes W1 BA core correctness/safety. Raw `0091cd1` has exact 24/24 local merged/split ranges; first finalization rejected new exact vLLM Inductor register signatures and wrote no marker. The two-hash repair is 69/69 green and its no-write full-chain preflight validates the exact 48-BF16-only delta; BF16 output and component performance remain open. qkvz, host-memory repair, SGLang and a newer binding result remain open | `GATING` | push the signature repair and re-finalize raw `0091cd1`; then close BF16 rounding and c2/c16 disposition before qkvz |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. Finalized `179a0fc` selects `KERNEL-GEMM-BF16`; BA code is `GATING`. Raw `0091cd1` passes local structure but has no completion marker after a fail-closed oracle-signature rejection. Re-finalize, then close rounding/component evidence before qkvz; repair the 22.920 GiB host mirror before 35B performance |
