...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 root takeover active: fail-closed harness merged at `b0acec2` (16 focused / 32 full CPU tests, CTest 2/2). Both pre-W2 campaigns are terminal/invalid. Fresh `0295e72` sm_121a build and 35B/27B model tests pass; the no-GPU plan preflight exposed a clean-shell corpus import defect before measurement, and the recorded command is repaired to a tested module invocation. W2 has genuine live SSE and is CPU/TSan-gated; regenerated current-main GPU evidence owns closure |
