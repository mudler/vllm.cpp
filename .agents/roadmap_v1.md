...
that gap. An exact async ON/OFF c2 timing + trace control is prepared and
`PENDING`; W3 remains `READY`, not credited or claimed for implementation.
...
| 6 | `ROAD-V1-C6` | Priority and async/overlap scheduling + AsyncLLM streaming serving — RE-PRIORITIZED 2026-07-10: `SERVE-ASYNC-LLM` re-promoted T1→T0 because it blocks order 0 and async scheduling is vLLM's default at the pin (B3) | [engine matrix](engine-matrix.md), [coverage view §1](feature-matrix.md#1-engine-core--scheduling), [async-serving spike](specs/async-serving.md) | W1 `ENG-CORE-BUSY-LOOP`, W2 `SERVE-ASYNC-LLM`, and W4 `ENG-PRIORITY-SCHED` are implemented/`GATING`; fixed HTTP capacity is retained and speed-neutral. W3 `ENG-ASYNC-SCHED` remains `READY`. The profiler can now force and record vLLM async `on`/`off`, but the exact c2 timing + trace credit is still `PENDING`, so no W3 implementation claim exists | `PARTIAL` | execute the order-0 async credit control. If positive at the size needed, claim/port W3 with graph-compatible copy events and GPU-resident sampled-token state; otherwise keep W3 as parity work and return the speed track to low-batch kernel mapping |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 immutable `3f256ab` is **FAILED/open** at 55/124. The scan selects an exact c2 vLLM async ON/OFF control; result is `PENDING`, not speed credit. Host PSS/RSS is grounded in a 22.920 GiB retained CPU weight mirror. Run the diagnostic, gate the selected speed lever, repair direct-device loading, then close every 27B axis before 35B performance |
