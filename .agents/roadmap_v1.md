...
|---:|---|---|---|---|---|---|
| 0 | `ROAD-V1-A` | Restore exact production-vLLM performance closure, online latency, and e2e/nightlies | [`BACKEND-GATE-CUDA-VLLM`](backend-matrix.md), [`SERVE-GATE-ONLINE`](engine-matrix.md), [`KV-PREFIX-CACHE`](engine-matrix.md), [`KV-DEVICE-RESIDENCY`](engine-matrix.md), [`SERVE-ASYNC-LLM`](engine-matrix.md), [`KERNEL-GEMM-NVFP4-W4A4`](kernel-matrix.md), [`SERVE-STREAM-USAGE`](engine-matrix.md), [`SERVE-E2E-NIGHTLY`](engine-matrix.md), [benchmark protocol](benchmark-protocol.md) | Exact pushed-`bce2627` 27B online evidence validates all 12 groups but is below the all-axis floor: c1→c32 median total ratios **0.9680/0.9317/0.9403/0.9516/0.9944/1.0073×**, only **4/4/5/4/4/12 of 20** performance axes and **2/4** memory axes pass. W1 exact buckets are fully classified: the component series is positive at c8/c32 but fails strict c16/memory no-regression; the trace proves independent plans and a 1.05% aggregate FP4-kernel-time reduction. The fresh oracle trace still attributes 25.1% to the absent 128x32x256 Stream-K/static pair, so W2's complete 32-tactic family is active. W0/W1 device-residency wins and inherited pool debt remain; 35B/nightly are deferred | `GATING` | implement and independently gate W2's full tactic/workspace family, then repeat the exact 27B component/trace/oracle loop until every axis passes; only then run 35B, repair teardown, and spike `SERVE-E2E-NIGHTLY` |
...
| # | Track | State |
|---|---|---|
| `SERVE-GATE-ONLINE` (formerly A1) | Serve-latency A/B vs `vllm serve` (TTFT/TPOT online, every-axis rule) | 🚧 active and below floor: exact pushed-`bce2627` 27B evidence validates 12/12 groups, but c1→c32 total ratios are 0.9680/0.9317/0.9403/0.9516/0.9944/1.0073×, only 4/4/5/4/4/12 of 20 performance axes and 2/4 memory axes pass. W1 from the accepted [FP4 spike](specs/nvfp4-small-m-dispatch.md) is fully safety/correctness/performance classified: exact buckets improve c8/c32 and materially improve the binding grid, but fail strict component c16/memory and full every-axis closure. Fresh execution traces confirm W2's absent narrow static-persistent/Stream-K family as the next runtime lever. Close every 27B axis before running 35B; W0/W1 device-residency wins and inherited zero-leak debt remain open context; later roadmap work is blocked |
