...
| `SGLANG-ORACLE-LEASE-WHEEL` | reach the pinned SGLang oracle from an `rc` lease through the PyPI wheels, because the 2026-07-28 image route is now forbidden and `rc run` has no `--image` | wheels `sglang-0.5.15-cp312-cp312-manylinux_2_34_aarch64.whl` sha256 `1c2d2602b4ba04c6a71d2f3bf2e3654da53987536f0d65dbe4f57cdc65c9812e` and `sglang_kernel-0.4.4-cp310-abi3-manylinux2014_aarch64.whl` sha256 `727e4bc53abeade20260186f99199200320b9fa51f8de7af90c01524cff73e5d`; upstream `docker/Dockerfile:210,242` installs the same kernel wheel from the same index at `CUDA_VERSION=13.0.1` | identity manifest [sglang-wheel-in-lease.json](specs/sglang-wheel-in-lease.json), 3338 files of the installed `sglang/` tree; spec [sglang-wheel-in-lease.md](specs/sglang-wheel-in-lease.md) | INVENTORIED | **W1 specified it; W2 built the driver and reached the pin in a lease.** Static analysis on 2026-08-19 settled the question that could have killed the route: `sgl_kernel/sm100/common_ops.abi3.so` holds 56 fatbins of 6 cubins each, architecture set 90/100/103/110/120/121, **zero fatbins without 121**, and 336 of 336 decompressed payloads agree with their declared architecture by ELF `e_flags`. `cuobjdump -lelf` (13.0, V13.0.85) names the six targets: the 121 cubins are **`sm_121a`**, the accelerated variant, 56 of them, one per container, and the plain ones are the 90 cubins, which `sgl-kernel/CMakeLists.txt:127,221` explains exactly. GB10's coverage is accelerated, not merely present. The 5.6% aarch64 size gap is one absent library, `flash_ops.abi3.so`, which capability refuses on GB10 anyway. **W2, 2026-08-23:** the replacement driver is `scripts/rc-sglang-oracle-lease.sh` with the gate at `scripts/sglang_lease_identity.py` and its mutation suite at `tests/scripts/test_sglang_lease_identity.py`; `scripts/dgx-sglang-low-concurrency.sh` stays unrunnable and was NOT patched. One `rc run` job on `dgx:gpu0` (`86282a1a`, 28 min 30 s, exit 0) installed both wheels from PyPI, hashed the bytes that landed (`WHEEL_SHA_OK=1` twice), installed `flashinfer-jit-cache==0.6.12+cu130` so the warm-cache difference is closed rather than accepted, and asserted `IDENTITY_RC=0` from `cd /`: **3338 of 3338** manifest files, 0 missing, 0 extra, 0 differing, at pin `f63458b5`. No compiler ran for any of the 199 resolved packages. `is_sm100_supported=False`, `is_sm120_supported=True`, `is_flashinfer_available=True` on GB10 capability `(12, 1)`, `torch 2.11.0+cu130`. `LGC_RC=4` reproduced #1354 on a fourth job. **The serving job is queued and has not reported**, so exit conditions 3, 4 and 5 are unmet and **`gateable` stays `no`**. [#1265](https://github.com/mudler/vllm.cpp/issues/1265) |
