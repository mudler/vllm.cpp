...
| **ISSUE-GH-1283: greedy decode is not reproducible at concurrency 16.** W5's OFF/OFF control caught it and it VOIDED workload B. NOT this row's defect — `VT_CUDA_GRAPH_DEDUP` is unset in both cells that disagree — but this row's future A/Bs cannot use that workload shape until it is understood | ISSUE-GH-1283 | it is a decode-path defect with its own issue and its own isolation plan. Folding it into this row would hide it behind a graph feature it has nothing to do with |
