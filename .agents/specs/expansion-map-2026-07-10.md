...
- **SGLang steals (Report 2):** Async/overlap scheduling is **vLLM's default at our pin** (`config/scheduler.py:158` → `True`) while we run fully synchronous — it is simultaneously a SGLang-proven lever *and* an unmet mirror obligation. The old premise that we already measured ≥1.0× against async-vLLM was invalidated by the 2026-07-11 sampling/token-budget audit. A current trace caps generic inter-step overlap near 0.2% on pure decode, so the real payoff remains TPOT/ITL and mirror closure after the larger mixed-state-residency lever; fresh exact A/Bs decide its throughput effect. No SGLang-proprietary kernel beats vLLM's resolved kernels — the kernel "steal" is a no-op.
