...
### Work breakdown
W0 config + registry stub (LANDED 2026-07-20). W1 runner generalization / `ENG-RUNNER-MODELSHAPE` (LANDED 2026-07-20, behaviour-preserving). W2 weight loader (LANDED 2026-07-20: shared BF16 helpers extracted to `dense_weight_loaders.h` byte-identical; `qwen3_weights.cpp` Qwen3 name map + merged qkv/gate_up + tied lm_head skipping the checkpoint `lm_head.weight`; dgx load gate 1567/1567, 311 tensors, no leftover, memcheck 0; 27B 235/235 + 35B 315/315 unchanged). W3 forward (LANDED 2026-07-20: `qwen3.cpp` `Qwen3DenseModel::Forward/ForwardDevice` composed from vt:: ops + the 2 new byte-exact recipes `kFusedAddRmsNormStd`/`kAttnQkNormRope`; bf16 attention numerics mirror vLLM; wired into the factory. Forced out + fixed 2 genuine latent bugs: tokenizer `kQwen2Classic` and `cuda_paged_attn.cu` WMMA prefill mistokenizing at head_dim≠256). W4 SACRED gate (GATING 2026-07-20 — **NOT token-exact yet: 11/16** prompts vs the vLLM 0.25.0 oracle; all 16 first-tokens exact, 5 late-decode near-ties = per-op bf16-rounding drift vs vLLM, confirmed structural-correctness via CPU-exact-f32 10/16, NOT a bug; regression 27B 235/235 + 35B 315/315 preserved, CUDA -Werror clean, memcheck 0). Row `ACTIVE`→`GATING` (NOT `DONE` — the gate is not 16/16). Remaining for `DONE`: a follow-on flash-exact d=128 attention / bf16-matmul bit-matching pass to close the last 5 near-ties. W-next: NVFP4 32B, then Llama-3.2-1B (download) as the genuine cross-family additivity proof.
