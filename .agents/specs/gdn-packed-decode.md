...
- Under one lock, run packed-default versus rollback c2/c16 in AB/BA/AB order,
  ...
  **Exception — the c2 TTFT-family (mean/median/p90/p99 of ttft):** at c2 those
  ...
  18-per-request distribution, (b) STABILITY-gated on a generous 50% pooled
...
- **The gated per-rep paired axes use a MAJORITY-CONSISTENCY rule.** A paired
  axis (per concurrency, per axis name) FAILS only when **≥ 2 of its 3 rep-pairs
  breach the acceptance band in the SAME direction (packed-worse)**; the
  ...
  FAILS. `contract.paired_gate = {rule: "majority-consistency", repetitions: 3,
  breach_majority: 2, grounding}` records it (revised test-first under
  `CLAIM-GDN-BA-ROUNDING-1`; the retired rule failed the gate on ANY single
  per-rep paired breach). Existing bands are unchanged (0.5% non-tail, 15% tail,
