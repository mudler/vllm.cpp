...
- Landing owner:
  [`BACKEND-ROCM` issue #2164](https://github.com/mudler/vllm.cpp/issues/2164)
...
Issue [#2164](https://github.com/mudler/vllm.cpp/issues/2164) integrates this
campaign's records from `pr/1936` at
...
`ACTIVE`. Position: ~103 tok/s (T18 idle-host gate 100.46 tok/s + T18 v_dot4
+2.7% matched-load). Adopted levers: T5a shared quant-body vectorization
(+23%), T5b d128 f32-Q DecodeGqa arm (+13.5%), T6a cooperative GDN scan
(+4.6%), T6b cooperative attn preamble (+4.6%), T8 cooperative rmsnorm row
(+3.2%), T9 cooperative gated norm (+2.6%), T10 warp postconv (+4.7%),
T11 row-split scan (+3.2%, BIT-IDENTICAL), T14 row-split argmax (−71%,
BIT-IDENTICAL), T16 YTILE=4 default (+1.8% contended, +8.1% idle),
T18 v_dot4 instruction selection (+2.7%, BIT-IDENTICAL).
