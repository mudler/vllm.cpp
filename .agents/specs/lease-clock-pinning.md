...
- **A pinned number is ~12% slower in absolute terms.** 2184 against a sampled
  ...
  unpinned one.
- **Persistence mode is not the DVFS lever, and it IS a gate rule.** The pinned,
  flat, unthrottled series ran with persistence `Disabled`; every throttling
  window of 2026-08-19 ran with it `Enabled`. Persistence governs whether the
  driver stays resident between clients, not DVFS under load. **Two clauses of
  this bullet were wrong and are corrected in §The 2026-08-23 re-measurement.**
  `nvidia-smi -pm 1` was measured to succeed inside a lease on 2026-08-22, so it
  is not unsettable there. And `clock_reasons` refuses any record whose
  `persistence_mode` is not `Enabled`, so the 2026-08-15 series this section
  cites cleared the spread rule and the throttle rule and not that one.

## The 2026-08-23 re-measurement: half one holds, and option 2 is partly available

An `rc` job ran on `dgx:gpu0` on 2026-08-23 and measured both halves again.
Evidence at `/mnt/nas_share/rc/clk1354/out/settle-20260823T004328Z/`. Three
results, each re-derived from the raw files rather than read out of the job log.

**Half one holds, on a new pod and a new boot.** `nvidia-smi -lgc 2190` and
`-rgc` both returned `4` with the same refusal text, on pod `rc-worker-4b8lj`
and boot `f50a1be6-7626-4a2f-90f0-08c77b16c9f1`. The pod reports
`CapPrm = CapEff = CapBnd = 0x00000000a80425fb`, the default OCI set, whose bit
21 is clear. `CAP_SYS_ADMIN` is therefore absent from the **bounding** set, so
no call inside the container can restore it, and the diagnosis in
[`lease-gpu-capability.md`](lease-gpu-capability.md) is confirmed on a second
pod. #1354 option 1 stays closed.

**Option 2, the settle-and-hold procedure, produced two windows clean on the
spread rule and the throttle rule at once.** They are the first such windows in
this campaign. The full derivation, the per-request mechanism, and the four
things the result does not buy are in
[`clock-gate-route.md`](clock-gate-route.md) §The 2026-08-23 settle run. The
short form: a window holding **one** long request is clean 2 times out of 3, a
window holding **six** short requests is clean 0 times out of 3, and the
per-request throttle-label probability that predicts both is `10/21 = 0.476`.
**No pairing was established and no ratio exists.** All four pairings returned
`PAIRING_VERDICT=DISCARD`.

**One reading the records carry is now falsified, and it is not this spec's
opening.** The opening says the instrument change "is not a route to a number",
and that is unchanged and still right.
What is now wrong elsewhere in the records is the stronger reading that **no**
unpinned lease window can pass both rules. Two did. What survives is the cause:
the excursion is generated by the request train, one excursion per request, and
a quiet box does not remove it.

### `nvidia-smi -pm 1` DOES work inside a lease, and this run did not use it

§The fleet change says persistence mode "is also not known to be settable from a
lease". **That is now false.** On 2026-08-22 the `SPEC-DFLASH2` speed gate ran
`nvidia-smi -pm 1` inside a lease, with no `ssh` and no host access, and it
succeeded: *"Enabled persistence mode via daemon for GPU 0000000F:01:00.0."*
`-lgc` in the same job still returned 4, so the two are separate driver
operations and only one is refused. `.agents/environment.md` carries the
measurement and the instruction.

The 2026-08-23 job read `persistence_mode: Disabled` at `job.log:12` and **never
ran `-pm 1`**. There is no `-pm` call in the log. All six windows therefore
carry a third refusal, `clock: <arm> ran with persistence mode 'Disabled', not
Enabled`, which `clock_reasons` has appended since `51ec6bed5`. This is a
procedure gap in one run, not a capability wall.

**Read persistence mode at the start of a leased measurement and set it if it is
off.** A two-hour paired measurement is otherwise discarded on a setting one
command fixes.

### The 2026-08-15 pinned series did not pass the persistence rule either

§The fleet change cites the 2026-08-15 host-path series as the measured reason
to take the grant. That series ran with persistence mode `Disabled`
([`../benchmark-record.md`](../benchmark-record.md), CLOCKS paragraph of the
2026-08-15 Qwen3.8-27B entry), and the persistence rule already existed when it
ran. So the series demonstrates a pass on the spread rule and the throttle rule,
and not on the persistence rule. The host recipe `.agents/environment.md`
records for 2026-08-21 runs `-pm 1` before `-lgc`, which is the repair. Whether
any `established` verdict resting on the 2026-08-15 records was produced by
`clock_reasons` over them is not established here, and it is listed under
`## Owed`.
...
- **A settle run that fixes the two procedure gaps of 2026-08-23.** Run
  `nvidia-smi -pm 1` at the start, and take both arms as one-request windows so
  a pairing can clear the throttle rule on both sides. Option 2 has produced
  clean windows and no clean pairing.
- **Whether any `established` verdict resting on the 2026-08-15 pinned records
  ...
  persistence mode `Disabled`, which the rule refuses. §The 2026-08-15 pinned
  series measures the input and does not resolve the verdict.
...
**Updated 2026-08-23** from the `rc` job at
`/mnt/nas_share/rc/clk1354/out/settle-20260823T004328Z/`. Half one holds: `-lgc`
is still refused, on a new pod and a new boot, and `CAP_SYS_ADMIN` is absent
from the pod's bounding set. Half two is still unimplemented and no threshold
moved. Two clauses of §The fleet change about persistence mode are corrected
above. This session took no lease, ran no job and used no GPU; it read the
artifacts the job left and re-derived every figure from them.
