...
| [#1452](https://github.com/mudler/vllm.cpp/issues/1452) | W5 | **PARTLY CLOSED on 2026-08-25: it compiles, it runs, and the bit-identity is now MEASURED rather than argued. Two of the four owed items remain.** The four were a compile, a `memcmp` CPU-vs-CUDA arm, an end-to-end pixel comparison, and the f16/bf16 storage the arm refuses by name. **(1) The compile: MET.** First compile ever, in an `rc` lease on `dgx:gpu0` (GB10), nvcc **13.0.88**, `-DVLLM_CPP_CUDA_ARCHITECTURES=121a`, `CMAKE_BUILD_TYPE=Release`: `cuda_conv3d.cu.o` built with **zero errors and zero warnings**, and `cuobjdump --list-elf` reports one ELF, `cuda_conv3d.cu.1.sm_121a.cubin`, so the object carries real `sm_121a` code rather than PTX alone. `src/vt/cuda/cuda_ltx2_vae.cu` (PR #1909, then unmerged) compiled clean in the same tree and same run. **(2) The `memcmp` arm: MET**, and it is `tests/vt/test_ops_conv3d.cpp`'s three new cases in the shape `test_ops_conv1d_general.cpp` ships for `kConv1d`. On the device: **7 cases / 2071 assertions green**, and the `vt::Conv3d` announcement fired with `device type 1`, so the CUDA provider was genuinely dispatched rather than a CPU fallback compared against itself. **The gate was mutation-proved on the same hardware and in the same run**: rewriting the kernel's `tap = __fadd_rn(tap, __fmul_rn(x, w))` as plain `tap += x * w`, which is exactly the FMA contraction the file's header says it refuses, turned it **RED — 2 of 7 cases, 15 of 2071 assertions** — and a byte-for-byte restore returned it to 7/7 and 2071/2071. **Read WHICH half caught it, because it decides how much the shape table is worth.** Only 4 of the 15 shapes moved at all (`single row` 16/60, `single col` 18/60, `ragged stride` 134/420, `kitchen sink` 203/720); the well-scaled majority stayed byte-identical under the mutation. The catastrophic-cancellation case moved **1423 of 3240** values. So a shape table on `Rng::Uniform`'s [-1, 1] data is NOT on its own an instrument for reduction order, and the cancellation case is what holds the contract — the same conclusion `test_ops_conv1d_general.cpp` reached for `kConv1d`. **(3) The end-to-end pixel comparison: STILL OWED**, and now with a cost. The staged lease kit at `/mnt/nas_share/rc/ltx25-fullmodel/` carries 4 of the 6 checkpoints its own `render.sh` references — the spatial upsampler and the distilled LoRA are on the NAS but outside the `rc/` subfolder a leased worker mounts, so they need staging first. The comparison also needs BOTH arms of a 22 B DiT render, and the host arm is the expensive one; no wall-clock for either was taken, and none is claimed. **(4) The narrow dtypes: STILL OWED, but now PINNED.** `conv3d: the CUDA arm refuses the narrow dtypes BY NAME` asserts the refusal message on the device, so the gap cannot change silently while it waits for a kernel. **One consequence handled in the same change:** `src/vt/ops.cpp`'s first-non-CPU-dispatch notice claimed "This arm has never been run on real hardware", which this run made false. It is NARROWED rather than deleted — CUDA now reports the gated state, every other accelerator type still reports never-run, because for them the sentence is still true |
