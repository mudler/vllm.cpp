...
and not the pinned oracle, which needs `nvcc`. A prebuilt wheel does not shorten
that route: an aarch64 vLLM wheel exists in general, and our pin is neither among
the wheels published for the one nightly commit nor on PyPI, so a source build or
a deliberate pin advance is the only way. Resume this row only after the oracle
