...
On `thor:gpu0`, a `torch` and `triton` tree staged on `/workspace` imports,
...
`sm_121a` and UNMEASURED, and the staged tree holds `torch` and `triton` and not
the pinned oracle, which needs `nvcc`. Resume this row only after the oracle
itself has a lease-compliant path, because the adjudication instrument is
committed and correct and has nothing to run on.
