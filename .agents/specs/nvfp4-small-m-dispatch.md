...
exact-oracle grid is complete and binding at **55/124 axes**; W3-D remains
`ACTIVE` because 69 axes fail, while its trace hands the next repair to the
separate `KERNEL-EW-NORM-QUANT` row.
...
v0.25 executed-cache audit promoted W3-C; C2 ready-map/lifecycle/5,000-us
runtime integration and frozen-map correctness are now gated. Immutable
`d211b8f` passes six-process 64/64 stability. C3R then proves 6/6 x 128-token
direct/fallback equality under identical sequential batch shape and reproduces
0/6 sequential-vs-c2 equality in both ours and production-default vLLM. The
stopped partial performance remains void. Corrected C3 completes all 12 legs
with identical frozen plans and exact counts; direct gains
1.004483x/1.005044x c2/c16 throughput, but W3-E strict-fails at 39/40 timing +
1/8 memory. W3-C reproduction control is complete and C4 remains blocked.
35B remains prohibited.
...
| timing/warmup/persistence (`kernel_warmup.py:133-213`, `flashinfer_autotune_cache.py:19-55`, installed `autotuner.py:799-818,978-1013,1745-1900`) | **W3-C CONTROL COMPLETE / OWNING ROW ACTIVE:** C2 implements loaded-plan priority, checked ready-map import/snapshot, source/device/tactic identity, 5,000-us miss timing, frozen misses and completed-only publication. Immutable `d211b8f` passes six fresh processes and every corrected component arm with byte-identical 64/64 maps, zero tuning/misses and exact 27B gates. C3R proves same-shape sequential direct/fallback 6/6 x 128 equality and shows independent production runs are batch-shape dependent in ours and vLLM. The stopped series remains void; corrected C3 completes exact counts but W3-E fails 39/40 timing + 1/8 memory. This is mandatory reproduction control, not speed credit; see [W3-C spike](nvfp4-persistent-plan-cache.md) |
...
| autotuner timing/cache behavior | **W1/W3-A/B classified; W3-C cache stability gated / component failed:** `tests/vt/test_nvfp4_persistent_cache.cpp:178-490` covers document/ready-map contracts under Release/sanitizers; `tests/vt/test_ops_nvfp4_fp4.cpp:1665-1884` covers exact frozen import/capture/miss and completed-only save lifecycle. Immutable `d211b8f` passes six byte-identical 64/64 native-only 27B processes, then fails first-pair long-output equality 2/6 under the same plans. See [the dedicated spike](nvfp4-persistent-plan-cache.md) |
...
| W3 | A/B historical tuning placement; C: v0.25 persistent/frozen plan cache and 5,000-us miss timing; D packed QKV; E direct scales | **ACTIVE** under `CLAIM-NVFP4-SMALL-M-3`. D closes topology and `3f256ab` binds at 55/124. C runtime, six-process stability, C3R correction and the corrected 12-leg frozen-plan control are complete. E gains 1.004483x/1.005044x c2/c16 mean throughput but strict-fails at 39/40 timing + 1/8 memory. C4 remains blocked; attribute failed axes and resume the executed-path scan. No speed credit/exact grid/35B performance |
...
clean and structurally classified. At that checkpoint immutable `9cc7191`
owned the binding c1/2/4/8/16/32 three-repetition v0.25 27B oracle result at
**54/124** axes, and accepted `def5f75` node tracing selected W3-D packed QKV.
Post-pack `3f256ab` now supersedes it at 55/124. Generated-code inspection
refutes the apparent norm+FP4 fusion, so residual selection is open outside
this row. Keep 35B held until all 27B throughput, latency and memory
axes pass.
...
### W3-D immutable component classification (2026-07-12)

Clean pushed `3f256abdbb558e162bf8a2196284deb119648560` was configured from a
detached source checkout with CUDA 13.0.88, sm_121a, the oracle FlashInfer
CUTLASS source, Triton AOT and FA2. Under one uncontended `/tmp/gpu` lock, the
packed and `VT_FP4_MERGED_QKV=0` model gates each pass **235/235 assertions and
16/16 vLLM tokens**. Six fresh server processes then execute c16/96,
input-1,024/output-128 in packed/split/split/packed/packed/split order, with
model/corpus/binary cache eviction, streaming preflight, process/GPU memory
sampling and post-exit memory return for every leg.

Packed runs are **815.886/810.759/810.047 tok/s** and split runs are
**811.294/805.779/807.377 tok/s**. Means are **812.231/808.150 = 1.005049×**;
population CV is **0.320%/0.287%**. Packed passes all four throughput axes and
ten of sixteen latency axes, for **14/20 timing axes**. It lowers mean sampled
GPU peak **38,059→37,765 MiB** and available-memory drop, while PSS/RSS are
about 60 MiB higher, for **2/4 memory axes**. All **576/576** requests and six
memory returns pass. Every packed process records 64/64 profiles and every
split process 80/80, with zero post-warmup lazy misses; tactic IDs remain noisy
across processes in both arms and are diagnostic.

Therefore the strict standalone component gate **fails**, while the packed
default is retained because it is the upstream-mirrored, trace-selected
topology, preserves exact model correctness, removes 16 profile shapes,
improves mean throughput/central latency and reduces GPU memory. This does not
...
component strict-failed and the completed exact grid binds at 55/124. The
paired trace closes W3-D topology. Generated-body inspection refutes the
apparent norm+FP4 mismatch; do not run 35B.
...
was **ACTIVE/PENDING at setup**. The vLLM 0.25 plan validates against the clean pushed
...
### W3-D exact-grid completion checkpoint (2026-07-13)

The same immutable evidence root completed under one uninterrupted `/tmp/gpu`
lock: passing model gate, **36/36** timed groups, **2,016/2,016** requests, six
memory/cache returns and the paired trace. All **124/124** axes are binding;
**55 pass and 69 fail**. Total-throughput ratios c1→c32 are
**0.993504/0.954464/0.966438/0.980678/1.027889/1.039417×**. PSS/RSS fail at
**0.584689/0.592269×**; GPU/drop pass at **1.829076/1.227760×**. Summary
all-runs/ratios/report SHA are `83b3f500…9f8` / `66d7f50e…b4bd` /
`df3d0539…e4d7`.

The paired trace passes and reconfirms the repaired FP4 topology. It records
**127,040 = 80 per 1,588 forwards** kernels whose generated names contain
`fused_add_rms_norm_scaled_fp4_quant`. Dumping the generated body shows that
each kernel stops after residual-add + RMSNorm and stores BF16; the wrapper
then invokes `torch.ops._C.scaled_fp4_quant.out` separately, yielding the
traced `cvt_fp16_to_fp4`. The oracle config has `fuse_norm_quant: False`.
Therefore vLLM and ours both use separate norm and FP4-quant launches, and no
`KERNEL-EW-NORM-QUANT` spike is selected from the trace name. Generated
graph/subgraph SHA are `d58f81b8…9401` / `466e359a…9dd8`. Residual
selection is open. Trace status/ours-Nsight/ours-SQLite/ours-kernel/vLLM-kernel SHA are
`9762c1e6…1d0c6` / `e397289d…8476` / `99cbd04d…93f8` /
`55a1631a…d2be` / `e4e916d1…565`. GPU, ports and lock exit idle. W3-D stays
`ACTIVE` because its component and exact every-axis gates fail; 35B remains
prohibited.
