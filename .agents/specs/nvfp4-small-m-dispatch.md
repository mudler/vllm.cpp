...
v0.25 executed-cache audit promoted W3-C; C1 document/import/atomicity is now
implemented and CPU/sanitizer-gated, while C2 runtime integration is `READY`.
...
| timing/warmup/persistence (`kernel_warmup.py:133-213`, `flashinfer_autotune_cache.py:19-55`, installed `autotuner.py:799-818,978-1013,1745-1900`) | **W3-C ACTIVE:** C1 implements native/import JSON, full metadata/tactic validation, path/frozen options, deterministic merge and atomic publication at `nvfp4_persistent_cache.{h,cpp}`; Release/ASan+UBSan/TSan pass **6/6 + 174/174**, exact oracle import 64/64 and full CPU 103/103. C2 loaded-plan priority, ready-map publication, 5,000-us miss timing and loader lifecycle remain `READY`. This is mandatory reproduction control, not speed credit; see [W3-C spike](nvfp4-persistent-plan-cache.md) |
...
| autotuner timing/cache behavior | **W1/W3-A/B classified; W3-C C1 gated:** existing bucket/capture tests remain; new `tests/vt/test_nvfp4_persistent_cache.cpp:178-415` covers native round-trip/stale/duplicates, merge, atomic readers and exact 64-entry FlashInfer import under Release/ASan+UBSan/TSan. Runtime loaded-plan and fresh-process gates remain C2/C3 under [the dedicated spike](nvfp4-persistent-plan-cache.md) |
...
| W3 | A/B historical tuning placement; C: v0.25 persistent/frozen plan cache and 5,000-us miss timing; D packed QKV; E direct scales | **ACTIVE** under `CLAIM-NVFP4-SMALL-M-3`. D closes topology, E strict-fails its component and `3f256ab` binds at 55/124. C1 cache document/import/atomicity is implemented and CPU/sanitizer-gated; C2 runtime integration is `READY`, C3/C4 benchmarks pending. No speed credit/exact grid/35B performance |
