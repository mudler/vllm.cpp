...
`d211b8f` passes six-process 64/64 stability, but C3 fails at the first
same-plan c2 pair because only 2/6 long outputs match; fixed-plan localization
is `READY` and all partial performance is void.
...
| W3 | A/B historical tuning placement; C: v0.25 persistent/frozen plan cache and 5,000-us miss timing; D packed QKV; E direct scales | **ACTIVE** under `CLAIM-NVFP4-SMALL-M-3`. D closes topology, E strict-fails its component and `3f256ab` binds at 55/124. C2 plus C3 six-process 64/64 stability pass; C3 same-plan output equality fails 2/6 at the first c2 pair, so fixed-plan localization is `READY` and C4 remains blocked. No speed credit/exact grid/35B performance |
