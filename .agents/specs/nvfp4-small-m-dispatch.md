...
stopped partial performance remains void and corrected C3 is `READY`.
...
| timing/warmup/persistence (`kernel_warmup.py:133-213`, `flashinfer_autotune_cache.py:19-55`, installed `autotuner.py:799-818,978-1013,1745-1900`) | **W3-C ACTIVE:** C2 implements loaded-plan priority, checked ready-map import/snapshot, source/device/tactic identity, 5,000-us miss timing, frozen misses and completed-only publication. Immutable `d211b8f` passes six fresh processes with byte-identical 64/64 maps, zero tuning/misses and exact 27B gates. C3R proves same-shape sequential direct/fallback 6/6 x 128 equality and shows independent production runs are batch-shape dependent in ours and vLLM; stopped series/rates remain void and corrected C3 is ready. This is mandatory reproduction control, not speed credit; see [W3-C spike](nvfp4-persistent-plan-cache.md) |
...
| W3 | A/B historical tuning placement; C: v0.25 persistent/frozen plan cache and 5,000-us miss timing; D packed QKV; E direct scales | **ACTIVE** under `CLAIM-NVFP4-SMALL-M-3`. D closes topology, E strict-fails its component and `3f256ab` binds at 55/124. C2 plus six-process 64/64 stability pass; C3R reclassifies the invalid cross-run text predicate with controlled ours/vLLM evidence and corrected C3 is `READY`. C4 remains blocked on its every-axis result. No speed credit/exact grid/35B performance |
