...
runtime integration and frozen-map correctness are now gated, while C3
fresh-process/same-plan performance is `READY`.
...
| timing/warmup/persistence (`kernel_warmup.py:133-213`, `flashinfer_autotune_cache.py:19-55`, installed `autotuner.py:799-818,978-1013,1745-1900`) | **W3-C ACTIVE:** C2 now implements loaded-plan priority, checked ready-map import/snapshot, source/device/tactic identity, 5,000-us miss timing, frozen misses, lifecycle/stats and load-before-warmup/save-after-complete. Exact 64/64 zero-tune import, runtime/save/memcheck and default/fallback 27B correctness pass. Six-process/same-plan performance remains C3 `READY`. This is mandatory reproduction control, not speed credit; see [W3-C spike](nvfp4-persistent-plan-cache.md) |
...
| autotuner timing/cache behavior | **W1/W3-A/B classified; W3-C C2 correctness-gated:** `tests/vt/test_nvfp4_persistent_cache.cpp:178-490` covers document/ready-map contracts under Release/sanitizers; `tests/vt/test_ops_nvfp4_fp4.cpp:1665-1884` covers exact frozen import/capture/miss and completed-only save lifecycle. Both frozen 27B arms pass. Commit-bound six-process/same-plan performance remains C3 under [the dedicated spike](nvfp4-persistent-plan-cache.md) |
...
| W3 | A/B historical tuning placement; C: v0.25 persistent/frozen plan cache and 5,000-us miss timing; D packed QKV; E direct scales | **ACTIVE** under `CLAIM-NVFP4-SMALL-M-3`. D closes topology, E strict-fails its component and `3f256ab` binds at 55/124. C2 runtime/frozen-map correctness is implemented/gated; C3 six-process/same-plan performance is `READY`, C4 exact grid conditional. No speed credit/exact grid/35B performance |
