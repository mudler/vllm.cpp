...
took the decision on `QUANT-GGUF-KEEPQ-LOADER`: `VT_GGUF_KEEP_F16` stays DEFAULT
ON, and 1.05 GiB of peak RSS is worth about 9% of prefill and about 1.4% of
decode as a product judgment over our own arms. The full record, including why
a gigabyte of resident weight outranks a slower prefill and why the tokens are
unaffected, is
[`gguf-keep-quant-loader.md`](gguf-keep-quant-loader.md) §"Decision
(2026-08-17)", which REPLACES the `:595` clause quoted above. So the paragraph
...
4. **keep-f16's "RSS gap CLOSED to `1.01x` llama.cpp", with "prefill `1.18x`
   ...
   **That exposure is CLOSED as of 2026-08-17**: the tie-break is now a product
   decision over our own same-binary arms, so this re-take corrects three
   published ratios and two source comments and cannot reopen the default. See
...
- **The keep-f16 default was owed a decision, not only a re-wording. It is
  ...
  keeps `VT_GGUF_KEEP_F16` DEFAULT ON and restates the tie-break as a product
  judgment over our own same-binary arms: 1.05 GiB of peak RSS (3.885 to
  2.832 GiB) is worth about 9% of prefill (224 to 204 t/s) and about 1.4% of
  decode (TPOT 40.4 to 40.95 ms), with tokens byte-identical on all three arms.
  Recorded in [`gguf-keep-quant-loader.md`](gguf-keep-quant-loader.md)
