...
|---:|---|---|---|---|
| W1 | **Prefix-cache statistics** — `PrefixCacheStats`, queries/hits/hit-rate recorded at `kv_cache_manager.cpp:139` and reset at `:268`; engine-level accessor | ported `test_prefix_cache_stats_disabled` + `test_kv_cache_metrics_collector_smoke`; CPU CTest | CPU | open |
