...
| M7 | `state_idx` ignored: row `s` for sequence `s` unconditionally | kernel | **RED, 1 of 10 cases, 3 of 538** |
...
| M13 | the `state_idx` range check removed | dispatcher | **RED, rc = -6 (SIGABRT), 1 of 10 cases, 1 of 535** — the refusal assertion fails and the unchecked row index then writes past the cache, so the process aborts. A red either way, and the abort is the reason the check is a refusal rather than a comment |
...
- **W5b-3 ([#2156](https://github.com/mudler/vllm.cpp/issues/2156)) lands
  ...
  - **Reaching the op from the runner's recurrent cache.** The op takes its state
    ...
    index, which is deliberately NOT upstream's `state_idx`: upstream's selects
