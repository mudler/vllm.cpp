...
## 6b. GATE RESULT — **FAILS 4/6**, and the failure is sharply localized
...
**The gate is NOT loosened.** This row does not claim correctness.
...
### Revised leading hypothesis: this is the PRE-EXISTING dense-forward bf16 drift
...
**This is a hypothesis, not a conclusion.** The project's ratified way to settle
it is the teacher-forcing measurement (`prompt_logprobs`, as
`scripts/qwen3-neartie-gap.py` / `scripts/qwen3coder-neartie-gap.py` do): feed
vLLM OUR exact prefix and ask whether OUR token is vLLM's own argmax there, and
with what logit gap. If the gaps are ≈0 the forward matches and the flips are
bf16 ties; if a gap is large it is a real bug in this row's new code. **That
measurement is NOT done and is the named next step** — until it is run, this row
must be treated as failing a strict gate for an unproven reason.

### Full gate evidence (dgx, clean `121a` build, `flock /tmp/gpu`)
...
| SACRED strict engine gate | **4/6 prompts, 67/96 tokens — GATING (not passed, not loosened)** |
| Execution counters (positive signal) | `marlin_gemms=18432`, `fused_gate_up=6144`, `fallback_gemms=0` — the W4A16 path PROVABLY ran |
| `VT_NVFP4_MARLIN=0` A/B | 3/6 (worse), SAME two divergences + one more |
...
### Disposition
Row lands **`ACTIVE` / correctness-GATING**, never `DONE`. Correctness is NOT
claimed; speed was never started.
...
7. **The strict gate FAILS 4/6 (§6b), and we do not loosen it.** The failure is
   invariant across both of our quantized GEMMs, so it is probably NOT this
   row's new code — but that is a hypothesis until the teacher-forcing
   measurement is run.
8. **Marlin matches vLLM better than a "purer" dequant does (4/6 vs 3/6)** —
