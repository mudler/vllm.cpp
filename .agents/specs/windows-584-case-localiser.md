...
byte-bound refusal that the log's last line pointed at. Every case that
constructs a `LoadedEngine` on Windows fast-fails; every case that does not,
passes.
...
- **The cure for #584.** The location is now known: `LoadedEngine` construction
  on Windows. The repair is a separate row with its own spec, because it is a
  `src/` change in code this spec deliberately does not touch.
...
both lanes name cases 56, 57 and 59 of 76, and the common factor is
`LoadedEngine` construction. #584 stays open for the repair.
