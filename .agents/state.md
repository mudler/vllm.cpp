...
## 2026-07-20 — ENG-ASYNC-SCHED robustness: runner `discard_request_mask` fixes the c8 + short-output async assertion crash

**Bug (pre-existing, reported).** `AsyncScheduler::update_request_with_output`'s `assert(num_output_placeholders >= 0)` (`src/vllm/v1/core/sched/async_scheduler.cpp`) aborted under concurrency-8 + short-output-length with `VT_ASYNC_RUNNER` ON (surfaced during the 35B c8 in-situ A/B; prior agents worked around it with `VT_ASYNC_RUNNER=0`).

**Root cause (NOT the scheduler).** The async scheduler mirrors vLLM correctly. The defect was in `GPUModelRunner`: it emitted a sampled token for EVERY scheduled request, including requests still consuming prefill tokens (partial prefill chunks). vLLM's scheduler REQUIRES the runner to return EMPTY token ids for a still-prefilling request (`scheduler.py:1888-1890`) via `discard_request_mask` — which our runner was missing entirely. When a short-output request under KV pressure is preempted and re-prefilled in chunks, each spurious prefill-chunk token was appended as output AND drained a placeholder that `update_after_schedule` never reserved (the `is_prefill_chunk` `continue` adds none) → underflow → abort. Latent on the single-request non-chunked gates (never chunk/preempt); exposed only by c8 + short-output. Diagnosed by an isolated CPU depth-2 driver: a FAITHFUL runner (empty for prefill chunks) makes the scheduler correct; the current runner behavior underflows — proving the runner is the bug.

**Fix (mirror vLLM's `discard_request_mask`, runner-side only).** `execute_model` computes `exec_state_.discard[i] = (step.seq_lens[i] < num_tokens_no_spec[i])` (`gpu_model_runner.py:2029-2051`); sync `sample_tokens` clears those rows to empty + skips write-back (`outputs.py:303` `valid_sampled_token_ids[i].clear()`); the async device path passes `invalid_req_indices` to `AsyncGPUModelRunnerOutput`, whose `get_output()` clears them (`gpu_model_runner.py:3625-3628`) and skips their `num_tokens_no_spec`/`last_sampled_tokens` write-back. Scheduler unchanged (assertion kept). Sync/non-chunked decode byte-identical (mask all-zero). Files: `src/vllm/v1/worker/gpu/runner.cpp`, `include/vllm/v1/worker/gpu/runner.h`, `src/vllm/v1/worker/gpu/async_output.cpp`, `include/vllm/v1/worker/gpu/async_output.h`.

**Gates (dgx `~/work/vllm-async-discard-fix`, clean CUDA `-Werror` 0 warn, one flock).** Token-exact 27B `test_qwen27_paged_engine` **235/235** + 35B `test_qwen36_paged_engine` **315/315** (inert on the non-chunked gates). New RED→GREEN unit test `runner: chunked prefill returns empty sampled tokens (discard_request_mask)` in `tests/vllm/v1/worker/test_runner.cpp` (RED verified: chunk samples a token + row grows without the mask). New scheduler contract test `AsyncScheduler: c8 short-output chunked-prefill + preemption stays balanced` in `tests/vllm/v1/test_async_scheduler.cpp`. `test_runner` 12/12, `test_async_scheduler` 7/7, `test_async_output` 5/5. Repro: `vllm-bench` synthetic engine `VT_ASYNC_RUNNER=1` c8 + output-len∈{3,4,8} + chunked prefill + KV pressure → all requests complete, no abort (async-OFF control clean). `compute-sanitizer memcheck` on the repro path **0 errors**. Full CPU ctest **124/124**. Robustness fix ⇒ `benchmark_binding=false`.

## 2026-07-20 — KERNEL-FUSION-FRAMEWORK W-series DONE + first additive-model bring-up SPIKED (roadmap pivot)

**Fusion framework (ORDER-1 extensibility cornerstone) — W-series proof milestone COMPLETE.** W0 adopt-one `611fd07`, W1 generalize POD `1115648`, W2 migrate hand-fusions (framework OWNS dispatch, perf-neutral via `fast_op` binding) `5122030`, W3 mechanical-sync proof (a new vLLM fusion pass = 2-code-file additive port) `ed2d4a7`, W4 executable backend-additivity proof (catalog 1→7, backend never grows per-recipe) `60d9e36`. + CPU-`-Werror` hygiene `9ca0d59`. Row stays SPIKE-open per named future work (Tier-1 quant-chain perf interpreter; real Metal/Vulkan realization = M4-blocked; per-recipe fast kernels). Delivers declare-once/realize-per-backend + mechanical upstream-sync + CPU/CUDA oracle-drift elimination — the PR-#4 remedy, proven concrete.

**Async runner discard_request_mask fix `eb6a174`** (see prior entry): a real vLLM-parity gap (chunked-prefill+preemption under concurrency).

**Roadmap pivot — first additive-model bring-up SPIKED.** With the extensibility seams all in place and the 35B engine levers exhausted (prefill compute-bound), the roadmap opened to GROW MODEL BREADTH. Spike `.agents/specs/first-additive-model-qwen3-dense.md` (`MODEL-TEXT-qwen3-qwen3-for-causal-lm` `INVENTORIED`→`SPIKE`, `CLAIM-MODEL-QWEN3-DENSE`): target = Qwen3 dense (`Qwen3ForCausalLM`) on `Qwen3-0.6B` BF16 (the only standard-dense arch with a checkpoint + runnable vLLM 0.25.0 oracle on dgx today; Llama needs a download → W-next). ADDITIVITY LEDGER: model-factory seam CLEAN (new TU + 1 in-TU REGISTER, zero shared-array edit); the first pure-dense model FORCES OUT genuine RUNNER seam gaps (#1 `runner.cpp:458` layer_types indexing crashes on non-hybrid; #2 `:645-730` unconditional GDN metadata/step) that hardcode the Qwen3.6 hybrid KV topology — **W1 runner-generalization IS the extensibility payload** (small, one-time, model-agnostic; after it Llama/Mistral add new-files-only). Fusion catalog reuses everything (≈2 new one-line recipes). W0 stub → W1 runner-gen → W2 loader → W3 forward → W4 SACRED token-exact `Qwen3-0.6B` vs oracle 16/16 (regression: 27B 235/235 + 35B 315/315 unchanged). Running autonomously (user-directed).

## 2026-07-20 — First additive-model bring-up W0+W1 LANDED: Qwen3 dense (`Qwen3ForCausalLM`) + the RUNNER GENERALIZATION (`CLAIM-MODEL-QWEN3-DENSE`)

**W0 (config + registry stub) + W1 (runner generalization) DONE and gated** on worktree `agent-a6e6d4e8326908282` (base main `9fd77ad`). Model-matrix `MODEL-TEXT-qwen3-qwen3-for-causal-lm` `SPIKE`→`ACTIVE`; new engine-matrix row `ENG-RUNNER-MODELSHAPE` `ACTIVE`.

**W0 — new files (additive):** `include/vllm/model_executor/models/qwen3.h` (decls: `Qwen3DenseWeights` placeholder, `ParseQwen3ForCausalLMConfig`, `MakeQwen3ForCausalLMKVCache`), `src/vllm/model_executor/models/qwen3_dense.cpp` (the registry TU: `REGISTER_VLLM_MODEL(qwen3, "Qwen3ForCausalLM", …)` — one in-TU line, ZERO shared-array edit; full-attention-ONLY KV spec = one `FullAttentionSpec` group, no MambaSpec; no-op config hook; `Qwen3DenseLoadedModel` whose load/forward THROW a clear "W2"/"W3" message — the forward is NOT implemented in W0/W1). Shared touches: `CMakeLists.txt` (+1 TU), `tests/vllm/models/test_model_registry.cpp` (3rd registration: size 2→3, sort/coverage/error-message updates + a dedicated `Qwen3ForCausalLM` resolve+FA-only-KV case), `tests/vllm/test_model_loader_gguf.cpp` (supported-arch error string). Sort order: `"Qwen3ForCausalLM"` < `"Qwen3_5*"` ('F' 0x46 < '_' 0x5F). Config: `HfConfig` already carries every field W0/W1 consume; `tie_word_embeddings`/`attention_bias` are W2/W3 loader/forward concerns (not parsed yet — recorded for W2).

**W1 — the ONE shared generalization (`src/vllm/v1/worker/gpu/runner.cpp`):** the runner had only ever run the Qwen3.6 HYBRID topology. Two hardcoded assumptions fixed, both driven off the resolved KV-group structure (MODEL-AGNOSTIC `has_mamba_group`/`gdn_group_id_ >= 0`, never a model-name check): (#1) `runner.cpp:458-470` KV-alloc loop — `has_mamba_group && !layer_types.empty() && layer_types[l]=="linear_attention"` (was a bare `layer_types[l]` index, OOB on empty); (#2) `runner.cpp:651-680` — the whole GDN metadata build (`gather_block_table(gdn_group_id_)`/`remap_gdn_state_slots`/`GDNAttentionMetadataBuilder`) is now gated on `gdn_group_id_ >= 0`; a full-attention-only model steps with a default-empty `gdn_meta`. Hybrid path BYTE-IDENTICAL (GDN group present ⇒ same code runs; only default-construct-then-move-assign vs move-construct of `gdn_meta`, identical result).

**Honest additional seam gap (NOT a runner gap):** the SHARED 27B `Qwen3_5DenseModel::Forward` asserts `gdn_meta.num_actual_tokens == T` (`qwen3_5.cpp:5463`) — a FORWARD-side hybrid assumption. Qwen3ForCausalLM gets its OWN dense forward in W3 (won't carry it), so the FA-only runner test asserts control cleanly REACHES that forward (a catchable throw, not an OOB), which proves the runner generalization. This is the one extra honest seam gap beyond spec #1/#2.

**New CPU tests (RED→GREEN, mirror test_runner.cpp):** `runner: full-attention-only KV config allocates without the GDN path` (edit #1) and `runner: full-attention-only step skips GDN metadata build (no OOB)` (edit #2). Verified RED: BOTH SIGSEGV against the pre-generalization runner (git-stash of runner.cpp, rebuild, crash) → GREEN post-fix.

**Gates (all passed).** dgx CUDA `-Werror` build **0 warnings** (`dgx:~/work/vllm-cpp-qwen3-dense-a6e/build`, prod flags CUTLASS sm120a + FA2 sm_121a + Triton AOT; runner.cpp + qwen3_dense.cpp clean). **Behaviour-preservation: 27B `test_qwen27_paged_engine` 235/235 + 35B `test_qwen36_paged_engine` 315/315 token-exact UNCHANGED** (both EXIT=0 under one flock). Registry test 163 asserts (resolves `Qwen3ForCausalLM`). Full test_runner 14/14 (incl. hybrid behaviour-preservation) + 2 new cases. ASan/UBSan **clean (0 err, 0 leak)** on the affected FA-only + registry paths (full-suite leaks are pre-existing DevicePool caching in hybrid MoE fixtures — `qwen3_5.cpp:427/691`, not the new code). Full CPU ctest green except `test_capi` (pre-existing timing-flaky async streaming early-stop — passes on rerun) and `test_bench` (slow, passes with a longer timeout).

**Additivity so far:** new files = `qwen3.h` + `qwen3_dense.cpp` (+ 2 new test cases). Shared touches = the designed/unavoidable ones (`CMakeLists.txt` TU add, the in-TU REGISTER line, the registration-count/arch-list test updates) + THE ONE generalization: `runner.cpp` (`ENG-RUNNER-MODELSHAPE`). Confirmed: the runner-generalization is the single shared behaviour change; everything else is additive or mechanical registration bookkeeping. After W1 every future dense/non-hybrid arch (Llama, Mistral) adds new-files-only, zero further runner edits.

**Remaining:** W2 weight loader (Qwen3 name map + tied lm_head; extract shared BF16 loader helpers per spec §3 gap #3) → W3 forward (≈2 new one-line fusion recipes: non-gemma add-rmsnorm + non-gated qk-norm-rope) → W4 SACRED token-exact `Qwen3-0.6B` vs vLLM 0.25.0 oracle 16/16. Committed on this worktree branch (NOT pushed — SHA reported to the caller).
