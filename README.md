...
| 35B FA2-prefill lever (oracle question) | ✅ INVESTIGATED / oracle grounded, flip not taken | The 35B FA2-prefill + fused-preamble lever ships opt-in because the deterministic greedy stream was seen (2026-07-09) to diverge; the MIRROR-vLLM oracle question is now RESOLVED (`CLAIM-35B-FA2-ORACLE`, [spec](.agents/specs/qwen36-35b-fa2-prefill-oracle-2026-07-18.md)): vLLM PRODUCTION runs FA2 bf16-q + the fused qk-norm-rope-gate preamble, and vLLM 0.25.0 graphed greedy == the stored oracle **16/16 identical** ⇒ the oracle is production-faithful, so **no re-baseline**. On an old build behind main the lever now passes the 35B gate **315/315** (nsys-confirmed FA2 ran; the 2026-07-09 divergence is stale) at ~1.86× the WMMA attention kernel | Enable-by-default is verified-ready but deferred to a current-main FULL-default-set battery + a fused-kernel tighten (round normed q/k→bf16 before RoPE, mirror `fused_qk_norm_rope.py:67`) + a realistic-prefill TTFT A/B |
