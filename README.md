...
**MLA / DeepSeek campaign — W0-W3 landed, no MLA model runs yet (2026-07-21).** The breadth
...
four steps of its plan are now in the tree. **No DeepSeek/Kimi model is supported** — the five model
rows stay `SPIKE`: there is still no MLA attention math, no MLA model and no MLA forward, so nothing
in this family can be loaded or run.
