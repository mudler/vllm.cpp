...
> non-tail/memory 4%). The c2 TTFT-family voids are grounded as an
> upstream-mirrored prefill co-schedule ARRIVAL LOTTERY (our waiting-queue
> admission is a 1:1 mirror of vLLM's — no scheduler divergence, scheduler
> unchanged). The packed component stays `GATING` (no speed credit)
