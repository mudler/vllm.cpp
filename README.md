...
actionable sweep is model breadth on GB10. **The Metal track now runs its first model: OPT-125m
generates on the Apple GPU and produces the SAME 96 tokens, exactly, that our NVIDIA path
produces** (2026-07-22, [study §12](.agents/specs/metal-mlx-reuse-study.md)) — measured against
the same recorded reference-implementation answers, which do not depend on which device runs
them, so Apple had to meet the bar NVIDIA already met rather than a new bar invented for it.
Attention, the KV cache and greedy sampling now exist on Apple GPU (fifteen operations of
seventy-five). **No speed is measured or claimed** — that Mac could not be made quiet enough for
an honest timing, and correctness and speed are separate bars of which only correctness is met.
