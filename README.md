...
**MLA / DeepSeek campaign — W0-W5 landed, no MLA model runs yet (2026-07-22).** The breadth
...
six steps of its plan are now in the tree. **No DeepSeek/Kimi model is supported** — the five model
rows stay `SPIKE`: the attention family (decode and prefill) now exists, but nothing composes it —
there is no MLA attention block, no weight absorption, no MLA model and no MLA forward, so nothing in
this family can be loaded or run.
...
inherited. Again: **no speed number**, and still no model. Read the
