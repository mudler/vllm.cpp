...
| Host-memory repair | **BINDING PASS for the established 27B windowed-release gate**: `LOAD-SAFETENSORS` progressive `madvise(MADV_DONTNEED)` remains in the `246a23c` binding binary (ours peak PSS 24.88 GB vs vLLM 28.18 GB), and the 35B routed-expert steady mirror/load interleave remains implemented. The additive plain-BF16 direct-device leaf (`LOAD-SAFETENSORS-DIRECT-DENSE`, [spike](.agents/specs/qwen35-plain-bf16-direct-load.md)) is **`ACTIVE` on a corrected Triton-AOT 4B rerun**. Root `/tmp/qwen35-transplant-4b-647a2a6a` preserves valid same-binary loader evidence: direct ON=OFF output IDs **128/128 in every pair**, ON improves throughput **1.21%**, and cuts peak/stable PSS **8.559→2.606 GiB / 8.558→0.727 GiB**. Its current-vs-previous/vLLM performance conclusion is **VOID**, however: the current project binary had `VLLM_CPP_TRITON=OFF`, while the previous **6607.04 tok/s** project binary used `VLLM_CPP_TRITON=ON`, `VT_GDN_DECODE_TRITON_AOT=1`, sm_120 and FlashInfer CUTLASS; vLLM also loaded AOT/Triton paths. Current source is being rebuilt with the matching previous-project AOT configuration before all 18 legs and graph-node traces are repeated. Current-v0.25 correctness/performance, sanitizer and strict ON<=OFF VRAM remain open; 27B/35B remain external-hardware follow-ups with no support extrapolation |
