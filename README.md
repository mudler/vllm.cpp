...
|---|---|---|
| CPU | x86-64 and arm64 reference | 🟡 Correctness/CI implementation with native threadpool. **Compute-in-quant is now LIVE and on by default** (2026-07-22): a GGUF weight in one of six supported block encodings (Q4_0, Q8_0, Q3_K, Q4_K, Q5_K, Q6_K) stays compressed from file to matrix multiply instead of being expanded to 16-bit at load, and the multiply runs directly on the compressed blocks. Correctness is intact: the 35B GGUF acceptance gate still reproduces llama.cpp's own greedy output token for token on both mixed real-model files, and on the benchmark model the output tokens are **byte-identical** to the previous path — no result moved. `VT_GGUF_KEEP_QUANT=0` turns it off and `VT_CPU_REF=1` restores the old reference path exactly. **The ordinary 16-bit matrix multiply has since been given real vector instructions too** (2026-07-22), which was the remaining gap: it now runs 16 independent accumulator chains instead of one, with per-architecture vector code (arm64 NEON, x86-64 SSE2/F16C) chosen at run time, and it is **bit-for-bit identical** to the old kernel — every result byte the same, so no stored expected output had to change. Measured on the same file and idle arm64 box, same binary: **prefill 3.41x faster, decode 3.11x faster** on top of the quantized win above. **Against llama.cpp on the same file, decode is now at parity — 1.03x, within 3% and inside llama.cpp's own run-to-run spread** (23.8 vs 24.5 tokens/s). **Prefill is 2.3x behind** (was 8.2x) and **peak memory 2.3x worse** (6.40 GiB against 2.80 GiB, unchanged — that is a model-loading problem, not a compute one). The next step is deliberately a re-measurement rather than a new optimisation: the last change made this multiply 1.63x faster at the operation level and moved end-to-end prefill by 0.0%, which proves prefill is now limited by something else, and we do not yet know what ([elementwise GEMM result](.agents/specs/cpu-elementwise-gemm.md), [quantized-compute result](.agents/specs/gguf-compute-in-quant-gemm.md), [measurement + ranked plan](.agents/specs/cpu-llamacpp-floor-remeasure-2026-07-22.md)) |
