...
| Async/overlap scheduling (`ENG-ASYNC-SCHED` W3) | **Host-side machinery + runner device-input half LANDED, CPU-gated (2026-07-16), `ACTIVE`.** The scheduler/engine half — `AsyncScheduler` output-placeholder accounting, depth-2 `step_with_batch_queue`, `async_scheduling` default-ON-when-compatible config resolution, `VT_ASYNC_SCHED` rollback — is implemented and CPU-tested RED-first (6 ported async-scheduler cases + a depth-2 engine cycle). The runner **device-input** half now lands too: `combine_sampled_and_draft_tokens` rebuilds each decode row's input id from the GPU-resident-analog `last_sampled_tokens` instead of the host `token_ids_cpu` round-trip (placeholder/prefill-aware, idx_mapping indirection, capture-safe host-side prep), behind `VT_ASYNC_RUNNER` / `set_async_input_combine`, **default OFF**. CPU-tested RED→GREEN (combine unit cases + last_sampled/prefill_len batch-lifecycle threading + a runner token-exactness async-ON≡sync case + a reads-`last_sampled`-over-a-stale-host case); full CPU ctest 110/110. The synchronous path stays byte-identical. **Remaining DGX-gated:** the non-blocking sampled-ID D2H on a copy stream + `vt::Backend` event/pinned primitives (the sampler-output half of the ~3.25 ms/step idle capture) + flipping `runner_supports_async` ON by default. Until then production stays synchronous, so this commit changes no GPU behavior |
