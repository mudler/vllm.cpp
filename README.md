...
**MLA / DeepSeek campaign — W0 + W1 landed, no MLA model runs yet (2026-07-21).** The breadth
...
two steps of its plan are now in the tree. **No DeepSeek/Kimi model is supported** — the five model
rows stay `SPIKE`, there is still zero MLA math, zero MLA ops and zero MLA model code.
...
(315/315), Qwen3-Coder (6/6) and Qwen3-dense (16/16) correctness gates are all unchanged. Read the
...
| compressed-tensors NVFP4A16 (W4A16), dense | 🟡 **New scheme, correctness GATING.** `RedHatAI/Qwen3-32B-NVFP4A16` (`Qwen3ForCausalLM`, 64 layers) loads and computes natively through the Marlin weight-only path — the same kernel vLLM itself forces for `use_a16` on sm_121 (observed: `Using MarlinNvFp4LinearKernel`), so this scheme needed **zero new kernel code**. The loader, scale conventions and merged-shard collapse mirror vLLM; a CPU synthetic check is bit-exact against the dequantized reference. The loader gate passes fully (all 1603 tensors mapped, 5144 assertions) and the engine path is memcheck-clean, but the strict token-exact engine gate currently **passes 4 of 6 prompts (67/96 tokens)** and is deliberately not relaxed: every prefill argmax is correct, both misses are single semantic near-ties at the first decode step, and disabling Marlin reproduces them, which points at the known BF16 rounding drift the dense Qwen3 path already carries rather than at the quantization. Speed is not yet measured |
