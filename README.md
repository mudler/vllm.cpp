...
> The first three-arm trace attempt is **VOID**: the prewarm capture and
> **144/144** requests completed, but mutable client artifacts changed the
> cache-drop inventory from 50 to 58 files, so the lifecycle validator stopped
> before lazy or vLLM ran. No kernel result is inferred; the corrected
> immutable-root trace is next. Optional versioned
