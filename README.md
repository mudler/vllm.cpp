...
> The retained process log also contains no frozen-plan lifecycle/selection
> records, while the status path only hashes that log instead of enforcing the
> exact 64-plan/zero-tune contract. The attempt therefore cannot be salvaged.
> H1d must repair both trace collection and semantic plan/workload validation
> before implementation. No speed credit, exact grid or 35B performance is
> accepted.
...
| CUDA | NVIDIA (first target: GB10 / DGX Spark, sm_121a) | 🟡 **gate-model paged-KV stack running on GB10** with both greedy correctness gates passing. Immutable `3f256ab` binds at **55/124**. W3-C frozen-map control passes; W3-E/W3-F strict-fail their components and earn no speed credit. W3-G implements the traced ratio-6 FA2 split-KV route: cast-free BF16 model/dispatcher selection, exact upstream group-swap strides and split heuristic, per-stream/per-shape capture-stable F32 scratch with queue cleanup, `VT_FA2_DECODE=0`, and ported ratio/fallback/capture/lifecycle tests. Clean immutable `ae9e8ff` passes sm_121a compile, **20/20 + 454,323** operator assertions, zero-error/zero-leak memcheck, both 27B arms, 35B correctness-only inertness and the exact paired structural trace. Its completed frozen c2/c16 component reaches **1.017668×/1.006548×** mean total throughput but strict-fails **35/40 timing + 5/8 memory**, so it earns no speed credit. W3-H is **ACTIVE / trace-first** for I/O-only normal BF16→FP4 vectorization. H1a is tactic-confounded; frozen-map H1b completed both profilers but lost at least 930 CUDA events and is void. Immutable `d1f8e33` H1c likewise failed closed on capture 1/3 after Nsight recorded 818,537 CUDA events and warned that not all events might have been collected; r2/r3 and vLLM never ran, and the retained log has no frozen-plan proof. H1d lossless collection and semantic plan/workload validation are pending. Exact grid and 35B performance remain blocked. |
...
- **Modernization and removal are trace-gated.** The v0.25.0 audit found no
  ...
  **FAILED / VOID**: after the **19.20-s** model-gate pass, capture 1/3 emitted
  ...
  Its process log also has no parseable frozen-plan lifecycle, so implementation
  remains prohibited and no speed credit is claimed. H1d must make collection
  lossless and enforce the plan/workload semantics before residual ranking.
