...
| Host-memory repair | **BINDING PASS for the established 27B windowed-release gate**: `LOAD-SAFETENSORS` progressive `madvise(MADV_DONTNEED)` remains in the `246a23c` binding binary (ours peak PSS 24.88 GB vs vLLM 28.18 GB), and the 35B routed-expert steady mirror/load interleave remains implemented. The additive plain-BF16 direct-device leaf (`LOAD-SAFETENSORS-DIRECT-DENSE`, [spike](.agents/specs/qwen35-plain-bf16-direct-load.md)) remains **`GATING`**. **Dense-4B H32 AOT repair performance is proven at `176d4926`:** exact operator **56/56**, full GDN **3,229/3,229**, real-model default/rollback **1,664/1,664** each and memcheck clean; same-binary AOT/fallback total **6427.07/6155.57 tok/s (+4.41%)**, TPOT **39.457/41.437 ms (-4.78%)**, with node-mode recurrence time **1.467331/2.585435 s (-43.25%)**. Fresh 18-leg direct ON/OFF/vLLM-0.25 total is **6418.26/6316.05/6744.29 tok/s**: repaired ON reaches **0.9714x** the previous AOT result but only **0.9517x vLLM**, failing TPOT/E2EL while winning TTFT and PSS. Direct ON=OFF outputs stay 128/128; the long AOT/fallback streams differ 9/128 and each matches the deterministic eager v0.25 oracle only 97/128. Strict loader VRAM also remains open (fresh mean ON/OFF **12890/12870 MiB** vs +8-MiB allowance), as do remaining decode parity and external 27B/35B regressions. No support extrapolation. [Repair evidence](docs/bench-evidence/qwen35-4b-h32-aot-repair-20260721.md) |
