...
| Host-memory repair | **BINDING PASS for the established 27B windowed-release gate**: `LOAD-SAFETENSORS` progressive `madvise(MADV_DONTNEED)` remains in the `246a23c` binding binary (ours peak PSS 24.88 GB vs vLLM 28.18 GB), and the 35B routed-expert steady mirror/load interleave remains implemented. The additive plain-BF16 direct-device leaf (`LOAD-SAFETENSORS-DIRECT-DENSE`, [spike](.agents/specs/qwen35-plain-bf16-direct-load.md)) is **`ACTIVE` / GATING**. Its last binding local Triton-AOT 4B root `/tmp/qwen35-transplant-4b-aot-557ab41d` completed 18/18 guarded legs: direct ON/OFF/local-vLLM-0.24 total **6155.10/6064.06/6730.46 tok/s**, peak PSS **2.405/8.571/7.569 GiB**, stable PSS **0.733/8.571/4.066 GiB**, peak VRAM **12892/12884/12942.7 MiB**, and ON=OFF outputs **128/128**. ON is +1.50% and cuts peak/stable PSS 71.9%/91.4%, but is **0.9316x** the previous matching-AOT 4B result and **0.9145x** local vLLM. Matched node-mode profiles attributed the loss to the absent current-ABI H=32 packed Triton recurrence. **The repair is now implemented:** vendored `gdn_decode_h32` uses the current raw-packed/FP32-state ABI and upstream warps1/stages3 configuration; exact Hv=48/32 AOT-vs-hand-vs-CPU coverage is **56/56**, full GDN is **53/53 (3,229/3,229)**, flag parsing is **10/10**, and generated sm_120/sm_121a manifests pass drift validation. No new speed is binding yet. Next is the clean-commit 4B token/sanitizer/repeated same-binary A/B plus node-mode profile; current-v0.25 correctness, strict ON<=OFF VRAM (+8 MiB) and external 27B/35B remain open with no support extrapolation. [Profile evidence](docs/bench-evidence/qwen35-4b-aot-regression-profile-20260721.md) |
