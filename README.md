...
| Tool-call parsing | 30 dialects, streaming | Every pure-text tool parser in the pinned vLLM registry, ported 1:1 with its upstream tests. Selected via `tool_parser` or auto-detected from the chat template; `tool_choice` required/named compiles the family's NATIVE syntax where a structural tag can express it, else runs unconstrained. Full tables: docs/BENCHMARKS.md + parser file headers |
