...
> 315/315 while the kernel stays 2.41× faster in isolation. A sibling
> batch-independent glue lever landed the SAME way (2026-07-17,
> `CLAIM-EW-NORM-GATED-1`): the **GDN gated RMSNorm** (`VT_RMSNORM_GATED_FAST`,
> +0.40 ms/step vs vLLM's fused gated norm per the c2/c8 attribution) now has a
> BIT-IDENTICAL (0-ulp) fast kernel — **isolated 2.04× at c16** (1.3–2.0× across
> c1–c16), full default set 27B 235/235 + 35B 315/315 — also flipped ON. The
> 52/124 binding was measured before these flips; the next binding grid re-measures
> the production default. See [BENCHMARKS](docs/BENCHMARKS.md). **Honest regression:** ours lost **−2.67% total throughput at c16** and
