...
> as **ACTIVE / trace-first**. W3-H1 has now made two complete paired-trace
> attempts, but both are **VOID**. H1a accidentally loaded a writable native
> 64-plan cache: **37/64** tactics differed from the accepted v0.25 fixture,
> including the active M=16/N=34,816/K=5,120 route. H1b fixed that confound—
> read-only mode loaded **64 FlashInfer / 0 native**, tuned/rejected/saved zero,
> and reproduced map SHA `f2d9be7f…1fa4`—then completed the model gate, three
> 48-request local legs in one Nsight capture, vLLM Torch trace and all cache
> drops under one lock.
> Its Nsight SQLite nevertheless reports **“Not all CUDA events might have
> been collected”**: the primary 1,107-node graph splits into **930 nodes at
> 1,372 replays and 177 at 1,373**, proving at least 930 missing events. Exact
> counts, duration sums and the cross-engine ranking therefore do not bind.
> Retained per-node samples only diagnose the normal producer at about
> **0.638 ms/decode forward** (ahead of fused production at about 0.544 ms), so
> no verified residual displaces W3-H, but H1 is not passed and implementation
> remains prohibited. The trace harness now isolates the three repetitions in
> three independent Nsight captures under the same lock, uses the
> Nsight-recommended 10-second CUDA flush, disables unrelated CPU
> context-switch tracing, drains a final flush interval, hashes every SQLite
> export, and fails before vLLM on any non-whitelisted severity diagnostic or
> uneven primary-graph replay count; it also requires the exact 1,107-node
> graph and the 208 FP4 GEMM / 144 normal / 64 fused / 48 recurrence / 16+16
> FA2 node cardinalities in every capture.
> Immutable H1c at clean pushed `d1f8e33` is **FAILED / VOID**. Its 27B model
> gate passed in **19.20 s**, but capture 1/3 emitted Nsight severity-2
> `Not all CUDA events might have been collected`; the fail-closed driver
> stopped before captures 2/3, SQLite acceptance or the paired vLLM profile.
> The immutable build was also invalid for this comparison: its clean source
> lacked CUTLASS, CMake explicitly disabled the NVFP4 CUTLASS path, and the
...
> selection records. H1d's diagnostic-only implementation is now CPU-gated:
> `VLLM_CPP_BENCH_PROFILE_CONTROL=ON` adds a SIGUSR2 arm that keeps the exact
> 16-warmup/48-measured workload outside collection, then captures exactly four
> warmed dense graph replays. The driver refuses timing mode while that build
> is selected, and production manifests must prove the observer is absent.
> Before GPU work, the schema-v3 harness hashes the external CUTLASS tree and
> configure/compile/binary provenance; afterward it requires three unique
> Nsight 2025.3.2.474 sessions, zero profiler exit status, direct runtime-child
> correlation, identical 1,107-node/resource multisets and the exact frozen
> plan lifecycle. It independently slices the vLLM raw trace into **1,476 clean
> decode windows out of 1,588 generation annotations**, with exact per-window
> family counts. Valid immutable H1d DGX evidence is still **PENDING**. No
> speed credit, exact grid or 35B performance is accepted.
...
> The third clean root, `5f8fab1`, compiled the full CUTLASS target, then
> failed the mandatory 27B model gate **3/3** with profile control enabled and
> **2/2** with it compiled out: all five runs produced the same 16-token stream
> and diverged from the production golden at token 8. This rules out the
> observer. Build provenance instead found that the H1d recipe had omitted the
> binding vendored Triton-AOT GDN path and used `Release`, while accepted
> `3f256ab`/`ae9e8ff` use `VLLM_CPP_TRITON=ON`, regeneration off, and
> `RelWithDebInfo`; the retained `ae9e8ff` binary still passes the same gate.
> No profiler capture ran. The repaired preflight now rejects any missing
> Triton-AOT/FA2 definition or build-type mismatch before the GPU lock.
> The next clean setup, `d063f20`, is **FAILED / VOID before plan/build/GPU**:
> the frozen corpus was copied into the SHA evidence directory before
> `--dry-run`, and the harness correctly refused to mix a new plan with
> pre-existing artifacts (failure-log SHA `b16476f0…de87`). No manifest,
> configure, model, lock or GPU command ran. The exact recipe now makes the
> ordering explicit: generate the plan in an empty evidence root first, then
> seed the frozen corpus.
> Clean `b1c7eb6` passed that plan-first setup and exact Triton-AOT/FA2/CUTLASS
> configuration, then stopped before build/GPU because the driver required an
> already-built `examples/server` before reaching its own provenance-recorded
> build step (driver-log SHA `f24c01c6…6e0a`). The bootstrap check now accepts
> a configured CMake tree, performs the exact server/model-gate build, and only
> then requires the executable. That root remains void; a fresh pushed SHA is
> pending.
> Clean `e1acb75` is the first exact-path H1d attempt to reach profiling. Its
> recorded build completed **154/154**, schema-v2 provenance proved the
> `RelWithDebInfo` Triton-AOT/FA2/CUTLASS/sm_121a path, and the mandatory 27B
> gate passed **1/1 in 17.42 s**. The profiled server then became ready with
> the exact read-only 64-plan map, but the harness incorrectly required its
> process group to equal the `nsys` PID. Nsight 2025.3.2 actually creates
> `nsys -> nsys-launcher -> target`, with the target as a separate session
> leader. The run stopped before its 48-request trace workload and generated no
> report, SQLite, status or vLLM trace, so it is **FAILED / VOID** and changes
> no speed result. The repair now records and validates that full ancestry and
> explicitly owns cleanup of both sessions; focused harness tests pass
> **29/29**. A new pushed SHA/root is still required.
> Clean `a96e899` then repeated the exact **154/154** build and passed the 27B
> gate **1/1 in 16.85 s**. Capture 1 completed the ordinary **48/48** workload,
> the **16/16** probe and the exact four-replay window (`prior_replays=484`,
...
> immutable evidence remained pending.
> Clean `219f4f2` then proved that FIFO path: the exact **154/154** build and
> 27B gate **1/1 in 17.22 s** passed, capture 1 completed **48/48 + 16/16**,
> and the FIFO lifecycle, removal, target exit zero and Nsight exit zero all
> passed. The exported SQLite is nevertheless **FAILED / VOID**. It contains
> all four launches and exact **1,107 kernel + 7 memcpy + 1 memset nodes x4**,
> but the continuous range also captures three between-replay sampler/input
> gaps (**9 eager kernels, 9 memcpys, 3 memsets**) and Nsight emits severity-2
> possible event loss. Validation remains strict. H1d is now specified as four
> isolated synchronized ranges via `--capture-range-end=repeat:4`. That repair
> is implemented and CPU-gated: each replay has its own profiler start → graph
> launch → stream sync → stop range, and schema v3 requires four distinct
> successful `cuProfilerStart` records before accepting the zero-eager graph.
> A new immutable DGX root is pending. No speed result changes.
> Clean `b2c940c` exercised that repair. The exact **154/154** build, 27B gate
> **1/1 in 17.30 s**, ordinary **48/48** workload, **16/16** probe, frozen
> 64-plan map, FIFO lifecycle/removal and target/Nsight zero exits all pass.
> Nsight's real `repeat:4` output is four indexed reports from one session, not
> the one combined `ours-r1.nsys-rep` assumed by schema v3, so the driver failed
> closed before captures 2/3 or vLLM. Read-only exports show each report has
> exactly one launch, **1,107 graph kernels + 7 graph memcpys + 1 graph memset**
> and zero eager CUDA work. All four still emit severity-2 possible event loss.
> The root is **FAILED / VOID** and changes no accepted number. H1d now requires
> schema v4 over **3 sessions x 4 reports**, `repeat:4:sync`, and a diagnostic
> device synchronization before each profiler stop. That repair is not yet
> implemented; no warning is whitelisted and W3-H2 remains prohibited.
...
| CUDA | NVIDIA (first target: GB10 / DGX Spark, sm_121a) | 🟡 **gate-model paged-KV stack running on GB10** with both greedy correctness gates passing on the accepted configuration. Immutable `3f256ab` binds at **55/124**. W3-C frozen-map control passes; W3-E/W3-F/W3-G strict-fail their components and earn no speed credit. W3-H is **ACTIVE / trace-first**. H1a/H1b/H1c and H1d attempts through `b2c940c` are void. The latest exact build/gate, clients, frozen plans, FIFO lifecycle and zero exits pass; four reports each contain one complete graph replay and zero eager CUDA rows, but all retain severity-2 possible loss and schema v3 expected one combined report. Schema v4 over 3 sessions x 4 reports plus synchronous generation/device flush is pending implementation. Exact grid and 35B performance remain blocked. |
...
| Format | Status |
|---|---|
| NVFP4 (W4A16 MoE / W4A4 dense, Blackwell) | ✅ **both running on GB10** with token-exact greedy gates passing on the accepted configuration. The W4A4 path includes all 32 SM12 tactics, merged/fused projections, pre-serve tuning and packed QKV. W3-C closes frozen-plan control; W3-E, W3-F and the separate W3-G attention repair preserve correctness/structure but strict-fail their components. `3f256ab` still fails **69/124** axes. The complete W3-H spike scopes only byte-identical, process-cached 256-bit-load/64-bit-store normal BF16→FP4 I/O behind an exact scalar fallback. H1a/H1b/H1c and H1d attempts through `b2c940c` are `VOID`; the latest proves exact build/gate, frozen plans, FIFO/zero-exit and four complete single-replay reports with zero eager CUDA rows, but every report emits severity-2 possible loss and schema v3 expected one combined report. Schema v4 over 3 sessions x 4 reports plus synchronous generation/device flush is pending implementation. No W3-H2 kernel, speed credit or support expansion is claimed. |
...
- **Modernization and removal are trace-gated.** The v0.25.0 audit found no
  ...
  **35/40 timing + 5/8 memory**; all 12 legs, 612 requests, memory returns,
  ...
  exact grid follows. W3-H records the complete vectorized normal FP4 producer
  spike as **ACTIVE / trace-first**. H1a completed but is `VOID` because a native
  cache changed 37/64 tactics. Frozen-map H1b also completed its model gate,
  three local trace legs in one capture, vLLM trace and cache lifecycle, but is
  `VOID` because Nsight reports lost CUDA events and the 1,107-node primary graph has two
  replay counts. Retained per-node means keep normal production near
  **0.638 ms/forward**, a diagnostic ranking only. The hardened harness now
  splits H1c into three independent captures and rejects any capture with that
  loss before running vLLM. Immutable `d1f8e33` H1c exercised that guard and is
  **FAILED / VOID**: after the **19.20-s** model-gate pass, capture 1/3 emitted
  the same severity-2 loss warning and the driver stopped before r2/r3/vLLM.
  ...
  CPU-gated. Clean `219f4f2` proves the FIFO lifecycle and zero-exit profiler,
  but is **VOID** because one continuous range includes three eager sampler/
  input gaps and Nsight emits severity-2 possible loss. Clean `b2c940c` then
  proves the four isolated reports contain one exact 1,107-kernel replay each
  and zero eager CUDA rows; it is also **VOID** because every report retains the
  loss warning and schema v3 expected one combined report. Validation is not
  relaxed. The schema-v4 3-session x 4-report contract plus synchronous
  generation/device flush is accepted and pending implementation.
