...
|---|---|---|
| CPU | x86-64 and arm64 reference | 🟢 Correctness/CI reference with a native threadpool; **on the GGUF benchmark file CPU prefill AND decode are now at llama.cpp parity, gap reduced to peak memory (1.39×)**. **Compute-in-quant is LIVE and default-on**: a GGUF weight in six block encodings (Q4_0, Q8_0, Q3_K, Q4_K, Q5_K, Q6_K) stays compressed from file to matrix multiply (no 16-bit expansion) and the multiply runs directly on the blocks (`VT_GGUF_KEEP_QUANT=0` off, `VT_CPU_REF=1` restores the reference path). The ordinary 16-bit multiply is vectorized (16 accumulator chains, per-arch NEON / SSE2·F16C), the linear-attention recurrence and paged attention are threaded, the Q8_0 quant multiply takes an Arm **i8mm** tier and — the last prefill lever, 2026-07-23 — is **repacked once at load into the i8mm interleave** (ported from llama.cpp's repack-at-load): the compressed multiply runs **3.7–5.9× faster** at prefill shapes and whole-model **prefill goes 1.92× faster same-binary to 223.8 tokens/s vs llama.cpp's 177.3 — at/beyond parity** (was ~1.3–1.5× behind), **decode at parity** (24.7 vs 25.4 tokens/s). Every one of these is **bit-for-bit identical** to its predecessor (byte-permute + non-fused MAC, memcmp-gated), so the benchmark output tokens are byte-identical (same md5) with each lever on, off, or in reference mode, and the 35B GGUF acceptance gate reproduces llama.cpp token for token on both real files — no stored expected output moved. Peak memory is **1.39×** llama.cpp (3.88 GiB vs 2.80): the loader reads compressed weights straight out of the memory-mapped file, shares the one tied vocabulary table between embedding and output head, and drops finished file pages. **The residual was previously blamed on the f16→bf16 expansion of the file's non-block tensors; a binding measurement (2026-07-23) DISPROVED that.** Keeping those f16 weights resident as f16 instead — an opt-in loader mode (`VT_GGUF_KEEP_F16=1`) mirroring llama.cpp's own f16 residency — leaves peak memory essentially unchanged (3.88→3.83 GiB, −52 MB) and slows the first prefill, because the earlier page-drop had already reclaimed the f16 file pages, so keeping f16 resident merely swaps an anonymous bf16 buffer for equal-size file-backed f16 pages. A memory breakdown shows our weight residency is ALREADY at llama.cpp parity (2.63 GiB file-backed ≈ its 2.68 GiB file); **the real ~1.1 GiB gap is the engine's own activation / KV working memory, not the weights** — a separate lever, now the sole open CPU RSS item. keep-f16 stays available (byte-identical greedy tokens, native-f16 compute closer to the file's values, useful for sharing weights across serving processes) but is **off by default** because it does not shrink single-process memory and would drop the first prefill below llama.cpp. Two honest scope limits: plain 16-/32-bit tensors have no block form so they still expand and run the general (now-vectorized) kernel; and this is CPU-only — a GPU build still expands every GGUF weight. Specs: [q8_0 repack-at-load + i8mm tier](.agents/specs/gguf-compute-in-quant-gemm.md), [GDN orientation](.agents/specs/cpu-gdn-proj-orientation-2026-07-23.md), [two-kernel threading](.agents/specs/cpu-thread-gdn-paged-2026-07-23.md), [elementwise GEMM](.agents/specs/cpu-elementwise-gemm.md), [keep-quant loader](.agents/specs/gguf-keep-quant-loader.md), [floor re-measurement](.agents/specs/cpu-llamacpp-floor-remeasure-2026-07-22.md) |
