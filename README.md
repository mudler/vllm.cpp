...
> p90 — now attributed to one wave-boundary two-prefill stall mechanism; see
> [BENCHMARKS](docs/BENCHMARKS.md)). **Honest regression:** ours lost **−2.67% total throughput at c16** and
...
> **attempted and PROOF-FAILED (2026-07-16)**: a register-resident
> packed-decode port failed its bit-exact oracle gate and ran ~12% slower
> (register-pressure collapse); the default is flipped back to the legacy
> kernel (`VT_GDN_PACKED_REG_TILE=1` keeps the experiment opt-in)
...
| Qwen3.6-27B performance | ❌ FAILED / `GATING` | **NEW BINDING `246a23c`: 49/124 pass** (fresh interleaved exact-grid rerun; supersedes `3f256ab`'s 55/124, retained immutable). Per concurrency: c1 **20/20**, c2 4, c4 5, c8 4, c16 6, c32 6, memory **4/4**. Memory + c1 + every TTFT axis sweep clean; failure mass is the decode-coupled family at c2–c32 (mean TPOT 2.2–6.5% slower) + two ITL tail anomalies (c8 p99_itl 0.5599, c32 p90_itl 0.7925). HONEST regression: ours c16/c32 total throughput −2.67%/−3.64% vs `3f256ab` while vLLM held; old c16/c32 wins gone. The binary carries slot-fix `c172336`, windowed-load `cb2d310` (memory now PASS), qkvz `45f9e6d` (DGX gates green), packed-decode default. The order-0 packed leaf is **CLOSED on equivalence** (`KERNEL-GDN-PACKED-DECODE` `DONE`, `e47b4d6`; eight seals + 8-pair A/B −0.205% ± 0.30 <1σ + 24-window trace, packed GPU-cheaper); no `complete-pass` marker, no packed speed credit | ATTRIBUTED + VALIDATED (era A/B, four rounds + probe3 at `6dd24df`): `3f256ab`'s high-concurrency throughput WAS inflated by the slot-sharing defect (collapsed slots = 1/16th the GDN state traffic ≈ 8–12 ms/step); the honest correct-state c16 floor is ~790–799 vs vLLM 794, gates 235/235×2, no recoverable host cost (≤15 µs/step; O(n) validation landed). Top lever (MEASURED 2026-07-16, correct-state c16 kernel traces): the ~8 ms/step wall gap (c16 TPOT 159.6 vs 167.5) = ~4.65 ms GPU-busy + ~3.25 ms host/idle; the busy part is ~2.06 ms GDN recurrence tiling (ours 21.31 vs vLLM 19.24 ms/step, state r/w fused in-kernel, ~83% vs ~92% of peak BW) + ~2.6 µs·k norm/quant glue (kernel-EFFICIENCY, not fusion), GEMM/MoE/attention at parity. The naive register-resident port FAILED its proof (oracle FAIL, −12%; default flipped OFF) — the recurrence lever stays open via an occupancy-aware redesign or the sanctioned Triton-AOT vendored-cubin path. Parallel levers: async-sched W3 runner leaf (+3.25 ms) and decode norm/quant kernel EFFICIENCY (the "fusion" framing is REFUTED — vLLM does not fuse rmsnorm+fp4quant, `fuse_norm_quant=False` + `3f256ab` body-dump stores bf16 + separate `cvt_fp16_to_fp4` (144/win == ours), silu+fp4quant already mirrored; residual cross-profiler-confounded + non-bit-exact ≤1.5×, reassigned to `KERNEL-EW-NORM-ACT`, see [reconciliation](.agents/specs/decode-norm-quant-fusion-reconcile-2026-07-16.md)). The ITL tails are ATTRIBUTED (2026-07-16, [spec](.agents/specs/tail-stall-analysis-2026-07-16.md)): one wave-boundary stall mechanism — ours packs TWO full 1024-token prefills into the 2048-token step (~860 ms batch-wide freeze) vs vLLM's graded ~500 ms single-prefill events (its arm runs async scheduling ON); capping the stall at one prefill is counterfactually sufficient to flip both axes. 35B only after 27B 124/124
...
| Remaining gap diagnosis | With memory now passing, the failing mass is the **c2–c32 decode-coupled family** (throughput inversely coupled to TPOT/ITL). The 2026-07-14 [parity rescan](.agents/specs/parity-rescan-2026-07-14.md) already grounded decode as host-side (TTFT passes, our GPU kernels net faster on the measured window). The prior RMSNorm/generated-partitions residual is **disproven** (vLLM's norm-quant fusion is FP8-only). Both open questions this binding raised are now ANSWERED: the c16/c32 throughput regression is corruption-subsidized state bandwidth (era A/B + probe3, `6dd24df`), and the c8 p99 / c32 p90 ITL tail mechanism is the wave-boundary two-prefill stall ([spec](.agents/specs/tail-stall-analysis-2026-07-16.md)) |
| Async/overlap scheduling (`ENG-ASYNC-SCHED` W3) | **Host-side machinery + runner device-input half + sampler-OUTPUT half all LANDED, CPU-gated (2026-07-16), `ACTIVE`.** The scheduler/engine half — `AsyncScheduler` output-placeholder accounting, depth-2 `step_with_batch_queue`, `async_scheduling` default-ON-when-compatible config resolution, `VT_ASYNC_SCHED` rollback — plus the runner **device-input** half (`combine_sampled_and_draft_tokens` rebuilds each decode row's input id from the GPU-resident-analog `last_sampled_tokens`) are implemented CPU-first. The sampler-**OUTPUT** half now lands too: `vt::Backend` gains pinned-host + cross-stream event primitives (CUDA `cudaHostAlloc`+`cudaEvent`, CPU synchronous degeneration), and `AsyncGPUModelRunnerOutput` issues the sampled-token-ID D2H on a dedicated **copy queue** with a completion event so the **main queue never blocks** — `get_output()`, called during output processing off the model's critical path, waits only that event (mirrors vLLM `async_utils.py`). The `Sampler` writes greedy ids device-resident (no host download), `sample_tokens_async` does the on-GPU post-update, and the deferred output threads through the depth-2 batch-queue seam. Everything is behind `VT_ASYNC_RUNNER` / `set_async_input_combine`, **default OFF**; `sample_tokens_async` degenerates to the byte-identical synchronous path when off. CPU-tested RED→GREEN (event/pinned contract + `AsyncGPUModelRunnerOutput` materialize/flush + a `sample_tokens_async` decode ≡ sync case); full CPU ctest 111/111. The synchronous path stays byte-identical. **ENABLE-FLIP now LANDED + CPU-gated (2026-07-16):** `LoadedEngine` reorders `runner_` before the scheduler and builds an `AsyncScheduler` + `max_concurrent_batches=2` (threaded into the async frontend's depth-2 `step_with_batch_queue`) when `ResolveAsyncScheduling(runner_.runner_supports_async())` resolves the overlap ON, else the byte-identical synchronous `Scheduler` + depth-1 — logging vLLM's "Asynchronous scheduling is enabled/disabled" for A/B audit; the device combine/scatter kernel (`src/vt/cuda/cuda_combine_tokens.cu`, 1:1 with `_combine_sampled_and_draft_tokens_kernel`) runs main-stream-ordered on the CUDA async path, deleting `sample_tokens_async`'s pre-scatter `Synchronize` (CPU keeps the host loop). Resolution matrix CPU-tested RED→GREEN. `VT_ASYNC_RUNNER=1` engages full W3 for the DGX A/B; `VT_ASYNC_SCHED=0` is the same-binary rollback; the no-env production default resolves OFF, so this commit changes no GPU behavior. **FULL W3 DGX proof PENDING:** token-exactness both models × {default, `VT_ASYNC_RUNNER=1`, `VT_ASYNC_RUNNER=1`+`VT_ASYNC_SCHED=0`}, then the interleaved c16 on/off A/B (~+3 ms/step target, TTFT-regression watch) |
