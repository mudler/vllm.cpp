...
> cache is disabled because of key collisions. W3-A now stages that exact
> delayed timing window with `VT_FP4_AUTOTUNE_DELAY=0` restoring W2, but its
> clean GB10 build, selected-plan A/B and performance result are still
> **PENDING**. W3-B pre-serve all-bucket warmup follows; optional versioned
...
|---|---|---|---|---|
| Qwen3.5/3.6 hybrid (GDN + gated attention, MoE + dense) | Qwen3.6-35B-A3B, Qwen3.6-27B | ✅ **text submodels** run end-to-end on GB10; token-exact greedy correctness gates pass. The exact clean `b5c6e4f` 27B online checkpoint is valid but below the all-axis floor: c1→c32 total ratios are 0.993/0.952/0.966/0.976/1.021/1.022×, with 4/4/5/4/17/14 of 20 performance axes and 2/4 memory axes. FP4 W2 is correctness-green; its trace shows runtime tactic selection, not tactic availability, is the next mismatch. 35B performance is held behind 27B closure. The upstream wrappers are multimodal; their vision path is not implemented. | ✅ 35B text path from real APEX k-quant `.gguf` on GB10 (greedy parity vs same-file llama.cpp oracle); 27B GGUF pending (no file exists) | 🟡 paged text engine + basic server/tool/grammar subsets; correctness gated, production-vLLM performance `GATING` |
...
- **Production-vLLM performance is below the exact gate.** The historical 35B
  ...
  `VT_FP4_AUTOTUNE_DELAY=0`, with clean GB10 correctness/selection/performance
  evidence pending. W3-B pre-serve all-bucket tuning follows, while W3-C's
  ...
  runtime repair, not a hardware ceiling. A same-binary W3 selection A/B and
  another exact 27B c1-c32 campaign are next; the 35B run waits behind complete
  27B closure.
- The optimized paths themselves remain implemented: vendored Triton-AOT GDN,
  ...
  W3 planner/warmup parity rather than another tactic implementation. W3-A now
  stages FlashInfer's pre-event stream sync plus one-thread 1-ms nanosleep and
  reports the timing arm with each selected stable ID. A disposable precommit
  sm_121a smoke passes 14/14 and 18,619/18,619 in both delayed/off processes and
  proves selection changes on synthetic shapes, but it is non-binding; no
  real-model ID or speed result is claimed until the clean same-binary gate completes.
...
- The CUDA low-concurrency comparison against SGLang is specified and pinned to
  ...
  W3-A delayed-event timing is implementation-staged/GPU-pending; W3-B owns
  pre-serve bucket coverage and the oracle-disabled file cache is not a speed
  denominator.
  W0 is
