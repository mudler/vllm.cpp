...
| MLA / DeepSeek family | DeepSeek-V2/V3/V3.2, Kimi-Linear, Kimi-K2.5 | BF16 (DeepSeek-V2-Lite) | — | 🟡 **CORRECT, NOT YET FAST — DeepSeek-V2-Lite PASSES its correctness gate (2026-07-22), so it is now `ACTIVE`; it is NOT `DONE`, because there is still no speed number of any kind for it** ([spike](.agents/specs/mla-deepseek-campaign.md)). **W0-W7 of the plan have landed.** W7 added the DeepSeek-V2 model itself — configuration parsing, weight loader and forward pass, registered under `DeepseekV2ForCausalLM` only. The real 30 GB DeepSeek-V2-Lite checkpoint loads with all **5291 of 5291** tensors accounted for (none unmapped, none left over) and runs a prompt end to end ("The capital of France is" -> " Paris", repeatable bit-for-bit), with targeted checks on the batch ordering MLA silently depends on, on the shared-expert path this family introduces, and on the GPU path at real head dimensions. **That is a sanity result, not the gate:** token-for-token agreement with vLLM has not been run (W8), so the row stays `SPIKE` and there is no speed number. The original `DeepseekForCausalLM` (not an MLA model at all), V3 (does not fit, and is stored in a format we do not load) and V3.2 (needs a sparse-attention module we do not have) are refused by name rather than claimed. Earlier steps: W0 fetched DeepSeek-V2-Lite and **OBSERVED** the vLLM oracle on GB10 selecting `TRITON_MLA` (dense MLA decode) + FlashAttention (MLA prefill), confirming the source read below, and confirmed the real `config.json` geometry (576-wide latent, 1 KV head, no separate V, `q_lora_rank=null`, no `e_score_correction_bias`); W1 made KV-cache allocation derive size and shape from the KV **spec** instead of hardcoded K+V arithmetic, and added an `MLAAttentionSpec` with upstream's factor-1 page formula — a deliberately behaviour-preserving refactor, verified by the 27B/35B/Coder/dense gates all staying unchanged. W2 completed the port of vLLM's attention-backend selection (the MLA half was previously skipped) as a per-architecture data table, so an MLA request on this GPU now resolves to `TRITON_MLA` as the oracle does, with existing non-MLA selection unchanged and a clean slot left for the sparse/DSA backend a future DeepSeek-V3.2 would need; W3 added the two building-block kernels — the MLA cache write (latent + rotary part packed into one cache row) and the DeepSeek grouped top-k MoE router — each gated against a CPU reference. W4 added MLA's decode attention itself — a structural port of the two-stage kernel pair vLLM was observed running on this GPU, gated at DeepSeek-V2-Lite's real dimensions against an independently written reference, bit-reproducible run to run, and clean under GPU memory/race/sync tooling. W6 composed all of it into a single MLA attention layer and added weight absorption — the load-time rearrangement plus two batched matrix multiplications that let decode read the compressed cache directly instead of expanding it — with both variants of the query path, the decoupled rotary embedding and the two normalizations; the equivalence of the folded and unfolded forms is proven numerically three ways, including by pushing the same batch through two independent code paths that share only the weights. There is still **no MLA model**: nothing registers, loads or runs a checkpoint. W5 added MLA's PREFILL half plus the chunked-context loop that lets a long prompt reuse previously cached context inside a bounded workspace — the FlashAttention path vLLM was observed selecting on this GPU, reached by generalizing the vendored kernel family we already ship (two new compiled instantiations of an unmodified template plus a new entry point; the code path every existing model's prefill uses is pure-addition, zero deletions, and all five existing gates are unchanged). It is gated against an independently written reference and, for the loop, against a reference that never chunks at all, over chunk boundaries, empty-context and empty-chunk requests, and scattered page assignments. There is still **no MLA model, no config parse, no weight loader and no model forward**, so no DeepSeek/Kimi model is usable, and there is no speed number for any of the new kernels. A second coverage limitation, recorded like the router one: the query path that DeepSeek-V3 and GLM-4.7-Flash use is implemented and unit-tested at their real dimensions, but the one checkpoint that fits this hardware does not use it, so it has no end-to-end coverage here. Note the grouped router's distinctive behaviour (sigmoid scoring, the learned selection bias, two-level expert-group masking) is **unit-tested only**: the one checkpoint that fits this hardware does not use any of it. Multi-head Latent Attention is a new attention family for us: we have **zero MLA code** today. The spike determined that on GB10/sm_121 vLLM selects **`TRITON_MLA`** for dense MLA decode and **FlashAttention** for MLA prefill, so the sm90/sm100-only MLA kernels (FlashMLA, CUTLASS MLA, FlashInfer MLA) are unreachable on this hardware and out of scope. The cross-cutting cost is the **compressed-latent KV cache** — MLA stores one 576-wide latent vector per token with a single KV head and no separate V, which our allocator (hardcoded K+V factor 2) and our paged-attention/cache-write ops (K/V-pair signatures) cannot express; the block manager and prefix caching are unaffected. **Hardware verdict: only DeepSeek-V2-Lite (~15.7B, ~29.3 GiB bf16) fits GB10** and it is the planned gate vehicle. DeepSeek-V3/V3.2 (671B), Kimi-K2.5 (~1T), MiniMax-M2 (~230B) and MiniMax-M3 are **HW-BLOCKED end-to-end on this box** — they do not fit in 119 GiB of unified memory, and two of them do not fit the free disk either; Kimi-Linear-48B (~89.4 GiB) is marginal. For the blocked families we will gate config resolution, weight mapping on a slice and unit parity at their dimensions, and record the rest as hardware-blocked rather than claiming a gate we cannot run. W0-W10 plan recorded; **nothing implemented** | **W8 ran the gate and DeepSeek-V2-Lite PASSES 8 of 8 prompts.** The bar is token-for-token agreement with vLLM 0.25.0, driven through the full paged engine (scheduler, paged MLA cache, MLA prefill and decode, DeepSeek MoE, sampler). Five prompts match vLLM's output exactly. The other three differ only where vLLM cannot tell the two candidates apart from itself — measured, not assumed: feeding vLLM our own generated text back shows that at 35 of the 36 differing positions **vLLM's own most-likely next token is the one we produced**, and the one remaining case is within a quarter of a nat, inside the tolerance used for every model in this project, with the per-position numbers committed next to the test. Nothing we produced fell outside vLLM's top 20 candidates, so there is no real disagreement anywhere. The gate also proves the engine really drives this path rather than merely compiling it: it checks the cache actually allocated is the compact MLA one, and it runs the prompts CONCURRENTLY with staggered arrival so the scheduler produces genuinely mixed batches (7 of them, up to 8 requests at once) in the exact order MLA silently depends on — an order the model refuses outright if violated — plus a cache-reuse case that exercises the harder "prefill with existing context" path. **The blocker turned out to be the tokenizer, not the model:** the first run refused to load the checkpoint, because DeepSeek splits text with a seven-stage pipeline over explicitly listed character ranges rather than the single pattern every previous model uses. That family was implemented and is checked character-for-character against the real Hugging Face tokenizer over a corpus written to stress each stage. **A plausible fix was also written and then discarded because measurement refuted it:** DeepSeek's configuration asks for a sentence-start marker on every prompt, which looked like exactly the silent bug that once cost us every OPT prompt — but vLLM itself does not add one, our tokenizer already agreed, and "fixing" it would have broken a passing gate; the measured behaviour is now pinned by a test. Everything else in the tree is unchanged (the 27B, 35B, Qwen3-Coder, Qwen3-dense and OPT gates all re-run identical, as do every tokenizer test), the build is clean, and the GPU memory/race/sync tools report nothing.
| MiniMax M2 / M3 | MiniMax-M2, MiniMax-M3-Sparse | — | — | 🔬 **SPIKED (M2) / INVENTORIED (M3), no implementation.** Determined to be **disjoint from MLA**, correcting an earlier source-scan assumption: M2 is plain dense GQA + MoE with partial RoPE and per-layer sliding window; M3 is GQA plus a learned sparse-attention indexer. Lightning attention is retired upstream. M2 needs the sliding-window track, not the MLA track, and is HW-BLOCKED e2e at ~428 GiB bf16 |
| GLM family | GLM-4.7-Flash, GLM-4-9B-0414, GLM-4.5-Air, GLM-5, ChatGLM3 | — | — | 🔬 **SPIKED ONLY — no implementation (2026-07-21)** ([spike](.agents/specs/glm-dsa-latest-deepseek.md)). The family splits three ways. **`Glm4MoeLiteForCausalLM` (GLM-4.7-Flash, 31.2B, 58.2 GiB bf16) FITS GB10** and is upstream a zero-override subclass of DeepSeek-V2's MLA attention, so it lands as a pure addition once the MLA track exists — and it exercises two routing/projection paths the MLA spike could only unit-test. **`Glm4ForCausalLM` and `GlmForCausalLM` (both 9B / 17.5 GiB) also fit**, but need two primitives we have none of: a partial rotary factor (rope only a leading slice of each head) and sandwich norms (extra norms on each sublayer's output before the residual add). **`Glm4MoeForCausalLM` is HW-BLOCKED** — its smallest genuine checkpoint is 205.8 GiB bf16. **`GlmMoeDsaForCausalLM` (GLM-5) is blocked twice over:** 1404 GiB, and it needs sparse attention that does not run on this GPU (see the DeepSeek row). Nothing is implemented; no row claims more than `SPIKE` |
...
**MLA / DeepSeek campaign — W0-W8 landed; DeepSeek-V2-Lite PASSES its correctness gate, and is now
CORRECT but NOT YET FAST (2026-07-22).** The breadth sweep's Tier-3 "MLA = new attention, new
campaign" item has its leaf spike
([`.agents/specs/mla-deepseek-campaign.md`](.agents/specs/mla-deepseek-campaign.md)), and nine steps
of its plan are now in the tree. **W8 ran the gate this project holds every model to — token-for-token
agreement with vLLM 0.25.0 over a fixed prompt set, driven through the full paged engine — and
DeepSeek-V2-Lite PASSES 8/8 prompts.** Five of the eight match vLLM's greedy output exactly; the other
three differ only at points where vLLM cannot separate the two candidates from itself, which was not
assumed but measured: feeding vLLM our own generated text back shows that at 35 of the 36 differing
positions **vLLM's own most-likely next token is the one we produced**, and the single remaining case
is within a quarter of a nat — well inside the tolerance this project has used for every model, with
the per-position numbers committed alongside the test. No output was outside vLLM's top 20 candidates,
so there is no real disagreement anywhere. So `DeepseekV2ForCausalLM` moves from `SPIKE` to `ACTIVE`
with **speed explicitly pending** — it is *not* `DONE`, because that additionally requires matching
vLLM's throughput on every axis, and **there is still no speed number of any kind for this model**
(W9). The other four rows in the campaign stay `SPIKE`.
Two things are worth knowing about how that gate was reached. **The blocker turned out to be the
tokenizer, not the model:** DeepSeek's text splitting is unlike anything the tree supported — a
seven-stage pipeline over explicitly enumerated character ranges rather than the single pattern every
previous model uses — so it had to be implemented from scratch and is checked character-for-character
against the real Hugging Face tokenizer over a corpus built to stress each stage. And **a plausible
"bug fix" was written and then thrown away because measurement refuted it:** DeepSeek's config asks
for a sentence-start marker on every prompt, which looked like a silent correctness bug of exactly the
kind that once cost us every OPT prompt — but vLLM itself does not add one, our tokenizer already
matched, and adding it would have broken a passing gate. The measured behaviour is now pinned by a
test so the next reader has to re-measure before "fixing" it.
*W0 (grounding, no code):* the DeepSeek-V2-Lite checkpoint was fetched to the GB10 box and run
through the vLLM 0.25.0 oracle, which **confirmed by observation** what the spike had only read from
source — on sm_121 vLLM selects `TRITON_MLA` for dense MLA decode and FlashAttention for MLA prefill
— and the shipped `config.json` confirmed the full MLA geometry (a 576-wide latent, one KV head, no
separate V). *W1 (engine seam, behaviour-preserving):* KV-cache allocation now derives its size and
shape from the KV **spec** rather than from hardcoded K+V arithmetic and the HF config, and an
`MLAAttentionSpec` carrying upstream's factor-1 page formula exists alongside it. This step
deliberately changes no behaviour, and that was verified rather than assumed: the 27B (235/235), 35B
(315/315), Qwen3-Coder (6/6) and Qwen3-dense (16/16) correctness gates are all unchanged.
*W2 (backend selection):* vLLM's attention-backend priority logic is now ported in full rather than
partially — the MLA half that earlier releases skipped is in, expressed as a data table with one row
per GPU architecture, so adding a future architecture is a table row rather than new branching code.
On this hardware an MLA request now resolves to the `TRITON_MLA` backend, matching what the vLLM
oracle was observed doing at W0. Backend selection for every existing (non-MLA) model is byte-for-byte
unchanged. The table deliberately keeps the *sparse* MLA entry that DeepSeek-V3.2-style models would
need, and excludes it with a real capability check rather than by omission, so that work can slot in
later without touching the selector. *W3 (two building-block kernels):* the MLA cache write — which
packs the compressed latent and its rotary part into a single cache row, something our existing
key/value cache-write op cannot express — and the DeepSeek MoE "grouped top-k" router, which picks
experts in two stages. Both are new numerics, so both are gated against a CPU reference on GPU and
CPU. An honest limitation, recorded rather than glossed: the only DeepSeek checkpoint that fits this
hardware (V2-Lite) does not use the grouped router's distinctive features at all, so that router's
correctness evidence is **unit tests only** — run at the real DeepSeek-V3 dimensions against an
independently written transcription of the upstream formula, but not yet exercised by any running
model.
*W4 (the decode kernel):* MLA's decode attention now exists — the step where every query head reads
the same single compressed latent row, and where the "values" are simply the first 512 numbers of
that same row rather than a separate tensor. It is a structural port of the two-stage kernel pair
vLLM was actually observed running on this GPU: one pass splits the sequence into chunks and produces
a partial result per chunk, a second pass merges them. The merge runs in a fixed order and uses no
atomic adds, so repeated runs are bit-identical. Because there is no MLA model to run it end to end
yet, the evidence is deliberately heavy at the unit level: it is checked against an independently
written, structurally different reference implementation at DeepSeek-V2-Lite's real dimensions, over
ragged sequence lengths, multiple pages, every chunk-count boundary including more chunks than
tokens, DeepSeek-V3's 128-head shape, head counts that do not fill a tile, and both bf16 and fp32 —
with the cache deliberately poisoned past each sequence's end so any read past the end fails, and
pages assigned in scattered descending order so any assumption about page layout fails. GPU memory
tooling reports zero errors, zero data races and zero synchronisation errors, and all existing model
correctness gates are unchanged. There is **no speed number** for it: it is written for correctness,
and tuning is a later step.
*W5 (prefill and the chunked-context loop):* the other half of MLA attention. During prefill the
compressed latent is expanded back into ordinary per-head keys and values, so this step is a normal
attention over 192-wide keys and 128-wide values — an asymmetry that would normally need a new
kernel. It does not, because vLLM does not need one either: on this GPU it pads the values with
zeros out to the key width and trims the result afterwards, so the existing FlashAttention kernel
family we already vendor covers it. Generalising it therefore meant adding two new compiled
instantiations of an unmodified template plus one new entry point; the code path every existing
model's prefill uses was not edited at all (the change to that file is pure addition, zero
deletions), and all five existing correctness gates are unchanged. The second half of this step is
the mechanism that lets a long prompt reuse previously cached context without exhausting memory:
the cached context is processed in bounded chunks and the partial attention results are combined
mathematically rather than recomputed — the alternative would need about 3 GB of scratch for a 64k
context instead of 144 MB. Chunk boundaries are where this kind of code hides bugs, so the evidence
targets them directly: the whole loop is checked against a reference that never chunks at all, over
contexts that end exactly on a chunk boundary, one token over, one token under, requests with no
cached context, and requests that contribute nothing to a given chunk, with cache pages deliberately
assigned in scattered order and output buffers pre-filled with invalid values so a kernel that fails
to write is caught. GPU memory, data-race and synchronisation tooling all report zero. One real
defect in the upstream FlashAttention code was found on the way — its handling of a request with no
keys writes its bookkeeping to the wrong place — and is worked around explicitly rather than
inherited. Again: **no speed number**, and still no model.
*W6 (the attention block and weight absorption):* the piece that turns the previous three steps into
a single usable attention layer, and adds the two parts only a model can own — the input projections
and "weight absorption". Absorption is the trick that makes MLA cheap at decode time: rather than
expanding the compressed cache back into full keys and values for every token, the expansion matrix
is folded into the query once at load time, so decode reads the compressed rows directly and a second
matrix converts the result back at the end. It turns out this needs no new attention kernel at all —
it is a one-off rearrangement of a weight plus two batched matrix multiplications — so the only new
GPU code is those two general-purpose primitives. Because the whole claim rests on the folded and
unfolded forms computing the same thing, that equivalence is **proven numerically three separate
ways** rather than argued: an independently written high-precision reference computes the attention
both ways and they agree to eleven decimal places; our folded path reproduces the *unfolded*
reference; and — the strongest check — the same batch of work is pushed through our folded decode
kernel and through our unfolded prefill kernel, which share nothing but the weights, and the two
outputs agree. Both variants of the query path are implemented, including the one no checkpoint that
fits this hardware can exercise, which is therefore tested at the larger model's real dimensions only
and is recorded as such rather than claimed as covered. Mixed batches (some requests generating, some
still reading their prompt) are covered, as are ragged lengths and chunk boundaries. GPU memory,
data-race and synchronisation tooling all report zero, and all five existing model correctness gates
are unchanged. Still **no speed number**, and still no model.
*W7 (the model):* the first MLA model — everything above, assembled into something a checkpoint runs
through. It adds DeepSeek-V2's configuration parsing, its weight loader and its forward pass, and
registers it under one architecture name. The weight loader is checked against the real 30 GB
DeepSeek-V2-Lite checkpoint and the check is exhaustive in both directions: every tensor the model
expects is present with the right shape, and every tensor in the checkpoint is consumed — 5291 of
5291, nothing unmapped, nothing left over. The forward pass then runs the real weights: given "The
capital of France is" it answers " Paris", with a sensible runner-up list, and repeats bit-for-bit.
Three things get their own targeted checks. First, the batch ordering MLA silently depends on — the
previous step discovered that getting it wrong produces plausible-looking but badly wrong numbers, so
the code now refuses a mis-ordered batch by name instead of computing quietly. Second, "shared
experts", which this model family has and no model we had before did: a mixture-of-experts layer with
its routed experts zeroed is checked to be bit-identical to a plain feed-forward layer holding the
same weights, and switching the shared expert off is checked to change the output, so the path is
proven to run rather than merely compile. Third, the GPU path is exercised at the real head
dimensions and agrees with the CPU path to within ordinary low-precision arithmetic noise. GPU
memory, data-race and synchronisation tooling all report zero, and all five existing model
correctness gates are unchanged. Only `DeepseekV2ForCausalLM` is registered: the original DeepSeek
(which is not an MLA model at all), V3 (which does not fit this hardware and is not stored in a
format we load) and V3.2 (which needs a sparse-attention module we do not have) are each refused by
name with an explanation rather than silently claimed. Read the
spike for the architecture, the KV-cache impact, and the honest per-family hardware verdict — the
short version is that one member of the family (DeepSeek-V2-Lite) is gateable on this hardware and
the flagship models are not.
