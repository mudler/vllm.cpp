...
| Mistral dense | Mistral-7B-v0.3, … | 🚧 | — | 🚧 **`MistralForCausalLM` — W0-W3 LANDED 2026-07-23; MODEL correctness oracle-validated tokenizer-free (30/30 greedy tokens vs vLLM 0.25.0), full paged-engine gate BLOCKED on a SentencePiece tokenizer, SPEED PENDING** ([spike](.agents/specs/sweep-mistral.md)). The breadth sweep's **fifth model family** and the closest-to-Llama dense arch — vLLM's own `mistral.py` is literally "Mistral adaptation of the LLaMA architecture". Mistral IS the shared Qwen3/Llama dense forward with **no new primitive**: plain RoPE (`rope_theta=1000000`, no rope_scaling), no per-head qk-norm, an **untied lm_head**, and `sliding_window: null` (full attention) — all four already existed (from Llama/Qwen3/OPT), so the whole model is three new files (a header aliasing the shared dense weights + forward, a weight loader, a registry TU) with zero changes to the runner, scheduler, platforms, attention registry, config parser, any CUDA kernel, the tokenizer, or any existing model. **Correctness:** the model forward was gated **tokenizer-free** by feeding vLLM's exact prompt token ids through our CUDA prefill and greedy-decoding — **30/30 continuation tokens match the vLLM 0.25.0 oracle** (29 strict token-exact + 1 in the near-tie band, where our token is one of the two vLLM itself flips between; 0 forward-divergent; vLLM greedy measured deterministic on 4 of 5 prompts). **Real finding — a tokenizer-family gap, not a model gap:** Mistral-7B-v0.3 ships a SentencePiece BPE tokenizer (a `Metaspace` pre-tokenizer replacing spaces with ▁ plus a byte-fallback vocab), which our ByteLevel-BPE-only tokenizer cannot load — so the full paged-engine SACRED gate (which tokenizes prompts through the engine) is blocked, and SentencePiece/Metaspace support is a separate tokenizer campaign (`LOAD-SENTENCEPIECE`). The W2 loader was validated on the real 3-shard checkpoint (1541 assertions: untied lm_head present, qk-norm empty, no leftover). Checkpoint `mistralai/Mistral-7B-v0.3` (SWA-disabled v0.3; the duplicate `consolidated.safetensors` excluded). **No throughput is measured or claimed** — SPEED is a separate increment, so the row stays `ACTIVE`, not `DONE`. Regressions unchanged (registry 299, Qwen3-dense forward 1031, Llama paged-engine 16/16; the additive-only diff — zero shared model/kernel/runner/tokenizer/config file touched — proves the 27B/35B/Qwen3-Coder/OPT/DeepSeek-V2 gates unaffected); clean CUDA rebuild `-Werror` 0 warnings; device-reference ratchet (DSR 32) unchanged |
