...
| Host-memory repair | **BINDING PASS for the established 27B windowed-release gate**: `LOAD-SAFETENSORS` progressive `madvise(MADV_DONTNEED)` remains in the `246a23c` binding binary (ours peak PSS 24.88 GB vs vLLM 28.18 GB), and the 35B routed-expert steady mirror/load interleave remains implemented. The additive plain-BF16 direct-device leaf (`LOAD-SAFETENSORS-DIRECT-DENSE`, [spike](.agents/specs/qwen35-plain-bf16-direct-load.md)) is now **`GATING` after the corrected local 4B comparison**. Immutable root `/tmp/qwen35-transplant-4b-647a2a6a` at `647a2a6a`, exact 128-request ShareGPT workload (131,784 input + 16,384 output tokens, c32, greedy), 3 memory + 3 timing reps per arm under one GPU lock: direct ON/OFF/local-vLLM-0.24 total throughput **4534.38/4480.04/6730.37 tok/s**, output throughput **501.40/495.39/744.22 tok/s**, mean TTFT **1114.40/1210.83/904.74 ms**, peak PSS **2.606/8.559/7.671 GiB**, stable PSS **0.727/8.558/4.058 GiB**, peak VRAM **12944/12936/12957 MiB**. Direct ON is **+1.21%** vs same-binary OFF and cuts peak/stable PSS **69.6%/91.5%**, but it is **31.4% below** the previous branch ON (**6607.04 tok/s**) and only **0.674x** local vLLM; local vLLM itself is stable at **1.002x** its previous 6716.47 tok/s result. Direct ON=OFF output IDs match **128/128 in every pair**. The local vLLM arm is diagnostic, not binding (v0.24 rather than current v0.25; one same-arm token run was unstable). Matching graph-node nsys traces completed and confirm materially different attention/GDN/GEMM execution paths without yet proving a single cause. Current-v0.25 correctness/performance, sanitizer and strict ON<=OFF VRAM remain open; 27B/35B remain external-hardware follow-ups with no support extrapolation |
