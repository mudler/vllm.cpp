...
| Loading | Safetensors for both gate models; supported 35B GGUF k-quant files materialized to BF16 (dense `qwen35` GGUF files now load too, via the model registry) |
...
|---|---|---|
| CPU | x86-64 reference | 🟡 Correctness/CI implementation with native threadpool; real-file GGUF speed/RSS and compute-in-quant gates remain open — the compute-in-quant track's type/traits/op foundation AND its portable tier-0 kernels (activation quant + the six `vec_dot`) landed 2026-07-22, but no model call site routes to them yet, so nothing runs in quant end to end. llama.cpp on the same file is the floor we must match or beat, and we are currently far behind it (decode 54-75x, prefill ~1,480x, peak RSS 2.7x) |
...
| GGUF F32, Q4_0, Q8_0, Q3_K/Q4_K/Q5_K/Q6_K | 🟡 Supported 35B files load through BF16 materialization and pass same-file llama.cpp greedy checks; **direct compute-in-quant is still NOT implemented — every GGUF weight is expanded to BF16 at load, so the file is small but the runtime memory and bandwidth are not.** The track to fix that is under way (2026-07-22): the dense-`qwen35` GGUF loader path is on main (landed as a squash of the 422-commit-old bench branch `7c91a42`, re-expressed through the `ModelRegistry` seam that had replaced its `IsDenseArch` symbol), `vt::` carries the ggml block dtypes and the llama.cpp CPU trait table, and the **portable tier-0 quant kernels now exist** — the Q8_0/Q8_K activation quantizers and the six generic `vec_dot` (Q4_0, Q8_0, Q3_K, Q4_K, Q5_K, Q6_K) wired into a `MatmulBTQuant` op, gated against an independent f64 reference. **Those kernels are not on any model's path yet:** no call site routes to the quant op, so every GGUF weight is still expanded to BF16 at load and no encoding computes in its native blocks end to end. Routing the call sites (which needs the keep-quantized loader) and the per-architecture SIMD tiers are the next increments. Several formats remain open |
