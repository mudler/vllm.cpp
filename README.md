...
> **40 timing + 8 memory** component is now **ACTIVE** from immutable `ae9e8ff`
> under one uninterrupted `/tmp/gpu` lock. Both FA2-on and fallback model gates
> passed **235/235 + 16/16** before timing began; partial legs remain non-binding
> until the driver emits its complete summary. No speed credit exists yet.
...
|---|---|---|---|---|
| Qwen3.5/3.6 hybrid (GDN + gated attention, MoE + dense) | Qwen3.6-35B-A3B, Qwen3.6-27B | ✅ **text submodels** run end-to-end on GB10 and retain token-exact greedy correctness. The binding v0.25.0 `3f256ab` 27B cache-off gate completed all 36 groups but failed strict parity at **55/124 axes**; c16/c32 total throughput pass while low-concurrency decode latency and host PSS/RSS remain open. Packed QKV closes at **208.192 vs 208 FP4 GEMMs/forward**. W3-C passes frozen-map control; W3-E and W3-F are mean-positive but strict-fail at **39/40 timing + 1/8 memory** and **27/40 + 3/8**, so neither earns speed credit. W3-G implements the bounded FA2 BF16 ratio-6 split-KV adapter, model/dispatch toggle, graph-stable scratch and upstream-derived operator/capture/lifecycle tests. Immutable `ae9e8ff` now passes its sm_121a operator/memcheck gates, both 27B default/fallback **235/235 + 16/16** arms, correctness-only 35B **315/315**, and paired structural trace. Its c2/c16 every-axis component is ACTIVE under one whole-series lock after both model arms passed; partial legs are non-binding, and the short correctness trace is performance-negative rather than speed evidence. The 35B ratio-8 path is explicitly inert and its performance remains held. The upstream wrappers are multimodal; their vision path is not implemented. | ✅ 35B text path from real APEX k-quant `.gguf` on GB10 (greedy parity vs same-file llama.cpp oracle); 27B GGUF pending (no file exists) | 🟡 paged-KV text engine + basic server/tool/grammar subsets; correctness gated, v0.25.0 27B production performance `FAILED/GATING`; 35B performance held |
...
| Format | Status |
|---|---|
| NVFP4 (W4A16 MoE / W4A4 dense, Blackwell) | ✅ **both running on GB10** with token-exact greedy gates passing. The W4A4 path includes all 32 SM12 tactics, merged/fused projections, pre-serve tuning and packed QKV. W3-C closes frozen-plan control; W3-E and W3-F preserve correctness/structure but strict-fail their components. `3f256ab` still fails **69/124** axes. The completed scan moves the next component to the separate W3-G attention row and ranks vectorized normal BF16→FP4 production second; no new quant implementation, speed credit or support expansion is claimed. |
...
- **Modernization and removal are trace-gated.** The v0.25.0 audit found no
  ...
  **23,616 / 7,061.921 ms** plus combine **23,488 / 123.245 ms**; unequal trace
  ...
  a gate workload; the input-1,024 c2/c16 component is active under its complete-
  series lock and publishes no partial result. Vectorized
  normal FP4 production remains the next separate lever.
