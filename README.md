...
**MLA / DeepSeek campaign — W0-W7 landed; DeepSeek-V2-Lite now LOADS and FORWARDS, but is NOT yet
supported (2026-07-22).** The breadth sweep's Tier-3 "MLA = new attention, new campaign" item has its
leaf spike ([`.agents/specs/mla-deepseek-campaign.md`](.agents/specs/mla-deepseek-campaign.md)), and
the first eight steps of its plan are now in the tree. As of W7 there IS an MLA model: the real
DeepSeek-V2-Lite checkpoint loads (every one of its 5291 tensors mapped, none left over) and a prompt
runs through it end to end — "The capital of France is" produces " Paris". **That is a sanity result,
not a correctness gate.** The bar this project holds every model to is token-for-token agreement with
vLLM over a fixed prompt set, and that gate (W8) has not been run, so **no DeepSeek/Kimi model is
supported**, the five model rows stay `SPIKE`, and there is no speed number for any of it.
