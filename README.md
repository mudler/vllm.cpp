...
| Async/overlap scheduling (`ENG-ASYNC-SCHED` W3) | **Host-side machinery LANDED + CPU-gated (2026-07-16), `ACTIVE`.** `AsyncScheduler` output-placeholder accounting, depth-2 `step_with_batch_queue`, `async_scheduling` default-ON-when-compatible config resolution, and the `VT_ASYNC_SCHED` rollback env are implemented and CPU-tested RED-first (6 ported async-scheduler cases + a depth-2 engine cycle; full CPU ctest 108/108). The synchronous path is byte-identical (placeholder sites inert while count 0). The runner-side device-input path + non-blocking sampled-token-ID D2H (the piece that captures the measured ~3.25 ms/step GPU-idle) is the remaining DGX-gated leaf; until it lands the production default stays synchronous, so this commit changes no GPU behavior |
