...
| GGUF F32, Q4_0, Q8_0, Q3_K/Q4_K/Q5_K/Q6_K | 🟡 Supported 35B files load through BF16 materialization and pass same-file llama.cpp greedy checks; **direct compute-in-quant is still NOT implemented — every GGUF weight is expanded to BF16 at load, so the file is small but the runtime memory and bandwidth are not.** The track to fix that has started at its two roots (2026-07-22): the dense-`qwen35` GGUF loader path is merged onto main, and `vt::` now carries the ggml block dtypes, the llama.cpp CPU trait table and a `MatmulBTQuant` op skeleton whose fallback still dequantizes. No encoding computes in its native blocks yet; the quantized dot kernels and the keep-quantized loader are the next increments. Several formats remain open |
