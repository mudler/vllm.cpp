...
| Qwen3 / Qwen2 dense | Qwen3-32B, Qwen3-0.6B, … | 🚧 | — | 🚧 **First additive-model bring-up — W0+W1 LANDED 2026-07-20** ([spike](.agents/specs/first-additive-model-qwen3-dense.md)): `Qwen3ForCausalLM` on `Qwen3-0.6B` (BF16). W0 = new registry TU (`qwen3_dense.cpp` + `qwen3.h`, one `REGISTER_VLLM_MODEL`, full-attention-only KV spec) resolves via the clean model-factory seam (zero shared-array edit). W1 = the **runner is now model-shape-agnostic**: a full-attention-only (empty `layer_types`, no GDN group) KV config allocates + steps without the hybrid GDN path — behaviour-preserving (27B 235/235 + 35B 315/315 token-exact UNCHANGED). Forward is a clear-throwing stub until W3. Remaining: W2 loader → W3 forward → W4 token-exact gate vs the vLLM 0.6B oracle |
