...
**MLA / DeepSeek campaign — W0-W4 landed, no MLA model runs yet (2026-07-22).** The breadth
...
five steps of its plan are now in the tree. **No DeepSeek/Kimi model is supported** — the five model
rows stay `SPIKE`: there is a decode kernel but still no MLA prefill, no MLA model and no MLA
forward, so nothing in this family can be loaded or run.
...
and tuning is a later step. Read the
