...
| Loading | Safetensors for both gate models; supported 35B GGUF k-quant files materialized to BF16 (dense `qwen35` GGUF files now load too, via the model registry). Block-resident (keep-quantized) GGUF loading exists but is default off — see the quantization table |
...
|---|---|---|
| CPU | x86-64 and arm64 reference | 🟡 Correctness/CI implementation with native threadpool; real-file GGUF speed/RSS and compute-in-quant gates remain open — the compute-in-quant track's type/traits/op foundation, its portable tier-0 kernels (activation quant + the six `vec_dot`) and the keep-quantized loader's block residency + routing all landed 2026-07-22, but no model call site routes to them yet, so keep-quant is default off and nothing runs in quant end to end. llama.cpp on the same file is the floor we must match or beat, and **we are still behind it on every axis** — re-measured 2026-07-22 on an idle arm64 box: **decode 11.6x, prefill 33.5x, peak RSS 2.65x** (7.43 GiB against llama.cpp's 2.80 GiB on a 2.68 GiB file). That supersedes the older 54-75x / ~1,480x figures, which predated the threadpool; the memory figure has not moved at all. The gap is now attributed to a **single operation** — the plain (non-quantized) matrix multiply is 95.4% of all run time, and the quantized kernels that already exist but are not yet called are 14-44x faster than it, so routing them (work row G4) is the one change that moves all three axes ([measurement + ranked plan](.agents/specs/cpu-llamacpp-floor-remeasure-2026-07-22.md)) |
...
| GGUF F32, Q4_0, Q8_0, Q3_K/Q4_K/Q5_K/Q6_K | 🟡 Supported 35B files load through BF16 materialization and pass same-file llama.cpp greedy checks; **direct compute-in-quant is still NOT implemented — every GGUF weight is expanded to BF16 at load, so the file is small but the runtime memory and bandwidth are not.** The track to fix that is under way (2026-07-22): the dense-`qwen35` GGUF loader path is on main, `vt::` carries the ggml block dtypes and the llama.cpp CPU trait table, the **portable tier-0 quant kernels now exist** — the Q8_0/Q8_K activation quantizers and the six generic `vec_dot` (Q4_0, Q8_0, Q3_K, Q4_K, Q5_K, Q6_K) wired into a `MatmulBTQuant` op, gated against an independent f64 reference — and the **keep-quantized loader can now hold a weight in its native blocks**, with a per-tensor routing policy and a `VT_CPU_REF` dequant-oracle switch, proven lossless per encoding. **None of that is on any model's path yet:** no call site routes to the quant op, so keep-quant residency is DEFAULT OFF, every GGUF weight is still expanded to BF16 at load, and no encoding computes in its native blocks end to end — the file is small but the runtime memory and bandwidth still are not. Routing the call sites and the per-architecture SIMD tiers are the next increments. Several formats remain open |
