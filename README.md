...
> reverted RMSNorm-fast kernel (c2 preflight: +1.446% tput / −0.887% TPOT),
> pending its combination-numerics fix; see [BENCHMARKS](docs/BENCHMARKS.md). **Honest regression:** ours lost **−2.67% total throughput at c16** and
...
|---|---|
| Binding gate | **NEW BINDING `a875397`: 52/124** (gate NO; supersedes `246a23c`'s 49/124 and `3f256ab`'s 55/124). Mem (4/4), c1 (20/20) pass; the async default flipped the c16/c32 ITL tails to PASS; the open axes are a nearly-uniform ~1–2% decode deficit at c2–c32 (55/72 fails within 2% of vLLM, only 3 >5%) — the reverted RMSNorm-fast kernel (combination-numerics fix pending) is the named ~1% lever |
