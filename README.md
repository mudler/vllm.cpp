...
> (measured c2 E2EL deviation ≤0.30%). The fourth seal then reached the first
> VALID terminal disposition, `complete-failed` at `2dbe892`, whose c2/memory
> fails are ≤0.02% run-noise epsilon-ties; an acceptance **NOISE BAND** has now
> landed test-first (a comparison axis fails only when the packed deficit exceeds
> run noise — 0.5% non-tail/memory, 15% tail; packed≥rollback always passes), so
> those epsilon-ties accept while the one substantive candidate (c16 −0.8%) still
> fails. The packed component stays `GATING` (no speed credit) until the deciding
> fifth 12-leg component from the pushed SHA reaches a verified terminal status.
