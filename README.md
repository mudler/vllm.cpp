...
> TTFT (0.906/0.943, the bimodal low-concurrency arrival lottery); and one real
> signal remains — **c8 p99_itl 0.86**, the wave-boundary prefill-co-schedule
> tail. Closing path: fold in the two landed bit-identical FP4-quant/SiLU glue
> kernels (`861b518`, opt-in) + the GDN conv-update lever for the decode-floor
> axes; characterize the c4 TTFT lottery vs run-noise; address the c8 p99 tail
> cadence. Memory PASSES via the windowed-load release (`cb2d310`, ours peak PSS
...
|---|---|
| Binding gate | **NEW BINDING `9ecd9d0`: 114/124** (gate NO; supersedes `a875397`'s 52/124, `246a23c`'s 49/124, `3f256ab`'s 55/124). Mem 4/4, c1 20/20, c2 20/20, c16 19/20; the bit-identical fast RMSNorm + gated-RMSNorm stack closed +62 axes. 10 remain: the batch-independent decode floor at 0.987–0.999 (7), c4 TTFT lottery (2), c8 p99_itl wave-boundary tail 0.86 (1) — closing via the landed FP4/SiLU + GDN conv-update glue levers, c4-noise characterization, and the c8 p99 tail cadence |
