...
| Host-memory repair | **BINDING PASS for the established 27B windowed-release gate**: `LOAD-SAFETENSORS` progressive `madvise(MADV_DONTNEED)` remains in the `246a23c` binding binary (ours peak PSS 24.88 GB vs vLLM 28.18 GB), and the 35B routed-expert steady mirror/load interleave remains implemented. The additive plain-BF16 direct-device leaf (`LOAD-SAFETENSORS-DIRECT-DENSE`, [spike](.agents/specs/qwen35-plain-bf16-direct-load.md)) stays **`GATING`**. Its corrected local Triton-AOT 4B root `/tmp/qwen35-transplant-4b-aot-557ab41d` completed 18/18 guarded legs: direct ON/OFF/local-vLLM-0.24 total **6155.10/6064.06/6730.46 tok/s**, peak PSS **2.405/8.571/7.569 GiB**, stable PSS **0.733/8.571/4.066 GiB**, peak VRAM **12892/12884/12942.7 MiB**, and ON=OFF outputs **128/128**. ON is +1.50% and cuts peak/stable PSS 71.9%/91.4%, but is **0.9316x** the previous matching-AOT 4B result and **0.9145x** local vLLM. **Regression attribution is complete:** matched node-mode nsys profiles show current synchronous GPU time **23.307 vs 21.960 s**; the dominant path swap is previous H=32 Triton decode **0.602 s / 47.7 us-call** to current hand packed decode **2.580 s / 234.2 us-call**. Shipping async adds a smaller 2.20% penalty. Packed-off is invalid (only +0.86%, outputs 117/128), while vLLM actually runs its H=32 packed Triton kernel at 104.6 us/call. Next gate is a current-ABI H=32 AOT specialization with token-exact same-binary A/B; current-v0.25 correctness, sanitizer, strict ON<=OFF VRAM (+8 MiB) and external 27B/35B remain open with no support extrapolation. [Profile evidence](docs/bench-evidence/qwen35-4b-aot-regression-profile-20260721.md) |
