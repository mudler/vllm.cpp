...
**MLA / DeepSeek campaign — W0-W9 landed. DeepSeek-V2-Lite is CORRECT, is now 1.7x-2.2x faster than
it was, and is STILL BEHIND vLLM on throughput (2026-07-22).** The breadth sweep's Tier-3 "MLA = new
attention, new campaign" item has its leaf spike
([`.agents/specs/mla-deepseek-campaign.md`](.agents/specs/mla-deepseek-campaign.md)); ten steps of its
plan are in the tree, and the track now has a binding speed number
...
a seven-stage text-splitting pipeline unlike anything the tree supported, and finally the DeepSeek-V2
model itself.

*Correctness — the gate this project holds every model to.* Token-for-token agreement with vLLM
0.25.0 over a fixed prompt set, driven through the full paged engine: **8 of 8 prompts pass.** Five
match vLLM exactly. The other three differ only where vLLM cannot separate the candidates from itself,
which was measured rather than assumed — feeding vLLM our own generated text back shows that at 35 of
the 36 differing positions vLLM's own most-likely next token is the one we produced, and the one
remaining case sits within a quarter of a nat; nothing we produced fell outside vLLM's top 20. The
gate also proves the engine really drives this path: it checks that the compact MLA cache was actually
allocated, and it runs the prompts concurrently with staggered arrival so the scheduler produces
genuinely mixed batches in the exact order MLA silently depends on — an order the model refuses
outright if violated — plus a cache-reuse case for the harder "prefill on top of existing context"
path. **This result did not change by a single token across either speed optimisation.**
...
GPU**: it was dividing the work into two pieces on a processor with dozens of independent cores,
because the upstream rule that chooses the split count looks only at sequence length and quietly
assumes many requests are in flight. Sizing the split to fill the machine — upstream's own stated
occupancy target, simply applied — made that kernel **18x faster** and raised end-to-end output rate
by **70% at one request and 20% at eight**. A second change records the decode step as a replayable
GPU command list, worth a further ~2%; that is small here, and the profile says why: this model's
decode is limited by the GPU itself, not by the time we spend issuing work. Both are on by default.
Against vLLM we now reach **0.86-0.95x its output rate** (it was 0.50x), and our time-to-first-token
is now **faster than vLLM at 4 and 8 concurrent requests** — but throughput is short at every
concurrency, so this is a miss, and it is recorded as one with a named cause: at small batch our dense
projections fall back to a vector-matrix routine for roughly a third of GPU time, where vLLM puts much
of the same work on tensor cores. That is the next thing to fix, and it is a routing change rather
than a new kernel.

*Two things worth stating plainly about the comparison.* First, vLLM's own default expert-layer
backend for this model **hard-reboots this machine** — five times now, including twice on a freshly
restarted box with every mitigation applied — so the comparison uses vLLM's best configuration that
actually runs here. That substitution is stated openly and does not flatter us: we lose to it. Second,
building the decode command list surfaced a genuine latent defect in our own code, where per-step
attention metadata was copied from short-lived scratch that the recorded command list then referred to
after it had been freed. It produced correct output on the first replay and silently wrong output
afterwards, and the standard memory-checking tool could not see it. It is fixed, the replay is now
bit-identical to running step by step, and the whole class of it is now caught by an explicit error
rather than a crash.

*Recorded limits, not glossed.* Only `DeepseekV2ForCausalLM` is registered: the original
`DeepseekForCausalLM` is not an MLA model at all, V3 does not fit this hardware and ships in a format
we do not load, and V3.2 needs a sparse-attention module we do not have — each refused by name with an
explanation. The query path DeepSeek-V3 and GLM-4.7-Flash use, and DeepSeek's distinctive
expert-router behaviour, are implemented and unit-tested at their real dimensions but have no
end-to-end coverage, because the one checkpoint that fits this box uses neither. Read the spike for the
architecture, the KV-cache impact, and the per-family hardware verdict — the short version is that one
member of the family (DeepSeek-V2-Lite) is gateable on this hardware and the flagship models are not.
