...
> zero such kernels. W3-F is **READY only**—no runtime code, component result,
> exact-grid rerun or speed credit exists at this checkpoint. Its implementation
> must retain `VT_FP4_DEVICE_ALPHA=0` as the same-binary staging fallback.
...
- **Modernization and removal are trace-gated.** The v0.25.0 audit found no
  ...
  **1.004483×/1.005044×**, but strict acceptance fails at **39/40 timing +
  ...
  and passes its pointer directly. Its complete spike is `READY`; implementation
  defaults to the device tensor only after ported tests, with
  `VT_FP4_DEVICE_ALPHA=0` preserving the same-binary old path. No W3-F number
  exists yet.
