...
| Tool-call parsing | 23 dialects, streaming | hermes/qwen3, mistral (both wire forms), llama3_json/llama4_json, pythonic, llama4_pythonic, granite, granite4, granite-20b-fc, deepseek_v3/v31/v32/v4, longcat, xlam, phi4_mini_json, internlm, jamba, step3, step3p5, minicpm5, hy_v3, olmo3 - each a 1:1 port of the pinned vLLM parser with its upstream tests, selected explicitly (`tool_parser`) or auto-detected from the chat template's markers |
...
Link `libvllm` (static or shared) and include [`include/vllm.h`](include/vllm.h). It exposes a flat, exception-free, llama.cpp-style C ABI (`VLLM_ABI_VERSION 4`, 19 exported symbols) suitable for `dlopen` / FFI / LocalAI integration.
