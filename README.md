...
**MLA / DeepSeek campaign — SPIKED, not started (2026-07-21).** The breadth sweep's Tier-3
"MLA = new attention, new campaign" item now has its leaf spike
([`.agents/specs/mla-deepseek-campaign.md`](.agents/specs/mla-deepseek-campaign.md)). Five model rows
moved to `SPIKE`; no code exists. Read that spike for the architecture, the KV-cache impact, and the
honest per-family hardware verdict — the short version is that one member of the family
(DeepSeek-V2-Lite) is gateable on this hardware and the flagship models are not.
