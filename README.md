...
| CUDA | NVIDIA (first target: GB10 / DGX Spark, sm_121a) | 🟡 **gate-model paged-KV stack running on GB10** with both greedy correctness gates passing. Immutable `3f256ab` binds at **55/124**. W3-C frozen-map control passes; W3-E/W3-F strict-fail their components and earn no speed credit. W3-G implements the traced ratio-6 FA2 split-KV route: cast-free BF16 model/dispatcher selection, exact upstream group-swap strides and split heuristic, per-stream/per-shape capture-stable F32 scratch with queue cleanup, `VT_FA2_DECODE=0`, and ported ratio/fallback/capture/lifecycle tests. Clean immutable `ae9e8ff` passes sm_121a compile, **20/20 + 454,323** operator assertions, zero-error/zero-leak memcheck, both 27B arms, 35B correctness-only inertness and the exact paired structural trace. Its completed frozen c2/c16 component reaches **1.017668×/1.006548×** mean total throughput but strict-fails **35/40 timing + 5/8 memory**, so it earns no speed credit. W3-H is **ACTIVE / trace-first** for I/O-only normal BF16→FP4 vectorization. H1a/H1b are void. Immutable `d1f8e33` H1c likewise failed closed on capture 1/3 after Nsight warned that not all 818,537 CUDA events might have been collected; it also configured with CUTLASS disabled and executed naive/WMMA FP4 GEMMs, not the target path. H1d build/profile control and exact schema-v2 validation are now CPU-gated; its immutable target-path DGX captures remain pending. Exact grid and 35B performance remain blocked. |
...
| Format | Status |
|---|---|
| NVFP4 (W4A16 MoE / W4A4 dense, Blackwell) | ✅ **both running on GB10** with token-exact greedy gates passing. The W4A4 path includes all 32 SM12 tactics, merged/fused projections, pre-serve tuning and packed QKV. W3-C closes frozen-plan control; W3-E, W3-F and the separate W3-G attention repair preserve correctness/structure but strict-fail their components. `3f256ab` still fails **69/124** axes. The complete W3-H spike scopes only byte-identical, process-cached 256-bit-load/64-bit-store normal BF16→FP4 I/O behind an exact scalar fallback. H1a/H1b traces are `VOID`. Immutable `d1f8e33` H1c is also `VOID`: capture 1 warned of event loss, and the build had CUTLASS disabled, so its naive/WMMA trace was the wrong NVFP4 path and produced no plan lifecycle. H1d's diagnostic implementation is CPU-gated and its immutable DGX evidence remains pending; no W3-H2 kernel, speed credit or support expansion is claimed. |
