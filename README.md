...
| KV offload to CPU / disk | Built, opt-in, off by default | CPU and disk tiers with identity-checked blocks, selected by a KVTransferConfig connector (a disk-offload tier and the LMCache lm:// client) over one abstract KVConnector ABI; scheduler and worker sides wired |
...
| Tool-call parsing | 30 dialects, streaming | Every pure-text tool parser in the pinned vLLM registry, ported 1:1 with its upstream tests (hermes, mistral, the llama/pythonic family, granite x3, deepseek v3-v4, qwen3, step3/3.5, minicpm5, hy_v3, olmo3, xlam, internlm, jamba, hunyuan, apertus, ernie45, gigachat3, lfm2, poolside, functiongemma, longcat, phi4). Selected via `tool_parser` or auto-detected from the chat template. `tool_choice` required/named compiles the FAMILY'S native syntax as the decode constraint (hermes/qwen3/longcat, llama-json, mistral, deepseek v3/v31); families a structural tag cannot express run unconstrained and rely on their parser |
| Reasoning parsing | 6 parsers, streaming | deepseek_r1 (<think>), mistral ([THINK]), minimax_m2 (+append_think), step3, olmo3 - reasoning split engine-side BEFORE tool parsing, streamed as `reasoning` deltas in the chat chunks |
...
The **Gemma family**: **Gemma 3** (`Gemma3ForCausalLM`, `google/gemma-3-1b-it`) is the **first landed member** (correctness-complete, speed-pending): STRICT token-exact 48/48 greedy vs the vLLM 0.25.0 oracle. It reuses infrastructure already in the tree (gemma-RMSNorm `(1+w)`, the GLM sandwich norms, the shared dense-attention path, sliding-window attention, tied embeddings) plus one genuinely-new compute kernel, GeGLU (`gelu_pytorch_tanh`), a bf16 embedding-scale multiply, dual per-layer RoPE theta, and query_pre_attn_scalar scaling. **Gemma 1 / 2 / 4** remain **spiked-only** ([spike](.agents/specs/sweep-gemma.md)): Gemma 2 adds the attention + final logit soft-caps, and the newest registered variant Gemma 4 needs a large new primitive stack (per-layer embeddings, YOCO KV-sharing, a Gemma-4 MoE) and has only multimodal-wrapped checkpoints. Larger DeepSeek / GLM / MiniMax / Gemma-4 variants are recorded as **hardware-blocked** (they do not fit 119 GiB of unified memory on this box) or **spiked-only**, per the [model matrix](.agents/model-matrix.md). The matrix opens with an architecture-support checklist (a per-architecture status roll-up covering every engaged model) that a CI checker keeps in lockstep with the detailed rows.
