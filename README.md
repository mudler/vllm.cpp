...
|---|---|---|
| CPU | x86-64 and arm64 reference | 🟡 Correctness/CI implementation with native threadpool. **Compute-in-quant is now LIVE and on by default** (2026-07-22): a GGUF weight in one of six supported block encodings (Q4_0, Q8_0, Q3_K, Q4_K, Q5_K, Q6_K) stays compressed from file to matrix multiply instead of being expanded to 16-bit at load, and the multiply runs directly on the compressed blocks. Correctness is intact: the 35B GGUF acceptance gate still reproduces llama.cpp's own greedy output token for token on both mixed real-model files, and on the benchmark model the output tokens are **byte-identical** to the previous path — no result moved. `VT_GGUF_KEEP_QUANT=0` turns it off and `VT_CPU_REF=1` restores the old reference path exactly. **The ordinary 16-bit matrix multiply has since been given real vector instructions too** (2026-07-22), which was the remaining gap: it now runs 16 independent accumulator chains instead of one, with per-architecture vector code (arm64 NEON, x86-64 SSE2/F16C) chosen at run time, and it is **bit-for-bit identical** to the old kernel — every result byte the same, so no stored expected output had to change. Measured on the same file and idle arm64 box, same binary: **prefill 3.41x faster, decode 3.11x faster** on top of the quantized win above. **Against llama.cpp on the same file, decode is now at parity — 1.03x, within 3% and inside llama.cpp's own run-to-run spread** (23.8 vs 24.5 tokens/s). **Prefill is 2.3x behind** (was 8.2x). **Peak memory is now 1.39x worse** (3.88 GiB against 2.80 GiB), down from 2.3x: the model loader was copying every compressed weight into fresh memory and expanding a tied vocabulary table twice — it now reads the compressed weights straight out of the memory-mapped file, shares the one vocabulary table between its two uses, and drops the file pages of tensors it has finished expanding, cutting peak memory from 6.40 to 3.88 GiB with the output tokens byte-identical and decode speed unchanged (2026-07-23). A fresh operation-level profile of the current binary re-ranked the remaining work: prefill is no longer bound by the matrix multiply (that runs at parity), it is split across the quantized multiply, the linear-attention recurrence and the paged attention. **The last two, which ran single-threaded, are now multi-threaded on the CPU** (2026-07-23): the linear-attention recurrence is split across its independent value-heads (each head owns a private state, so the per-head time-ordered recurrence is untouched — matching how the GPU splits it), and paged attention is split across query positions; both are **bit-for-bit identical** to the single-thread path — the benchmark output tokens are byte-identical (same md5) at 1, 4 and 20 threads and under `VT_CPU_REF=1`, and no stored expected output moved. Measured on the idle arm64 box under one lock, same binary: **prefill is now 1.38x faster (73.0 → 100.9 tokens/s), moving from 2.43x to 1.76x behind llama.cpp**, with **decode still at parity**. A fresh operation-level profile confirms the two kernels dropped from 35% of prefill to under 9% combined, so prefill is now bound by the matrix multiplies again (80%: the quantized multiply 50%, the two elementwise multiplies 30%) — the next lever is the per-architecture SIMD/repack GEMM tiers, not another non-matrix kernel. **The first of those tiers has now landed** (2026-07-23): an Arm **i8mm** (int8 matrix-multiply) kernel for the compressed multiply, ported from llama.cpp for the four block encodings that have one upstream (Q8_0, Q4_0, Q4_K, Q6_K; Q3_K/Q5_K have none and stay on the portable kernel), selected at run time only where the CPU actually has the instruction (the GB10 does) and only on the wide prefill shapes — decode (single-row) is untouched. It is **bit-for-bit identical** to the portable kernel for Q8_0/Q4_0 and within the accepted numerical tolerance for the K-quants, run-to-run and across thread counts; the benchmark output tokens are **byte-identical** (same md5) with it on, off, or in reference mode, and the 35B acceptance gate still reproduces llama.cpp token for token on both real files — no token moved. The compressed multiply itself gets much faster at the model's prefill shapes: the **K-quant kernels go 4–8× faster** (Q4_K about eight times, Q6_K about four), Q8_0 about 1.2× (its portable kernel was already near-optimal). On the benchmark file — which is Q8_0-plus-16-bit, the least favourable case for this lever — whole-model **prefill goes 1.08× faster, to 1.44× behind llama.cpp**, with decode unchanged; the large K-quant win lands on the K-quant-heavy 35B production files ([Arm i8mm tier](.agents/specs/gguf-compute-in-quant-gemm.md); [two-kernel threading](.agents/specs/cpu-thread-gdn-paged-2026-07-23.md); [elementwise GEMM](.agents/specs/cpu-elementwise-gemm.md), [keep-quant loader](.agents/specs/gguf-keep-quant-loader.md), [measurement + ranked plan](.agents/specs/cpu-llamacpp-floor-remeasure-2026-07-22.md)). **The last slow-layout matrix multiply on the prefill path has now been removed** (2026-07-23): a fresh operation-level profile of the current binary found that the linear-attention layers' four input projections — 72 matrix multiplies, 18 % of prefill — were the one weight family the loader still transposed into the slower memory layout and ran through the general kernel, while every other weight had already been left in the file's own layout for the faster, cache-blocked kernel. They are now kept in that layout too (only where the CPU quantized path is active; a CUDA build is untouched), which is **bit-for-bit identical** to the transpose — the same sequential accumulation, only the weight offset differs — so the benchmark output tokens are **byte-identical** (same md5) with the change on, off (`VT_GGUF_GDN_NK=0`), or in reference mode, at 1, 4 and 20 threads, and the 35B GGUF acceptance gate still reproduces llama.cpp token for token on both APEX files. Measured same-binary on the idle arm64 box: **prefill 1.09× faster and decode 1.09× faster** (the projections run in decode too), moving prefill from **1.44× to ~1.32× behind** llama.cpp with peak memory unchanged. A fresh post-change profile confirms the general-kernel calls dropped to zero (absorbed into the faster kernel) and the next prefill lever is the quantized multiply itself (now 55 % of prefill; its repack tier) ([GDN projection orientation](.agents/specs/cpu-gdn-proj-orientation-2026-07-23.md)) |
