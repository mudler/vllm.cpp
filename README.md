...
| MoE decoders | Mixtral, Qwen3-MoE | 🚧 | — | 🚧 **Qwen3-Coder-30B-A3B (`Qwen3MoeForCausalLM`) — W0+W1 LANDED 2026-07-21** ([spike](.agents/specs/sweep-qwen3-coder-30b.md)); the first sweep model, a BF16 full-attention MoE that composes the done dense attention + 35B MoE experts (zero runner change). **W0** = new registry TU (`qwen3_moe_registry.cpp` + `qwen3_moe.h`, one `REGISTER_VLLM_MODEL`, full-attention-only KV spec, `is_dense_model=false`) resolves via the clean model-factory seam (zero shared-array edit); load/forward are clear-throwing W2/W3 stubs. **W1** = three behaviour-preserving refactors that make the two done pieces reusable: (#1) the dense self-attention block + device glue extracted verbatim to `dense_attn_block.h` (Qwen3-dense byte-identical), (#2) the bf16 `MoeBlock` exposed cross-TU via `RunMoeBlock` (`qwen3_5_moe_block.h`; 35B untouched), (#3) a no-shared-expert guard in `MoeBlock` (`shared_expert_intermediate_size==0` → skip shared, nullptr to `MoeCombine`; inert for the shared-expert-having 35B). Regression UNCHANGED: Qwen3-dense 0.6B+4B near-tie 16/16, 27B 235/235, 35B 315/315, CUDA `-Werror` 0-warn, memcheck 0. Next: W2 BF16 per-expert loader → W3 forward → W4 near-tie token-exact vs vLLM 0.25.0 → W5 fast BF16 grouped-MoE GEMM (the SPEED gap; only NVFP4-Marlin exists today). Each sweep model is held to the Qwen3 bar (near-tie-robust token-exact + vLLM-speed on every axis) |
