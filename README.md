...
|---|---|---|
| CPU | x86-64 and arm64 reference | 🟡 Correctness/CI implementation with native threadpool. **Compute-in-quant is now LIVE and on by default** (2026-07-22): a GGUF weight in one of six supported block encodings (Q4_0, Q8_0, Q3_K, Q4_K, Q5_K, Q6_K) stays compressed from file to matrix multiply instead of being expanded to 16-bit at load, and the multiply runs directly on the compressed blocks. Correctness is intact: the 35B GGUF acceptance gate still reproduces llama.cpp's own greedy output token for token on both mixed real-model files, and on the benchmark model the output tokens are **byte-identical** to the previous path — no result moved. `VT_GGUF_KEEP_QUANT=0` turns it off and `VT_CPU_REF=1` restores the old reference path exactly. Measured on the same file and idle arm64 box as before, same binary: **decode 3.45x faster, prefill 4.16x faster, peak memory 1.16x smaller**. llama.cpp on the same file remains the floor we must match or beat, and **we are still behind it on every axis** — but by much less: **decode 3.4x** (was 11.6x), **prefill 8.2x** (was 33.5x), **peak memory 2.3x** (6.40 GiB against llama.cpp's 2.80 GiB; was 2.65x). The earlier projection of a 9-17x jump did **not** hold, and the measurement says exactly why: 56 of that file's tensors are plain 16-bit — 60% of its weight bytes, including the largest matrix in the model — and no block encoding covers those, so they still take the slow general kernel. **The remaining CPU gap is therefore the ordinary 16-bit matrix multiply, not the quantized one**, which makes giving it real vector instructions the next thing to do ([result](.agents/specs/gguf-compute-in-quant-gemm.md), [measurement + ranked plan](.agents/specs/cpu-llamacpp-floor-remeasure-2026-07-22.md)) |
