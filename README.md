...
> gates on NVIDIA GB10. Production parity is nearly closed. The **new binding 27B
> result** is the fresh, fully-interleaved exact-grid rerun at `9ecd9d0` on the
> full production default set (async scheduling + vendored Triton GDN decode cubin
> + bit-identical fast RMSNorm + bit-identical fast gated-RMSNorm): **114/124**
> required axes pass against vLLM v0.25.0 (gate NO — 10 axes remain). It
> **supersedes** `a875397` (52/124), `246a23c` (49/124) and `3f256ab` (55/124),
> all retained immutable. Evidence `dgx:~/work/vllm.cpp-online-gate/evidence/9ecd9d0…`,
> ratios.json sha256 `8c81083e…`; ZERO void, 12/12 binding-eligible.
>
> This is +62 axes over the prior binding — a decode-kernel-efficiency close:
> **memory 4/4, c1 20/20, c2 20/20, c16 19/20, c4 & c32 18/20, c8 15/20.** The
> lever that did it is a family of **bit-identical (0-ulp) fast decode kernels** —
> each reproduces its shipped reference's exact float-op order (so the fast set
> yields identical logits and can never cross the 27B tok6 razor near-tie), while
> vectorizing memory access for the speedup: `RmsNormRowFastKernel` (2.41×
> isolated, `348d12d`), `RmsNormGatedRowFastKernel` (2.04× at c16, `9ecd9d0`),
> both `VT_*` default ON with `=0` rollback, on top of the async default
> (`a0013a2`) and the vendored GDN cubin (`a321d7c`). Correctness holds: the full
> default set is 27B 235/235 + 35B 315/315 token-exact.
>
> **The 10 remaining axes** are all near-parity or low-concurrency noise: seven
> are the batch-independent decode floor sitting at 0.987–0.999 (c8 mean/median
> itl+tpot, c16/c32 median_itl, c32 median_tpot — a whisker short); two are c4
> TTFT (0.906/0.943, the bimodal low-concurrency arrival lottery); and one —
> **c8 p99_itl 0.86** (ours 557 vs vLLM 479 ms) — is now ROOT-CAUSED as the
> honest residual: at c8, our deterministic forward keeps co-admitted requests in
> lockstep (uniform 840 ms two-prefill wave-boundary stalls) where vLLM's async
> runtime jitter de-phases them; but ours BEATS vLLM on this same tail at c16
> (1.055) and c32 (1.078), so it is the trailing edge of the per-step determinism
> that wins higher concurrency, not a capability gap — no code fix, no fake fix
> (see `.agents/specs/c8-p99-itl-tail-2026-07-18.md`). Closing path for the
> decode-floor seven: the batch-independent glue is now all landed BIT-IDENTICAL
> and DEFAULT ON — the two FP4-quant/SiLU fast kernels flipped ON (parity-enabler
> policy) and the new GDN decode conv-update fast kernel (`CausalConv1dUpdateFastKernel`,
> **1.92× isolated**, 0-ulp) — so the next binding grid runs the full bit-identical
> fast-decode stack by default; characterize the c4 TTFT lottery vs run-noise.
> Memory PASSES via the windowed-load release (`cb2d310`, ours peak PSS
> 24.88 GB vs vLLM 28.18 GB). No 35B performance result is claimed until 27B
> reaches 124/124. See [Benchmarks](docs/BENCHMARKS.md).
...
| Qwen3.6-27B performance | ❌ FAILED / `GATING` | **NEW BINDING `9ecd9d0`: 114/124 pass** (gate NO; fresh interleaved exact-grid rerun on the full production default set — async + Triton GDN cubin + bit-identical fast RMSNorm + bit-identical fast gated-RMSNorm; supersedes `a875397`'s 52/124, `246a23c`'s 49/124, `3f256ab`'s 55/124, all retained immutable; ZERO void, 12/12 binding-eligible; evidence `dgx:~/work/vllm.cpp-online-gate/evidence/9ecd9d0…`, ratios.json sha256 `8c81083e…`). Per concurrency: mem **4/4**, c1 **20/20**, c2 **20/20**, c16 **19/20**, c4 18/20, c32 18/20, c8 15/20. +62 axes over the prior binding — a decode-kernel-efficiency close via a family of **bit-identical (0-ulp) fast decode kernels** (`RmsNormRowFastKernel` 2.41×, `RmsNormGatedRowFastKernel` 2.04× at c16, both default ON, reproducing their shipped reference's exact float-op order so the fast set yields identical logits — no near-tie crossing), on the async (`a0013a2`) + vendored GDN cubin (`a321d7c`) defaults; full set 27B 235/235 + 35B 315/315 token-exact. **10 axes remain:** seven are the batch-independent decode floor at 0.987–0.999 (c8 mean/median itl+tpot, c16/c32 median_itl, c32 median_tpot), two are c4 TTFT low-concurrency arrival lottery (0.906/0.943), one is the c8 p99_itl wave-boundary tail (0.86) — now ROOT-CAUSED as the honest residual (irreducible-as-mirrored: ours' deterministic forward lockstep vs vLLM's async runtime-jitter de-phase; ours BEATS vLLM on the same tail at c16/c32, so not a capability gap; no code fix — `.agents/specs/c8-p99-itl-tail-2026-07-18.md`). Closing path: fold in the landed bit-identical FP4-quant/SiLU glue (`861b518`) + GDN conv-update for the decode floor; characterize the c4 TTFT lottery. Memory PASSES (windowed-load `cb2d310`, ours peak PSS 24.88 GB vs vLLM 28.18 GB); packed-decode CLOSED on equivalence (`KERNEL-GDN-PACKED-DECODE` `DONE`, `e47b4d6`) | Decode-kernel-efficiency close CONFIRMED by the `9ecd9d0` binding: the bit-identical fast RMSNorm + gated-RMSNorm stack moved 52→114/124, closing c2 entirely (3→20) and c16 to 19/20. The measured attribution held — the uniform decode deficit was the norm/quant/act kernel glue, and vectorizing it (bit-identically, so token-exact) closed it. Remaining: the batch-independent glue tail (FP4/SiLU/conv levers, landed/ready), the c8 p99 wave-boundary tail, and c4 TTFT arrival-lottery noise. 35B only after 27B reaches 124/124 |
...
|---|---|
| Binding gate | **NEW BINDING `9ecd9d0`: 114/124** (gate NO; supersedes `a875397`'s 52/124, `246a23c`'s 49/124, `3f256ab`'s 55/124). Mem 4/4, c1 20/20, c2 20/20, c16 19/20; the bit-identical fast RMSNorm + gated-RMSNorm stack closed +62 axes. 10 remain: the batch-independent decode floor at 0.987–0.999 (7), c4 TTFT lottery (2), c8 p99_itl wave-boundary tail 0.86 (1, ROOT-CAUSED as the honest residual — ours' deterministic-forward lockstep vs vLLM's async runtime-jitter de-phase, ours BEATS vLLM on this tail at c16/c32, no code fix) — closing the seven decode-floor via the landed FP4/SiLU + GDN conv-update glue levers + c4-noise characterization |
