...
**MLA / DeepSeek campaign — W0-W6 landed, no MLA model runs yet (2026-07-22).** The breadth
sweep's Tier-3 "MLA = new attention, new campaign" item has its leaf spike
([`.agents/specs/mla-deepseek-campaign.md`](.agents/specs/mla-deepseek-campaign.md)), and the first
seven steps of its plan are now in the tree. **No DeepSeek/Kimi model is supported** — the five model
rows stay `SPIKE`. A complete MLA attention *layer* now exists, but nothing registers, loads or runs a
checkpoint through it: there is no MLA model, no config parse, no weight loader and no model forward,
so nothing in this family can be loaded or run.
...
are unchanged. Still **no speed number**, and still no model. Read the
