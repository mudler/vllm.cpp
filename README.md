...
| Remaining gap diagnosis | The 2026-07-14 [parity rescan](.agents/specs/parity-rescan-2026-07-14.md) grounds the failing mass as **host-side**: TTFT passes 24/24, our GPU kernels are net faster on the measured window, and the open axes are c2–c8 decode latency plus host memory. The prior RMSNorm/generated-partitions residual is **disproven** (vLLM's norm-quant fusion is FP8-only; cross-profiler artifact). Parallel host workstreams: TCP_NODELAY on the SSE server (now **implemented + CPU-tested**, below), memory precheck → weight streaming, and nsys c2 attribution before async-sched W3 |
| Serving transport (TCP_NODELAY) | **IMPLEMENTED + CPU-tested; GPU sizing PENDING** (`SERVE-HTTP-TRANSPORT`). We now mirror vLLM's uvicorn/asyncio default and call `set_tcp_nodelay(true)` on the SSE server so per-token frames are not held by Nagle; a behavioral accepted-socket test pins it (RED `TCP_NODELAY` 0 → GREEN 1, full server suite 22/22). The one non-binding localhost sizing is deferred while the GPU is held by the c16 diagnostic; parity-axis credit comes only from the authorized exact-grid rerun |
