...
| Qwen3-Coder-30B-A3B (`Qwen3MoeForCausalLM`) BF16 MoE — W5 | **ACCEPTED NUMBERS / PARITY MISSED — `ACTIVE`** (2026-07-21, `CLAIM-MODEL-QWEN3-CODER`; [spike §9](../.agents/specs/sweep-qwen3-coder-30b.md)) | Breadth-sweep model #1's SPEED row. W5 shipped the fast BF16 grouped-MoE GEMM `vt::MoeGroupedGemmBf16` (**default ON**, `VT_MOE_BF16_FAST=0` rolls back) replacing the per-expert host-gather loop with ~3 on-device grouped launches, plus a host-mirror release (the experts are ~57 GiB = ~94% of the checkpoint; retaining host + device copies cost ~114 GiB of GB10's 119 GiB unified pool, left `free=1 GiB`, and COLLAPSED c>=2 — c2 output throughput fell BELOW c1 and c4 never completed; **all pre-fix c>=2 numbers are VOID**). Correctness re-verified and STRICTER (`test_qwen3coder_paged_engine` **6/6**; STRICT token-exact **4/6 -> 5/6**; max teacher-forced gap **0.125 -> 0.0000 nats**). SAME-BINARY A/B (`VT_MOE_BF16_FAST` 0->1, c1): median TTFT **3092 -> 904 ms (3.4x)**, median TPOT **64.4 -> 40.4 ms (1.59x)**, per-stream decode **15.9 -> 24.8 tok/s (1.56x)**. **BINDING GRID** vs vLLM 0.25.0 PRODUCTION GRAPHED (`enforce_eager=False`, `CUDAGraphMode.FULL_AND_PIECEWISE`, Inductor `VLLM_COMPILE`, `moe_backend='triton'` — the OOM-safe backend for a 57 GiB bf16 model on a 119 GiB unified pool), random 1024/128 `--ignore-eos`, num-prompts 16/24/48/96, idle box under one `flock /tmp/gpu`, checkpoint page-cache evicted between runs — ours / vLLM -> ratio (>1 = we win): **median TTFT** c1 904.5/370.0 **0.41x**, c2 1684.6/248.9 **0.15x**, c4 2489.6/458.4 **0.18x**, c8 2501.4/600.4 **0.24x**; **median TPOT** c1 40.40/32.22 **0.80x**, c2 43.81/44.71 **1.02x**, c4 98.98/74.09 **0.75x**, c8 148.85/81.80 **0.55x**; **output throughput** c1 18.55/28.76 **0.65x**, c2 31.85/42.32 **0.75x**, c4 32.20/51.24 **0.63x**, c8 46.55/94.52 **0.49x**. Only c2 TPOT reaches parity. **`nsys --cuda-graph-trace=node` + `cuda_gpu_kern_sum` ATTRIBUTION (ours, c1):** prefill `MoeGroupedGemmBf16Wmma<float>` **35.9%** + `<bf16>` **20.1%** = **56.0% of all GPU time** = 48 layers x (2x5.71 + 6.39) ms = **855 ms ~= our entire 904 ms TTFT**; at 3.71 TFLOP of MoE GEMM per 1024-token prefill that is **~4.3 TFLOP/s ~= 1.7% of GB10 bf16 tensor-core peak** vs vLLM's Triton **~10.0 TFLOP/s** (vLLM itself untuned — it logs the missing `E=128,N=768,device_name=NVIDIA_GB10.json` config); root cause = the WMMA tile is a first-cut naive GEMM (BK=32, no `cp.async` double-buffering, BN=64, no register blocking). Decode `MoeGroupedGemmBf16Naive<float>` **15.7%** + `<bf16>` **5.6%** = **21.3%** (~21.7 ms of our 40 ms TPOT); gate/up moves 25.2 MB in 166.7 us = **151 GB/s (55% of ~273 GB/s)** vs down's 211 GB/s (77%) — BLOCK-PARALLELISM STARVATION at N=768 (24 blocks vs down's 64). Memory axis CLOSED by the host-mirror release: ours ~62 GiB vs vLLM's 56.93 GiB weights + 11.38 GiB KV. Gates: `-Werror` 0-warn; 6/6; `test_ops_moe_grouped_bf16` 4/4; 27B 235/235 + 35B 315/315 UNCHANGED; memcheck 0 on the MoE GEMM + engine path | **Named levers to close the row to `DONE`:** (i) w13 gate+up FUSION — one grouped GEMM over a concatenated `[K,2I]` operand, exactly what vLLM `fused_moe` does and what we ALREADY ship for Marlin as `VT_MOE_FUSED_W13` (doubles N to 1536, halves the sort/launch count); (ii) DETERMINISTIC split-K (two-pass partials + fixed-order reduce — explicitly NOT `atomicAdd`, which would break token determinism and the SACRED gate); (iii) a **bf16 decode CUDA graph** (the 35B's is fp4-gated, `qwen3_5_moe.cpp:76-82` — also why our TPOT degrades 3.4x c2->c8 while vLLM's degrades 1.83x); (iv) prefill WMMA tile/pipeline rework. Repro: `examples/vllm-bench --model <coder-snapshot> --input-len 1024 --output-len 128 --concurrency {1,2,4,8} --num-prompts {16,24,48,96}` vs `vllm serve <snapshot> --moe-backend triton --gpu-memory-utilization 0.58 --max-model-len 2048 --max-num-seqs 32` + `vllm bench serve --dataset-name random --random-input-len 1024 --random-output-len 128 --random-range-ratio 0 --ignore-eos --max-concurrency C` |
