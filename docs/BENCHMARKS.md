...
| Host-weight ownership | **Established 27B binding memory axes remain PASS at `246a23c`. Local plain-BF16 4B (`LOAD-SAFETENSORS-DIRECT-DENSE`) remains `GATING`.** Immutable `176d4926` completes the current-ABI H32 AOT repair gates: exact Hv=48/32 AOT-vs-hand-vs-CPU **56/56**, full GDN **53/53 (3,229/3,229)**, real-model default/rollback **1,664/1,664** each, memcheck 0-error/0-leak and manifest drift green. Exact locked AOT/fallback means: total **6427.07/6155.57 tok/s (+4.41%)**, output **710.69/680.67 (+4.41%)**, TTFT **722.61/723.32 ms**, TPOT/ITL **39.457/41.437 ms (-4.78%)**, E2EL **5733.27/5985.83 ms (-4.22%)**. Node-mode nsys attributes the gain to packed recurrence **1.467331/2.585435 s (-43.25%)** over the same 11,016 calls. | **REPAIR ACCEPTED / PARITY FAILED-OPEN (`benchmark_binding=false`).** Fresh vLLM-0.25 18-leg direct ON/OFF/reference means: total **6418.26/6316.05/6744.29 tok/s**, output **709.71/698.41/745.76**, request **5.547/5.460/5.826 req/s**, TTFT **722.68/819.49/900.04 ms**, TPOT/ITL **39.513/39.487/33.492 ms**, E2EL **5741.03/5834.21/5153.57 ms**, peak PSS **2.573/8.571/7.636 GiB**, stable PSS **0.739/8.571/4.334 GiB**, VRAM **12890/12870/12956 MiB**. ON is **0.9714x** historical AOT but only **0.9517x vLLM**; TPOT/E2EL and strict +8-MiB loader VRAM fail. Direct ON=OFF is 128/128 every rep, while AOT/fallback differ 9/128 and both match the deterministic eager v0.25 oracle only 97/128, so end-to-end correctness remains open despite operator correctness. Next: isolate the remaining decode path and resolve oracle/VRAM, then repeat; external 27B/35B remain pending with no support extrapolation. [Evidence](bench-evidence/qwen35-4b-h32-aot-repair-20260721.md). |
