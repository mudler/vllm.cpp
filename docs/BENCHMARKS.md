...
Local disposition is **FAILED CORRECTNESS / NOT A PERFORMANCE RUN**: Python compile,
shell syntax, focused harness tests **26/26** and a full CUDA-off Release build
...
| `KERNEL-GEMM-NVFP4-W4A4` small-M dispatch | **ACTIVE / GATING — H1d REPAIR PENDING** | W3-C control is complete; W3-E/W3-F/W3-G fail. H1a/H1b/H1c are `VOID`; `5f8fab1` is void on the non-binding build path and `d063f20` is void before plan on artifact-order rejection. The repaired recipe requires empty-root plan first, then corpus, plus `RelWithDebInfo`, Triton-AOT/FA2/CUTLASS and pinned tools. Retained-vLLM reaggregation finds **1,476** clean windows and normal production at **0.342627 ms/window**, diagnostic only. No W3-H2 vector kernel or accepted performance result exists. | Run a fresh immutable H1d. Implement only byte-identical, process-cached 256-bit-load/64-bit-store I/O if correctness and all three captures are lossless, exact-plan and launch/workload gates pass; exact grid/35B performance remain held. |
