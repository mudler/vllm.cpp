...
| Host-weight ownership | **Established 27B binding memory axes remain PASS at `246a23c`**: windowed `madvise(MADV_DONTNEED)` yields ours peak PSS/RSS 24,879,201/24,881,800 KiB vs vLLM 28,184,400/28,563,020 KiB; the 35B host-free/load-stream work remains recorded separately. **Corrected local plain-BF16 4B scoreboard (`LOAD-SAFETENSORS-DIRECT-DENSE`, `GATING`):** immutable project commit `647a2a6a978b3f47c80d8be9249f5f740299da1a`, root `/tmp/qwen35-transplant-4b-647a2a6a`, ShareGPT SHA-256 `9ea136…`, 128 requests / 131,784 actual input tokens / 16,384 output tokens / c32 / greedy ignore-EOS / MNB 2048 / 1280 blocks / max-len 4096. Three memory plus three unmonitored timing reps per arm ran under one `flock /tmp/gpu`; all 18 pre-leg compute-app snapshots were empty, cache proofs succeeded, and thermal/throttle guards were clean. Means for direct ON / direct OFF / local production-graphed vLLM-0.24: request rate **3.913/3.870/5.814 req/s**; output throughput **501.40/495.39/744.22 tok/s**; total throughput **4534.38/4480.04/6730.37 tok/s**; TTFT mean/median/p99 **1114.40/688.75/5304.86**, **1210.83/688.57/5687.61**, **904.74/675.53/3109.33 ms**; TPOT mean/median/p99 **55.30/60.15/62.71**, **55.32/60.20/62.76**, **33.54/34.95/45.90 ms**; ITL mean/median/p99 **55.30/21.71/346.86**, **55.32/21.78/347.19**, **33.54/16.84/190.55 ms**; E2EL mean/median/p99 **8136.94/8109.18/12929.95**, **8235.81/8195.93/13310.76**, **5164.79/5097.28/7562.28 ms**. Peak PSS **2.606/8.559/7.671 GiB**; stable PSS **0.727/8.558/4.058 GiB**; peak VRAM **12944/12936/12957 MiB**. Aggregate SHA-256 `0f95d6763e1a87d6b6e9e4028d33a84ae9351c5495703d4f2a13dc3d81a9b749`. | **Disposition: local diagnostic complete, performance gate FAIL/open.** Direct ON gains **1.21% total/output throughput** over same-binary OFF, cuts peak PSS **69.6%** and stable PSS **91.5%**, and lowers mean TTFT **8.0%**; paired ON/OFF output IDs are stable and equal **128/128 in all three repetitions**. But ON is only **0.686×** the previous branch ON (**6607.04 tok/s; −31.4%**) and **0.674×** current local vLLM. OFF is **0.678×** its previous 6603.28 result, while current local vLLM is **1.002×** previous 6716.47, locating the regression in current project behavior rather than the machine/reference. ON peak PSS is 47.4% above the previous 1.768 GiB ON result and VRAM is 8 MiB worse than same-binary OFF, so strict loader closure is not met. Local vLLM is diagnostic rather than binding: it is v0.24, not current v0.25, and its second same-arm output file matches rep 1 for only 102/128 requests; no cross-engine correctness claim is made. Required matching nsys traces completed with `--cuda-graph-trace=node`: project SHA `86f0946e…`, vLLM SHA `3b4d8114…`; project GPU time is led by bf16 CUTLASS GEMM (27.2%), custom `PagedFlashKernel` (24.3%) and packed GDN (8.0%), while vLLM resolves bf16 CUTLASS GEMM (23.5%), `flash_fwd_kernel` (16.1%), different WMMA/GEMM tactics and fused recurrent/Triton kernels. This proves structural execution differences, not a single cause. Reproduce with `flock /tmp/gpu tools/bench/run_qwen35_4b_compare.sh /tmp/qwen35-transplant-4b-<commit>` after building the immutable commit; current-v0.25 correctness/performance, sanitizer, strict ON<=OFF VRAM and external 27B/35B gates remain `PENDING`. No 27B/35B support extrapolation. |
