...
Local disposition is **PENDING GPU / NOT A PERFORMANCE RUN**: Python compile,
shell syntax, focused harness tests **25/25** and a full CUDA-off Release build
...
retained rather than represented as a clean 106/106 run. No H1d SQLite/status,
throughput, latency, memory ratio or speed credit exists yet. H1d must be
...
| `SERVE-GATE-ONLINE` | **FAILED / GATING — `3f256ab` BINDS 55/124; H1d GPU PENDING** | Immutable `3f256ab` remains **55/124**. W3-C passes frozen-map control; W3-E, W3-F and W3-G strict-fail and earn no speed credit. W3-H1a/H1b/H1c are `VOID`; H1c lost events and used a CUTLASS-disabled fallback. H1d's trace-only four-replay controller, exact target-build/production-build separation, schema-v2 SQLite/session/plan/client validation and clean vLLM window reaggregation are implemented and CPU-gated (**25/25** focused tests plus CUDA-off Release build). No GPU capture, accepted ratio, exact grid or 35B performance exists. | Execute the immutable target-path H1d series and require three uniquely linked, zero-exit, lossless, exact-plan captures before residual ranking. Only a later 48/48 component authorizes the exact vLLM grid; 35B performance remains held. |
...
| `KERNEL-GEMM-NVFP4-W4A4` small-M dispatch | **ACTIVE / GATING — H1d GPU PENDING** | W3-C reproduction control is complete; W3-E/W3-F/W3-G remain strict failures. W3-H inventories the normal BF16/direct-swizzled producer and prior neutral `f787cf8` constraint. H1a/H1b/H1c are `VOID`. H1d's diagnostic controller and fail-closed build/trace/plan/vLLM validation are now implemented and CPU-gated, but no immutable target-path capture exists. Reaggregation of the retained vLLM raw trace finds **1,476** clean windows and normal production at **0.342627 ms/window**; this is diagnostic only. No W3-H2 vector kernel or accepted performance result exists. | Run immutable H1d. Implement only byte-identical, process-cached 256-bit-load/64-bit-store I/O if all three captures are lossless, every process proves the exact plan map, launch/workload reconciliation passes and the residual ranking confirms it; exact grid/35B performance remain held. |
