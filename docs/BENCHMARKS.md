...
| MiniMax-Music3 DiT attention kernel, `thor:gpu0` sm_110 (#672, [#1555](https://github.com/mudler/vllm.cpp/issues/1555)) | ONE binary, arms by `VT_OP_PROVIDER_DISABLE`, engine-announced provider as the control, staged ckpt, 3 alternated pairs spread 0.080%: `denoise.dit_device` **24.770 -> 15.046 s, 1.6461x**; kernel alone **7.62x** | 30-step pair PENDING a second lease. `ncu` UNAVAILABLE here, so the mechanism is ABLATION: occupancy and bandwidth both REFUTED. Detail: benchmark-record |
