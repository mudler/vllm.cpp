...
| Host-weight ownership | **Established 27B binding memory axes remain PASS at `246a23c`**. **Local plain-BF16 4B checkpoint (`LOAD-SAFETENSORS-DIRECT-DENSE`, `ACTIVE`):** root `/tmp/qwen35-transplant-4b-647a2a6a` used the exact 128-request / 131,784-input / 16,384-output / c32 corpus and completed 18/18 guarded legs. Its same-binary loader evidence remains valid: direct ON/OFF output IDs equal 128/128 in all three pairs; ON is +1.21% throughput and peak/stable PSS **2.606/0.727 GiB** vs OFF **8.559/8.558 GiB**. | **Cross-build performance result VOID — mismatched Triton configuration.** Current project commit `647a2a6a` was built from `build-nix-cuda-transplant` with `VLLM_CPP_TRITON=OFF`; the previous 6607.04 tok/s project arm invoked `build-nix-cuda-triton-sm120-debug`, whose cache proves `VLLM_CPP_TRITON=ON`, `VLLM_CPP_TRITON_REGEN=ON`, `VT_GDN_DECODE_TRITON_AOT=1`, sm_120 and FlashInfer CUTLASS. The vLLM reference also loaded Torch AOT and executed Triton/FLA kernels. Therefore **4534.38 vs 6607.04 tok/s cannot be attributed to current-main code**, and the 0.674x vLLM ratio is not the intended optimized-project comparison. Rebuild current source with the exact previous-project AOT/CUTLASS flags, gate token equivalence, then rerun all 18 legs and both `--cuda-graph-trace=node` traces. Reproduction: `CPP_BENCH=$PWD/build-nix-cuda-transplant-triton/examples/vllm-bench CMAKE_CACHE=$PWD/build-nix-cuda-transplant-triton/CMakeCache.txt flock /tmp/gpu tools/bench/run_qwen35_4b_compare.sh /tmp/qwen35-transplant-4b-aot-<commit>`. Current-v0.25, sanitizer, strict ON<=OFF VRAM and 27B/35B remain `PENDING`; no support extrapolation. |
