...
Last updated: **2026-07-11** (`KV-DEVICE-RESIDENCY` W0 CPU checkpoint;
corrected 27B baseline active).
...
| `KV-DEVICE-RESIDENCY` | **ACTIVE — CPU CHECKPOINT ONLY** | W0 replaces CUDA host-vector ownership for full-attention KV and GDN state with stable backend allocations, stream-ordered zeroing, and a `VT_DEVICE_KV_CACHE=0` fallback. Focused CPU build and 4/4 tests pass; real CUDA tests also assert pointer type in both gate-model binaries. No W0 CUDA binary, pointer, model, memory, trace, or performance result exists yet. | Finish the exact `f065fce` cache-off 27B baseline already running under one DGX lock; then build this W0 on GB10 and run default/fallback correctness, pointer, sanitizer, memory, trace, and repeated same-binary A/B gates before W1 indexed state I/O. |
