...
| `ENG-ASYNC-SCHED` (W3) | **Host-side machinery LANDED + CPU-gated (2026-07-16), `ACTIVE`; no GPU behavior change this commit** | `AsyncScheduler` output-placeholder accounting (1:1 with `async_scheduler.py:12-75`), depth-2 `step_with_batch_queue` (`core.py:519-632`), `async_scheduling` default-ON-when-compatible config resolution + `MaxConcurrentBatches` + the `VT_ASYNC_SCHED` rollback env are implemented and CPU-tested RED-first (`tests/vllm/v1/test_async_scheduler.cpp` 6 ported cases / 54 asserts, verified RED against the base `Scheduler` 2/6 fail; depth-2 engine cycle; config resolution). Full CPU ctest **108/108**, clean full CPU `-Werror` rebuild 0 warnings. The synchronous path is byte-identical (placeholder sites inert while count 0); production wiring unchanged (`runner_supports_async` FALSE keeps the default synchronous), so no benchmark number moves. `benchmark_binding=false`, no speed credit | The runner-side leaf (placeholder-aware device-input `combine_sampled_and_draft_tokens` + non-blocking sampled-token-ID D2H on a copy stream + `vt::Backend` event/pinned) is the piece that captures the measured ~3.25 ms/step GPU-idle; then the DGX both-model-gate both-rollback-arm A/B (interleaved c16 W3-on vs W3-off, ~+3 ms/step ≈ +15 tok/s target, with a TTFT-regression watch). Prior `3812d8` control freezes the c2 speed budget at neutral (total 1.002153×) |
