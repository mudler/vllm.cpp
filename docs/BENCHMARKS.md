...
trace gates; its strict c2/c16 component and every new accepted performance
number remain explicitly `PENDING`).
...
CORRECTNESS, SAFETY AND STRUCTURAL TRACE PASS; COMPONENT PENDING**.
...
Its operator/sanitizer/model/inertness/trace gates pass; execute the strict
c2/c16 same-binary **40 timing + 8 memory** component. The conditional exact
...
| `KERNEL-ATTN-FA2` ratio-6 split-KV decode | **GATING — CUDA/SAFETY/MODEL/STRUCTURAL PASS; PERFORMANCE PENDING** | Immutable `ae9e8ff` passes **20/20 + 454,323** CUDA assertions, zero-error/zero-leak memcheck, both 27B arms and 35B correctness. Default/fallback traces switch exactly between **240 main+combine / 0 old** and **0 combine / 240 old**, while FP4 topology/plans are identical. The short trace is performance-negative (**3.246400/1.395488 ms**) and earns no credit. Binding remains **22,893 / 8,793.238 ms** local versus unequal-window vLLM split main **23,616 / 7,061.921 ms** + combine **23,488 / 123.245 ms**. | Run the frozen input-1,024 c2/c16 all-40-timing/all-8-memory component; fail closed before the exact grid. |
