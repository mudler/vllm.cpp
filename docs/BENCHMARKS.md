...
source/trace-grounded small-M FP4 W1 repair is implemented and CPU/TSan-green,
but has no CUDA or performance result yet. The 35B campaign waits for 27B
...
| `KERNEL-GEMM-NVFP4-W4A4` small-M dispatch | **W1 IMPLEMENTED / CPU-GREEN — GPU + PERFORMANCE PENDING** | Default dispatch now mirrors FlashInfer's hybrid M mapping (distinct 1/2/4/8/16), keys by bucket/N/K/device/SM/output dtype/tactic ABI, publishes one tune per key, wakes waiters on failure, retries without partial state, and rejects an uncached capture before creating CUDA events. `VT_FP4_EXACT_BUCKETS=0` restores the exact aliased baseline. Focused Release builds cleanly and passes **100/100** repetitions; ThreadSanitizer passes **9 cases / 615 assertions**. This checkpoint has no CUDA/model/capture/AB-BA-AB/oracle result and claims no speed ratio. The clean `4e1d8ca` ladder remains the before-state. Four wide persistent tactics remain; W2's traced 32-tactic family is deliberately not stacked here. | Push the CPU checkpoint, then run clean sm_121a build, focused CUDA correctness/capture/memcheck and both-model default/fallback gates. Only then run exact-bucket vs legacy component AB/BA/AB, paired trace and the full exact 27B oracle campaign; keep W2 separate. |
