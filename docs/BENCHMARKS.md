...
v0.25 persistent/frozen FP4 tactic caching to W3-C `READY`, with implementation
and same-plan performance still `PENDING`).
...
no accepted number: code, stable 64/64 fresh-process plans, same-plan output
hashes, component axes and any conditional exact grid are all **PENDING**.
...
| `SERVE-GATE-ONLINE` | **FAILED / GATING — `3f256ab` BINDS 55/124** | Immutable `3f256ab` completed all 36 groups, 2,016 requests, six returns and the paired trace; **69/124 axes fail**. Total ratios are **0.993504/0.954464/0.966438/0.980678/1.027889/1.039417×**. Clean packed/split c16 remains **1.005049×** with **14/20 timing + 2/4 memory**; post-pack topology is 208.192 vs 208 FP4 GEMMs/forward. Generated code refutes the apparent norm+FP4 fusion gap. W3-E removes 624 activation swizzles and gains 0.211%/0.622% mean total throughput at c2/c16, but strict-fails **32/40 timing + 6/8 memory**. The conditional exact grid therefore did not run. The binding v0.25 log proves a persistent 64-plan hit while W3-E fresh processes are tactic-unstable; W3-C is `READY` as a same-plan control, without speed credit. The 35B correctness-only inertness gate ran; no 35B performance run occurred. | Implement/gate W3-C load/save/import/frozen mode, require 64/64 identical plans and same-plan c2/c16 every-axis acceptance, then run an exact grid only if that component accepts. 35B performance remains held until full 27B closure. |
...
| `KERNEL-GEMM-NVFP4-W4A4` small-M dispatch | **ACTIVE — W3-C SPIKED/READY; IMPLEMENTATION + BENCHMARK PENDING** | W3-E remains implemented/`GATING`: tracing removes all 624 activation-scale swizzles and its 12-leg component gains **1.002108× c2 / 1.006222× c16** mean total throughput, but accepts only **32/40 timing + 6/8 memory**. The v0.25 oracle log/cache SHA `367af18d…2da` / `b41a8ecc…677` proves a 64-entry persistent hit; ours retuned every process, paired IDs match only 18--33/64 and five of six long-output hashes differ. The accepted [W3-C spike](../.agents/specs/nvfp4-persistent-plan-cache.md) specifies native JSON round-trip, exact FlashInfer import, stale rejection, atomic publication, frozen read-only mode and the current 5,000-us miss method. No W3-C code or benchmark has run; `3f256ab` remains binding at 55/124. | Implement C1/C2; pass schema/sanitizer/model gates and six-process 64/64 stability. Then rerun direct/fallback c2/c16 with one frozen oracle map; exact 27B grid remains conditional and 35B performance prohibited. |
