...
Last updated: **2026-07-11** (the exact pushed-`a531e05` 27B online campaign is
below floor. The confirmed c32 HTTP-capacity repair is implemented and
CPU/sanitizer-gated, with its exact GPU A/B pending; the source/trace-grounded
small-M FP4 implementation spike is accepted, but no FP4 repair is implemented
yet. The 35B campaign waits for 27B closure. External KV-cache/LMCache
...
| `SERVE-GATE-ONLINE` | **BELOW FLOOR — VALID `a531e05` 27B CHECKPOINT** | Exact greedy/cache-off/closed-loop input-1024→output-128 c1–c32 evidence has 2,016/2,016 successful timed requests, three interleaved repetitions, six verified memory returns, model gate **1/1**, and passing paired-trace status. Median total-throughput ratios are **0.9679/0.9338/0.9482/0.9547/1.0028/0.9268×**; only **4/2/5/3/10/8 of 20** throughput+latency axes pass. Median mean-TPOT ratios are **0.9665/0.9204/0.9342/0.9204/0.9676/0.9858×**. Memory passes **2/4**: ours/vLLM median PSS **48,175,251/28,075,085 KiB**, RSS **48,177,656/28,401,904 KiB**, GPU **39,067/72,704 MiB**, and available-memory drop **66,305,132/80,687,860 KiB**. Campaign/trace-status hashes are `24d78fbc…e9d2a` / `1c702ef9…142a`; ours nsys/kernel `22d5a0f4…47d1` / `ab7d0131…d6a3`; vLLM trace/kernel `83fd0f41…2a66` / `7056183f…e417`. 35B was not run. | Gate the implemented HTTP fix at c32/full ladder, then repair the confirmed FP4 bucket/tactic mismatch and repeat exact 27B after each iteration. Only once every 27B axis passes, run 35B. Pool teardown remains separately required. |
| `SERVE-ASYNC-LLM` HTTP capacity | **IMPLEMENTED + CPU-GREEN — GPU A/B PENDING** | Production replaces cpp-httplib's racy 19→76 dynamic pool with a fixed **`max_num_seqs + 4`** floor (36 workers at the c32 gate); `VLLM_CPP_HTTP_FIXED_POOL=0` selects the exact legacy pool in the same binary for attribution. A real-socket regression parks 32 keep-alive connections, then proves the control reserve reads and answers another socket. Focused Release/help pass; the API binary passes **100/100** repetitions, ASan+UBSan **1/1**, and TSan **1/1** under the host's required `setarch -R`. Full serial CTest is **104/105** only on the already-recorded intermittent C-API merged-delta count; isolated C API and focused tests pass. No GPU ratio is claimed yet. | Commit/push the CPU checkpoint, then run legacy/fixed AB/BA/AB at exact c32 plus the fresh full 27B oracle ladder. The row remains `ACTIVE` until that evidence passes. |
...
| `KERNEL-GEMM-NVFP4-W4A4` small-M dispatch | **CONFIRMED GAP — SPIKE ACCEPTED / IMPLEMENTATION READY** | `NextPow2M` clamps every M=1/2/4/8/16 call to one M=16 key. Because the exact ladder starts at c1, its M=1 tactic is reused through c16. Three fresh-server traces started directly at c16 retune real M=16 and give TPOT **161.747/161.719/161.729 ms**, effectively the standard vLLM mean **161.698 ms**, versus **167.484 ms** in our ascending standard series. The paired vLLM trace resolves 128×32×256 Stream-K/static-persistent FP4 tactics absent from our four wide persistent candidates. The accepted [dependency-chain spike](../.agents/specs/nvfp4-small-m-dispatch.md) inventories FlashInfer 0.6.12's hybrid mapping, exact 32-tactic order, raw template/workspace/capture behavior, upstream tests and gates. This documentation checkpoint adds no runtime code or new number. | After the immutable HTTP campaign is classified, port W1 exact buckets plus per-key single-flight behind `VT_FP4_EXACT_BUCKETS=0`, run its AB/BA/AB and full 27B oracle gate, then separately port and measure the full tactic family. |
