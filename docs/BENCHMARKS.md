...
shell syntax, focused harness tests **27/27** and a full CUDA-off Release build
...
| `SERVE-GATE-ONLINE` | **FAILED / GATING — `3f256ab` BINDS 55/124; H1d REPAIR PENDING** | Immutable `3f256ab` remains **55/124**. W3-E/W3-F/W3-G strict-fail; H1a/H1b/H1c are `VOID`. Clean `7bae38a`/`2d16c68` failed before build/GPU on omitted Ninja/`nvcc`. Clean `5f8fab1` built, but its invalid `Release`/no-Triton-AOT recipe failed the 27B gate identically **3/3 observer-on + 2/2 observer-off** before capture; retained `ae9e8ff` still passes. Clean `d063f20` is void pre-plan because corpus was seeded before dry-run. Clean `b1c7eb6` passed exact plan/configuration but is void pre-build on the now-fixed driver bootstrap check. No GPU trace, accepted ratio, exact grid or 35B performance exists. | Execute a fresh immutable `RelWithDebInfo` root: plan first in an empty evidence directory, then frozen corpus; bind Triton-AOT, FA2, CUTLASS, oracle Ninja and CUDA 13.0.88; let the driver build before executable validation; require correctness plus three uniquely linked, zero-exit, lossless, exact-plan captures. Only a later 48/48 component authorizes the exact grid; 35B performance remains held. |
...
| `KERNEL-GEMM-NVFP4-W4A4` small-M dispatch | **ACTIVE / GATING — H1d REPAIR PENDING** | W3-C control is complete; W3-E/W3-F/W3-G fail. H1a/H1b/H1c are `VOID`; `5f8fab1` is void on the non-binding build path, `d063f20` is void before plan on artifact-order rejection, and `b1c7eb6` is void before build on the repaired bootstrap check. The recipe now requires empty-root plan first, then corpus, exact `RelWithDebInfo` Triton-AOT/FA2/CUTLASS configuration, and driver-owned target build before executable validation. Retained-vLLM reaggregation finds **1,476** clean windows and normal production at **0.342627 ms/window**, diagnostic only. No W3-H2 vector kernel or accepted performance result exists. | Run a fresh immutable H1d. Implement only byte-identical, process-cached 256-bit-load/64-bit-store I/O if correctness and all three captures are lossless, exact-plan and launch/workload gates pass; exact grid/35B performance remain held. |
