...
not a performance run. Clean `3c1d7b7` then exercised the synchronous-SIGUSR1
repair: build/gate/clients/four-replay close all passed, but SIGUSR1 terminated
the target and Nsight returned 138 before validation. That root is also
...
also `VOID`. Four isolated `repeat:4` single-replay ranges are now specified;
implementation and fresh immutable evidence are pending.
...
The repair did not accept 143 and attempted synchronous SIGUSR1 shutdown.
Clean pushed `3c1d7b79243ac173e2828e1fb3a74de4e72fbd35` exercised that
path from immutable root
`~/work/vllm.cpp-executed-path-refresh-h1d/3c1d7b79243ac173e2828e1fb3a74de4e72fbd35`.
The exact **154/154** build passed, as did the 27B gate **1/1 in 16.83 s**,
ordinary client **48/48 in 66.803863 s**, probe **16/16 in 23.780080 s**, and
the exact profile window (`prior_replays=483`, `captured_replays=4`).
Nsight wrote a 448,789-byte report (SHA `4cc9deef…0c1`). Process-directed
SIGUSR1 nevertheless terminated the target before requested/completed
shutdown markers, so Nsight propagated exit **138**. No SQLite, captures 2/3,
vLLM trace or ratio exists. Driver/profile/command/client/probe SHA-256 are
`3e936aa8…030` / `62c773f5…bc9` / `b9f0c465…c3f` /
`4f37de0a…184` / `7d94fd6b…78f`. The root is **FAILED / VOID**, is never
reused, and changes no accepted number. GPU, `/tmp/gpu` and port 8001
returned idle.

The accepted diagnostic path no longer uses a terminating signal for shutdown.
For each capture the driver creates a mode-0600 named FIFO; the diagnostic
target opens and validates it read-only, records its ready marker, and calls
the HTTP server's thread-safe `stop()` only after reading one `Q`. The driver
writes that byte only after the exact four-replay close marker. The record
requires ready/requested/completed FIFO markers with the target PID, FIFO
removal and profiler exit zero; SIGTERM/KILL remain failure cleanup only.
Production builds compile the FIFO path out.

Clean pushed `219f4f2449aaf6afe7a9a8a41a0dabb58c85e0f3` exercised that
repair from immutable root
`~/work/vllm.cpp-executed-path-refresh-h1d/219f4f2449aaf6afe7a9a8a41a0dabb58c85e0f3`.
The exact **154/154** build and 27B gate **1/1 in 17.22 s** passed. Capture 1
completed the ordinary **48/48** client in 66.910178 s, the **16/16** probe in
23.824916 s and four graph launches after 484 prior replays. The FIFO
ready/requested/completed lifecycle, removal, target exit zero and Nsight exit
zero all passed. Those client rates remain structural diagnostics only.

The 395,367-byte report and 2,068,480-byte SQLite have SHA-256
`6b8667c7…161` / `da637485…0c`; profile/command/control are
`c14f68d6…59f` / `f29369db…4c5` / `9c36ca83…6d7`. The validator rejected
severity-2 `Not all CUDA events might have been collected.` and stopped before
captures 2/3 or vLLM. Read-only SQL proves all intended data exists exactly:
four `cudaGraphLaunch` rows, **4,428 kernel rows / 1,107 nodes x4**, **28
memcpy rows / 7 nodes x4**, **4 memset rows / 1 node x4**, uniform replay
counts and exact FP4/FA2/GDN family cardinalities. It also proves why the range
is invalid: the three decode-step gaps contribute **9 eager kernels, 9 eager
memcpys and 3 eager memsets** for argmax, token readback and next-input
embedding. The root is **FAILED / VOID**, never reused, and changes no accepted
number. GPU, `/tmp/gpu` and port 8001 returned idle.

The next H1d capture keeps every rejection intact and changes the capture
boundary: Nsight uses `--capture-range-end=repeat:4`, while the diagnostic
controller performs start → one graph launch → stream synchronize → stop four
times. Sampler/input work must execute between ranges and every SQLite must
contain zero eager kernel/memcpy/memset rows. No severity is whitelisted and no
existing evidence is reclassified.

Current H1d disposition is **FAILED / NOT A PERFORMANCE RUN**. Python and shell
syntax, diagnostic-macro syntax, the CUDA-off server build and focused
client/summary/trace tests **31/31** pass. Two full CUDA-off runs pass
**105/106** and fail only the unrelated timing-sensitive C API early-stop case;
that case passes in isolation. No accepted H1d SQLite/status, throughput,
latency, memory ratio or speed credit exists. H1d must complete three
...
| `SERVE-GATE-ONLINE` | **FAILED / GATING — `3f256ab` BINDS 55/124; H1d REPEATED-RANGE REPAIR PENDING** | Immutable `3f256ab` remains **55/124**. W3-E/W3-F/W3-G strict-fail; H1a/H1b/H1c and H1d attempts through `219f4f2` are `VOID`. `219f4f2` passes exact build/gate, 48/48 + 16/16 clients, FIFO lifecycle/removal and target/Nsight zero exit. Its SQLite has every intended graph child x4 but also three eager sampler/input gaps and severity-2 possible loss, so no trace is accepted. Four isolated `repeat:4` ranges are specified but not implemented. No ratio, exact grid or 35B performance exists. | Implement and CPU-gate the repeated-range controller, then execute a fresh immutable `RelWithDebInfo` root; require exact build, correctness plus three uniquely linked, zero-exit, zero-eager, lossless, exact-plan captures. Only a later 48/48 component authorizes the exact grid; 35B performance remains held. |
...
| `KERNEL-GEMM-NVFP4-W4A4` small-M dispatch | **ACTIVE / GATING — H1d REPEATED-RANGE REPAIR PENDING** | W3-C control is complete; W3-E/W3-F/W3-G fail. H1a/H1b/H1c and H1d attempts through `219f4f2` are `VOID`; the latest proves FIFO/zero-exit and exact graph-child counts, but one continuous range captures three eager sampler/input gaps and emits severity-2 possible loss. Four isolated `repeat:4` ranges are specified without relaxing validation. Retained-vLLM reaggregation finds **1,476** clean windows and normal production at **0.342627 ms/window**, diagnostic only. No W3-H2 vector kernel or accepted performance result exists. | Implement and gate repeated ranges, then run a fresh immutable H1d. Implement producer I/O only if correctness and all three captures are lossless, zero-eager, exact-plan and launch/workload gates pass; exact grid/35B performance remain held. |
