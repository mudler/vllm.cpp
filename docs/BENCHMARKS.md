...
The current bounded checkpoint is **W3-G / ACTIVE — IMPLEMENTATION SOURCE +
CUDA-OFF COMPILE CHECKPOINT; CUDA/BENCHMARK PENDING**. Qwen3.6-27B uses Hq/Hkv
...
capacity rollover and two-queue cleanup. The warning-as-error CUDA-off Release
build and focused paged-attention CTest pass; those FA2-only cases are compiled
out without CUDA, so this is not CUDA correctness evidence. Existing prefill is
unchanged. The 35B ratio-8 path remains explicitly ineligible. Vectorized
normal BF16→FP4 production stays separate.

Clean sm_121a compile, CUDA operator execution, sanitizer, 27B ON/OFF model
gates, 35B inertness, node traces and c2/c16 performance are **PENDING**. No
new throughput, latency or memory number is accepted. The component
...
| `SERVE-GATE-ONLINE` | **FAILED / GATING — `3f256ab` BINDS 55/124** | Immutable `3f256ab` remains **55/124**. W3-C passes frozen-map control; W3-E and W3-F strict-fail and earn no speed credit. W3-G ratio-6 FA2 decode source and upstream-derived fallback/capture/lifecycle tests now exist; only its CUDA-off compile/test checkpoint is green. Clean sm_121a/operator/sanitizer/model/trace and every runtime axis remain `PENDING`. No exact grid or 35B performance command ran. | Freeze this implementation, require immutable CUDA/operator/sanitizer/27B-ON+OFF/35B-inertness/trace gates and all **40 timing + 8 memory** component axes. Only then run the exact vLLM grid; 35B performance remains held. |
...
| `KERNEL-ATTN-FA2` ratio-6 split-KV decode | **ACTIVE — SOURCE/PORTED TESTS PRESENT; CUDA RUNTIME PENDING** | Exact swap-stride params, split heuristic, stable shape scratch/queue cleanup, model+operator dispatch and `VT_FA2_DECODE=0` are implemented. Ported tests cover upstream paged fallback, ratio-6 B ladder, invalid eligibility, graph replay/capacity and two queues. CUDA-off warning-as-error build + focused CTest pass; no FA2 kernel executed and no accepted ratio exists. Binding remains **22,893 / 8,793.238 ms** local versus unequal-window vLLM split main **23,616 / 7,061.921 ms** + combine **23,488 / 123.245 ms**. | Build immutable sm_121a source, then pass operator/capture/lifecycle, sanitizer, both 27B arms, 35B inertness, paired node trace and all 40+8 component axes. |
