...
source/trace-grounded small-M FP4 W1 repair is now active. The 35B campaign
waits for 27B closure. External KV-cache/LMCache
...
| `KERNEL-GEMM-NVFP4-W4A4` small-M dispatch | **CONFIRMED GAP — W1 ACTIVE** | `NextPow2M` clamps every M=1/2/4/8/16 call to one M=16 key. Because the exact ladder starts at c1, its M=1 tactic is reused through c16. Three fresh-server traces started directly at c16 retune real M=16 and give TPOT **161.747/161.719/161.729 ms**, effectively the prior standard vLLM mean **161.698 ms**, versus **167.484 ms** in our ascending series. The paired vLLM trace resolves 128×32×256 Stream-K/static-persistent FP4 tactics absent from our four wide persistent candidates. The accepted [dependency-chain spike](../.agents/specs/nvfp4-small-m-dispatch.md) inventories FlashInfer 0.6.12's hybrid mapping, exact 32-tactic order, raw template/workspace/capture behavior, upstream tests and gates. The clean `4e1d8ca` ladder confirms the gap remains after HTTP capacity is removed from the timed path. | W1 now ports exact buckets, a complete plan key, per-key single-flight and capture-miss rejection behind `VT_FP4_EXACT_BUCKETS=0`; run its unit/CUDA/model AB/BA/AB and full 27B oracle gate before separately porting and measuring the full tactic family. |
