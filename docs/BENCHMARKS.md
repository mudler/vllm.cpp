...
| Host-weight ownership | **Established 27B binding memory axes remain PASS at `246a23c`**. **Local plain-BF16 4B checkpoint (`LOAD-SAFETENSORS-DIRECT-DENSE`, `GATING`):** corrected AOT root `/tmp/qwen35-transplant-4b-aot-557ab41d` used the exact 128-request / 131,784-input / 16,384-output / c32 corpus and completed 18/18 guarded legs. Direct ON/OFF/local-vLLM-0.24 means: total **6155.10/6064.06/6730.46 tok/s**, output **680.61/670.54/744.24**, request **5.317/5.240/5.814 req/s**, mean TTFT **722.56/815.05/903.20 ms**, mean TPOT/ITL **41.44/41.43/33.56 ms**, mean E2EL **5985.90/6076.37/5164.87 ms**. Peak PSS **2.405/8.571/7.569 GiB**, stable PSS **0.733/8.571/4.066 GiB**, peak VRAM **12892/12884/12942.7 MiB**. ON=OFF outputs equal 128/128 in all three pairs; ON is +1.50% total and −71.9%/−91.4% peak/stable PSS. | **Performance regression diagnosed; repair remains `PENDING`.** Current ON is **0.9316x** previous AOT ON (6155.10 vs 6607.04, −6.84%) and **0.9145x** local vLLM; the reference itself is stable at 1.002x previous. Exact-workload node-mode nsys root `/tmp/qwen35-transplant-4b-aot-profile-832ff89d` isolates a GPU-path regression: synchronous current/previous GPU kernel time **23.306561/21.960202 s**. The dominant change is previous sm120 H=32 Triton recurrence **0.601555 s / 12,624 calls / 47.652 us** becoming current hand `GdnPackedDecodeKernel` **2.579749 s / 11,016 / 234.182 us**; newer FA2/fusions offset part of the +1.978-s raw recurrence delta, leaving +1.346 s net. Shipping async costs another 2.20% under profiler but does not explain the residual. Old pool cap ON/OFF changes speed only +0.39%, so it explains the +1200-MiB memory denominator, not throughput. `VT_GDN_PACKED_DECODE=0` is rejected as a remedy: only +0.86%, 117/128 output identity, and its decomposed recurrence is still 2.652 s. The vLLM trace proves actual H=32 packed Triton execution at 104.619 us/call. Next reproduction/gate: add a current raw-packed/f32-state H=32 AOT specialization, then require current-default vs same-binary rollback 128/128 plus repeated every-axis A/B. Existing compare command remains `REQUIRE_TRITON_AOT=1 CPP_BENCH=$PWD/build-nix-cuda-transplant-triton/examples/vllm-bench CMAKE_CACHE=$PWD/build-nix-cuda-transplant-triton/CMakeCache.txt flock /tmp/gpu tools/bench/run_qwen35_4b_compare.sh /tmp/qwen35-transplant-4b-aot-<commit>`. Current-v0.25 correctness, sanitizer, strict ON<=OFF VRAM (+8 MiB) and 27B/35B stay `PENDING`; no support extrapolation. [Profile evidence](bench-evidence/qwen35-4b-aot-regression-profile-20260721.md). |
