...
| Host-weight ownership | **Established 27B binding memory axes remain PASS at `246a23c`**. **Local plain-BF16 4B checkpoint (`LOAD-SAFETENSORS-DIRECT-DENSE`, `ACTIVE` / GATING):** the last binding root `/tmp/qwen35-transplant-4b-aot-557ab41d` used the exact 128-request / 131,784-input / 16,384-output / c32 corpus and completed 18/18 guarded legs. Direct ON/OFF/local-vLLM-0.24 means: total **6155.10/6064.06/6730.46 tok/s**, output **680.61/670.54/744.24**, request **5.317/5.240/5.814 req/s**, mean TTFT **722.56/815.05/903.20 ms**, mean TPOT/ITL **41.44/41.43/33.56 ms**, mean E2EL **5985.90/6076.37/5164.87 ms**. Peak PSS **2.405/8.571/7.569 GiB**, stable PSS **0.733/8.571/4.066 GiB**, peak VRAM **12892/12884/12942.7 MiB**. ON=OFF outputs equal 128/128 in all three pairs; ON is +1.50% total and −71.9%/−91.4% peak/stable PSS. | **Regression repair implemented; new performance remains `PENDING`.** Matched node-mode profiles attributed the prior loss to current hand recurrence (**2.579749 s / 234.182 us-call**) replacing previous H=32 Triton (**0.601555 s / 47.652 us-call**). `gdn_decode_h32` now mirrors the current vLLM FLA raw-packed/FP32-state ABI at exact Hk=16/Hv=32/Dk=Dv=128 strides with warps1/stages3; the old decomposed/BF16-state ABI remains absent. Exact Hv=48/32 AOT-vs-hand-vs-CPU is **56/56**, full GDN **53/53 (3,229/3,229)**, flag parsing **10/10**, and sm_120/sm_121a manifest drift checks pass. No throughput number is accepted from the dirty implementation tree. Next clean-commit gate: default versus `VT_GDN_PACKED_DECODE_TRITON=0` token equality, sanitizer, repeated same-binary A/B and `--cuda-graph-trace=node`, followed by the exact compare command `REQUIRE_TRITON_AOT=1 CPP_BENCH=$PWD/build-nix-cuda-transplant-triton/examples/vllm-bench CMAKE_CACHE=$PWD/build-nix-cuda-transplant-triton/CMakeCache.txt flock /tmp/gpu tools/bench/run_qwen35_4b_compare.sh /tmp/qwen35-h32-aot-<commit>`. Current-v0.25 correctness, strict ON<=OFF VRAM (+8 MiB) and external 27B/35B stay `PENDING`; no support extrapolation. [Profile evidence](bench-evidence/qwen35-4b-aot-regression-profile-20260721.md). |
