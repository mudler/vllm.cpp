...
6. This changes no speed denominator: repeated selection/component, trace and
exact oracle performance remain pending. The
...
| `KERNEL-GEMM-NVFP4-W4A4` small-M dispatch | **W3-B IMMUTABLE SAFETY GREEN — SELECTION/PERFORMANCE PENDING; W2 BINDING BELOW FLOOR** | W1/W2 and immutable W3-A evidence remain unchanged. W3-B adds an actual-W4A4-gated maximum-token request in the shared `LoadedEngine`, exact FlashInfer profile enumeration, process-local completion statistics, capture-safe ready hits and diagnostic-only later misses; `VT_FP4_PRE_SERVE_WARMUP=0` restores W3-A. Clean pushed `d7cdf66` (tree `24abb109…c488`) builds with CUDA 13.0.88/sm_121a/CUTLASS 4.5. Registry/loader pass **12/12 + 4/4**; exact/legacy CUDA each pass **14/14 / 26,819/26,819**; native 27B passes **235/235 + 16/16** and tunes **80/80** profiles into 80 entries with zero lazy misses; memcheck passes **24,586/24,586, zero errors**. HTTP answers `/health` and `/v1/models`, with completion line 3 before listening line 6 and zero misses. Evidence root `~/work/vllm.cpp-nvfp4-small-m/d7cdf66…/w3b`; manifest/provenance SHA `6f372fbe…89b1` / `1e8db7b7…e936`; model/memcheck/server logs `5ea053fe…b6475` / `2ef8f758…b124` / `04d04fce…6951`. This closes immutable safety, not speed; W3-A's failed +0.816% component and clean `b5c6e4f` denominator remain binding. | Repeat fresh-process selection/component AB/BA/AB, then run paired trace. Only after that rerun the exact vLLM 27B ladder. W3-C persistence stays optional; do not run 35B. |
...
### Reproduce the W3-B immutable preflight
