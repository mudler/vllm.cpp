...
`466e359a…9dd8`. Residual selection is **PENDING** until another executed
kernel-body difference survives this same check. GPU, ports and lock exit idle.
...
Next: execute that one-lock exact grid and re-rank the now-structural-parity residual. Every
...
| `SERVE-GATE-ONLINE` | **FAILED / GATING — `3f256ab` BINDS 55/124** | Immutable `3f256ab` completed all 36 groups, 2,016 requests, six returns and the paired trace; **69/124 axes fail**. Total ratios are **0.993504/0.954464/0.966438/0.980678/1.027889/1.039417×**. Clean packed/split c16 remains **1.005049×** with **14/20 timing + 2/4 memory**; post-pack topology is 208.192 vs 208 FP4 GEMMs/forward. Generated code refutes the apparent norm+FP4 fusion gap, so residual selection is pending. 35B did not run. | Continue body-level trace diffing, require a clean local slice and full dependency-chain proof, then same-binary A/B and another exact grid; only then consider 35B. |
