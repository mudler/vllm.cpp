...
production-faithful default stays without steady-state speed credit; paired
trace and exact-oracle performance remain open, and no denominator changes.
Trace attempt 1 is **VOID**: prewarm's three retained c16/48 runs completed
**144/144**, but placing newly created client results under a cache-drop root
changed its inventory **50→58 files**; the lifecycle validator stopped before
lazy or vLLM. No trace number or kernel conclusion is binding. Evidence
`~/work/vllm.cpp-nvfp4-small-m/d7cdf66…/w3b/trace-ab-oracle`, driver/report/tree
SHA `c1162d8f…2743` / `eb8a0996…1cc5c` / `69a16a4d…b62f`; corrected repro uses
an immutable corpus-only eviction root. The
...
| `KERNEL-GEMM-NVFP4-W4A4` small-M dispatch | **W3-B COMPONENT STRICT FAIL / FIRST-USE WIN; TRACE ATTEMPT 1 VOID; W2 BINDING BELOW FLOOR** | W1/W2 and immutable W3-A evidence remain unchanged. W3-B adds an actual-W4A4-gated maximum-token request in the shared `LoadedEngine`, exact FlashInfer profile enumeration, process-local completion statistics, capture-safe ready hits and diagnostic-only later misses; `VT_FP4_PRE_SERVE_WARMUP=0` restores W3-A. Clean pushed `d7cdf66` (tree `24abb109…c488`) is immutable correctness/safety-green: registry/loader **12/12 + 4/4**, exact/legacy CUDA **14/14 / 26,819/26,819** each, native 27B **235/235 + 16/16**, **80/80** profiles, zero lazy misses, memcheck **24,586/24,586 / zero errors**, and completion-before-listening. The repeated c16/96 AB/BA/AB then passes **576/576** requests, six cache evictions and six memory returns. Prewarm runs **806.723/806.094/812.555**, lazy **809.209/807.292/808.160 tok/s**; means **808.457/808.220 = 1.000293×**, CV **0.360%/0.097%**. Strict result is **15/20 timing** and **2/4 memory**; prewarm GPU/available-drop are worse (**38,048/37,615 MiB**, **65,615,217/64,893,877 KiB**) while PSS/RSS are slightly better. Prewarm keys are **80/80/80**, but only **20/80** IDs are stable; lazy keys **35/40/40**, stable **9/30**; paired equal IDs are **13/35, 18/40, 17/40**. Untimed preflight mean first-chunk/full-request is **0.779/14.929 s** prewarm versus **5.662/20.249 s** lazy. Evidence `~/work/vllm.cpp-nvfp4-small-m/d7cdf66…/w3b/component-ab`; summary/selection/driver/provenance/tree SHA `c371848a…cd11` / `fec3bf11…99c8` / `e996e6dd…662` / `1df8bdbe…23cb` / `85910147…7b6`; GPU exits empty and lock is free. Shipping prewarm remains for production parity and first-use behavior, but receives no steady-state speed credit. Trace attempt 1 is **VOID** after prewarm-only **144/144** because mutable client outputs changed cache inventory **50→58**; lifecycle validation stopped before lazy/vLLM, so no trace interpretation is allowed. | Rerun the paired prewarm/lazy/vLLM nsys with client artifacts outside the immutable corpus-only eviction root and classify actual kernel/tactic mix. Then choose the next trace-grounded lever and rerun the exact vLLM 27B ladder. W3-C persistence stays optional; do not run 35B. |
