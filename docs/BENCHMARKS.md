...
| Serving transport (TCP_NODELAY) | **IMPLEMENTED / CPU-TESTED; SIZING PENDING** | Mirror landed (`SERVE-HTTP-TRANSPORT`): `set_tcp_nodelay(true)` on the SSE server now matches vLLM's uvicorn/asyncio default (asyncio sets `TCP_NODELAY` on every accepted socket). Test-first behavioral proof: accepted-socket `getsockopt(TCP_NODELAY)` RED **0** → GREEN **1**, full `test_openai_api_server` 22/22 cases / 242 assertions, `-Werror` clean. The non-binding localhost sizing is unmeasured (magnitude illustrative only) | One non-binding localhost SSE A/B sizing on an idle GPU window (deferred — GPU held by the c16 diagnostic); binding parity-axis credit only via the authorized exact-grid rerun |
