...
|---|---|---|---|
| `SERVE-GATE-ONLINE` | **FAILED / GATING** | Immutable `3f256ab` remains **55/124**; packed correctness/structure is accepted. The `4a450f9` reproduction captured the c16 `duplicate live GDN state index` **3/3**. **Repair proven on DGX at `c172336`**: `--diagnostic-c16` passes 3/3, both model gates 235/235, and the first sealed 12-leg component reached `complete-void` (stable means, non-regressing forensic medians; voided on c2 TTFT max-of-6 tails). Component rerun executing | Component rerun to a verified terminal status; the exact grid remains unauthorized until a `complete-pass` |
...
| `KERNEL-GDN-PACKED-DECODE` | **ACTIVE / W1D3 repair PROVEN on DGX; component VOID on TTFT tails, rerun executing** | Root cause captured 3/3 at `4a450f9` (6 requests, 5 live + 27 free of 32 slots; `packed_decode=0` lead-up). **Fixed test-first**: `remap_gdn_state_slots` keyed the compact slot pool on the mamba block-id, which collapses to the shared null block-id 0 once a sequence exceeds one mamba block (sub-sequence `block_size`), so two long c16 sequences shared one slot; the pool now keys on the request identity. RED runner test threw `duplicate live GDN state index`, GREEN after fix (`test_runner` 8/8; tools 132/132). DGX at `c172336`: fresh `--diagnostic-c16` passes **3/3** (zero engine-fatal lines), both direct model gates **235/235**, and the FIRST sealed 12-leg component reached marker-last **`complete-void`** — all throughput/mean axes stable (≤2.34%) with forensic medians non-regressing (c2 packed tput +0.32%, TPOT 108.736 vs 109.100 ms), voided only by c2 TTFT max-of-6 tail deviations (4.10/5.57/10.58%) | Rerun executing from fresh `-r2` root; if tails void again while means are stable, revise the 6-sample tail stability rule test-first before a third run |
