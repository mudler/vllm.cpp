...
A/B strict-fails at 32/40 timing + 6/8 memory).
...
Next: do not run the conditional W3-E exact grid; its component failed. Re-scan
the executed vLLM/local hot paths and spike the next unstacked trace-grounded
lever. Every 27B throughput, latency and memory axis must pass before 35B; broader roadmap
...
| `KERNEL-GEMM-NVFP4-W4A4` small-M dispatch | **ACTIVE — W3-E IMPLEMENTED / COMPONENT STRICT-FAILED / EXACT GRID NOT RUN** | W3-E mirrors vLLM's direct scale address in normal and both fused producers, zeroes padded bytes, defaults only on CUDA true-W4A4, and retains `VT_FP4_DIRECT_SF=0`. CPU/CUDA direct-vs-composed bytes/GEMM output pass; focused no-pool sanitizer is zero-error/leak-free; direct/fallback 27B pass 235/235 + 16/16; 35B is inert at 315/315. Paired tracing removes all 624 activation-scale swizzles. The 12-leg component gains **1.002108× c2 / 1.006222× c16** mean total throughput, but accepts only **32/40 timing + 6/8 memory**; five of six paired long-output hashes differ amid unstable tactic IDs. `3f256ab` remains binding at 55/124. | No conditional exact grid or 35B performance run. Re-scan the executed residual, spike the next isolated lever, and retain W3-E as `GATING` without accepted speed credit. |
