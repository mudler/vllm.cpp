...
| `SERVE-GATE-ONLINE` | **FAILED / GATING — `3f256ab` BINDS 55/124; H1d GPU PENDING** | Immutable `3f256ab` remains **55/124**. W3-E/W3-F/W3-G strict-fail; H1a/H1b/H1c are `VOID`. H1d's controller/schema is CPU-gated (**25/25** plus CUDA-off Release build). Clean `7bae38a`/`2d16c68` failed before build/GPU because login `PATH` omitted Ninja and then `nvcc`; configure SHAs are `ac9e4854…f616` / `378fdd7a…4c3c`, both roots are retained and yield no evidence. No GPU capture, accepted ratio, exact grid or 35B performance exists. | Execute a new immutable root with oracle Ninja plus CUDA 13.0.88 `nvcc` pinned and hashed; require three uniquely linked, zero-exit, lossless, exact-plan captures. Only a later 48/48 component authorizes the exact grid; 35B performance remains held. |
...
| `KERNEL-GEMM-NVFP4-W4A4` small-M dispatch | **ACTIVE / GATING — H1d GPU PENDING** | W3-C control is complete; W3-E/W3-F/W3-G fail. H1a/H1b/H1c are `VOID`. H1d's diagnostic controller/build/trace validation is CPU-gated. Clean `7bae38a`/`2d16c68` failed pre-build/GPU on omitted Ninja/`nvcc`; the fully pinned replacement has no capture yet. Retained-vLLM reaggregation finds **1,476** clean windows and normal production at **0.342627 ms/window**, diagnostic only. No W3-H2 vector kernel or accepted performance result exists. | Run immutable H1d. Implement only byte-identical, process-cached 256-bit-load/64-bit-store I/O if all three captures are lossless, exact-plan and launch/workload gates pass; exact grid/35B performance remain held. |
