...
| Non-CUDA backends (Metal, Vulkan, Intel XPU) | **NOT APPLICABLE — SPIKE ONLY; NO NUMBER MEASURED OR CLAIMED** (2026-07-22, `CLAIM-BACKEND-FANOUT-1`; [spike](../.agents/specs/backend-fanout-metal-vulkan-xpu.md)). Design/records-only: no source file, no CMake edit, no kernel, no shader, nothing downloaded. Three rows `INVENTORIED` -> `SPIKE`. **No backend is implemented, so no throughput, latency or memory number is owed, produced or claimed.** Every native-competitor row (`BACKEND-GATE-METAL-*`, `BACKEND-GATE-VULKAN-LLAMACPP`, `BACKEND-GATE-XPU-VLLM`) stays `INVENTORIED` | The benchmark-relevant output is a per-backend HARDWARE and ORACLE verdict, MEASURED rather than assumed. **Metal REACHABLE and GATEABLE:** the tree builds on the M4 under CLT-only AppleClang 21 and **108,952 portable-tier assertions pass on Apple arm64** across 7 suites (incl. `test_ops_quant_dot` 78,052 and `test_gguf_keep_quant` 5,574) — a third-architecture confirmation of the GGUF quant tier — after a one-line `-force_load` fix for `CMakeLists.txt:304-306`, which as shipped silently drops every static registrar on macOS (observed: `test_backend` 5/7 FAIL, `no backend registered for device type 0`; after the fix 7/7). Runtime MSL compilation plus a numerically-correct dispatched kernel verified with no Xcode and no MLX. **Vulkan REACHABLE and GATEABLE:** GB10 enumerates at API 1.4.312 with `VK_KHR_cooperative_matrix` v2 + `VK_NV_cooperative_matrix2` and one unified 89.72 GiB heap, with `llvmpipe` on the dev box for GPU-free CI. **Intel XPU HW-BLOCKED:** no Intel GPU here, and vLLM has no in-tree SYCL source to mirror. Bars are chosen so they can actually be RUN — vLLM does not run on the M4 and has no Vulkan path at all, so **Vulkan gates against our own CUDA backend on the same GB10 box** and **Metal against our own CPU backend on the M4**; XPU gets NO e2e gate. Bit-exactness is explicitly NOT promised across CPU/GPU for reductions (CPU determinism comes from a fixed sequential order) — the bar is the already-ported NMSE <= 5e-4, with bit-exactness retained for pure copy/layout ops | **No benchmark is possible or owed until a backend exists.** The first speed number this track can produce is a small-model run on the M4 against llama.cpp Metal (`BACKEND-GATE-METAL-LLAMACPP`), gated behind work rows `W0` (8 shared seam repairs) and `M1`-`M3`. Correctness precedes speed unconditionally. **Apple gate-scale is permanently out of reach on this host** (16 GB / 11.84 GiB working set cannot hold the 27B/35B gate models), so no Apple result may ever be extrapolated to them. Repro: nothing was executed as a benchmark. Re-derive the hardware evidence with the throwaway probes recorded in [state.md](../.agents/state.md) — M4 `~/vllmcpp-probe`; dgx `/tmp/vkprobe` + `/tmp/vkext` under `$HOME/gpu.lock` |
