...
| `SERVE-GATE-ONLINE` | **FAILED / GATING — `3f256ab` BINDS 55/124; H1d GPU PENDING** | Immutable `3f256ab` remains **55/124**. W3-C passes frozen-map control; W3-E/W3-F/W3-G strict-fail. H1a/H1b/H1c are `VOID`. H1d's four-replay controller and schema-v2 validation are CPU-gated (**25/25** focused tests plus CUDA-off Release build). Clean `7bae38a` then failed before build/GPU because login `PATH` omitted Ninja; its configure SHA is `ac9e4854…f616`, its root is retained, and it yields no evidence. No GPU capture, accepted ratio, exact grid or 35B performance exists. | Execute a new immutable root with the oracle venv's Ninja pinned by absolute path; require three uniquely linked, zero-exit, lossless, exact-plan captures before residual ranking. Only a later 48/48 component authorizes the exact vLLM grid; 35B performance remains held. |
