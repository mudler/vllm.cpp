...
while the fresh exact W3-B grid remains mandatory. Corrected evidence
...
| `SERVE-GATE-ONLINE` | **BELOW FLOOR — VALID CLEAN `b5c6e4f` 27B CHECKPOINT; W3-B TRACE CLEAN** | Exact greedy/cache-off/closed-loop input-1024→output-128 c1-c32 evidence has **2,016/2,016** successful timed requests, three interleaved repetitions, six verified memory returns, model gate **1/1**, and passing paired-trace status. Median total-throughput ratios are **0.9933/0.9520/0.9657/0.9760/1.0213/1.0218×**; **4/4/5/4/17/14 of 20** throughput+latency axes pass. Median mean-TPOT normalized ratios are **0.9915/0.9417/0.9476/0.9404/0.9827/0.9837×**. Memory passes **2/4**: ours/vLLM median PSS **48,272,873/28,096,858 KiB**, RSS **48,275,264/28,424,060 KiB**, GPU **38,746/72,608 MiB**, and available-memory drop **66,089,528/80,435,540 KiB**. The canonical 27-only runs/ratios/trace-status hashes are `0056bf62…c5c59` / `632e087b…192c` / `0190a7e1…ad3e`; the separate 16/16 commit-bound model gate owns correctness. W3-B at clean `d7cdf66` remains correctness/safety-green; its component is **1.000293×**, strict **15/20 timing + 2/4 memory**, and first-use-positive. Corrected attempt 2 now adds one lifecycle-clean steady-state prewarm/lazy/vLLM trace: **144/144** requests per arm, identical 49-file inventories, three returns, FP4 sums **110.623/114.229/109.932 s**, and narrow-pair **70.333/70.986 s** prewarm/vLLM. The trace structurally closes W2's wide-tactic mismatch, but diagnostic normalized mean TPOT remains **0.9673x**. `b5c6e4f` therefore remains the denominator and 35B was not run. | Run the fresh exact W3-B c1-c32 campaign from a clean pushed SHA. If any 27B axis remains red, rank the next executed-kernel gap from the corrected trace and repair it before another exact grid. Gate optional W3-C persistence separately because pip-vLLM disables its file cache. Only then run 35B. Pool teardown remains separately required. |
...
| `KERNEL-GEMM-NVFP4-W4A4` small-M dispatch | **W3-B TRACE SUCCESS / FP4 STRUCTURAL PARITY; EXACT GRID PENDING** | W1/W2 and immutable W3-A evidence remain unchanged. Clean pushed `d7cdf66` W3-B is immutable correctness/safety-green and its component remains a strict fail/first-use win. Corrected trace attempt 2 holds one uncontended lock across shipping prewarm, lazy W3-A and pinned vLLM, excludes startup/autotune with separate warmups and retained interactive Nsight ranges, completes **432/432** retained requests, preserves all six cache inventories at 49 files/digest `b1789458…7523`, and passes three memory returns. FP4 sums are **110.623/114.229/109.932 s**: prewarm is **3.157%** lower than lazy and only **0.629%** above vLLM. Its 128x32x256 pair is **70.333 s / 218,434 calls** versus vLLM **70.986 s / 220,465 calls**, though prewarm resolves **65.800 s Stream-K + 4.533 s static** versus vLLM **43.259 + 27.728 s**. Lazy still executes **480 / 0.492 s** delay kernels and sampled alternate candidates inside the retained range; prewarm executes none. Diagnostic means are prewarm/lazy/vLLM **804.860/810.250/798.324 tok/s**; prewarm total is **1.0082x**, but normalized mean TPOT is **0.9673x**, so no profiled rate binds. Evidence tree/driver and three report hashes are `2aab1197…a137` / `af29681e…22fbf`, `a73d6032…1194a` / `9d74b6c8…37ea2` / `f89ffd4a…2d1b8`; GPU exits empty and lock is free. | Run the clean exact W3-B c1-c32 oracle ladder. If decode remains red, use the corrected trace's remaining dependent launch/traffic differences rather than revisiting tactic availability. W3-C persistence stays optional; do not run 35B. |
