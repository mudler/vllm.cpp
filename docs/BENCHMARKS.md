...
| Speculation depth (`ROAD-V1-D3-SPEC-K`, #81) | **PENDING, and now measurable.** MTP depth is configurable as of `SPEC-MTP-K-GT-1`, and correctness is gated on CPU at k=1..4 (identical greedy tokens to speculative-off, with per-depth acceptance as the witness). NO speed number exists at any k>1: the GPU was held for that whole flow, so nothing was measured on it | k=2..4 three-way greedy gate on 27B and 35B (our-ON == our-OFF == vLLM-ON), then the c1/c>1 A/B at matched k + the per-workload (prose vs code) acceptance-vs-depth curve any dynamic or adaptive depth policy needs |
