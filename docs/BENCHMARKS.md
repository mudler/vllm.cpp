...
| 35B FA2-prefill + fused-preamble lever | **INVESTIGATION COMPLETE / NOT LANDED; `benchmark_binding=false`, no speed credit** ([spec](../.agents/specs/qwen36-35b-fa2-prefill-oracle-2026-07-18.md), `CLAIM-35B-FA2-ORACLE`) | ORACLE DECISION: **DO NOT re-baseline** — vLLM 0.25.0 PRODUCTION (graphed) 35B greedy == the stored 0.24.0-eager oracle **16/16 identical** (`dgx:/tmp/prod_greedy_35b.log`); vLLM production runs FA2 bf16-q + the fused qk-norm-rope-gate preamble (`platforms/cuda.py:145-160`, `fa_utils.py:154-162`, `qwen3_next.py:318-357`, dispatch `enforce_eager`-independent), so the oracle is production-faithful. BONUS (old build behind main): `VT_FUSE_ATTN_PREAMBLE=1 VT_FA2_PREFILL=1` → 35B gate **315/315** both cases, nsys-CONFIRMED `flash_fwd_splitkv` ran; the 2026-07-09 divergence is STALE. Attention prefill kernel ~1.86× (WMMA 17.8 → FA2 9.55 µs/full-attn-layer, no tile-build) on the oracle prompt (not a representative-prefill TTFT number) | Enable-by-default is verified-ready but NOT taken: tighten the fused kernel (round normed q/k → output dtype before RoPE, bit-identical to unfused/vLLM `fused_qk_norm_rope.py:67`), then re-gate 315/315 with the FULL current-main default set (RMSNorm-fast combination-tie lesson) + 27B 235/235 + memcheck + a realistic-prefill TTFT A/B, then flip 35B `FuseAttnPreambleOn` |
