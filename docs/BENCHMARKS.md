...
axes passing. W2's 32 raw tactics now pass forced/reference/sanitizer probes;
the model investigation proves the remaining correctness prerequisite is
vLLM's merged gate/up operand with maximum logical-shard CT divisors and one
alpha. No W2 speed result exists yet, and 35B still waits for complete 27B
...
| `KERNEL-GEMM-NVFP4-W4A4` small-M dispatch | **W1 MEASURED — STRICT GATES FAIL; W2 RAW TACTICS GREEN, MODEL REPAIR ACTIVE** | W1 retains all prior CPU/TSan/capture/memcheck/model proof. Its same-binary exact/legacy AB/BA/AB completes **2,016/2,016** requests: total-throughput ratios c1-c32 are **1.0002/1.0012/1.0006/1.0093/0.9996/1.0072×**, with **10/16/18/18/5/19 of 20** axes and only **1/4** memory axes passing. Paired traces reduce FP4 kernel time **1.05%**. W2's 32/32 forced tactics, graph probes, sanitizer shapes and raw FlashInfer comparisons pass. Default swap tactics first diverge in layer-1 MLP; forced narrow ordinary IDs 1/3/7 recover 16/16 tokens, but nsys exactly matches vLLM's dominant kernels to swap IDs 4/6. Source inspection resolves the apparent contradiction: vLLM concatenates gate/up and takes max input/weight divisors before one quant/alpha/GEMM, while ours still uses two independently scaled GEMMs. This is diagnostic evidence, not a W2 performance result. | Implement and gate the spike-expanded merged gate/up resident and exact max-divisor/one-alpha semantics with a split fallback. Require 16/16 oracle tokens, then repeat sanitizer, component, trace and binding 27B campaigns. Do not run 35B yet. |
