...
| `ENG-ASYNC-SCHED` (W3) | **Host-side machinery + runner device-input half LANDED, CPU-gated (2026-07-16), `ACTIVE`; no GPU behavior change this commit** | Scheduler/engine half: `AsyncScheduler` output-placeholder accounting (1:1 with `async_scheduler.py:12-75`), depth-2 `step_with_batch_queue` (`core.py:519-632`), `async_scheduling` default-ON-when-compatible config resolution + `MaxConcurrentBatches` + `VT_ASYNC_SCHED` rollback (all RED-first CPU-tested). Runner **device-input** half now landed: `combine_sampled_and_draft_tokens` (`input_batch.py:304-406`) rebuilds each decode row's input id from the GPU-resident-analog `last_sampled_tokens` (per-req_state, threaded through the batch lifecycle) instead of the host `token_ids_cpu` read — placeholder/prefill-aware, idx_mapping indirection, host-side + outside CUDA-graph capture (capture-safe), behind `VT_ASYNC_RUNNER` / `set_async_input_combine`, **default OFF**. CPU-tested RED→GREEN: `tests/vllm/v1/worker/test_combine_tokens.cpp` (7 cases, RED verified by splicing the stale value → 5/7 fail), `last_sampled`/`prefill_len` lifecycle in `test_input_batch.cpp`, runner token-exactness async-ON≡sync + reads-`last_sampled`-over-stale-host in `test_runner.cpp`. Full CPU ctest **110/110**, tools 164/164, clean full CPU `-Werror` rebuild 0 warnings. Sync path byte-identical; `runner_supports_async` still FALSE (production synchronous), so no benchmark number moves. `benchmark_binding=false`, no speed credit | **Remaining DGX-gated:** the sampler-output half — non-blocking sampled-ID D2H on a copy stream + `vt::Backend` event/pinned primitives — plus flipping `runner_supports_async` ON by default; together they capture the measured ~3.25 ms/step GPU-idle. Then the DGX both-model-gate both-rollback-arm A/B (both models × `VT_ASYNC_SCHED` {unset, 0} = 235/235 + 16/16 token-exact; interleaved c16 W3-on vs W3-off same binary, ~+3 ms/step ≈ +15 tok/s target, with a TTFT-regression watch — `3812d8`'s TTFT 0.862× ON/OFF is the caution). Prior `3812d8` control freezes the c2 speed budget at neutral (total 1.002153×) |
