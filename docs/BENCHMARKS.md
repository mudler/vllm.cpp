...
| `ENG-ASYNC-SCHED` (W3) | **Host-side + runner device-input + sampler-OUTPUT half + the ENABLE-FLIP all LANDED, CPU-gated (2026-07-16), `ACTIVE`; no GPU behavior change this commit (production default resolves OFF)** | Scheduler/engine half: `AsyncScheduler` placeholder accounting, depth-2 `step_with_batch_queue`, config resolution + `VT_ASYNC_SCHED` rollback. Runner **device-input** half: `combine_sampled_and_draft_tokens` (`input_batch.py:304-406`) rebuilds each decode row's input id from `last_sampled_tokens`. Runner **sampler-OUTPUT** half now landed (1:1 with `async_utils.py:12-70` + `gpu_model_runner.py:242-332`): `vt::Backend` gains `AllocPinned`/`FreePinned` + `CreateEvent`/`RecordEvent`/`SynchronizeEvent`/`QueueWaitEvent` (CUDA `cudaHostAlloc`+`cudaEvent`+`cudaStreamWaitEvent`; CPU null-handle sync-degeneration); `AsyncGPUModelRunnerOutput` owns the device sampled-id snapshot and issues the non-blocking D2H on a dedicated **copy queue** (which first waits the main queue) + records a completion event — the **main queue is never synchronized**; `get_output()` (called at batch-consume in `step_with_batch_queue`, off the model's critical path) blocks only that event. `Sampler::forward(sampled_ids_out)` writes greedy ids device-resident (no host download); `sample_tokens_async` does the on-GPU post-update with the host token write-back deleted; the deferred `AsyncModelRunnerOutput` threads through `Executor` + `BatchQueueItem`. All behind `VT_ASYNC_RUNNER` / `set_async_input_combine`, **default OFF** (`sample_tokens_async` degenerates to byte-identical sync `sample_tokens`). CPU-tested RED→GREEN: `tests/vt/test_backend.cpp` (event/pinned contract), `tests/vllm/v1/worker/test_async_output.cpp` (materialize/flush/snapshot; RED = +1 splice → fail), `test_runner.cpp` (`sample_tokens_async` decode ≡ sync), depth-2 seam `test_engine_core_proc.cpp:479`. Full CPU ctest **111/111**, tools 164/164, clean full CPU `-Werror` rebuild 0 warnings. Sync path byte-identical; `runner_supports_async` resolves FALSE in production (no env), so no benchmark number moves. `benchmark_binding=false`, no speed credit. **ENABLE-FLIP now LANDED + CPU-gated (2026-07-16, same claim):** `LoadedEngine` reorders `runner_` before the scheduler and builds `AsyncScheduler` + `max_concurrent_batches=2` (threaded to `AsyncLLM`→`EngineCoreProc`→`step_with_batch_queue`) when `ResolveAsyncScheduling(runner_.runner_supports_async())` resolves ON, else the byte-identical `Scheduler`+depth-1, mirroring vLLM's "Asynchronous scheduling is enabled/disabled" log; the device combine/scatter kernel (`src/vt/cuda/cuda_combine_tokens.cu`, 1:1 `_combine_sampled_and_draft_tokens_kernel`) runs main-stream-ordered on the CUDA async path and DELETES `sample_tokens_async`'s pre-scatter `Synchronize` (CPU keeps the host loop). Construction resolution matrix CPU-tested RED→GREEN (`test_loaded_engine_dense.cpp`; RED = un-flipped engine → 3/3 ON-arm asserts fail). Full CPU ctest **111/111**, tools 164/164, clean `-Werror` rebuild 0 warnings. No-env production stays synchronous byte-identical | **FULL W3 DGX proof PENDING:** the both-model-gate both-rollback-arm A/B — both models × `VT_ASYNC_SCHED` {unset, 0} under `VT_ASYNC_RUNNER=1` = 235/235 + 16/16 token-exact; interleaved c16 W3-on vs W3-off same binary, ~+3 ms/step ≈ +15 tok/s target, with a TTFT-regression watch (`3812d8`'s TTFT 0.862× ON/OFF is the caution). Prior `3812d8` control freezes the c2 speed budget at neutral (total 1.002153×) |
