...
builds are restored after H1d/G4. The schema-v3 record hashes the exact external
...
boundary. The implementation now runs Nsight with
`--capture-range-end=repeat:4`; the diagnostic controller performs start → one
graph launch → stream synchronize → stop four times. Schema v3 requires four
unique, zero-return `cuProfilerStart` rows, each preceding its corresponding
launch and separated from the prior launch. Sampler/input work must execute
between ranges and every SQLite must contain zero eager kernel/memcpy/memset
rows. No severity is whitelisted and no existing evidence is reclassified.
...
that case passes in isolation. The repeated-range implementation also passes
Python/shell syntax, ShellCheck, focused tests **31/31** and the CUDA-off server
build. No accepted H1d SQLite/status, throughput,
