...
| DFlash2 | `Qwen3.8-27B` + `z-lab/Qwen3.8-27B-DFlash2`, k=7, GB10, oracle BEYOND-PIN on `TRITON_ATTN` (not vLLM's default here, #1456) | **Correctness gates READ**: 4/4 token-exact, 45/47 draft blocks identical, acceptance IDENTICAL per prompt (209 both engines). **No speed number** | `ACTIVE` |
