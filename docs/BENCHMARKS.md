...
| Settled: the HOST rebooted | (c1 landed) | (cell stands) | OBSERVED: `boot_id` moved `3fd9745a-...` -> `64c495a3-...` in a later `dgx:gpu0` job, so not a lost pod. DERIVED from `/proc/uptime`: the boot lands ~10:42Z, inside the c8 window. Causation is NOT traced |
