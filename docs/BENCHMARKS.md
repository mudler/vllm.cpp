...
alpha and accepts its spike as `READY`, with no implementation or new number).
...
The next bounded checkpoint is **W3-F / PENDING — READY, NOT IMPLEMENTED**.
...
[W3-F spike](../.agents/specs/nvfp4-device-alpha.md) requires a model-owned
one-element device tensor, `VT_FP4_DEVICE_ALPHA=0` same-binary fallback,
ported upstream tests, sanitizer/model/node-trace proof, and a frozen-plan
c2/c16 every-axis A/B. **No runtime file, accepted ratio, binding denominator,
exact-grid authorization or 35B performance state changes in this spike.**
...
Next: keep W3-C's now-complete frozen-plan control and implement the accepted
W3-F device-alpha spike without stacking it into W3-E. First run the ported
operator/capture tests and local build; after a pushed immutable checkpoint,
run CUDA safety, both correctness arms, node trace and the frozen-plan c2/c16
component. The conditional exact grid remains
...
| `KERNEL-GEMM-NVFP4-W4A4` small-M dispatch | **ACTIVE — W3-F SPIKED/READY; NO RUNTIME RESULT** | W3-C reproduction control is complete and W3-E remains failed at **39/40 timing + 1/8 memory**. The next executed mismatch is 296,575 steady graph `SetScalar` launches / 1,425 forwards (**208.123/forward**) locally versus zero in vLLM. Whole-chain source confirms vLLM/FlashInfer pass one model-owned device F32 alpha pointer. W3-F inventories exact API/residency/fallback/tests/safety/trace/A-B gates; `3f256ab` remains 55/124. | Implement device alpha with `VT_FP4_DEVICE_ALPHA=0` fallback, port the upstream tensor-alpha tests, then run pushed CUDA/model/node-trace and frozen-plan c2/c16 gates. Exact grid/35B performance remain held. |
