...
denominator. W3-B's shared-loader pre-serve implementation is now staging-
green: CPU focused tests pass **3/3**; disposable CUDA exact/legacy processes
each pass **14/14 and 26,819/26,819**; the native 27B gate passes **235/235 +
16/16**; and both model and HTTP startup materialize **80/80** profiles (16
buckets × five real shapes) with zero lazy misses. The warmup completion is log
line 3 and HTTP listening line 6. This is explicitly non-binding until a clean
pushed-SHA build, sanitizer, repeated selection/component, trace and exact
oracle campaign complete. The
...
| `KERNEL-GEMM-NVFP4-W4A4` small-M dispatch | **W3-B IMPLEMENTED / STAGING GREEN — IMMUTABLE PERFORMANCE PENDING; W2 BINDING BELOW FLOOR** | W1/W2 and immutable W3-A evidence remain unchanged. W3-B adds an actual-W4A4-gated maximum-token request in the shared `LoadedEngine`, exact FlashInfer profile enumeration, process-local completion statistics, capture-safe ready hits and diagnostic-only later misses; `VT_FP4_PRE_SERVE_WARMUP=0` restores W3-A. CPU focused tests pass **3/3**, including the true-W4A4/BF16 runtime-capability split. Disposable CUDA exact/legacy tests each pass **14/14 / 26,819/26,819**. The 27B stage passes **235/235 + 16/16**, tunes **80/80** profiles into 80 cache entries across five N/K shapes, and reports no lazy miss. The HTTP stage answers `/health` and `/v1/models`, with warmup completion at line 3 before listening at line 6 and zero lazy misses. Evidence root is `~/work/vllm.cpp-nvfp4-small-m/precommit-w3b-195b475/gates`; model/server/models hashes are `96afc6ed…de401` / `2264a306…7e9e` / `80e10055…8315`. This disposable stage is `PENDING`, not a speed result; W3-A's failed +0.816% component and clean `b5c6e4f` denominator remain binding. | Clean-build the pushed W3-B SHA, run exact/legacy focused tests, memcheck, native 27B and server ordering, then repeat fresh-process selection/component AB/BA/AB and paired trace. Only after that rerun the exact vLLM 27B ladder. W3-C persistence stays optional; do not run 35B. |
...
### Reproduce the pending W3-B immutable preflight

Run this from the clean pushed W3-B checkpoint on `dgx.casa`; do not reuse the
disposable staging build. The model log must contain exactly one
