...
Next: repeat the exact grid and re-rank the now-structural-parity residual. Every
...
| `SERVE-GATE-ONLINE` | **FAILED / GATING — `9cc7191` v0.25.0 27B: 54/124 AXES PASS; W3-D STRUCTURAL TRACE PASS** | Immutable `9cc7191` completed the correctness gate, all 36 cache-off groups, 2,016 requests and six returns under one uncontended lock. All 124 axes bind; 70 fail. Total-throughput ratios c1→c32 are **0.9901/0.9491/0.9633/0.9770/1.0288/1.0467×**. Clean `3f256ab` packed/split correctness passes 16/16; c16 means are **812.231/808.150 = 1.005049×**, but only **14/20 timing + 2/4 memory** axes pass. Its post-pack trace confirms **208.192 vs vLLM 208 FP4 GEMMs/forward**. This does not replace the vLLM grid; 35B did not run. | Repeat the exact grid and re-rank until every axis passes; only then run 35B. |
