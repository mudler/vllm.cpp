...
| `VT_CPU_Q8_DOT` | `auto` | Selects the CPU Q8_0 x Q8_0 inner dot: `portable`, compiler-generated `sdot`, or `a76-asm`. `auto` selects the gated assembly schedule only on Cortex-A76 with DotProd and otherwise stays portable. Unsupported ISA selections safely fall back; `vllm-cpu-kernel-bench --variant` fails loudly instead |
