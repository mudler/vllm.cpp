...
| Qwen3.8-27B quantized arms (Q4_K_M GGUF, its `clip` mmproj, the `unsloth` "NVFP4") | Projector LOADS, nothing runs it; both quantized arms spec'd only | BF16 is gated (#915). `--mmproj` (C ABI v22) loads a second `clip` GGUF into the Qwen3-VL tower; no forward reads it. "NVFP4" is `mixed-precision`, zero `input_scale`. [spec](../.agents/specs/qwen38-27b-quant-arms.md) |
