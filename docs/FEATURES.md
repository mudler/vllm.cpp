...
| Block-wise (fine-grained 128x128) FP8, the `weight_scale_inv` layout | ◐ RUNS on CPU (#1189 M4); the `sm_12xa` CUTLASS arm (M5) is BUILD-VERIFIED ONLY (registered, NEVER EXECUTED, no token gate) ([spec](../.agents/specs/vt-matmul-fp8-block-cuda.md)) | ✅ | ✅ | ☐ |
