...
| Routed-expert weight streaming from disk | ◐ default OFF (`VT_MOE_EXPERT_STREAM=1`), CPU keep-quant towers only; bounded slot cache, LFU + LRU tiebreak; refuses an unfittable slice by name. c1-c4 capacity, not throughput. An `[expert-stream]` line at exit (#1091) | ☐ blanket `cpu_offload_gb`, not expert-granular | ☐ | ◐ mmap only |
