...
| GGUF gather tables kept QUANTIZED (one row dequantized per gathered token) | ✅ CPU. `vt::Embedding` takes a block-quantized table, decoding one row per id — a port of llama.cpp's `ggml_compute_forward_get_rows_q`. Without it a 51.2 G-parameter n-gram table expands from 28.8 GB of IQ4_NL to 102.4 GB of bf16, which no device here has ([#1989](https://github.com/mudler/vllm.cpp/issues/1989)). **The CUDA arm is OWED**: `EmbeddingKernelCuda` still refuses a block table, so on CUDA such a table keeps its expand-bf16 residency | ☐ | ☐ | ✅ `get_rows` for ~20 types, CPU and CUDA |
