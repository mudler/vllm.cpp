...
| Speech / audio GENERATION (TTS, vLLM-Omni lane) | ◐ IndexTTS-2.5 only: the ACOUSTIC CHAIN renders on real weights, codes to a WAV via the regulator, CFM loop and BigVGAN; text reaches mel codes. No route (#634) | ✅ (vllm-omni: MOSS-TTS, Qwen3-TTS, Higgs Audio v3, Voxtral TTS, IndexTTS-2.5) | not assessed | not assessed |
