...
| LTX-2.5 DFR temporal rounds (`temporal_upsample_rounds`) | LTX-2.5 (21.00B video+audio) | 7 `test_ltx2_video` cases through `LoadVideoEngine`/`Generate`; deleting the upsampler call site reds the reachability counter while both frame counts stay green. The four per-tile guarantees (seed, 0.95 anchor, eta 0.5, first-wins dedupe) are each red-first by mutation | `--temporal-upsample-rounds` + `--temporal-upsampler`. The temporal x2 upsampler's FIRST caller. `(n-1)*2**r+1` frames; conditioning fps capped at 60, playback not. Tiles clamp to segments and the unclamped arm is ungated (#1493). Fixture-gated |
