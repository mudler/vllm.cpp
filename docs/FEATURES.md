...
| MTP speculation DEPTH (`num_speculative_tokens` > 1) | Qwen3.5/3.6 `mtp.*` heads | the configured depth reaches the verify path and the greedy tokens are unmoved: `test_mtp_depth` 5/5 (47 assertions), k=1..4 through the loader, each arm witnessed by the DRAFT DECODE FORWARDS the propose actually ran (`forwards == propose_calls * (k-1)`). The earlier per-depth-acceptance witness was WITHDRAWN: a fresh review passed it with a one-forward propose that padded all k columns | Default stays k=1 (the checkpoints' `n_predict`). NO speed claim at k>1. The CPU gate proves k drafts are PROPOSED and VERIFIED, never ACCEPTED: acceptance is zero at every depth on the synthetic gate model, so the accept path at depth stays unexercised until the owed DGX three-way at k=2..4 runs, alongside the matched-k throughput A/B ([#81](https://github.com/mudler/vllm.cpp/issues/81)) |
