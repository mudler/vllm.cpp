...
| MiniMax-Music3 (`MiniMaxMusic3ForConditionalGeneration`, diffusers lane) | MiniMax-Music3 (8.6B Qwen3 LLM + 0.646B 8-codebook RVQ decoder + 2.4B fp32 DiT + 0.054B DAC Flow-VAE) | `SPIKE`. Scoped only, nothing implemented. Geometry measured from each `config.json` and the safetensors headers; port target is the diffusers arm, ~28.5 GB resident | Not measured. The denominator will be SGLang-Omni in its production configuration (both CUDA graphs, compiled DIT and DAV, batched seeded sampling) |
