...
| Hybrid CPU/GPU expert placement (routed-expert COMPUTE on the CPU, attention and dense on the GPU) | ☐ `ENG-HYBRID-PLACEMENT` is `READY`: spec committed ([hybrid-placement.md](../.agents/specs/hybrid-placement.md)), no product code, no per-tensor-group device assignment anywhere in the engine. The INVERSE of the row above, not a second spelling of it: streaming moves weights toward the compute, placement moves compute toward the weights ([#149](https://github.com/mudler/vllm.cpp/issues/149), [#2012](https://github.com/mudler/vllm.cpp/issues/2012)) | ☐ the CPU MoE kernels exist (`fused_moe/experts/cpu_moe.py`, `cpu_fused_moe.py:398,430`) and every selection site gates on `current_platform.is_cpu()` (`oracle/fp8.py:129`, `oracle/mxfp4.py:533`, `oracle/unquantized.py:97,202`), so they are reachable only when the WHOLE model runs on the CPU platform | ☐ same shape as vLLM: module-level `_is_cpu = is_cpu()` platform flags (`layers/moe/topk.py:129`, `fused_moe_triton/layer.py:83` @ `f63458b5`), no per-layer device override | ✅ `-cmoe` / `-ncmoe`, applied at load as tensor-name pattern → buffer type, so compute follows the tensor's buffer (`common/arg.cpp:2451-2478`, `common/common.h:1046-1054` @ `237ad9b96`) |
