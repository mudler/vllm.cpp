...
| LTX-2.5 DFR temporal rounds (`temporal_upsample_rounds`) | LTX-2.5 (21.00B video+audio) | `test_ltx2_video` 6 cases via `LoadVideoEngine`/`Generate`; deleting the upsampler call site reds the reachability counter while both frame counts STAY green, which is why the counter exists | `--temporal-upsample-rounds` + `--temporal-upsampler`. DRIVES the temporal x2 upsampler, its FIRST caller. `(n-1)*2**r+1` frames; cond fps capped 60, playback not. Tiles CLAMP to segments. Fixture-gated |
