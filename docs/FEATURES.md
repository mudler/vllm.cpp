...
| MTP speculation DEPTH (`num_speculative_tokens` > 1) | Qwen3.5/3.6 `mtp.*` heads | the configured depth reaches the verify path and the greedy tokens are unmoved: `test_mtp_depth` 5/5 (34 assertions), k=1..4 through the loader, each arm witnessed by per-depth acceptance | Default stays k=1 (the checkpoints' `n_predict`). NO speed claim at k>1: the DGX three-way at k=2..4 and the matched-k throughput A/B are owed ([#81](https://github.com/mudler/vllm.cpp/issues/81)) |
