...
| MTP speculation DEPTH (`num_speculative_tokens` > 1) | Qwen3.5/3.6 `mtp.*` heads | k=1..4 through the loader, greedy tokens unmoved, each arm witnessed by the draft decode forwards the propose RAN (`forwards == calls * (k-1)`): `test_mtp_depth` 5/5, 47 assertions | Default stays k=1. NO speed claim at k>1. Drafts are proposed and verified, never ACCEPTED: acceptance is 0 at every depth on the CPU model, so the accept path at depth awaits the owed DGX gate (#81) |
