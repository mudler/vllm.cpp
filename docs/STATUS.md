...
| LTX-2.5 video VAE decode, device arm (`KERNEL-CONV3D`) | **`ACTIVE` (#1007).** The decode's convolution dispatches through the new `vt::Conv3d` op on the queue the engine resolved at load, as upstream places it at build time. CPU arm byte-identical to the old host loop | **NO GPU has run the CUDA arm** (#1452), so nothing here is measured on an accelerator and no speed number is published. Only the convolution is on the device; the other decode stages are still host loops (#1451) |
