...
| Speculative decoding | Partial | DFlash2 drafts now DRAFT end to end from BOTH containers -- safetensors and GGUF, in the published bf16, Q8_0 and mixed Q4_K_M ENCODINGS, though no published artifact has been LOADED yet -- with the convolution, candidate selector and path walk all running; greedy only, no throughput number yet, and a GGUF drafter is dequantized to bf16 at load (#1314) A preference-ordered DRAFTER CHAIN has its FIELD but no behaviour (`SPEC-DRAFTER-CHAIN`, #1522): `--speculative-config` now accepts `vllm_cpp.drafter_chain`, an additive list of speculator entries that is parsed, validated and refused by name, and a well-formed chain is refused at the loader before any weight I/O because nothing resolves one yet. With the field absent every existing speculative path is unchanged. |
