...
| LTX-2.5 DFR pipeline + temporal rounds (`LTX25-DFR-PIPELINE`, `LTX25-DFR-ROUNDS`) | **`DONE` (#986).** The DFR base, its generated keyframe slots and the `temporal_upsample_rounds` loop are landed and gated; the rounds are the temporal x2 latent upsampler's FIRST production caller, which retires the `UNDRIVEN` state `docs/FEATURES.md` carried for it since #644 | **REFUSED in practice on real weights**: DFR's base needs a `keyframe_slot_sft` transformer and #1137 established by authenticated reads at the pinned revision that none is published, so every claim here is a reduced-dimension fixture claim and NO real-weight DFR result exists. NO measurement of any kind: no lease, no oracle run, no throughput, latency or memory number, so `docs/BENCHMARKS.md` carries no row. Unclamped `2**round` tiling ungated (#1493); stage-2 detailing IC-LoRA refused (#975) |
