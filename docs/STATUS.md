...
| Speculative decoding | Partial | DFlash2 drafts now DRAFT end to end from BOTH containers -- safetensors and GGUF, including the published Q8_0 and Q4_K_M arms -- with the convolution, candidate selector and path walk all running; greedy only, no throughput number yet, and a GGUF drafter is dequantized to bf16 at load (#1314) A preference-ordered DRAFTER CHAIN is specified and unimplemented (`SPEC-DRAFTER-CHAIN`, #1522): an additive `--speculative-config` field, inert when absent, one winner per sequence rather than an ensemble. |
