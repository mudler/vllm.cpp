...
| DeepSeek-V4-Flash-Vision GGUF, the RUNNABLE single-device vehicle | `DeepSeek-V4-Flash-Vision-Exp-UD-IQ1_S-0000{1..3}-of-00003.gguf` plus `mmproj-BF16.gguf` | 82,438,622,112 bytes total (76.78 GiB) across three shards (5,305,248 + 49,991,832,128 + 32,441,484,736); the `clip` projector is a further 934,462,656 bytes (0.870 GiB), so 77.65 GiB resident before KV cache and activations | `unsloth/DeepSeek-V4-Flash-Vision-Exp-GGUF` @ `b977d3c0ea2da58dbc12ddae8fb8951a7b3854d0`, path `UD-IQ1_S` | Shards `be862fb3ecdeb99a9a47fabd091b9c7bd32d0de89c9a85589cd007b822bb6305`, `c21604991c40674ac1612f16dcedf84b857bb5a2bace00b47a7ab7e5f5e3296e`, `8326a8a98fb224a16f8e83e6236fc346222bc9131f6495644c5c988b8a6101f4`; projector `e4914c6c8063d01f4cbb6dafdf2f959c7d06fbe8ad11ae5b11ad032edd42642e`. **These are the digests the Hugging Face `paths-info` API publishes for that revision, NOT bytes this tree has hashed.** No `sha256sum` has been run over the staged copy, so a re-quantization in place under an unchanged name would not be caught here | **The weights LOAD, and how far the engine then gets DEPENDS ON THE BUILD.** `deepseek-v4-gen --load-only --gpu` on `thor:gpu0` reports `LOADED: layers=43 experts=256 vocab=129280 has_gguf=1`, RSS 73.0 GiB, and the example path GENERATES greedily (` Paris. The capital`, ids `11111 16 455 6102`, peak 81.19 GiB). On `dgx:gpu0` the PRODUCTION entry `vllm-cli` completes every LOAD stage on both builds: file open, header parse, tensor mapping, KV-cache config (`auto-fit max_model_len` 1048576 -> 65536) and the full 82 GB weight load (1026 s on the CUDA-OFF leg). **Engine construction completes only on a CUDA-OFF build**; a CUDA `sm_121a` build fails there, so the two refusals below are at DIFFERENT stages | **REFUSED, zero tokens, and the stage depends on the build — the `:4658` answer below is the CUDA-OFF one and is SUPERSEDED for a CUDA build.** (1) **CUDA-OFF builds refuse at the FIRST FORWARD**: `vllm-cli` exits 1 with `OUT_BYTES=0` at `src/vllm/model_executor/models/deepseek_v4.cpp:4658`, `vt: DeepseekV4 DEVICE forward (W7-device) not implemented`. Production `ModelRegistry::Forward` routes to `DeepseekV4Model::ForwardDevice` (`deepseek_v4_registry.cpp:253`), gated on `V4DeviceKernelsAvailable()`, which requires `kDeepseekV4{Mhc,Dsa,Compressor,Moe}` registered for **`kCUDA`** — so a CUDA-OFF build can never emit a token regardless of `--device cpu`. (2) **A CUDA `sm_121a` build gets FURTHER and refuses EARLIER in the engine**: measured 2026-09-14 (rc job `fc593c9f-f550-4f1a-9adf-a8c734f48822`, `dgx:gpu0`, GB10, Release `-O3 -DNDEBUG`), `V4DeviceKernelsAvailable()` is TRUE, the weights load, and `vllm-cli` then exits with `OUT_BYTES=0` at **ENGINE CONSTRUCTION**: `vllm_engine_load: Block size must be a multiple of 16.` `MakeDeepseekV4KVCache` publishes seven KV-cache groups at `{256,256,256,64,4,4,8}` (`deepseek_v4_registry.cpp:546`-`:556`) and the 4/4/8 DSA groups fail the `{16}` every attention backend here declares (`include/vllm/v1/attention/backend.h:444`, `:546`, `:604`); the first forward is never reached. **No production token gate, no speed figure, and no image answer exists for this arm on either build.** `.agents/specs/deepseek-v4-flash-vision.md` `## Owed` carries the mechanism, the upstream reconciliation this tree lacks, and the residuals |
