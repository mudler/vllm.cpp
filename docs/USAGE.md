...
**A `glm5next` file LOADS and FORWARDS, on the CPU device, one sequence at a
time.** W1 ([#2067](https://github.com/mudler/vllm.cpp/issues/2067)) gave
...
the forward, so `ModelRegistry::Forward` now runs the model: the tower stays
