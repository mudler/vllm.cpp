...
safetensors `DFlash2DraftModel` draft is admitted as far as its CANDIDATE
SELECTOR and no further: it loads, it runs the grouped dynamic depthwise
convolution around every attention and every MLP sublayer of every draft layer,
it takes the target head's top-K over the draft's hidden states, and it scores
the selector's transition lattice over its two codebooks. It is then refused BY
