#!/bin/bash
# MODEL-MM-QWEN4-EXP -- WAVE 6: the F1 red-first only. Wave 5 VOIDED it -- the poison orphaned `eps` and -Wextra -Werror refused the build. This one keeps `eps` live.
...
set -u
HEAD_SHA=7b737f882a1bd9047e891cdf2cbb6baa5bd28a8d
...
ROOT=/tmp/vllmcpp-hcnorm-w6-$$
OUT=/workspace/q4exp-hcnorm/w6-$(date -u +%Y%m%dT%H%M%SZ)
...
bld(){ ninja -C "$ROOT/b" -j 4 test_backend_cross_device test_qwen4_exp_rocm_reductions > /tmp/w6b.log 2>&1; }
...
[ -x "$BINR" ] || { echo FATAL no rocm reductions binary; tail -20 /tmp/w6b.log; exit 93; }
...
if ! bld; then echo "F1_1 BUILD FAILED -- VOID"; tail -20 /tmp/w6b.log; else
...
if ! bld; then echo "F1_2 BUILD FAILED -- VOID"; tail -20 /tmp/w6b.log; else
...
echo "=====W6_TAR_B64_BEGIN====="; tar czf - -C "$OUT" . | base64 -w 200; echo "=====W6_TAR_B64_END====="
say "DONE $OUT"
