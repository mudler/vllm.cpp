#!/bin/bash
# MODEL-MM-QWEN4-EXP -- WAVE 8: the F1 red-first AND the declared gate, re-run on the POST-MERGE head 0515493e8 (origin/main 7588c1e6f). Wave 6 measured 7b737f882, which is now a stale base. Wave 5 VOIDED it -- the poison orphaned `eps` and -Wextra -Werror refused the build. This one keeps `eps` live.
...
set -u
HEAD_SHA=f35457cc0ac964f2294e8908509d90fdc873736f
...
ROOT=/tmp/vllmcpp-hcnorm-w8-$$
OUT=/workspace/q4exp-hcnorm/w8-$(date -u +%Y%m%dT%H%M%SZ)
...
bld(){ ninja -C "$ROOT/b" -j 4 test_backend_cross_device test_qwen4_exp_rocm_reductions > /tmp/w8b.log 2>&1; }
...
[ -x "$BINR" ] || { echo FATAL no rocm reductions binary; tail -20 /tmp/w8b.log; exit 93; }
...
if ! bld; then echo "F1_1 BUILD FAILED -- VOID"; tail -20 /tmp/w8b.log; else
...
if ! bld; then echo "F1_2 BUILD FAILED -- VOID"; tail -20 /tmp/w8b.log; else
...
echo "=====W8_TAR_B64_BEGIN====="; tar czf - -C "$OUT" . | base64 -w 200; echo "=====W8_TAR_B64_END====="
say "DONE $OUT"
