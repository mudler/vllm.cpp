...
MATRICES = {
    ...
    # row EXISTS, never to make a transition pass.
    # 370 since 2026-08-13: +8 rows for the architectures behind official
    # `vllm-project/recipes` models that had no row at all (#609, #610). Two are
    # pin-lag — `Qwen3_5MoeForCausalLM` and `BailingMoeV3ForCausalLM` are
    # registered on vLLM `main` and absent only at `555967922`. Six are
    # out-of-repo: `MossTTSDelayModel`, `MossTTSRealtime`,
    # `Qwen3TTSForConditionalGeneration` and
    ...
    # of an invented anchor. None of the eight touches the at-the-pin model
    # inventory below (324/373/356/310/261 is unchanged), because like the
    # MuseGlimmer, KimiK3 and MiniMaxH3DiT rows they carry no pinned-registry
    # target. Bumped because eight new rows EXIST, never to make a transition
    # pass.
