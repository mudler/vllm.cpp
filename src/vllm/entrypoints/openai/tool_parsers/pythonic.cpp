...
// (shared pythonic helpers: vllm/tool_parsers/utils.py @ e24d1b24 -
//  handle_single_tool, get_parameter_value, make_valid_python,
//  compute_tool_delta, find_common_prefix).
...
struct ParsedCall {
  std::string name;
  ojson arguments;  // always an object
};
...
// (get_parameter_value / handle_single_tool) - both bubble up to the caller's
// try/except and yield the plain-content fallback.
struct ParseError {};
...
class PythonicParser {
 public:
  ...
  std::vector<ParsedCall> ParseCallList() {
    SkipWs();
    Expect('[');
    SkipWs();
...
// Parse the whole `clean_text` as a pythonic list-of-calls. Returns nullopt on
// any failure (the caller maps that to the plain-content fallback).
std::optional<std::vector<ParsedCall>> ParsePythonicList(
    const std::string& clean_text) {
  try {
    PythonicParser p(clean_text);
...
std::string Rstrip(const std::string& s) {
  std::size_t e = s.size();
  while (e > 0 && IsSpaceCh(s[e - 1])) --e;
  return s.substr(0, e);
}

// tool_parsers/utils.py:405 (make_valid_python). Close all open
// brackets/quotes to make a partial pythonic expression parseable. Returns
// (completed_text, added_suffix) or nullopt when the text is too incomplete to
// complete meaningfully (mid parameter-name / mid dict-key / etc.). The final
// upstream ast.parse + ast.Set walk is reproduced by re-parsing the candidate
// with our strict parser (which itself rejects set literals).
std::optional<std::pair<std::string, std::string>> MakeValidPython(
  ...
  const std::string candidate = text + added_text;
  // Upstream: ast.parse(candidate) must succeed, have a body, and contain no
  // ast.Set. Our strict parser both parses AND rejects set literals, so a
  // successful ParsePythonicList is the faithful equivalent.
  if (!ParsePythonicList(candidate).has_value()) return std::nullopt;
  return std::make_pair(candidate, added_text);
}

// tool_parsers/utils.py:658 (compute_tool_delta). Returns the incremental
// DeltaToolCall, or nullopt when nothing new. Throws ParseError on the
// withheld-suffix mismatch (upstream raises ValueError -> caught -> None).
std::optional<DeltaToolCall> ComputeToolDelta(const std::string& previously_sent,
  ...
  if (!withheld_suffix.empty()) {
    if (!EndsWith(new_args, withheld_suffix)) throw ParseError{};
...
ExtractedToolCallInformation PythonicToolParser::ExtractClean(
  ...
  const std::optional<std::vector<ParsedCall>> calls =
      ParsePythonicList(clean_text);
  if (!calls.has_value()) {
...
std::optional<DeltaMessage> PythonicToolParser::StreamClean(
  ...
  try {
    ...
    for (std::size_t index = 0; index < calls->size(); ++index) {
      ...
      const ParsedCall& call = (*calls)[index];
      const std::string call_args = call.arguments.dump();
      const std::string call_id = make_tool_call_id();

      const std::optional<DeltaToolCall> delta =
          ComputeToolDelta(streamed_args_for_tool[index], call_id, call.name,
                           call_args, static_cast<int>(index), withheld_suffix);
      if (delta.has_value()) {
