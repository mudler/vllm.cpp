// Weight loader for `Gemma2ForCausalLM` (gemma-2-2b-it, BF16) — sweep W4. Loads
// the checkpoint safetensors into Gemma2Weights (gemma2.h) via the shared
// dense_weight_loaders.h helpers.
//
// Grounding: vllm/model_executor/models/gemma2.py @ e24d1b24 —
//   packed_modules_mapping (:325-328): qkv_proj<-[q,k,v]_proj,
//   gate_up_proj<-[gate,up]_proj. tie_word_embeddings (:335,338-339): asserted
//   true; lm_head aliases embed_tokens, the checkpoint's lm_head.weight skipped.
//
// Name map (gemma-2-2b-it, flat `model.` prefix, no multimodal wrapper). Same as
// Gemma-3 MINUS the per-head self_attn.{q,k}_norm.weight (Gemma-2 has no QK-norm):
...
Gemma2LayerWeights LoadGemma2Layer(const TensorResolver& get, int64_t layer) {
  const std::string base = "model.layers." + std::to_string(layer) + ".";
  const std::string sa = base + "self_attn.";
  const std::string mlp = base + "mlp.";

  Gemma2LayerWeights w;
...
Gemma2Weights LoadGemma2ForCausalLMWeights(
  ...
  const TensorResolver get =
    ...
    VT_CHECK(it != where.end(), "gemma2: tensor not found: " + name);
    return it->second->Get(name);
  };

  VT_CHECK(config.num_hidden_layers > 0,
           "gemma2: num_hidden_layers must be positive");
  ...
  for (int64_t l = 0; l < config.num_hidden_layers; ++l)
    w.layers.push_back(LoadGemma2Layer(get, l));
  return w;
}
