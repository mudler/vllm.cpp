...
//    seq_lens, block_table = block_table_tensor). The cache READ is the NHD
...
// context = seq_lens[r] - query_len_r. Keys 0..p (causal) are gathered from the
// paged cache: position j lives in block_table[r, j/block_size], offset
// j%block_size, at [block, offset, kv_head, :] — read via cache strides.
void PagedAttentionKernel(Queue&, Tensor& out, const Tensor& query, const Tensor& k_cache,
  ...
  for (int64_t r = 0; r < num_reqs; ++r) {
    ...
    for (int64_t local = 0; local < query_len; ++local) {
      ...
      const int64_t jmax = args.causal ? p : seqlen - 1;
      if (jmax < 0) continue;
      probs.assign(static_cast<size_t>(jmax + 1), 0.0f);
      for (int64_t h = 0; h < hq; ++h) {
        ...
        const float inv = 1.0f / denom;  // denom >= 1 (the j==p term is exp(0)=1)
