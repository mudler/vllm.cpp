// vllm.cpp — torch-free lift of vLLM's gptq_marlin_repack_kernel
// (csrc/libtorch_stable/quantization/marlin/gptq_marlin_repack.cu @ e24d1b24).
//
// This header exposes ONLY the (num_bits=4, has_perm=false, is_a_8bit=false)
// instantiation — the exact config the NVFP4 W4A16 MoE path needs — as a plain
// device kernel + host launcher, with the torch::stable wrapper stripped. The
// kernel body is a VERBATIM copy of vLLM's; only the launcher (torch::Tensor →
// raw pointers) is rewritten. Depends on the already-vendored marlin.cuh for
// tile constants / cp_async helpers.
#pragma once
...
// VERBATIM from gptq_marlin_repack.cu:14-268 (only the surrounding torch wrapper
// is removed). num_bits=4, has_perm=false, is_a_8bit=false is all we instantiate.
...
#pragma unroll
    ...
    constexpr int tile_size_local =
        target_tile_k_size * target_tile_n_size / pack_factor;
    int out_offset = (k_tile_id * n_tiles + n_tile_id) * tile_size_local;
