...
// snapshot/restore API: the warmup's scatter commit IS the geometry the
// capture's first read serves, so restoring pre-warmup shadows only
// discarded the warmup's state update — the frozen GDN recurrence.
