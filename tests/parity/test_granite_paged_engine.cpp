// vllm.cpp original (Granite-3 W4 — THE SACRED correctness gate for the first
// IBM Granite model); no upstream mirror (upstream's Granite coverage is the
// generic text-generation entry — spec §"Tests to port").
//
// THE PAGED-ENGINE GraniteForCausalLM (granite-3.3-2b-instruct, bf16) GREEDY
...
// --model ibm-granite/granite-3.3-2b-instruct). STRICT where vLLM is deterministic;
...
//
// Goldens (tests/parity/goldens/granite_greedy_2b/, captured on dgx):
//   greedy_ids.npy / greedy_dist.npy / our_ids.npy / neartie_gap_mnats.npy.
// Checkpoint-GATED + dgx-only: resolves ~/.cache/huggingface/hub/
// models--ibm-granite--granite-3.3-2b-instruct. On CPU/CI it emits a loud SKIP.
...
void RunGate(const std::string& repo_dir, const std::string& golden_subdir,
  ...
  if (!have_gap) {
    MESSAGE(label << " gap goldens absent; skipping — run under VT_DUMP_IDS=1 "
            "then glm4-neartie-gap.py --model ibm-granite/granite-3.3-2b-instruct");
    return;
  }
...
// granite-3.3-2b-instruct (dense, Llama + 4 scalar multipliers) — the first IBM
// Granite SACRED gate. Proves the four scalars + custom attention scale are applied.
TEST_CASE("granite-3.3-2b dense paged-engine greedy correctness gate (dgx-only, SACRED)") {
  RunGate("models--ibm-granite--granite-3.3-2b-instruct", "granite_greedy_2b",
          "granite-3.3-2b");
}
