// vllm.cpp original (cross-family additive-model W4 — THE SACRED correctness
// gate); no upstream mirror.
//
// THE PAGED-ENGINE Llama DENSE GREEDY CORRECTNESS GATE. Drives the standard prompt
...
// the pinned vLLM 0.25.0 oracle for Llama-3.2-1B (LlamaForCausalLM: GQA 32/8,
// head_dim 64, llama3 rope-scaling, tied lm_head).
//
// METHODOLOGY (identical to the Qwen3-dense gate, [[near-tie-distributional-gate]]).
...
//   * at a bf16 near-tie (vLLM's one-shot prefill argmax vs its incremental decode
//     disagree, and two bf16 decoders resolve it either way): our token must be
//     within kNearTieMnats of vLLM's OWN teacher-forced argmax GIVEN OUR PREFIX
//     (gap captured by scripts/qwen3-neartie-gap.py). Strict where the token IS
//     vLLM's argmax; near-tie-robust ONLY where vLLM itself cannot separate the
//     tokens. A token outside vLLM's near-tie band is a REAL forward divergence
//     the gate FAILS on (the first divergent position + gap is reported).
//
// Goldens (committed under tests/parity/goldens/llama_greedy_1b/, captured on dgx):
...
//
// Checkpoint-GATED + dgx-only: resolves models--unsloth--Llama-3.2-1B under
...
void RunGate(const std::string& repo_dir, const std::string& golden_subdir,
  ...
  if (!fs::exists(gdir / "greedy_ids.npy")) {
    MESSAGE(label << " greedy golden absent; skipping — capture on dgx: "
            "qwen3-oracle-capture.py --per-prompt --model unsloth/Llama-3.2-1B "
...
// Llama-3.2-1B (dense) — the cross-family additive-model SACRED gate.
TEST_CASE("llama-3.2-1B dense paged-engine greedy near-tie correctness gate (dgx-only, SACRED)") {
  RunGate("models--unsloth--Llama-3.2-1B", "llama_greedy_1b", "llama-3.2-1B");
}
