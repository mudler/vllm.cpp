// vllm.cpp original (OLMo-2 W4 — THE SACRED correctness gate for the first
// OLMo-family model); no upstream mirror (upstream's OLMo-2 coverage is the generic
// text-generation entry — spike §"Tests to port").
//
// THE PAGED-ENGINE Olmo2ForCausalLM (OLMo-2-0425-1B, bf16) GREEDY CORRECTNESS GATE.
...
// --model allenai/OLMo-2-0425-1B). The gate carries the near-tie band as the honest
// fallback where vLLM's own bf16 prefill/decode disagree on a tie; a token OUTSIDE
// vLLM's own near-tie band (gap > kNearTieMnats in vLLM's teacher-forced logits on
// OUR prefix) is a REAL forward divergence the gate FAILS on. This is what proves
// the PURE POST-NORM placement and the FULL-WIDTH qk-norm are actually applied — a
// misplaced norm or wrong qk-norm width emits FLUENT WRONG tokens far outside the
// band (the OPT BOS silent-corruption mode).
//
// Goldens (tests/parity/goldens/olmo2_greedy_1b/, captured on dgx):
...
//
// Checkpoint-GATED + dgx-only: resolves ~/.cache/huggingface/hub/models--allenai
// --OLMo-2-0425-1B. On CPU/CI the snapshot + goldens are absent, so the case emits
...
// Qwen3-dense / GLM-4 gates: vLLM's argmax must beat OUR token by <= this in vLLM's
// OWN teacher-forced logits given OUR prefix for a divergence to count as a bf16
...
void RunGate(const std::string& repo_dir, const std::string& golden_subdir,
  ...
  if (!fs::exists(gdir / "greedy_ids.npy")) {
    MESSAGE(label << " greedy golden absent; skipping — capture on dgx: "
            "glm4-oracle-capture.py --per-prompt --model allenai/OLMo-2-0425-1B");
    return;
  }
  ...
  if (!have_gap) {
    MESSAGE(label << " gap goldens absent; skipping — run under VT_DUMP_IDS=1 "
            "then glm4-neartie-gap.py --model allenai/OLMo-2-0425-1B");
    return;
  }
...
// OLMo-2-0425-1B (dense, pure post-norm + full-width qk-norm) — the first
// OLMo-family SACRED gate (spike W4). Proves the two new wiring facts are applied.
TEST_CASE("olmo2-0425-1B dense paged-engine greedy correctness gate (dgx-only, SACRED)") {
  RunGate("models--allenai--OLMo-2-0425-1B", "olmo2_greedy_1b", "olmo2-0425-1B");
}
