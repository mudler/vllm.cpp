// vllm.cpp original (OLMo-3 W5 — THE SACRED correctness gate for the first
// IBM Granite model); no upstream mirror (upstream's Granite coverage is the
// generic text-generation entry — spec §"Tests to port").
//
// THE PAGED-ENGINE Olmo3ForCausalLM (OLMo-3-1025-7B, bf16) GREEDY
...
// --model allenai/OLMo-3-1025-7B). STRICT where vLLM is deterministic;
...
//
// Goldens (tests/parity/goldens/olmo3_greedy_7b/, captured on dgx):
//   greedy_ids.npy / greedy_dist.npy / our_ids.npy / neartie_gap_mnats.npy.
// Checkpoint-GATED + dgx-only: resolves ~/.cache/huggingface/hub/
// models--allenai--OLMo-3-1025-7B. On CPU/CI it emits a loud SKIP.
...
void RunGate(const std::string& repo_dir, const std::string& golden_subdir,
  ...
  if (!have_gap) {
    MESSAGE(label << " gap goldens absent; skipping — run under VT_DUMP_IDS=1 "
            "then glm4-neartie-gap.py --model allenai/OLMo-3-1025-7B");
    return;
  }
...
// OLMo-3-1025-7B (dense, dual-rope interleaved sliding window) — the OLMo-3
// SACRED gate. Proves the YaRN full-attn rope + plain sliding rope are wired.
TEST_CASE("OLMo-3-7B paged-engine greedy correctness gate (dgx-only, SACRED)") {
  RunGate("models--allenai--OLMo-3-1025-7B", "olmo3_greedy_7b",
          "OLMo-3-7B");
}
