...
class AgentRecordMutationTests(unittest.TestCase):
    ...
    def test_qwen4_exp_row_is_inside_the_model_ratchet(self) -> None:
        ...
        # ...and an ACTIVE row without a live claim is the shape the audit
        # reports as ABANDONED, so the owner is pinned beside the state rather
        # than left to the generic claim rule, which does not name this row.
        self.assertIn("CLAIM-MODEL-MM-QWEN4-EXP-W1", found[0].field("owner"), item_id)
