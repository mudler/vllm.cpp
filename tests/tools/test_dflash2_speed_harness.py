...
class ShellDriverTest(unittest.TestCase):
    ...
    def test_the_gate_hands_our_arm_the_STREAM_the_sampler_writes(self) -> None:
        ...
        The binding is nonetheless a real cross-file one -- the path the sampler
        is told to WRITE against the path the arm is told to READ -- so a rename
        of either, or a deletion of the flag, reds this.

        Delete the `--clock-samples` line and `OurArmSpannedWindowTest` still
        passes, because that class drives `main()` directly. This case is what
        makes the deletion visible.
        """

        text = self.SCRIPT.read_text(encoding="utf-8")
        self.assertIn('--output "${EVIDENCE}/clock-$1-samples.jsonl"', text)
        ...
        # And the ORACLE arm is NOT given one, because it cannot use it: it does
        # not own its window on this branch, and restricting a window the shell
        # closes after the arm exits would restrict nothing. Owed, not silent.
...
class OurArmSpannedWindowTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        (self.root / "draft.gguf").write_bytes(payload)
        self.digest = hashlib.sha256(payload).hexdigest()
        self.samples = self.root / "clock-ours-samples.jsonl"
    ...
    def argv(self, *, samples: bool = True) -> list[str]:
        argv = [
            item
            for item in our_arm_argv(self.root, self.digest)
            if item != "--precheck-only"
        ...
        if samples:
            argv += ["--clock-samples", str(self.samples)]
        return argv
    ...
    def test_a_stream_the_spans_MISS_refuses_AFTER_the_evidence_is_written(self) -> None:
        ...
        self.samples.write_text(
            "".join(
                json.dumps(_clock_sample(_SPAN_EPOCH - 500.0 + n, utilization=97),
                           sort_keys=True) + "\n"
                for n in range(40)
            ),
            encoding="utf-8",
    ...
    def test_an_ABSENT_stream_is_a_refusal_and_never_an_unattributed_number(self) -> None:
        self.samples.unlink()
        with self.assertRaises(HarnessError) as raised:
            self.run_arm()
        self.assertIn("sampled no clock window", str(raised.exception))
