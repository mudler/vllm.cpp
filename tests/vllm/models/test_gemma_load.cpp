// W5 loader gate for `GemmaForCausalLM` (Gemma-1, gemma-2b, BF16). Loads the real
// checkpoint and asserts the name map is complete and exact: TWO norms per layer
// (no sandwich pre/post-ff, no q/k norm), tied lm_head (aliases embed_tokens; no
// lm_head.weight), and NO unmapped/leftover tensor. Loud SKIP without the ckpt.
...
using vllm::HfConfig;
using vllm::LoadGemmaForCausalLMWeights;
...
TEST_CASE("gemma W5 loader: gemma-2b safetensors -> GemmaWeights") {
  const std::string snap = FindSnapshot();
  if (snap.empty()) {
    MESSAGE("SKIP: gemma-2b checkpoint absent (run on dgx for the W5 load gate).");
    return;
  }

  const HfConfig config = LoadHfConfig(snap + "/config.json");
  // Config sanity (gemma-2b): hidden 2048, 18L, MQA 8/1, head_dim 256.
  REQUIRE(config.num_hidden_layers == 18);
  ...
  const GemmaWeights w = LoadGemmaForCausalLMWeights(shards, config);
  ...
  // NO unmapped/leftover tensors (Gemma-1: two norms/layer, no q/k norm, tied).
