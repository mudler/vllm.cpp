...
//
// `vt::Glm5NextKpoolCompress` and `vt::Glm5NextKpoolSelect` are the device arm
// of the k-pool indexer (CUDA and ROCm). There is no CPU provider and there
// deliberately is not one: the CPU answer already exists as `glm5_next_dsa.cpp`,
// gated by `test_glm5_next_dsa` against the same goldens, and registering it a
// second time under an `OpId` would make the seam its own oracle.
//
...
//
// The device cases SKIP on a CPU-only build (no CUDA or ROCm backend
// registered). A doctest `assertions: 0` line is a skip wearing a pass; the
// device job reads that line rather than the exit code.
...
// The device types whose k-pool ops are registered AND whose backend is live.
// On a CPU-only build this returns an empty vector and every device case skips.
// On CUDA or ROCm it returns that type, so the SAME golden comparison runs on
// whichever arm is present.
std::vector<DeviceType> KpoolDeviceTypes() {
  std::vector<DeviceType> out;
  for (DeviceType dt : {DeviceType::kCUDA, DeviceType::kROCM}) {
    try {
      vt::GetBackend(dt);
      if (vt::OpRegistered(vt::OpId::kGlm5NextKpoolCompress, dt) &&
          vt::OpRegistered(vt::OpId::kGlm5NextKpoolSelect, dt))
        out.push_back(dt);
...
class DeviceTensor {
 public:
  // The device is read from the queue so the same class serves CUDA and ROCm:
  // `CreateQueue` tags the queue with the backend's `DeviceType`, and the
  // tensor's device field stays consistent with the allocator that owns it.
...
TEST_CASE("glm5_next k-pool: the device ops are registered on CUDA/ROCm and absent on CPU") {
  ...
  // ROCm mirrors CUDA: both ops registered together, or neither. On a CPU-only
  // build neither is registered and this holds; on a ROCm build both are.
  const bool rocm_compress = vt::OpRegistered(vt::OpId::kGlm5NextKpoolCompress,
                                             DeviceType::kROCM);
  const bool rocm_select = vt::OpRegistered(vt::OpId::kGlm5NextKpoolSelect,
                                            DeviceType::kROCM);
  CHECK(rocm_compress == rocm_select);
...
TEST_CASE("glm5_next k-pool device: compress agrees with the transformers goldens") {
  for (DeviceType dt : KpoolDeviceTypes()) {
    CAPTURE(DeviceName(dt));
    Backend& backend = vt::GetBackend(dt);
