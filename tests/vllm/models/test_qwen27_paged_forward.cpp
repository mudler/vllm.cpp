...
TEST_CASE("qwen27 full-attention registry accepts unused GDN metadata") {
  ...
  const auto dense_prefill = Qwen3_5DenseModel::ForwardDense(ids, positions, w, c, q);
  ...
  const auto dense_full = Qwen3_5DenseModel::ForwardDense({5, 9, 2, 8}, {0, 1, 2, 3}, w, c, q);
...
TEST_CASE("qwen27 full-attention registry keeps generic counts") {
  HfConfig c = MakeConfig();
  c.num_hidden_layers = 1;
  c.layer_types = {"full_attention"};
...
TEST_CASE("qwen27 registry preserves each hybrid GDN guard") {
  const HfConfig c = MakeConfig();
  const Qwen3_5DenseWeights w = MakeWeights(c);
