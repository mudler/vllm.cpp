...
def main() -> None:
    ...
    parser.add_argument(
        "--benchmark-output",
        type=Path,
        help="optionally write elapsed time and token throughput as JSON",
    )
    parser.add_argument(
        "--top-logits-output",
        type=Path,
        help="optionally dump vLLM's raw top logits for each generated token",
    )
        ...
        for output in outputs:
            ...
            for position in output.outputs[0].logprobs:
                ...
                request_trace.append(
                    [
                        {"token_id": token_id, "logit": entry.logprob, "rank": entry.rank}
                        for token_id, entry in ranked
                    ]
                )
            traces.append(request_trace)
        args.top_logits_output.write_text(
            json.dumps(traces) + "\n", encoding="utf-8"
        )
