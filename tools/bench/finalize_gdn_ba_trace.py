#!/usr/bin/env python3
"""Finalize the exact batch-2 merged/split GDN BA trace series.

The GPU driver captures two complete paired profiles under one lock.  Each
local arm carries an explicit ``VT_GDN_MERGED_BA`` value and is independently
revalidated against its exact graph contract.  This CPU-only finalizer proves
that the merged arm removes exactly 48 BF16 GEMMs and no other graph nodes.
Kernel durations remain diagnostic and earn no benchmark speed credit.
"""
...
DERIVED_PATHS = frozenset(
    {
        "trace/27/gdn-ba-manifest.json",
        "trace/27/gdn-ba-summary.json",
        "trace/27/status-gdn-ba.json",
    }
)
...
def _trace_paths(evidence: pathlib.Path, mode: str) -> dict[str, Any]:
    if mode not in TRACE_GDN_BA_MODES:
        raise HarnessError(f"unknown GDN BA trace mode: {mode}")
    trace = evidence / "trace" / MODEL_KEY / f"gdn-ba-{mode}"
    return {
        "trace": trace,
...
def _validate_run_log(path: pathlib.Path) -> dict[str, Any]:
    ...
    required = {
        "c2 GDN BA merged raw paired trace capture complete; status remains "
        "PENDING until GDN BA finalization",
        "c2 GDN BA split raw paired trace capture complete; status remains "
        "PENDING until GDN BA finalization",
        "model 27 GDN BA merged/split node-level paired traces complete",
    }
    if not required.issubset(set(lines)):
        raise HarnessError("run log lacks the complete merged/split trace terminus")
...
def _build_arm(
    ...
    *,
    ...
    contract = TRACE_PRIMARY_GRAPH_CONTRACTS_BY_GDN_BA_MODE[
...
def summarize_gdn_ba_topology(
    arms: Mapping[str, Mapping[str, Any]],
) -> dict[str, Any]:
    """Require the merged arm to remove only one BA GEMM per GDN layer."""

    if set(arms) != set(TRACE_GDN_BA_MODES):
        raise HarnessError("GDN BA topology requires exactly merged and split arms")
    expected = {
        "merged": {"kernels": 963, "bf16": 145},
        "split": {"kernels": 1_011, "bf16": 193},
    }
    family_counts: dict[str, dict[str, int]] = {}
    topology_hashes = {}
    for mode in TRACE_GDN_BA_MODES:
        local = arms[mode].get("local")
        if not isinstance(local, Mapping):
            raise HarnessError(f"GDN BA {mode} local summary is absent")
        if local.get("kernel_count_per_window") != expected[mode]["kernels"]:
            raise HarnessError(f"GDN BA {mode} total kernel count drifted")
        families = local.get("family_summary")
        if not isinstance(families, Mapping):
            raise HarnessError(f"GDN BA {mode} family summary is absent")
        counts = {}
        for family, record in families.items():
            if not isinstance(family, str) or not isinstance(record, Mapping):
                raise HarnessError(f"GDN BA {mode} family summary is malformed")
            count = record.get("count_per_window")
            if isinstance(count, bool) or not isinstance(count, int):
                raise HarnessError(f"GDN BA {mode} family count is malformed")
            counts[family] = count
        if counts.get("bf16_cutlass_gemm") != expected[mode]["bf16"]:
            raise HarnessError(f"GDN BA {mode} BF16 GEMM count drifted")
        topology_hash = local.get("node_multiset_sha256")
        if not isinstance(topology_hash, str):
            raise HarnessError(f"GDN BA {mode} topology hash is absent")
        family_counts[mode] = counts
        topology_hashes[mode] = topology_hash
    if topology_hashes["merged"] == topology_hashes["split"]:
        raise HarnessError("GDN BA merged/split topology hashes unexpectedly match")
    merged_other = {
        family: count
        for family, count in family_counts["merged"].items()
        if family != "bf16_cutlass_gemm"
    }
    split_other = {
        family: count
    ...
    if merged_other != split_other:
        raise HarnessError("GDN BA merged/split non-BF16 family counts differ")
    kernel_delta = expected["split"]["kernels"] - expected["merged"]["kernels"]
    bf16_delta = expected["split"]["bf16"] - expected["merged"]["bf16"]
    if kernel_delta != 48 or bf16_delta != 48:
        raise HarnessError("GDN BA structural delta is not exactly 48 BF16 GEMMs")
    return {
        "bf16_gemm_delta_split_minus_merged": bf16_delta,
        "kernel_delta_split_minus_merged": kernel_delta,
        "merged_bf16_gemms": expected["merged"]["bf16"],
        "merged_kernel_count": expected["merged"]["kernels"],
        "non_bf16_family_counts": merged_other,
        "split_bf16_gemms": expected["split"]["bf16"],
        "split_kernel_count": expected["split"]["kernels"],
        "speed_credit": False,
    }
...
def build_summary(
    ...
    *,
    ...
    for field in ("ordered_names_sha256", "steady_b2_kernel_count_per_window"):
        values = {arms[mode]["oracle"].get(field) for mode in TRACE_GDN_BA_MODES}
        if len(values) != 1:
            raise HarnessError(f"merged/split vLLM oracle {field} differs")
    return {
        "schema_version": 1,
        "status": STATUS,
        ...
        "arms": arms,
        "topology": summarize_gdn_ba_topology(arms),
    }
...
def build_manifest(
    ...
    if not files:
        raise HarnessError(f"no immutable GDN BA trace artifacts under {evidence}")
    external = [
...
def finalize(
    ...
    *,
    ...
    summary_path = trace / "gdn-ba-summary.json"
    manifest_path = trace / "gdn-ba-manifest.json"
    marker_path = trace / "status-gdn-ba.json"
    existing = [
...
def main() -> int:
    ...
    if run_log is None:
        run_log = args.evidence.parent.parent / "gdn-ba-run.log"
    ...
    print(
        canonical_json(
            {
                "merged_bf16_gemms": summary["topology"]["merged_bf16_gemms"],
                "split_bf16_gemms": summary["topology"]["split_bf16_gemms"],
...
if __name__ == "__main__":
    try:
        ...
        print(f"finalize-gdn-ba-trace: {error}", file=sys.stderr)
